...
---
### <a name="sse-and-sse2-comparegreaterthan-methods-properly-handle-nan-inputs"></a><span data-ttu-id="2418b-101">SSE 和 SSE2 CompareGreaterThan 方法正确处理 NaN 输入</span><span class="sxs-lookup"><span data-stu-id="2418b-101">SSE and SSE2 CompareGreaterThan methods properly handle NaN inputs</span></span>

<span data-ttu-id="2418b-102">以下 <xref:System.Runtime.Intrinsics.X86.Sse?displayProperty=nameWithType> 和 <xref:System.Runtime.Intrinsics.X86.Sse2?displayProperty=nameWithType> 方法已修复为正确处理 `NaN` 输入，并匹配 <xref:System.Runtime.Intrinsics.X86.Avx?displayProperty=nameWithType> 类中等效方法的硬件行为：</span><span class="sxs-lookup"><span data-stu-id="2418b-102">The following <xref:System.Runtime.Intrinsics.X86.Sse?displayProperty=nameWithType> and <xref:System.Runtime.Intrinsics.X86.Sse2?displayProperty=nameWithType> methods have been fixed to properly handle `NaN` inputs and match the hardware behavior of the equivalent methods in the <xref:System.Runtime.Intrinsics.X86.Avx?displayProperty=nameWithType> class:</span></span>
...
#### <a name="change-description"></a><span data-ttu-id="2418b-103">更改描述</span><span class="sxs-lookup"><span data-stu-id="2418b-103">Change description</span></span>

<span data-ttu-id="2418b-104">以前，向列出的 <xref:System.Runtime.Intrinsics.X86.Sse> 和 <xref:System.Runtime.Intrinsics.X86.Sse2> 方法提供 `NaN` 输入所返回的结果不正确。</span><span class="sxs-lookup"><span data-stu-id="2418b-104">Previously, `NaN` inputs to the listed <xref:System.Runtime.Intrinsics.X86.Sse> and <xref:System.Runtime.Intrinsics.X86.Sse2> methods returned an incorrect result.</span></span> <span data-ttu-id="2418b-105">此结果也不同于 <xref:System.Runtime.Intrinsics.X86.Avx> 类中相应方法生成的结果。</span><span class="sxs-lookup"><span data-stu-id="2418b-105">The result also differed from the result generated by the corresponding method in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="2418b-106">自 .NET 5.0 起，这些方法可以正确地处理 `NaN` 输入，并返回与 <xref:System.Runtime.Intrinsics.X86.Avx> 类中相应方法相同的结果。</span><span class="sxs-lookup"><span data-stu-id="2418b-106">Starting in .NET 5.0, these methods correctly handle `NaN` inputs and return the same results as the corresponding methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="2418b-107">流式处理 SIMD 扩展 (SSE) 和流式处理 SIMD 扩展 2 (SSE2) 行业标准体系结构 (ISA) 没有为这些比较方法提供直接的硬件支持，因此它们是在软件中实现的。</span><span class="sxs-lookup"><span data-stu-id="2418b-107">The Streaming SIMD Extensions (SSE) and Streaming SIMD Extensions 2 (SSE2) industry standard architectures (ISAs) don't provide direct hardware support for these comparison methods, so they're implemented in software.</span></span> <span data-ttu-id="2418b-108">以前，这些方法没有得到正确的实现，也没有正确地处理 `NaN` 输入。</span><span class="sxs-lookup"><span data-stu-id="2418b-108">Previously, the methods were improperly implemented, and they incorrectly handled `NaN` inputs.</span></span> <span data-ttu-id="2418b-109">对于从本机移植的代码，不正确的行为可能会引入 bug。</span><span class="sxs-lookup"><span data-stu-id="2418b-109">For code ported from native, the incorrect behavior may introduce bugs.</span></span> <span data-ttu-id="2418b-110">对于 256 位代码路径，这些方法可能还会生成与 <xref:System.Runtime.Intrinsics.X86.Avx> 类中等效方法不同的结果。</span><span class="sxs-lookup"><span data-stu-id="2418b-110">For a 256-bit code path, the methods can also produce different results to the equivalent methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="2418b-111">有关说明这些方法以前是如何不正确的示例，可以为常规整数将 `CompareNotGreaterThan(x,y)` 实现为 `CompareLessThanOrEqual(x,y)`。</span><span class="sxs-lookup"><span data-stu-id="2418b-111">As an example of how the methods were previously incorrect, you can implement `CompareNotGreaterThan(x,y)` as `CompareLessThanOrEqual(x,y)` for regular integers.</span></span> <span data-ttu-id="2418b-112">不过，对于 `NaN` 输入，相应逻辑计算出的结果不正确。</span><span class="sxs-lookup"><span data-stu-id="2418b-112">However, for `NaN` inputs, that logic computes the wrong result.</span></span> <span data-ttu-id="2418b-113">相反，使用 `CompareNotLessThan(y,x)` 可以正确地比较数字，并  考虑 `NaN` 输入。</span><span class="sxs-lookup"><span data-stu-id="2418b-113">Instead, using `CompareNotLessThan(y,x)` compares the numbers correctly *and* takes `NaN` inputs into consideration.</span></span>

#### <a name="version-introduced"></a><span data-ttu-id="2418b-114">引入的版本</span><span class="sxs-lookup"><span data-stu-id="2418b-114">Version introduced</span></span>

<span data-ttu-id="2418b-115">5.0 预览版 4</span><span class="sxs-lookup"><span data-stu-id="2418b-115">5.0 Preview 4</span></span>

#### <a name="recommended-action"></a><span data-ttu-id="2418b-116">建议操作</span><span class="sxs-lookup"><span data-stu-id="2418b-116">Recommended action</span></span>

- <span data-ttu-id="2418b-117">如果以前的行为是 bug，则不需要进行任何更改。</span><span class="sxs-lookup"><span data-stu-id="2418b-117">If the previous behavior was a bug, no change is required.</span></span>

- <span data-ttu-id="2418b-118">如果需要以前的行为，则可以通过更改相关调用来维护此行为，如下所示：</span><span class="sxs-lookup"><span data-stu-id="2418b-118">If the previous behavior was desired, you can maintain that behavior by changing the relevant invocation as follows:</span></span>
...
#### <a name="category"></a><span data-ttu-id="2418b-119">类别</span><span class="sxs-lookup"><span data-stu-id="2418b-119">Category</span></span>

<span data-ttu-id="2418b-120">Core .NET 库</span><span class="sxs-lookup"><span data-stu-id="2418b-120">Core .NET libraries</span></span>

#### <a name="affected-apis"></a><span data-ttu-id="2418b-121">受影响的 API</span><span class="sxs-lookup"><span data-stu-id="2418b-121">Affected APIs</span></span>
