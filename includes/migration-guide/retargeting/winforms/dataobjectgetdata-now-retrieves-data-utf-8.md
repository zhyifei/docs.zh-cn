...
|   |   |
|---|---|
|详细信息|对于面向 .NET Framework 4 或者在 .NET Framework 4.5.1 或更早版本上运行的应用，<code>DataObject.GetData</code> 将 HTML 格式的数据检索为 ASCII 字符串。 因此，非 ASCII 字符（ASCII 代码大于 0x7F 的字符）由两个随机字符表示。对于面向 .NET Framework 4.5 或更高版本以及在 .NET Framework 4.5.2 上运行的应用，<code>DataObject.GetData</code> 将按 UTF-8 检索 HTML 格式的数据，从而可以正确表示大于 0x7F 的字符。|
