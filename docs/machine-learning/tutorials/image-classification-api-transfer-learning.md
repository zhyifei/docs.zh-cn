...
1. 安装 Microsoft.ML 1.4.0-preview2 NuGet 包  ：
    ...
    1. 对 Microsoft.ML.Dnn 0.16.0-preview2 和 Microsoft.ML.ImageAnalytics 1.4.0-preview2 重复这些步骤   。
...
1. 定义包含 `mapLabelEstimator` 和 `ImageClassification` 转换的训练 [`EstimatorChain`](xref:Microsoft.ML.Data.EstimatorChain%601) 管道。
    ...
    `ImageClassification` 估算器采用以下几个参数：

    - `featuresColumnName` 是用作模型的输入的列。
    - `labelColumnName` 是要预测的值的列。
    - `arch` 定义要使用的预先训练的模型体系结构。 本教程使用 ResNetv2 模型的 101 层变体。
    - `epoch` 指定整个训练过程中整个数据集的最大迭代次数。 该数字越大，模型的训练时间就越长，并且生成的模型可能就越好。
    - `batchSize` 是在训练时要使用的示例数。 在一个学习周期，使用等于 batchSize 的多个批来训练和更新模型。 该数字越小，处理每个批时所需的内存就越少。
    ...
    - `disableEarlyStopping` 告知 ImageClassification 估算器是否采用尽早停止策略。 由于模型会搜索可帮助它在训练期间进行准确预测的最佳值，因此性能可能会增加或降低。 最终，如果模型达到最后一个学习周期，那么它从训练中学到的模式可能并非最佳。 尽早停止监视这些性能下降的训练，并停止训练过程，以努力保留模型的最佳版本。
