...
**✓ 务必**在所有具有 <xref:System.Runtime.Serialization.DataMemberAttribute> 的属性上实现 getter 和 setter。数据协定序列化程序需要将类型的 getter 和 setter 视为可序列化。（在 .NET Framework 3.5 SP1 中，某些集合属性可以是只读的。）如果该类型不会在部分信任环境下使用，则这两个属性访问器中的一个或两个都可以是非公共的。
