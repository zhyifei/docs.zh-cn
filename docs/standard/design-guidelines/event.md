...
事件是最常用的回调形式（允许框架调用用户代码的结构）。其他回调机制包括接受委托的成员、虚拟成员和基于接口的插件。来自可用性研究的数据表明，大多数开发人员相比使用其他回调机制，更习惯使用事件。事件能与 Visual Studio 和许多语言很好地集成。
...
**X 切忌** 在引发非静态事件时传递 null 值 作为 sender。

**✓ 务必** 在引发静态事件时传递 null 值作为 sender。  

**X 切忌** 在引发事件时传递 null 值作为事件数据参数。

如果您不想将任何数据传递给事件处理方法，则应传递`EventArgs.Empty`。开发人员希望此参数不为null。
...
**✓ 务必** 为事件处理程序使用 void 的返回类型。  
  
事件处理程序可以调用多个事件处理方法，可能在多个对象上。如果允许事件处理方法返回值，则每个事件调用都会有多个返回值。
  
**✓ 务必** 使用 `object` 作为事件处理程序的第一个参数的类型，并将其命名为`sender`。  
  
**✓ 务必** 使用 <xref:System.EventArgs?displayProperty=nameWithType> 或其子类作为事件处理程序中第二个参数的类型，并将其命名为 `e`。  
  
**X 切忌** 对事件处理程序使用两个以上的参数。
