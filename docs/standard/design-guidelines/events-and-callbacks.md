...
**✓ 务必** 倾向使用事件而非普通回调，因为一般开发人员更熟悉事件，而且事件能与 Visual Studio 的语句完成功能集成在一起。

**X 避免** 在性能敏感的 API 中使用回调。

**✓ 务必** 在定义带回调的 API 时，使用新的 `Func<...>`、`Action<...>` 或 `Expression<...>` 类型，而不是自定义委托。

`Func<...>` 和 `Action<...>` 表示泛型委托。`Expression<...>` 表示函数定义，可在运行时编译并随后调用，但也可进行序列化并传递给远程进程。

**✓ 务必** 估量和了解使用 `Expression<...>` 来代替 `Func<...>` 和 `Action<...>` 委托的性能影响。

在大多数情况下，`Expression<...>` 类型在逻辑上等同于 `Func<...>` 和 `Action<...>` 委托。它们之间的主要区别是，委托适用于本地进程场景；表达式适用于可以在远程进程或计算机中进行表达式评估的情况。

**✓ 务必** 了解在调用委托时执行的任意代码可能会对安全性、正确性和兼容性产生影响。
