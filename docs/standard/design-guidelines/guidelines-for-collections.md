 ...
 **✓ DO**作为参数类型使用的最小专用类型可能。 如何集合，因为参数使用的大多数成员`IEnumerable<T>`接口。  
 ...
 **✓ CONSIDER**使用键控的集合，如果在集合中存储的项具有唯一键 （名称、 Id，等等。）。 键控的集合是集合可以通过一个整数和一个密钥建立索引并通常实现通过继承`KeyedCollection<TKey,TItem>`。  
  
 键控的集合通常更大的内存需求量，并且不能如果内存开销超过了带来密钥的好处。  
  
 **X DO NOT**返回 null 值，从集合属性或方法返回集合。 而是返回空集合或为空数组。  
 ...
 **✓ DO**实现`IEnumerable<T>`设计新的集合时。 请考虑实施`ICollection<T>`甚至`IList<T>`有意义。  
 ...
 **✓ CONSIDER**前缀同名的项类型的集合名称。 例如，存储类型的项集合`Address`(实现`IEnumerable<Address>`) 应该被命名为`AddressCollection`。 如果项的类型是接口，"I"前缀的项类型，则可以省略。 因此，集合<xref:System.IDisposable>可调用项`DisposableCollection`。  
