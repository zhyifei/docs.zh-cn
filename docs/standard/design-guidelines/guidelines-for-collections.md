...
**✓ 请对**表示只读集合的​​属性或返回值使用 <xref:System.Collections.ObjectModel.ReadOnlyCollection%601>、`ReadOnlyCollection<T>` 的子类或 `IEnumerable<T>`（在极少数情况下使用）。 
...
**✓ 考虑**如果框架中可能已添加或已存在相应的可写集合，则在只读集合的​​名称中使用“ReadOnly”前缀。
