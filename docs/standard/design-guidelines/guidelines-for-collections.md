 ...
 **✓ DO** 作为参数类型使用的最小专用类型可能。 将集合作为参数的大多数成员使用 `IEnumerable<T>` 接口。  
 ...
 **✓ CONSIDER** 使用键控的集合，如果在集合中存储的项具有唯一键 （名称、 Id，等等。）。 键控集合是可以由整数和键索引的集合，通常通过继承自`KeyedCollection<TKey,TItem>` 进行实现。  
 ...
 **X DO NOT** 如继承自非泛型基集合`CollectionBase`。 相反，使用 `Collection<T>`、`ReadOnlyCollection<T>` 和 `KeyedCollection<TKey,TItem>`。  
 ...
 **✓ CONSIDER** 前缀同名的项类型的集合名称。 例如，存储类型为 `Address` 的项的集合（实现 `IEnumerable<Address>`）应该命名为 `AddressCollection`。 如果项类型是接口，则可以省略项类型的 “I” 前缀。 因此，<xref:System.IDisposable> 项的集合可以称为 `DisposableCollection`。  
