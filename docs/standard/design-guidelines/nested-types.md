...
**X 切忌** 使用嵌套类型，如果类型可能在包含类型之外引用的话。  

例如，传递给类上定义的方法的枚举不应该被定义为类中的嵌套类型。

**X 切忌** 使用嵌套类型，如果它们需要通过客户端代码实例化的话。如果类型具有公共构造函数，则它可能不应该被套。

如果一个类型可以被实例化，那表明该类型在框架中占有一席之地（你可以创建它，使用它，并在不使用外部类型的情况下销毁它），因此不应该嵌套。内部类型不应在与外部类型无任何关系的情况下在外部类型之外广泛重用。
