 ...
 **✓ 执行**尝试使用描述性的参数名称以指示较短的重载使用的默认值。  
  
 **请避免 x**随意更改重载中的参数名称。 如果某一重载中的参数表示相同的输入中另一个重载的参数，参数应具有相同的名称。  
  
 **请避免 x**在排序中的参数中不一致状态重载成员。 具有相同名称的参数应出现在所有重载中的同一位置。  
  
 **✓ 执行**使只有最长的重载成为虚方法 （如果可扩展性是必需的）。 较短的重载应只需通过调用较长的重载。  
  
 **X 不**使用`ref`或`out`修饰符来重载成员。  
  
 某些语言不能解决如下重载的调用。 此外，此类重载通常具有完全不同的语义，并且可能不应为重载，但两个单独的方法相反。  
  
 **X 不**具有重载随着在相同的位置以及相似类型的参数，而使用不同的语义。  
  
 **✓ 执行**允许`null`要为可选自变量传递。  
  
 **✓ 执行**使用重载，而不是定义具有默认自变量的成员的成员。  
