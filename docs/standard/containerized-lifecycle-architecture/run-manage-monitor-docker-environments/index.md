...
设想：企业应用程序需要使用高可用性和高可伸缩性; 才能运行IT 操作需要能够管理和监视环境及应用程序本身。

容器化 Docker 应用程序生命周期的最后一个关键点在于，如何能够在可缩放的高可用性 (HA) 生产环境中运行、管理和监视应用程序。

在生产环境 （基础结构体系结构和平台技术） 中运行容器化应用程序的方式是非常相关，基于此电子书的第 1 章中所述的所选体系结构和开发平台。

本章检查特定产品和技术从 Microsoft 和其他供应商提供可用于有效地运行可缩放的 HA 分布式应用程序以及如何管理和监视它们从 IT 角度来看。
