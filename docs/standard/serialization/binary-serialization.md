...
二进制序列化允许修改对象内部的私有成员，从而更改其状态。 因此，建议在公共 API 图面上运行其他序列化框架，如 <xref:System.Text.Json?displayProperty=fullName>。

## <a name="net-core"></a>.NET Core

对于类型的子集，.NET Core 支持二进制序列化。 可在后面的 "[可序列化类型](#serializable-types)" 部分中查看支持的类型列表。 在 .NET Framework 4.5.1 和更高版本之间以及 .NET Core 2.0 及更高版本之间，可以保证列出的类型是可序列化的。 其他 .NET 实现（如 Mono）不是正式支持的，但也应该有效。
...
> | <xref:System.DBNull?displayProperty=nameWithType> | 从 .NET Core 2.0.2 及更高版本开始。 |
...
> | <xref:System.Data.DataTable?displayProperty=nameWithType> | 如果将 `RemotingFormat` 设置为 `SerializationFormat.Binary`，则只能与 .NET Core 2.1 和更高版本交换。 |
...
> | <xref:System.Security.SecurityException?displayProperty=nameWithType> | 从 .NET Core 2.0.4 开始。<br/>限制的序列化数据。 |
...
> | <xref:System.ValueTuple?displayProperty=nameWithType> | 在 .NET Framework 4.7 及更早版本中不可序列化。 |
...
本文介绍和说明如何使用 ASP.NET 创建 XML Web services。
