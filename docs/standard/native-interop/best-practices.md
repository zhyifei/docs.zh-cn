...
| <xref:System.Runtime.InteropServices.DllImportAttribute.CharSet> | `CharSet.None`，这会退回到 `CharSet.Ansi` 行为  | 定义中存在字符串或字符时显式使用 `CharSet.Unicode` 或 `CharSet.Ansi` | 这将指定字符串的封送行为以及为 `false` 时 `ExactSpelling` 的操作。 请注意，`CharSet.Ansi` 在 Unix 上实际为 UTF8。 __ 大部分时间，Windows 使用 Unicode，而 Unix 使用 UTF8。 有关更多信息，请查看[有关字符集的文档](./charset.md)。 |
...
当字符集为 Unicode 或参数显式标记为 ，`[MarshalAs(UnmanagedType.LPWSTR)]` __ 并且通过值（不是 `ref` 或 `out`）传递字符串时，将固定该字符串并直接由本机代码使用（而非复制）。
...
2. Invoke
   1. 分配本机缓冲区**{2}**
   2. 如果为 （`StringBuilder` 参数的默认值）`[In]` __，则复制内容
   3. 如果为 `[Out]` **{3}** _（也是 `StringBuilder` 的默认值）_，则将本机缓冲区复制到新分配的托管数组中
3. `ToString()`分配另一个托管数组**{4}**

这是*{4}* 从本机代码中获取字符串的分配。 可用来限制此操作的最佳方法是在其他调用中重用 `StringBuilder`，但这仍只能保存 1** 个分配。 最好从 `ArrayPool` 使用并缓存字符缓冲区 - 然后可以在后续调用中直接获得 `ToString()` 的分配。
...
> __特定于窗口__对于`[Out]`字符串，CLR 默认情况下`CoTaskMemFree`将使用释放字符串或`SysStringFree`标记为`UnmanagedType.BSTR`的字符串。
> **对于大多数具有输出字符串缓冲区的 API：** 传入的字符计数必须包括 null。 如果返回的值小于传入的字符计数，则调用成功，并且该值是** 不带尾随 Null 的字符数。 否则，该计数是包括** Null 字符的缓冲区的所需大小。
>
> - 传递 5，获取 4：字符串长 4 个字符，尾随 null。
> - 传递 5，获取 6：字符串为 5 个字符长，需要 6 个字符缓冲区来容纳 null。
> [Windows 字符串数据类型](/windows/desktop/Intl/windows-data-types-for-strings)
...
❌请勿用于`[MarshalAs(UnmanagedType.LPStruct)]`GUID 参数`ref`以外的任何内容。
...
`char`在一维数组中是可 blit 的，**或者如果**它是包含它显式用`[StructLayout]``CharSet = CharSet.Unicode`标记的类型的一部分。
...
[`HandleRef`](xref:System.Runtime.InteropServices.HandleRef)允许编组在 P/Invoke 的持续时间内使对象保持活动状态。 方法签名中可以使用该类型，而不是 `IntPtr`。 `SafeHandle` 可有效地替换此类，应改为使用此类型。

[`GCHandle`](xref:System.Runtime.InteropServices.GCHandle)允许固定托管对象，并将本机指针获取到该对象。 基本模式是：
...
❌AVOID`System.Delegate`使用`System.MulticastDelegate`或字段来表示结构中的函数指针字段。

由于<xref:System.Delegate?displayProperty=fullName>并且<xref:System.MulticastDelegate?displayProperty=fullName>没有必需的签名，它们不保证传入的委托与本机代码所需的签名匹配。 此外，在 .NET Framework 和 .NET Core 中，如果`System.Delegate`本机`System.MulticastDelegate`表示中的字段值不是包装托管委托的函数指针，则将包含 或从其本机表示形式为管理对象的结构封送到托管对象可能会破坏运行时的稳定。 在 .NET 5 和更高版本中，`System.Delegate`不支持`System.MulticastDelegate`将 或 字段从本机表示形式封送到托管对象。 使用特定的委托类型而不是`System.Delegate`或`System.MulticastDelegate`。
