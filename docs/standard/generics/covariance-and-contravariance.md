...
- `Covariance`  
     ...
     你可以将 `IEnumerable<Derived>`（在 Visual Basic 中为 `IEnumerable(Of Derived)`）的实例分配给 `IEnumerable<Base>` 类型的变量。  
  
- `Contravariance`  
  
     使你能够使用比原始指定的类型更泛型（派生程度更小）的类型。  
  
     你可以将 `Action<Base>`（在 Visual Basic 中为 `Action(Of Base)`）的实例分配给 `Action<Derived>` 类型的变量。  
