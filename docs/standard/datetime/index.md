...
* <xref:System.TimeZone>
  ...
  使用此类处理系统的本地时区和协调世界时 (UTC) 区域。 类的功能<xref:System.TimeZone>在很大程度上<xref:System.TimeZoneInfo>由类取代。
...
[如何：枚举计算机](../../../docs/standard/datetime/enumerate-time-zones.md)上存在的时区提供一些示例，这些示例列举了在计算机的注册表中定义的时区，并允许用户从列表中选择预定义的时区。

[如何：访问预定义的 UTC 和本地时区对象](../../../docs/standard/datetime/access-utc-and-local.md)介绍如何访问协调世界时和本地时区。

[如何：实例化 TimeZoneInfo 对象](../../../docs/standard/datetime/instantiate-time-zone-info.md)介绍如何<xref:System.TimeZoneInfo>实例化本地系统注册表中的对象。
...
[如何：创建不含调整规则](../../../docs/standard/datetime/create-time-zones-without-adjustment-rules.md)的时区介绍如何创建不支持夏令时转换的自定义时区。

[如何：创建具有调整规则](../../../docs/standard/datetime/create-time-zones-with-adjustment-rules.md)的时区介绍了如何创建支持一次或多次转换夏令时的自定义时区。
...
[如何：将时区保存到嵌入的资源](../../../docs/standard/datetime/save-time-zones-to-an-embedded-resource.md)说明了如何创建自定义时区，并将其信息保存到资源文件中。

[如何：从嵌入的资源](../../../docs/standard/datetime/restore-time-zones-from-an-embedded-resource.md)还原时区介绍如何实例化已保存到嵌入的资源文件中的自定义时区。
...
[如何：在日期和时间算法](../../../docs/standard/datetime/use-time-zones-in-arithmetic.md)中使用时区讨论如何执行反映时区调整规则的日期和时间算法。
...
[如何：解决不明确](../../../docs/standard/datetime/resolve-ambiguous-times.md)时间介绍如何通过将不明确时间映射到时区的标准时间来解决它。

[如何：让用户解决不明确](../../../docs/standard/datetime/let-users-resolve-ambiguous-times.md)时间介绍如何让用户确定不明确的本地时间与协调世界时之间的映射。
