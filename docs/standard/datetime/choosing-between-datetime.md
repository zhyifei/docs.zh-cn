...
* 若要从.NET 外部源中检索日期和时间信息，通常其中日期和时间信息存储在一个简单数据类型。
...
.NET 包括<xref:System.DateTime>， <xref:System.DateTimeOffset>， <xref:System.TimeSpan>，和<xref:System.TimeZoneInfo>它们都可以用于构建使用日期和时间的应用程序的类型。
...
<xref:System.DateTime> 值定义特定日期和时间。 它包括<xref:System.DateTime.Kind%2A>属性，可提供有关时区的信息的日期和时间所属有限。 <xref:System.DateTimeKind>返回值<xref:System.DateTime.Kind%2A>属性指示是否<xref:System.DateTime>值表示本地时间 (<xref:System.DateTimeKind.Local?displayProperty=nameWithType>)，格式为协调世界时 (UTC) (<xref:System.DateTimeKind.Utc?displayProperty=nameWithType>)，或未指定的时间 (<xref:System.DateTimeKind.Unspecified?displayProperty=nameWithType>)。
...
* 从外部.NET 中，如 SQL 数据库的源中检索日期和时间信息。 通常，这些源按与 <xref:System.DateTime> 结构兼容的简单格式存储日期和时间信息。
...
<xref:System.DateTimeOffset> 类型包括 <xref:System.DateTime> 类型的所有功能以及时区感知功能。 这使其适用于应用程序执行以下操作：
...
* 测量运行时间。 例如，<xref:System.Diagnostics.Stopwatch.Elapsed%2A?displayProperty=nameWithType>属性返回<xref:System.TimeSpan>值，该值反映自之一调用以来经过的时间间隔<xref:System.Diagnostics.Stopwatch>开始测量运行时间的方法。

<xref:System.TimeSpan> 值还可用于在 <xref:System.DateTime> 值反映一个时间而不引用一天的特定时间时替代它。 这种用法是类似于<xref:System.DateTime.TimeOfDay%2A?displayProperty=nameWithType>和<xref:System.DateTimeOffset.TimeOfDay%2A?displayProperty=nameWithType>属性，它们返回<xref:System.TimeSpan>值，该值表示未引用日期的时间。 例如， <xref:System.TimeSpan> 结构可用于反映商店每天的开张或打烊时间，还可用来表示任何常规事件发生的时间。
...
在某些情况下，可能需要进一步的开发工作才可以充分利用 <xref:System.TimeZoneInfo> 类。 如果日期和时间值不会与它们可以与其所属，进一步的工作的时区紧密耦合是必需的。 除非你的应用程序提供了一些机制用于链接的日期和时间与其关联的时区，很容易与其所属与其时区的特定日期和时间值。 链接此信息的一种方法是，定义一个同时包含日期和时间值及其关联时区对象的类或结构。
