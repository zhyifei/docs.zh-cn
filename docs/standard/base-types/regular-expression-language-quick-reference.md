...
|`\G`|匹配必须出现在上一个匹配结束的地方。|`\G\(\d\)`|`"(1)(3)(5)[7](9)"` 中的 1、`"(1)"`、`"(3)"` 和 `"(5)"`|  
...
|`(?<` *名称1* `-` *名称2* `>` *子表达式* `)`|定义平衡组定义。 有关详细信息，请参阅 [分组构造](grouping-constructs-in-regular-expressions.md)中的"平衡组定义"部分。|`(((?'Open'\()[^\(\)]*)+((?'Close-Open'\))[^\(\)]*)+)*(?(Open)(?!))$`|`"3+2^((1-3)*(3-1))"` 中的 `"((1-3)*(3-1))"`|  
|`(?:` *子表达式* `)`|定义非捕获组。|`Write(?:Line)?`|`"Console.WriteLine()"` 中的 `"WriteLine"`<br /><br /> `"Console.Write(value)"` 中的 `"Write"`|  
|`(?imnsx-imnsx:` *子表达式* `)`|应用或禁用 *子表达式*中指定的选项。 有关详细信息，请参阅 [正则表达式选项](regular-expression-options.md)。|`A\d{2}(?i:\w+)\b`|`"A12xl A12XL a12xl"` 中的 `"A12xl"` 和 `"A12XL"`|  
