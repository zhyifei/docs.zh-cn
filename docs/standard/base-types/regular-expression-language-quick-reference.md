...
|`$`|默认情况下，匹配必须出现在字符串的末尾，或在字符串末尾的 `\n` 之前；在多行模式中，必须出现在该行的末尾之前，或在该行末尾的 `\n` 之前。|`-\d{3}$`|“-333-333”中的<br /><br /> “-333-333”中的|  
|`\A`|匹配必须出现在字符串的开头。|`\A\d{3}`|“901-333-”中的<br /><br /> “901”|  
|`\Z`|匹配必须出现在字符串的末尾或出现在字符串末尾的 `\n` 之前。|`-\d{3}\Z`|“-333-333”中的<br /><br /> “-333-333”中的|  
|`\z`|匹配必须出现在字符串的末尾。|`-\d{3}\z`|“-333-333”中的<br /><br /> “-333-333”中的|  
...
|`(?(`expression`)`yes<code>&#124;</code>no`)`|如果正则表达式模式由 *expression* 匹配指定，则匹配 *yes* ；否则，匹配可选 *no* 部分。 *expression* 被解释为零宽度断言。|<code>(?(A)A\d{2}\b&#124;\b\d{3}\b)</code>|“A10 C103 910”中的“A10”和“910”|  
