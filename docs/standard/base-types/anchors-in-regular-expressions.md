...
|`\A`|匹配必须仅出现在字符串的开头位置（无多行支持）。 有关详细信息，请参阅 [仅字符串的开头](#start-of-string-only-a)。|  
|`\Z`|匹配必须出现在字符串的末尾，或出现在字符串末尾的 `\n` 之前。 有关详细信息，请参阅 [字符串的末尾或结束换行之前](#end-of-string-or-before-ending-newline-z)。|  
|`\z`|匹配必须仅出现在字符串的末尾。 有关详细信息，请参阅 [仅字符串的末尾](#end-of-string-only-z)。|  
|`\G`|匹配必须从上一个匹配结束的位置开始。 有关详细信息，请参阅 [连续匹配](#contiguous-matches-g)。|  
|`\b`|匹配必须出现在字边界。 有关详细信息，请参阅 [字边界](#word-boundary-b)。|  
