...
-   `xx` 是称为“精度说明符”的可选整数。 精度说明符的范围从 0 到 99，并且影响结果中的位数。 请注意，精度说明符控制数字的字符串表示形式中的数字个数。 它不舍入该数字。 若要执行舍入运算，请使用 <xref:System.Math.Ceiling%2A?displayProperty=nameWithType>、<xref:System.Math.Floor%2A?displayProperty=nameWithType> 或 <xref:System.Math.Round%2A?displayProperty=nameWithType> 方法。  
     ...
     当*精度说明符*控制数的结果字符串中的小数位数，结果字符串反映远离零舍入的数字 (即，使用<xref:System.MidpointRounding.AwayFromZero?displayProperty=nameWithType>)。  
...
- 某些重载`ToString`的所有数值类型的方法。 例如，可以提供一个数值格式字符串到<xref:System.Int32.ToString%28System.String%29?displayProperty=nameWithType>和<xref:System.Int32.ToString%28System.String%2CSystem.IFormatProvider%29?displayProperty=nameWithType>方法。 
 
- .NET[复合格式设置功能](../../../docs/standard/base-types/composite-formatting.md)，由它的一些`Write`和`WriteLine`方法<xref:System.Console>和<xref:System.IO.StreamWriter>类，<xref:System.String.Format%2A?displayProperty=nameWithType>方法，与<xref:System.Text.StringBuilder.AppendFormat%2A?displayProperty=nameWithType>方法。 复合格式功能允许你将多个数据项的字符串表示形式包含在单个字符串中，以指定字段宽度，并在字段中对齐数字。 有关更多信息，请参见[复合格式设置](../../../docs/standard/base-types/composite-formatting.md)。  

- [内插字符串](../../csharp/language-reference/keywords/interpolated-strings.md)在 C# 和 Visual Basic 中，提供简化的语法为复合格式字符串进行比较时。
...
|“P”或“p”|百分比|结果：乘以 100 并显示百分比符号的数字。<br /><br /> 受以下类型支持：所有数值类型。<br /><br /> 精度说明符：所需的小数位数。<br /><br /> 默认值精度说明符： 由定义<xref:System.Globalization.NumberFormatInfo.PercentDecimalDigits%2A?displayProperty=nameWithType>。<br /><br /> 有关详细信息，请参阅[百分比（“P”）格式说明符](#PFormatString)。|1 ("P", en-US) -> 100.00 %<br /><br /> 1 ("P", fr-FR) -> 100,00 %<br /><br /> -0.39678 ("P1", en-US) -> -39.7 %<br /><br /> -0.39678 ("P1", fr-FR) -> -39,7 %|  
|“R”或“r”|往返过程|结果：可以往返至相同数字的字符串。<br /><br /> 受以下类型支持：<xref:System.Single>、<xref:System.Double> 和 <xref:System.Numerics.BigInteger>。<br /><br /> 注意： 建议<xref:System.Numerics.BigInteger>仅键入。 有关<xref:System.Double>类型，使用"G17"; 对于<xref:System.Single>类型，使用"G9"。 </br> 精度说明符：忽略。<br /><br /> 有关详细信息，请参阅[往返（“R”）格式说明符](#RFormatString)。|123456789.12345678 ("R") -> 123456789.12345678<br /><br /> -1234567890.12345678 ("R") -> -1234567890.1234567|  
...
-   可以传递给拥有 `ToString` 参数的 `format` 方法的重载。 下面的示例设置为货币字符串中 （在此情况下，EN-US 区域性） 的当前区域性的格式的数字值。  
...
-   它可作为提供`formatString`等方法一起使用的格式项中的自变量<xref:System.String.Format%2A?displayProperty=nameWithType>， <xref:System.Console.WriteLine%2A?displayProperty=nameWithType>，和<xref:System.Text.StringBuilder.AppendFormat%2A?displayProperty=nameWithType>。 有关更多信息，请参见[复合格式设置](../../../docs/standard/base-types/composite-formatting.md)。 下面的示例使用格式项在字符串中插入货币值。  
     ...
     此外，你可以提供`alignment`自变量以指定的数字字段以及是右对齐还是左对齐其值的宽度。 下面的示例在 28 位字符的字段中左对齐货币值，在 14 位字符的字段中右对齐货币值。  
...
|<xref:System.Globalization.NumberFormatInfo.NegativeSign%2A>|定义时使用的负号<xref:System.Globalization.NumberFormatInfo.CurrencyNegativePattern%2A>指示不使用括号。|  
...
## <a name="the-fixed-point-f-format-specifier"></a>定点（“F”）格式说明符  
 定点 ("F") 格式说明符将数字转换为格式的字符串"-ddd.ddd …" 其中每个“d”表示一个数字 (0-9)。 如果该数字为负，则该字符串以减号开头。  
...
请注意，如果用于<xref:System.Double>值，使用"G17"格式说明符，可确保原始<xref:System.Double>值成功往返。 这是因为<xref:System.Double>是 IEEE 754 2008年符合双精度 (`binary64`) 浮点数，提供多达 17 有效位数的精度。 建议使用它而不是["R"格式说明符](#RFormatString)，因为在某些情况下"R"无法成功往返双精度浮点值。 下面的示例阐释了这样一种情况。

[!code-csharp[Round-tripping a Double](../../../samples/snippets/standard/base-types/format-strings/csharp/g17.cs)]   
[!code-vb[Round-tripping a Double](../../../samples/snippets/standard/base-types/format-strings/vb/g17.vb)]   

如果用于<xref:System.Single>值，"G9"格式说明符，可确保原始<xref:System.Single>值成功往返。 这是因为<xref:System.Single>是 IEEE 754 2008年符合单精度 (`binary32`) 浮点数，最多 9 个有效位精度。 建议使用它而不是["R"格式说明符](#RFormatString)，因为在某些情况下"R"无法成功往返单精度浮点值。
...
## <a name="the-round-trip-r-format-specifier"></a>往返过程（“R”）格式说明符  
 往返 ("R") 格式说明符尝试确保转换为字符串的数值分析回相同的数值。 只有 <xref:System.Single>、<xref:System.Double> 和 <xref:System.Numerics.BigInteger> 类型支持此格式。  

有关<xref:System.Double>和<xref:System.Single>值，"R"格式说明符在某些情况下失败以成功往返原始值，并且还提供相对较差的性能。 相反，我们建议你使用["G17"](#GFormatString)格式说明符的<xref:System.Double>值和["G9"](#GFormatString)格式说明符以成功往返<xref:System.Single>值。
...
|<xref:System.Globalization.NumberFormatInfo.PositiveSign%2A>|定义指示指数为正值的字符串。|  
 ...
 下面的示例格式<xref:System.Numerics.BigInteger>使用往返过程格式说明符的值。  
