...
A*子任务*(或*嵌套的任务*) 是<xref:System.Threading.Tasks.Task?displayProperty=nameWithType>的另一个任务，这被称为的用户委托中创建的实例*父任务*。 可以分离或附加子任务。 A*分离的子任务*是独立于其父执行的任务。 *附加子任务*是使用创建的嵌套的任务<xref:System.Threading.Tasks.TaskCreationOptions.AttachedToParent?displayProperty=nameWithType>其父级不显式或默认禁止其被附加的选项。 一个任务可以创建任意数量的附加和分离子任务，这仅受系统资源限制。  
...
## <a name="preventing-a-child-task-from-attaching-to-its-parent"></a>阻止子任务附加到其父任务  
 ...
 可能还想要防止子任务在没有及时完成时附加到其父任务。 因为父任务只有在所有子任务完成后才会完成，所以长时间运行的子任务会使整个应用执行得非常缓慢。 有关演示如何通过阻止任务附加到其父任务来提高应用性能的示例，请参阅[如何： 防止子任务附加到其父](../../../docs/standard/parallel-programming/how-to-prevent-a-child-task-from-attaching-to-its-parent.md)。  
