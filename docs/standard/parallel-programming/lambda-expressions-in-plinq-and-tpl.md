...
## <a name="action-delegate"></a>Action 委托  
 <xref:System.Action?displayProperty=nameWithType> 委托封装了不返回值或返回 [void](~/docs/csharp/language-reference/keywords/void.md) 的方法（[!INCLUDE[vbprvb](../../../includes/vbprvb-md.md)] 中的 Sub）。 在 Action 类型签名中，类型参数仅表示输入参数。 与 Func 一样，Framework 类库定义了 Action 的 17 个版本，从没有类型参数的版本到具有 16 个类型参数的版本。  
