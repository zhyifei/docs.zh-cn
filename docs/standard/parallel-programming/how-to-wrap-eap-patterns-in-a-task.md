...
下面的示例演示如何通过使用 <xref:System.Threading.Tasks.TaskCompletionSource%601> 将任意基于事件的异步模式 (EAP) 操作序列公开为一个任务。 该示例还演示如何使用<xref:System.Threading.CancellationToken>要调用的内置取消方法<xref:System.Net.WebClient>对象。  
