...
## <a name="overview"></a><span data-ttu-id="bf86b-111">概述</span><span class="sxs-lookup"><span data-stu-id="bf86b-111">Overview</span></span>

<span data-ttu-id="bf86b-112">![XSLT 转换体系结构](media/xslttransformationswithxsltransformclass.gif "xsltTransformationsWithXslTransformClass")</span><span class="sxs-lookup"><span data-stu-id="bf86b-112">![XSLT Transformation Architecture](media/xslttransformationswithxsltransformclass.gif "xsltTransformationsWithXslTransformClass")</span></span>  
<span data-ttu-id="bf86b-113">转换结构</span><span class="sxs-lookup"><span data-stu-id="bf86b-113">Transformation Architecture</span></span>

<span data-ttu-id="bf86b-114">XSLT 建议使用 XML 路径语言 (XPath) 选择 XML 文档的各部分，其中 XPath 是用于浏览文档树节点的查询语言。</span><span class="sxs-lookup"><span data-stu-id="bf86b-114">The XSLT recommendation uses XML Path Language (XPath) to select parts of an XML document, where XPath is a query language used to navigate nodes of a document tree.</span></span> <span data-ttu-id="bf86b-115">如图所示，[!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] 实现的 XPath 用来选择存储在几个类中的 XML 部分，如 <xref:System.Xml.XmlDocument>、<xref:System.Xml.XmlDataDocument> 和 <xref:System.Xml.XPath.XPathDocument> 类。</span><span class="sxs-lookup"><span data-stu-id="bf86b-115">As shown in the diagram, the [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] implementation of XPath is used to select parts of XML stored in several classes, such as an <xref:System.Xml.XmlDocument>, an <xref:System.Xml.XmlDataDocument>, and an <xref:System.Xml.XPath.XPathDocument>.</span></span> <span data-ttu-id="bf86b-116"><xref:System.Xml.XPath.XPathDocument> 是一个优化的 XSLT 数据存储区，当与 <xref:System.Xml.Xsl.XslTransform> 一起使用时，可以提供性能良好的 XSLT 转换。</span><span class="sxs-lookup"><span data-stu-id="bf86b-116">An <xref:System.Xml.XPath.XPathDocument> is an optimized XSLT data store, and when used with <xref:System.Xml.Xsl.XslTransform>, it provides XSLT transformations with good performance.</span></span>

<span data-ttu-id="bf86b-117">下表列出了在使用 <xref:System.Xml.Xsl.XslTransform> 和 XPath 时常用的类及其功能。</span><span class="sxs-lookup"><span data-stu-id="bf86b-117">The following table list commonly uses classes when working with <xref:System.Xml.Xsl.XslTransform> and XPath and their function.</span></span>

|<span data-ttu-id="bf86b-118">类或接口</span><span class="sxs-lookup"><span data-stu-id="bf86b-118">Class or Interface</span></span>|<span data-ttu-id="bf86b-119">函数</span><span class="sxs-lookup"><span data-stu-id="bf86b-119">Function</span></span>|
|------------------------|--------------|
|<xref:System.Xml.XPath.XPathNavigator>|<span data-ttu-id="bf86b-120">它是提供用于在存储区上浏览的游标样式模型以及 XPath 查询支持的 API。</span><span class="sxs-lookup"><span data-stu-id="bf86b-120">It is an API that provides a cursor style model for navigating over a store, along with XPath query support.</span></span> <span data-ttu-id="bf86b-121">它不提供对基础存储区的编辑。</span><span class="sxs-lookup"><span data-stu-id="bf86b-121">It does not provide editing of the underlying store.</span></span> <span data-ttu-id="bf86b-122">要进行编辑，请使用 <xref:System.Xml.XmlDocument> 类。</span><span class="sxs-lookup"><span data-stu-id="bf86b-122">For editing, use the <xref:System.Xml.XmlDocument> class.</span></span>|
|<xref:System.Xml.XPath.IXPathNavigable>|<span data-ttu-id="bf86b-123">它是为存储区提供 `CreateNavigator` 的 <xref:System.Xml.XPath.XPathNavigator> 方法的接口。</span><span class="sxs-lookup"><span data-stu-id="bf86b-123">It is an interface that provides a `CreateNavigator` method to an <xref:System.Xml.XPath.XPathNavigator> for the store.</span></span>|
|<xref:System.Xml.XmlDocument>|<span data-ttu-id="bf86b-124">它启用对此文档的编辑。</span><span class="sxs-lookup"><span data-stu-id="bf86b-124">It enables editing of this document.</span></span> <span data-ttu-id="bf86b-125">它实现 <xref:System.Xml.XPath.IXPathNavigable>，从而允许随后需要 XSLT 转换的文档编辑方案。</span><span class="sxs-lookup"><span data-stu-id="bf86b-125">It implements <xref:System.Xml.XPath.IXPathNavigable>, allowing document-editing scenarios where XSLT transformations are subsequently required.</span></span> <span data-ttu-id="bf86b-126">有关详细信息，请参阅 [XslTransform 的 XmlDocument 输入](xmldocument-input-to-xsltransform.md)。</span><span class="sxs-lookup"><span data-stu-id="bf86b-126">For more information, see [XmlDocument Input to XslTransform](xmldocument-input-to-xsltransform.md).</span></span>|
|<xref:System.Xml.XmlDataDocument>|<span data-ttu-id="bf86b-127">它从 <xref:System.Xml.XmlDocument> 派生。</span><span class="sxs-lookup"><span data-stu-id="bf86b-127">It is derived from the <xref:System.Xml.XmlDocument>.</span></span> <span data-ttu-id="bf86b-128">它根据 <xref:System.Data.DataSet> 的指定映射，使用 <xref:System.Data.DataSet> 优化 XML 文档中的结构化数据存储，从而将关系世界与 XML 世界联系起来。</span><span class="sxs-lookup"><span data-stu-id="bf86b-128">It bridges the relational and XML worlds by using a <xref:System.Data.DataSet> to optimize storage of structured data within the XML document according to specified mappings on the <xref:System.Data.DataSet>.</span></span> <span data-ttu-id="bf86b-129">它实现 <xref:System.Xml.XPath.IXPathNavigable>，从而允许可以对从数据库中检索到的关系数据执行 XSLT 转换的方案。</span><span class="sxs-lookup"><span data-stu-id="bf86b-129">It implements <xref:System.Xml.XPath.IXPathNavigable>, allowing scenarios where XSLT transformations can be performed over relational data retrieved from a database.</span></span> <span data-ttu-id="bf86b-130">有关详细信息，请参阅[与关系数据和 ADO.NET 的 XML 集成](xml-integration-with-relational-data-and-adonet.md)。</span><span class="sxs-lookup"><span data-stu-id="bf86b-130">For more information, see [XML Integration with Relational Data and ADO.NET](xml-integration-with-relational-data-and-adonet.md).</span></span>|
|<xref:System.Xml.XPath.XPathDocument>|<span data-ttu-id="bf86b-131">此类针对 <xref:System.Xml.Xsl.XslTransform> 处理和 XPath 查询进行了优化，并提供只读的高性能缓存。</span><span class="sxs-lookup"><span data-stu-id="bf86b-131">This class is optimized for <xref:System.Xml.Xsl.XslTransform> processing and XPath queries, and it provides a read-only high performance cache.</span></span> <span data-ttu-id="bf86b-132">它实现 <xref:System.Xml.XPath.IXPathNavigable>，是用于 XSLT 转换的首选存储区。</span><span class="sxs-lookup"><span data-stu-id="bf86b-132">It implements <xref:System.Xml.XPath.IXPathNavigable> and is the preferred store to use for XSLT transformations.</span></span>|
