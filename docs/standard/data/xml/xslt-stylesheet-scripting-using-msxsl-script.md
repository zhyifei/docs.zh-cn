...
```  
 ...
 如果脚本的调用方没有 [UnmanagedCode](frlrfSystemSecurityPermissionsSecurityPermissionFlagClassTopic) 访问权限，则样式表中的脚本将永不编译并且对 <xref:System.Xml.Xsl.XslTransform.Load%2A> 的调用将失败。  
