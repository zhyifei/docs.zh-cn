...
XmlDocument 将命名空间声明和 xmlns 属性公开为文档对象模型的一部分。 这些声明和属性存储在 XmlDocument 中，因此在可以保存文档时暂留这些属性的位置。 更改这些属性对树中现有其他节点的 Name、NamespaceURI 和 Prefix 属性没有影响。 例如，如果加载以下文档，则 `test` 元素具有**NamespaceURI** `123.`  
...
```  
 ...
 同样，如果将不同 `xmlns` 特性添加到 `doc` 元素，如下所示，`test` 元素仍具有**NamespaceURI** `123`。  
