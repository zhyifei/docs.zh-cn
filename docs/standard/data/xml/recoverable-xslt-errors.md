...
|--------------------|-------------|-----------------------------------|  
|文本节点同时与 `xsl:strip-space` 和 `xsl:preserve-space` 匹配。|3.4|Recover|  
|源节点与多个模板规则匹配。|5.5|Recover|  
|某个命名空间 URI 声明为多个命名空间 URI 的别名，所有这些 URI 都具有相同的导入优先级。|7.1.1|Recover|  
|通过属性值生成的 `name` 和 `xsl:attribute` 中的 `xsl:element` 属性不是 QName。|7.1.2, 7.1.3|错误*|  
|具有相同导入和扩展名称的两个属性集具有一个公共的属性，没有其他包含该公共属性的同名属性集具有更高的优先级。|7.1.4|Recover|  
...
|`xsl:processing-instruction` 内容的实例化结果中包含字符串“?>”|7.3|Recover|  
|`xsl:processing-instruction` 内容的实例化结果中包含字符串“--”或以“-”结尾。|7.4|Recover|  
...
|存在多个名称相同但是值不同的属性，这些属性不是具有相同导入优先级的 `xsl:output` 中的命名 cdata 节元素。|16|Recover|  
|处理器不支持 `xsl:output` 编码属性中的编码。|16.1|Recover|  
...
|`value` 的 `xsl:number` 属性为 NAN、无限大或小于 0.5。|errata 24|Recover|  
|文档函数的第二个自变量 node-set 为空，且 URI 引用是相对的。|errata 14|Recover|  
