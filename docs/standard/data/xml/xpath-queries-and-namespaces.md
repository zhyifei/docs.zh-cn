---
title: "XPath 查询和命名空间 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
XPath 查询支持 XML 文档中的命名空间，可以使用命名空间前缀来限定元素和属性的名称。  使用命名空间前缀来限定元素和属性的名称可以限制 XPath 查询只返回属于特定命名空间的节点。  
 ...
 例如，如果前缀 `books` 映射到命名空间 `http://www.contoso.com/books`，以下 XPath 查询 `/books:books/books:book` 将只选择命名空间 `http://www.contoso.com/books` 中的 `book` 元素。  
...
-   使用 <xref:System.Xml.XPath.XPathExpression> 对象的 <xref:System.Xml.XPath.XPathExpression.SetContext%2A> 方法将 <xref:System.Xml.XmlNamespaceManager> 对象与现有的 <xref:System.Xml.XPath.XPathExpression> 对象关联。  还可以使用静态 <xref:System.Xml.XPath.XPathExpression.Compile%2A> 方法编译新的 <xref:System.Xml.XPath.XPathExpression> 对象，该方法使用表示 XPath 表达式的字符串和 <xref:System.Xml.XmlNamespaceManager> 对象作为参数，并返回一个新的 <xref:System.Xml.XPath.XPathExpression> 对象。  
...
```  
 ...
 以下代码为默认命名空间定义前缀，并选择 `http://www.contoso.com/books` 命名空间中的所有 `book` 元素。  
