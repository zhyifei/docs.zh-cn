...
如果你有烹饪经验，便可通过异步方式执行这些指令  。 你会先开始加热平底锅以备煎蛋，接着再从培根着手。 然后将面包放进烤面包机，再煎鸡蛋。 在此过程的每一步，你都可以先开始一项任务，然后将注意力转移到准备进行的其他任务上。
...
对于并行算法而言，你则需要多名厨师（或线程）。 一名厨师煎鸡蛋，一名厨师煎培根，依次类推。 每名厨师将仅专注于一项任务。 且每名厨师（或线程）都会同步地停下来（被阻塞），以等待翻动培根的时机或面包弹出的瞬间。
...
计算机不会按人类的方式来解释这些指令。 计算机将阻塞每条语句，直到工作完成，然后再继续运行下一条语句。 这将创造出令人不满意的早餐。 后续任务要到早前任务完成后才会启动。 这样做早餐花费的时间要长得多，有些食物在上桌之前就已经凉了。
...
上述代码展示了可以使用 <xref:System.Threading.Tasks.Task> 或 <xref:System.Threading.Tasks.Task%601> 对象来保存运行中的任务。 你首先需要 `await` 每项任务，然后再使用它的结果。 下一步是创建表示其他工作组合的方式。 在提供早餐之前，你希望先等待代表烤面包的任务完成，再添加黄油和果酱。 你可以使用以下代码表示此工作：
...
上述更改说明了使用异步代码的一项重要技术。 你可以通过将操作分离到一个返回任务的新方法中来组合任务。 可以选择何时等待此任务。 可以同时启动其他任务。
