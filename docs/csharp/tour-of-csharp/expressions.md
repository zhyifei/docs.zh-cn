...
* 基本
  ...
  - `new {...}`:匿名对象初始值设定项
  - `new T[...]`:数组创建
  - `typeof(T)`:获取 <xref:System.Type> 对象 `T`
...
* 关系和类型测试
  ...
  - `x as T`:返回类型化为 `T` 的 `x`；或返回 `null`，前提是 `x` 不是 `T`
* 相等
  - `x == y`:等于
  - `x != y`:不等于
* 逻辑“与”
...
* 逻辑“或”
  - `x | y`:整型按位“或”，布尔型逻辑“或”
* 条件“与”
  - `x && y`:计算 `y`，前提是 `x` 不是 `false`
* 条件“或”
  - `x || y`:计算 `y`，前提是 `x` 不是 `true`
* null 合并
  - `x ?? y`:如果 `x` 为 null，计算结果为 `y`；否则，计算结果为 `x`
* 条件运算
  - `x ? y : z`:计算 `y`，前提是 `x` 为 `true`；或计算 `z`，前提是 `x` 为 `false`
* 赋值或匿名函数
  - `x = y`:赋值
  - `x op= y`:复合赋值；支持以下运算符：
    - `*=`   `/=`   `%=`   `+=`   `-=`   `<<=`   `>>=`   `&=`  `^=`  `|=`
