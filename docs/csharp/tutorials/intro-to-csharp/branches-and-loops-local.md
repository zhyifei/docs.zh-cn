...
再次键入 `dotnet run`。 由于答案小于 10，因此什么也没有打印出来。 要测试的*条件*为 false。 因为仅为 `if` 语句编写了一个可能分支（true 分支），所以没有任何可供执行的代码。
...
`==` 符号执行*相等*测试。 使用 `==` 将相等测试与赋值符号区分开来，如在 `a = 5` 中所见。
...
在本节使用**循环**以重复执行语句。 请在 `Main` 方法中试用以下代码：
...
此示例新引入了另外一个运算符。 `++` 变量后面的 `counter` 是**增量**运算符。 它将 `counter` 值加 1，并将计算后的值存储在 `counter` 变量中。

> [!IMPORTANT]
> 请确保 `while` 循环条件在你执行代码时更改为 false。 否则，创建的就是**无限循环**，即程序永不结束。 本示例中没有演示上述情况，因为你必须使用 **CTRL-C** 或其他方法强制退出程序。
