...
> [!TIP]
> 在探索 C#（或任何编程语言）的过程中，可能会在编写代码时犯错。 **编译器**会发现并向你报告这些错误。 如果输出中包含错误消息，请仔细比对示例代码和你的窗口中的代码，看看哪些需要纠正。
> 这样做有助于了解 C# 代码结构。
...
通过组合多个不同的运算，进一步探索运算顺序。 在 `Main` 方法底部添加类似以下行的内容。 再执行一次 `dotnet run`试试。
...
继续之前，让我们把你在本节编写的所有代码放在新的方法中。 调用新方法 `OrderPrecedence`。
最后应以与下面类似的代码结束：
...
在上一个示例中，整数除法截断了结果。
可以使用**取模**运算符（即`%`字符）来计算余数。 在 `Main` 方法中试用以下代码：
...
如果运算生成的值超过这些限值，则会出现**下溢**或**溢出**的情况。 答案似乎是从一个限值覆盖到另一个限值的范围。 例如，将以下两行添加到 `Main` 方法：
...
`double` 数字类型表示双精度浮点数。 这些词可能是第一次听说。 **浮点**数可用于表示数量级可能非常大或非常小的非整数。 **双精度**意味着存储这些数字时使用的精度高于**单精度**。 在新式计算机上，使用双精度数字比使用单精度数字更为常见。
接下来，我们将探索双精度类型。 添加以下代码并查看结果：
...
花了一些时间应对挑战之后，把刚刚编写的代码放在一个新方法中。 将新方法命名为 `WorkWithDoubles`。
