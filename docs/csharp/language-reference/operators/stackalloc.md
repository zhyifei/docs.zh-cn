...
- 从 C# 7.2 开始，<xref:System.Span%601?displayProperty=nameWithType> 或 <xref:System.ReadOnlySpan%601?displayProperty=nameWithType> 将如下例所示：
  ...
  将堆栈中分配的内存块分配给 [ 或 ](../keywords/unsafe.md) 变量时，不必使用 <xref:System.Span%601>unsafe<xref:System.ReadOnlySpan%601> 上下文。

  使用这些类型时，可以在`stackalloc`条件表达式[或赋值表达式中使用 ](conditional-operator.md) 表达式，如以下示例所示：

  [!code-csharp[stackalloc expression](snippets/StackallocOperator.cs#AsExpression)]

  从 C#8.0 开始，只要允许使用 `stackalloc` 或 <xref:System.Span%601> 变量，就可以在其他表达式中使用 <xref:System.ReadOnlySpan%601> 表达式，如下例所示：
...
有关详细信息，请参阅 [C# 语言规范](~/_csharplang/spec/unsafe-code.md#stack-allocation)以及[允许嵌入上下文中的 ](~/_csharplang/spec/introduction.md)[ 功能建议说明的`stackalloc`堆栈分配](~/_csharplang/proposals/csharp-8.0/nested-stackalloc.md)部分。
