...
可以通过[定义从 ](user-defined-conversion-operators.md) 到 `int` 的隐式转换`Fraction`来扩展前面的示例。 然后，重载运算符将支持这两种类型的参数。 也就是说，可以将一个整数添加到一个分数中，得到一个分数结果。
