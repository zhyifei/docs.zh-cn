...
---
# <a name="database-per-microservice"></a>每微服务一个数据库
...
当跨微服务查询数据很困难时，跨多个微服务实现事务甚至更加复杂。 在不同微服务中跨独立数据源维护数据一致性的固有挑战不能 understated。 云本机应用程序中缺少分布式事务意味着必须以编程方式管理分布式事务。 从一开始就是，从一开始就是*最终的一致性*。
