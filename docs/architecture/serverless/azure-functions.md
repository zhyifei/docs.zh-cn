...
> **在1.x 中存在一个已知问题, 其中包含[绑定重定向支持](https://github.com/Azure/azure-functions-host/issues/992)。** 此问题特定于 .NET 开发。 对于依赖于运行时中包含的库不同版本的库, 具有依赖项的项目会受到影响。 函数团队已致力于对问题进行具体的进度。 团队将在2.x 中处理绑定重定向, 然后再将其公开上市。 可在此处获取带有建议的修补程序和解决方法的官方团队声明:[Azure Functions 中的程序集解析](https://github.com/Azure/azure-functions-host/wiki/Assembly-Resolution-in-Azure-Functions)。
...
在此示例中, 该函数是通过对`images`容器中的 blob 存储的更改触发的。 传递文件的信息, 因此触发器也充当绑定。 另一个绑定用于将信息放到名为`images`的队列。
...
代理使用发送到根 URL (`https://--shorturl--/`) 的任何内容, 并将其重定向到文档站点。 `Root`

视频[Azure 中显示了使用代理的示例:利用无服务器 Azure Functions](https://channel9.msdn.com/events/Connect/2017/E102)将你的应用引入云。 在本地 SQL Server 上运行的 ASP.NET Core 应用程序将被迁移到 Azure 云。 代理用于帮助重构传统 Web API 项目, 以使用函数。
