...
本文中的示例可用于F#应用程序或F#脚本。 若要创建F#脚本，请在F#开发环境中创建一个具有 `.fsx` 扩展名的文件，例如 `tables.fsx`。

接下来，使用[程序包管理器](package-management.md)（如[Paket](https://fsprojects.github.io/Paket/)或[NuGet](https://www.nuget.org/) ）通过 `#r` 指令在脚本中安装 `WindowsAzure.Storage` 包和引用 `WindowsAzure.Storage.dll`。 为获取 Microsoft Azure 命名空间，请再次对 `Microsoft.WindowsAzure.ConfigurationManager` 执行此操作。
...
实体必须具有从 `TableEntity`继承的类型。 您可以采用任何喜欢的方式扩展 `TableEntity`，但您的类型*必须*具有无参数的构造函数。 只有具有 `get` 和 `set` 的属性存储在 Azure 表中。
...
现在，将 `Customer` 添加到表中。 为此，请创建对表执行的 `TableOperation`。 在这种情况下，你将创建 `Insert` 操作。
...
如果不想查询分区中的所有实体，则可以通过结合使用分区键筛选器与行键筛选器来指定一个范围。 在这里，您将使用两个筛选器来获取 "Smith" 分区中的所有实体，其中，行键（名字）以字母表中字母 "M" 前面的字母开头。
...
你可以编写查询以检索单个特定实体。 此处，可以使用 `TableOperation` 来指定客户 "Ben Smith"。 返回 `Customer`，而不是使用集合。 在查询中同时指定分区键和行键是从表服务中检索单个实体的最快方法。
...
有时，您不知道实体是否存在于表中。 如果是这样，则不再需要存储在其中的当前值。 你可以使用 `InsertOrReplace` 来创建实体，或将其替换为（如果存在），而不考虑其状态。

[!code-fsharp[TableStorage](~/samples/snippets/fsharp/azure/table-storage.fsx#L134-L141)]

### <a name="query-a-subset-of-entity-properties"></a>查询一部分实体属性

表查询可以只检索实体中的几个属性，而不是所有这些属性。 此方法称为 "投影"，可提高查询性能，尤其适用于大型实体。 此处，只使用 `DynamicTableEntity` 和 `EntityResolver`返回电子邮件地址。 注意，本地存储模拟器不支持投影，因此，此代码仅在使用表服务中的帐户时才能运行。
...
- [.NET 存储客户端库参考](https://docs.microsoft.com/dotnet/api/overview/azure/storage)
