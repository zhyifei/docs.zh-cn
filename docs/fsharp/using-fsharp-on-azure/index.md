...
## <a name="using-azure-virtual-machines-with-f"></a>将 Azure 虚拟机与 F\# 一起使用
...
## <a name="using-azure-functions-with-f"></a>将 Azure Functions 与 F\# 一起使用
...
* [将 Azure 类型提供程序用于 Azure Functions](https://compositional-it.com/blog/2017/08-30-using-the-azure-type-provider-with-azure-functions/index.html)

## <a name="using-azure-storage-with-f"></a>将 Azure 存储与 F\# 一起使用

Azure 存储是一种基层存储服务，用于依赖于持久性、可用性和可缩放性来满足其客户需求的现代应用程序。 F#使用以下文章中所述的技术，程序可以直接与 Azure 存储服务进行交互。
...
## <a name="using-azure-app-service-with-f"></a>将 Azure App Service 与 F\# 一起使用
...
## <a name="using-azure-cosmos-db-with-f"></a>将 Azure Cosmos DB 与 F\# 一起使用

[Azure Cosmos DB](https://azure.microsoft.com/services/cosmos-db)是一种 NoSQL 服务，适用于高度可用的全球分布式应用。

可以通过F#两种方式使用 Azure Cosmos DB：

1. 通过创建对 Azure Cosmos DB F#集合做出更改的 Azure Functions。 请参阅[Azure Functions 的 Azure Cosmos DB 绑定](/azure/azure-functions/functions-bindings-cosmosdb)，或
2. 使用[.NET SDK FOR SQL API Azure Cosmos DB](/azure/cosmos-db/sql-api-sdk-dotnet)。 相关的示例位于C#。
...
## <a name="using-azure-notification-hubs-with-f"></a>将 Azure 通知中心与 F\# 配合使用
...
## <a name="implementing-webhooks-on-azure-with-f"></a>在 Azure 上通过 F\# 实现 Webhook
...
## <a name="using-webjobs-with-f"></a>将 Web 作业与 F 一起使用\#
...
## <a name="implementing-timers-on-azure-with-f"></a>在 Azure 上通过 F\# 实现计时器
...
同样，其他 Azure 资源也可用相同的组件通过 F# 脚本进行部署和管理。 例如，你可以创建存储帐户、部署 Azure 云服务、从F#脚本以编程方式创建 Azure Cosmos DB 实例和管理 Azure 通知 hub。

通常，使用 F# 脚本来部署和管理资源并不必要。 例如，Azure 资源还可以直接从 JSON 模板说明（可以参数化）部署。 请参阅 [Azure 资源管理器模板](/azure/azure-resource-manager/resource-manager-template-best-practices)，包括 [Azure 快速入门模板](https://azure.microsoft.com/resources/templates/)等示例。
