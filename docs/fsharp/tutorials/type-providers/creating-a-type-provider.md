...
- <span data-ttu-id="6d5dc-130">它会更改在编码过程吗？</span><span class="sxs-lookup"><span data-stu-id="6d5dc-130">Will it change during coding?</span></span>
...
<span data-ttu-id="6d5dc-138">请注意静态已知类型和成员提供的组。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-138">Note that the set of types and members provided is statically known.</span></span> <span data-ttu-id="6d5dc-139">此示例不会利用提供程序能够提供依赖于架构的类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-139">This example doesn't leverage the ability of providers to provide types that depend on a schema.</span></span> <span data-ttu-id="6d5dc-140">类型提供程序的实现下面的代码中所述，本主题的后面部分介绍详细信息。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-140">The implementation of the type provider is outlined in the following code, and the details are covered in later sections of this topic.</span></span>
...
<span data-ttu-id="6d5dc-143">然后寻找下类型`Samples.HelloWorldTypeProvider`类型提供程序生成的命名空间。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-143">Then look for the types under the `Samples.HelloWorldTypeProvider` namespace that the type provider generated.</span></span>
...
<span data-ttu-id="6d5dc-167">接下来，生成 100 提供的类型：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-167">Next, generate the 100 provided types:</span></span>
...
### <a name="choosing-representations-for-erased-provided-types"></a><span data-ttu-id="6d5dc-228">有关清除选择表示形式提供类型</span><span class="sxs-lookup"><span data-stu-id="6d5dc-228">Choosing Representations for Erased Provided Types</span></span>
...
- <span data-ttu-id="6d5dc-245">如果你想要创建启用从任何现有的.NET API 显著不同的 API，最好创建将为类型擦除和提供的类型的表示形式的运行时类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-245">If you want to create an API that differs significantly from any existing .NET API, it makes sense to create runtime types that will be the type erasure and representations for the provided types.</span></span>
...
## <a name="a-type-provider-that-uses-static-parameters"></a><span data-ttu-id="6d5dc-255">使用静态参数类型提供程序</span><span class="sxs-lookup"><span data-stu-id="6d5dc-255">A Type Provider That Uses Static Parameters</span></span>
...
- <span data-ttu-id="6d5dc-283">在调用`Regex`构造函数引发<xref:System.ArgumentException>正则表达式无效。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-283">The call to the `Regex` constructor throws a <xref:System.ArgumentException> when a regular expression isn’t valid.</span></span> <span data-ttu-id="6d5dc-284">编译器会捕获此异常，并在编译时或在 Visual Studio 编辑器中向用户报告一条错误消息。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-284">The compiler catches this exception and reports an error message to the user at compile time or in the Visual Studio editor.</span></span> <span data-ttu-id="6d5dc-285">此异常将正则表达式来验证但不运行的应用程序。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-285">This exception enables regular expressions to be validated without running an application.</span></span>
...
<span data-ttu-id="6d5dc-304">由于您创建实例方法，因此`args.[0]`表示`RegexTyped`实例在其调用方法，和`args.[1]`是输入的参数。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-304">Because you are creating an instance method, `args.[0]` represents the `RegexTyped` instance on which the method is being called, and `args.[1]` is the input argument.</span></span>
...
<span data-ttu-id="6d5dc-326">本部分演示如何提供可用于获取的行的类型`Distance`类型的属性`float<meter>`和一个`Time`类型的属性`float<second>`。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-326">This section shows how to provide a type that you can use to get rows with a `Distance` property of type `float<meter>` and a `Time` property of type `float<second>`.</span></span> <span data-ttu-id="6d5dc-327">为简单起见，进行以下假设：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-327">For simplicity, the following assumptions are made:</span></span>
...
<span data-ttu-id="6d5dc-342">下面的代码演示实现的核心。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-342">The following code shows the core of the implementation.</span></span>
...
- <span data-ttu-id="6d5dc-349">可以使用`ProvidedMeasureBuilder`类型，若要查找的 SI 单位并生成相关`float<_>`类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-349">You can use the `ProvidedMeasureBuilder` type to look up the SI units and to generate the relevant `float<_>` types.</span></span>
...
<span data-ttu-id="6d5dc-357">如示例所示，该类型的所有提及`Type1`和`InstanceProperty`属性都会被清除，离开所涉及的仅对运行时类型的操作。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-357">As the example shows, all mentions of the type `Type1` and the `InstanceProperty` property have been erased, leaving only operations on the runtime types involved.</span></span>
...
<span data-ttu-id="6d5dc-359">创作类型提供程序时，应遵守以下约定。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-359">Observe the following conventions when authoring type providers.</span></span>

<span data-ttu-id="6d5dc-360">**连接协议的提供程序**一般情况下，为数据和服务的连接协议，如 OData 或 SQL 连接时，大多数提供程序 Dll 的名称应以结尾`TypeProvider`或`TypeProviders`。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-360">**Providers for Connectivity Protocols** In general, names of most provider DLLs for data and service connectivity protocols, such as OData or SQL connections, should end in `TypeProvider` or `TypeProviders`.</span></span> <span data-ttu-id="6d5dc-361">例如，使用一个 DLL 名称类似于以下字符串：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-361">For example, use a DLL name that resembles the following string:</span></span>

```
  Fabrikam.Management.BasicTypeProviders.dll
```

<span data-ttu-id="6d5dc-362">请确保你提供的类型成员的相应的命名空间，并指示你实现的连接协议：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-362">Ensure that your provided types are members of the corresponding namespace, and indicate the connectivity protocol that you implemented:</span></span>
...
<span data-ttu-id="6d5dc-363">**对于常规编码的实用程序提供程序**。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-363">**Utility Providers for General Coding**.</span></span>  <span data-ttu-id="6d5dc-364">对于的实用程序类型提供程序，如正则表达式，类型提供程序可能有某个基库的一部分，如以下示例所示：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-364">For a utility type provider such as that for regular expressions, the type provider may be part of a base library, as the following example shows:</span></span>

```fsharp
  #r "Fabrikam.Core.Text.Utilities.dll"
```

<span data-ttu-id="6d5dc-365">在这种情况下，所提供的类型将出现在合适的点根据普通.NET 设计约定：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-365">In this case, the provided type would appear at an appropriate point according to normal .NET design conventions:</span></span>
...
<span data-ttu-id="6d5dc-366">**单一实例数据源**。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-366">**Singleton Data Sources**.</span></span> <span data-ttu-id="6d5dc-367">某些类型提供程序连接到单个的专用的数据源，并仅提供数据。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-367">Some type providers connect to a single dedicated data source and provide only data.</span></span> <span data-ttu-id="6d5dc-368">在这种情况下，应删除`TypeProvider`后缀并使用正常的约定为.NET 命名：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-368">In this case, you should drop the `TypeProvider` suffix and use normal conventions for .NET naming:</span></span>
...
<span data-ttu-id="6d5dc-369">有关详细信息，请参阅`GetConnection`设计本主题后面所述的约定。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-369">For more information, see the `GetConnection` design convention that's described later in this topic.</span></span>

### <a name="design-patterns-for-type-providers"></a><span data-ttu-id="6d5dc-370">类型提供程序的设计模式</span><span class="sxs-lookup"><span data-stu-id="6d5dc-370">Design Patterns for Type Providers</span></span>

<span data-ttu-id="6d5dc-371">以下部分介绍创作类型提供程序时，可以使用的设计模式。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-371">The following sections describe design patterns you can use when authoring type providers.</span></span>

#### <a name="the-getconnection-design-pattern"></a><span data-ttu-id="6d5dc-372">GetConnection 设计模式</span><span class="sxs-lookup"><span data-stu-id="6d5dc-372">The GetConnection Design Pattern</span></span>

<span data-ttu-id="6d5dc-373">大多数类型提供程序应编写为使用`GetConnection`由 FSharp.Data.TypeProviders.dll 中的类型提供程序，如以下示例所示的模式：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-373">Most type providers should be written to use the `GetConnection` pattern that's used by the type providers in FSharp.Data.TypeProviders.dll, as the following example shows:</span></span>
...
#### <a name="type-providers-backed-by-remote-data-and-services"></a><span data-ttu-id="6d5dc-374">由远程数据和服务支持的类型提供程序</span><span class="sxs-lookup"><span data-stu-id="6d5dc-374">Type Providers Backed By Remote Data and Services</span></span>

<span data-ttu-id="6d5dc-375">创建由远程数据和服务提供支持的类型提供程序之前，必须考虑一系列连接的编程中固有的问题。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-375">Before you create a type provider that's backed by remote data and services, you must consider a range of issues that are inherent in connected programming.</span></span> <span data-ttu-id="6d5dc-376">这些问题包括以下注意事项：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-376">These issues include the following considerations:</span></span>

- <span data-ttu-id="6d5dc-377">架构映射</span><span class="sxs-lookup"><span data-stu-id="6d5dc-377">schema mapping</span></span>

- <span data-ttu-id="6d5dc-378">活跃度和出现架构更改时失效</span><span class="sxs-lookup"><span data-stu-id="6d5dc-378">liveness and invalidation in the presence of schema change</span></span>

- <span data-ttu-id="6d5dc-379">架构缓存</span><span class="sxs-lookup"><span data-stu-id="6d5dc-379">schema caching</span></span>

- <span data-ttu-id="6d5dc-380">数据访问操作的异步实现</span><span class="sxs-lookup"><span data-stu-id="6d5dc-380">asynchronous implementations of data access operations</span></span>

- <span data-ttu-id="6d5dc-381">支持的查询，包括 LINQ 查询</span><span class="sxs-lookup"><span data-stu-id="6d5dc-381">supporting queries, including LINQ queries</span></span>

- <span data-ttu-id="6d5dc-382">凭据和身份验证</span><span class="sxs-lookup"><span data-stu-id="6d5dc-382">credentials and authentication</span></span>

<span data-ttu-id="6d5dc-383">本主题不会介绍这些问题更多。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-383">This topic doesn't explore these issues further.</span></span>

### <a name="additional-authoring-techniques"></a><span data-ttu-id="6d5dc-384">其他创作技术</span><span class="sxs-lookup"><span data-stu-id="6d5dc-384">Additional Authoring Techniques</span></span>

<span data-ttu-id="6d5dc-385">在编写你自己的类型提供程序时，你可能想要使用以下其他技术。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-385">When you write your own type providers, you might want to use the following additional techniques.</span></span>

### <a name="creating-types-and-members-on-demand"></a><span data-ttu-id="6d5dc-386">类型和成员通过按需创建</span><span class="sxs-lookup"><span data-stu-id="6d5dc-386">Creating Types and Members On-Demand</span></span>

<span data-ttu-id="6d5dc-387">ProvidedType API 已延迟 AddMember 的版本。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-387">The ProvidedType API has delayed versions of AddMember.</span></span>
...
<span data-ttu-id="6d5dc-388">这些版本用于创建按需空间的类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-388">These versions are used to create on-demand spaces of types.</span></span>

### <a name="providing-array-types-and-generic-type-instantiations"></a><span data-ttu-id="6d5dc-389">提供数组类型和泛型类型实例化</span><span class="sxs-lookup"><span data-stu-id="6d5dc-389">Providing Array types and Generic Type Instantiations</span></span>

<span data-ttu-id="6d5dc-390">通过使用普通的提供的成员 （其签名包含数组类型、 byref 类型和实例化的泛型类型） `MakeArrayType`， `MakePointerType`，并`MakeGenericType`上的任何实例<xref:System.Type>，其中包括`ProvidedTypeDefinitions`。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-390">You make provided members (whose signatures include array types, byref types, and instantiations of generic types) by using the normal `MakeArrayType`, `MakePointerType`, and `MakeGenericType` on any instance of <xref:System.Type>, including `ProvidedTypeDefinitions`.</span></span>

> [!NOTE]
> <span data-ttu-id="6d5dc-391">在某些情况下，您可能需要使用该帮助器`ProvidedTypeBuilder.MakeGenericType`。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-391">In some cases you may have to use the helper in `ProvidedTypeBuilder.MakeGenericType`.</span></span>  <span data-ttu-id="6d5dc-392">请参阅[类型提供程序 SDK 文档](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations)的更多详细信息。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-392">See the [Type Provider SDK documentation](https://github.com/fsprojects/FSharp.TypeProviders.SDK/blob/master/README.md#explicit-construction-of-code-makegenerictype-makegenericmethod-and-uncheckedquotations) for more details.</span></span>

### <a name="providing-unit-of-measure-annotations"></a><span data-ttu-id="6d5dc-393">提供的度量值批注的单元</span><span class="sxs-lookup"><span data-stu-id="6d5dc-393">Providing Unit of Measure Annotations</span></span>

<span data-ttu-id="6d5dc-394">ProvidedTypes API 提供帮助器提供度量值批注。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-394">The ProvidedTypes API provides helpers for providing measure annotations.</span></span> <span data-ttu-id="6d5dc-395">例如，若要提供的类型`float<kg>`，使用以下代码：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-395">For example, to provide the type `float<kg>`, use the following code:</span></span>
...
```
  ...
  <span data-ttu-id="6d5dc-396">若要提供的类型`Nullable<decimal<kg/m^2>>`，使用以下代码：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-396">To provide the type `Nullable<decimal<kg/m^2>>`, use the following code:</span></span>
...
### <a name="accessing-project-local-or-script-local-resources"></a><span data-ttu-id="6d5dc-397">访问项目的本地或脚本本地资源</span><span class="sxs-lookup"><span data-stu-id="6d5dc-397">Accessing Project-Local or Script-Local Resources</span></span>

<span data-ttu-id="6d5dc-398">类型提供程序的每个实例只能获取`TypeProviderConfig`在构造期间的值。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-398">Each instance of a type provider can be given a `TypeProviderConfig` value during construction.</span></span> <span data-ttu-id="6d5dc-399">此值包含"解决方法文件夹"的提供程序 （即，在编译或包含的脚本的目录的项目文件夹）、 引用程序集的列表和其他信息。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-399">This value contains the "resolution folder" for the provider (that is, the project folder for the compilation or the directory that contains a script), the list of referenced assemblies, and other information.</span></span>

### <a name="invalidation"></a><span data-ttu-id="6d5dc-400">失效</span><span class="sxs-lookup"><span data-stu-id="6d5dc-400">Invalidation</span></span>

<span data-ttu-id="6d5dc-401">提供程序可以引发失效信号来通知架构假设可能已更改的 F # 语言服务。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-401">Providers can raise invalidation signals to notify the F# language service that the schema assumptions may have changed.</span></span> <span data-ttu-id="6d5dc-402">失效时，如果提供程序托管在 Visual Studio 中，是重做一次。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-402">When invalidation occurs, a typecheck is redone if the provider is being hosted in Visual Studio.</span></span> <span data-ttu-id="6d5dc-403">当在 F # Interactive 中或通过 F # 编译器 (fsc.exe) 托管提供程序时，将忽略此信号。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-403">This signal will be ignored when the provider is hosted in F# Interactive or by the F# Compiler (fsc.exe).</span></span>

### <a name="caching-schema-information"></a><span data-ttu-id="6d5dc-404">缓存的架构信息</span><span class="sxs-lookup"><span data-stu-id="6d5dc-404">Caching Schema Information</span></span>

<span data-ttu-id="6d5dc-405">提供程序通常必须缓存对架构信息的访问。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-405">Providers must often cache access to schema information.</span></span> <span data-ttu-id="6d5dc-406">使用给定的文件名称的静态参数或作为用户数据应存储缓存的数据。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-406">The cached data should be stored by using a file name that's given as a static parameter or as user data.</span></span> <span data-ttu-id="6d5dc-407">架构缓存的一个示例是`LocalSchemaFile`中的类型提供程序中的参数`FSharp.Data.TypeProviders`程序集。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-407">An example of schema caching is the `LocalSchemaFile` parameter in the type providers in the `FSharp.Data.TypeProviders` assembly.</span></span> <span data-ttu-id="6d5dc-408">在这些提供程序实现中，此静态参数指示要使用指定的本地文件而不是通过网络访问数据源中的架构信息的类型提供程序。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-408">In the implementation of these providers, this static parameter directs the type provider to use the schema information in the specified local file instead of accessing the data source over the network.</span></span> <span data-ttu-id="6d5dc-409">若要使用缓存的架构信息，还必须设置的静态参数`ForceUpdate`到`false`。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-409">To use cached schema information, you must also set the static parameter `ForceUpdate` to `false`.</span></span> <span data-ttu-id="6d5dc-410">可以使用类似的技术来启用联机和脱机数据访问。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-410">You could use a similar technique to enable online and offline data access.</span></span>

### <a name="backing-assembly"></a><span data-ttu-id="6d5dc-411">后备程序集</span><span class="sxs-lookup"><span data-stu-id="6d5dc-411">Backing Assembly</span></span>

<span data-ttu-id="6d5dc-412">在编译时`.dll`或`.exe`文件，支持.dll 文件生成的类型以静态方式链接到生成的程序集。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-412">When you compile a `.dll` or `.exe` file, the backing .dll file for generated types is statically linked into the resulting assembly.</span></span> <span data-ttu-id="6d5dc-413">通过将中间语言 (IL) 类型定义和任何托管的资源复制到最终的程序集的后备程序集从创建此链接。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-413">This link is created by copying the Intermediate Language (IL) type definitions and any managed resources from the backing assembly into the final assembly.</span></span> <span data-ttu-id="6d5dc-414">当您使用 F # Interactive 时，支持.dll 文件不会复制和改为直接加载到 F # 交互式过程。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-414">When you use F# Interactive, the backing .dll file isn't copied and is instead loaded directly into the F# Interactive process.</span></span>

### <a name="exceptions-and-diagnostics-from-type-providers"></a><span data-ttu-id="6d5dc-415">异常和来自类型提供程序的诊断</span><span class="sxs-lookup"><span data-stu-id="6d5dc-415">Exceptions and Diagnostics from Type Providers</span></span>

<span data-ttu-id="6d5dc-416">使用提供的类型中的所有成员的所有可能会引发异常。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-416">All uses of all members from provided types may throw exceptions.</span></span> <span data-ttu-id="6d5dc-417">在所有情况下，如果类型提供程序引发异常，宿主编译器错误向属性的特定类型提供程序。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-417">In all cases, if a type provider throws an exception, the host compiler attributes the error to a specific type provider.</span></span>

- <span data-ttu-id="6d5dc-418">类型提供程序异常应永远不会导致内部编译器错误。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-418">Type provider exceptions should never result in internal compiler errors.</span></span>

- <span data-ttu-id="6d5dc-419">类型提供程序无法将报告警告。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-419">Type providers can't report warnings.</span></span>

- <span data-ttu-id="6d5dc-420">当类型提供程序托管在 F # 编译器、 F # 开发环境中，或 F # Interactive 中时，从该提供程序的所有异常被都捕获。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-420">When a type provider is hosted in the F# compiler, an F# development environment, or F# Interactive, all exceptions from that provider are caught.</span></span> <span data-ttu-id="6d5dc-421">消息属性始终为错误文本，并没有堆栈跟踪显示。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-421">The Message property is always the error text, and no stack trace appears.</span></span> <span data-ttu-id="6d5dc-422">如果您要引发异常，可能会引发下面的示例： `System.NotSupportedException`， `System.IO.IOException`， `System.Exception`。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-422">If you’re going to throw an exception, you can throw the following examples: `System.NotSupportedException`, `System.IO.IOException`, `System.Exception`.</span></span>

#### <a name="providing-generated-types"></a><span data-ttu-id="6d5dc-423">提供生成的类型</span><span class="sxs-lookup"><span data-stu-id="6d5dc-423">Providing Generated Types</span></span>

<span data-ttu-id="6d5dc-424">到目前为止，本文档介绍了如何提供已清除的类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-424">So far, this document has explained how to provide erased types.</span></span> <span data-ttu-id="6d5dc-425">您可以在 F # 中使用类型提供程序机制提供生成添加到用户的程序的实际.NET 类型定义为的类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-425">You can also use the type provider mechanism in F# to provide generated types, which are added as real .NET type definitions into the users' program.</span></span> <span data-ttu-id="6d5dc-426">您必须引用生成提供的使用类型定义的类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-426">You must refer to generated provided types by using a type definition.</span></span>
...
<span data-ttu-id="6d5dc-427">ProvidedTypes 0.2 帮助器代码是 F # 3.0 发行版的一部分，仅提供有限支持用于提供生成的类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-427">The ProvidedTypes-0.2 helper code that is part of the F# 3.0 release has only limited support for providing generated types.</span></span> <span data-ttu-id="6d5dc-428">下面的语句必须是生成的类型定义，则返回 true:</span><span class="sxs-lookup"><span data-stu-id="6d5dc-428">The following statements must be true for a generated type definition:</span></span>

- <span data-ttu-id="6d5dc-429">`isErased` 必须设置为`false`。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-429">`isErased` must be set to `false`.</span></span>

- <span data-ttu-id="6d5dc-430">生成的类型必须添加到新构造`ProvidedAssembly()`，它表示生成的代码片段的容器。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-430">The generated type must be added to a newly constructed `ProvidedAssembly()`, which represents a container for generated code fragments.</span></span>

- <span data-ttu-id="6d5dc-431">提供程序必须具有匹配的.dll 文件在磁盘上的实际后备.NET.dll 文件的程序集。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-431">The provider must have an assembly that has an actual backing .NET .dll file with a matching .dll file on disk.</span></span>

## <a name="rules-and-limitations"></a><span data-ttu-id="6d5dc-432">规则和限制</span><span class="sxs-lookup"><span data-stu-id="6d5dc-432">Rules and Limitations</span></span>

<span data-ttu-id="6d5dc-433">当您编写类型提供程序时，请记住以下规则和限制。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-433">When you write type providers, keep the following rules and limitations in mind.</span></span>

### <a name="provided-types-must-be-reachable"></a><span data-ttu-id="6d5dc-434">提供的类型必须是可访问</span><span class="sxs-lookup"><span data-stu-id="6d5dc-434">Provided types must be reachable</span></span>

<span data-ttu-id="6d5dc-435">所有提供的类型应该是可从非嵌套类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-435">All provided types should be reachable from the non-nested types.</span></span> <span data-ttu-id="6d5dc-436">对的调用中将给出非嵌套类型`TypeProviderForNamespaces`构造函数或调用`AddNamespace`。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-436">The non-nested types are given in the call to the `TypeProviderForNamespaces` constructor or a call to `AddNamespace`.</span></span> <span data-ttu-id="6d5dc-437">例如，如果提供程序提供了一种类型`StaticClass.P : T`，必须确保 T 是非嵌套类型或嵌套在下一个。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-437">For example, if the provider provides a type `StaticClass.P : T`, you must ensure that T is either a non-nested type or nested under one.</span></span>

<span data-ttu-id="6d5dc-438">例如，对于某些提供程序有一个静态类，如`DataTypes`，包含这些`T1, T2, T3, ...`类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-438">For example, some providers have a static class such as `DataTypes` that contain these `T1, T2, T3, ...` types.</span></span> <span data-ttu-id="6d5dc-439">否则，该错误指出找对程序集 A 中的 T 类型的引用，但该程序集中找不到类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-439">Otherwise, the error says that a reference to type T in assembly A was found, but the type couldn't be found in that assembly.</span></span> <span data-ttu-id="6d5dc-440">如果出现此错误，请验证可从提供程序类型访问所有的子类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-440">If this error appears, verify that all your subtypes can be reached from the provider types.</span></span> <span data-ttu-id="6d5dc-441">注意： 这些`T1, T2, T3...`类型被称为*实时上*类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-441">Note: These `T1, T2, T3...` types are referred to as the *on-the-fly* types.</span></span> <span data-ttu-id="6d5dc-442">请记住将其放在可访问的命名空间或父类型。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-442">Remember to put them in an accessible namespace or a parent type.</span></span>

### <a name="limitations-of-the-type-provider-mechanism"></a><span data-ttu-id="6d5dc-443">类型提供程序机制的限制</span><span class="sxs-lookup"><span data-stu-id="6d5dc-443">Limitations of the Type Provider Mechanism</span></span>

<span data-ttu-id="6d5dc-444">F # 中的类型提供程序机制具有以下限制：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-444">The type provider mechanism in F# has the following limitations:</span></span>

- <span data-ttu-id="6d5dc-445">在 F # 类型提供程序的底层基础结构不支持提供的泛型类型或提供泛型方法。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-445">The underlying infrastructure for type providers in F# doesn't support provided generic types or provided generic methods.</span></span>

- <span data-ttu-id="6d5dc-446">该机制不支持嵌套的类型的静态参数。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-446">The mechanism doesn't support nested types with static parameters.</span></span>

## <a name="development-tips"></a><span data-ttu-id="6d5dc-447">开发提示</span><span class="sxs-lookup"><span data-stu-id="6d5dc-447">Development Tips</span></span>

<span data-ttu-id="6d5dc-448">您可能会发现以下提示有助于开发过程中：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-448">You might find the following tips helpful during the development process:</span></span>

### <a name="run-two-instances-of-visual-studio"></a><span data-ttu-id="6d5dc-449">运行 Visual Studio 的两个实例</span><span class="sxs-lookup"><span data-stu-id="6d5dc-449">Run two instances of Visual Studio</span></span>

<span data-ttu-id="6d5dc-450">可以在一个实例中开发类型提供程序和其他测试提供程序，因为测试 IDE 将获取一个锁可防止类型提供程序正在重新生成的.dll 文件。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-450">You can develop the type provider in one instance and test the provider in the other because the test IDE will take a lock on the .dll file that prevents the type provider from being rebuilt.</span></span> <span data-ttu-id="6d5dc-451">因此，您必须关闭 Visual Studio 的第二个实例时提供程序生成中的第一个实例，并生成提供程序后然后必须重新打开第二个实例。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-451">Thus, you must close the second instance of Visual Studio while the provider is built in the first instance, and then you must reopen the second instance after the provider is built.</span></span>

### <a name="debug-type-providers-by-using-invocations-of-fscexe"></a><span data-ttu-id="6d5dc-452">使用调用 fsc.exe 调试类型提供程序</span><span class="sxs-lookup"><span data-stu-id="6d5dc-452">Debug type providers by using invocations of fsc.exe</span></span>

<span data-ttu-id="6d5dc-453">可以使用以下工具来调用类型提供程序：</span><span class="sxs-lookup"><span data-stu-id="6d5dc-453">You can invoke type providers by using the following tools:</span></span>

- <span data-ttu-id="6d5dc-454">fsc.exe （命令行编译器的 F #）</span><span class="sxs-lookup"><span data-stu-id="6d5dc-454">fsc.exe (The F# command line compiler)</span></span>

- <span data-ttu-id="6d5dc-455">fsi.exe （F # 交互式编译器）</span><span class="sxs-lookup"><span data-stu-id="6d5dc-455">fsi.exe (The F# Interactive compiler)</span></span>

- <span data-ttu-id="6d5dc-456">devenv.exe (Visual Studio)</span><span class="sxs-lookup"><span data-stu-id="6d5dc-456">devenv.exe (Visual Studio)</span></span>

<span data-ttu-id="6d5dc-457">通常可以通过在测试脚本文件 (例如，script.fsx) 上使用 fsc.exe 非常轻松地调试类型提供程序。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-457">You can often debug type providers most easily by using fsc.exe on a test script file (for example, script.fsx).</span></span> <span data-ttu-id="6d5dc-458">您可以启动命令提示符中的调试器。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-458">You can launch a debugger from a command prompt.</span></span>

```
  devenv /debugexe fsc.exe script.fsx
```

  <span data-ttu-id="6d5dc-459">可以使用打印到 stdout 日志记录。</span><span class="sxs-lookup"><span data-stu-id="6d5dc-459">You can use print-to-stdout logging.</span></span>

## <a name="see-also"></a><span data-ttu-id="6d5dc-460">请参阅</span><span class="sxs-lookup"><span data-stu-id="6d5dc-460">See also</span></span>

- [<span data-ttu-id="6d5dc-461">类型提供程序</span><span class="sxs-lookup"><span data-stu-id="6d5dc-461">Type Providers</span></span>](index.md)
- [<span data-ttu-id="6d5dc-462">类型提供程序 SDK</span><span class="sxs-lookup"><span data-stu-id="6d5dc-462">The Type Provider SDK</span></span>](https://github.com/fsprojects/FSharp.TypeProviders.SDK)
