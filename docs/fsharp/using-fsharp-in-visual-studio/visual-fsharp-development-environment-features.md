...
|图标和清单|否|可通过编译器命令行选项。|
|ASP.NET 客户端服务|否||
|ClickOnce|否|如果适用，请在另一个.NET Framework 语言中，使用客户端项目。|
|强命名|否|可通过编译器命令行选项。|
|发布的程序集和版本管理|否||
|代码分析|否|手动或作为后期生成命令的一部分，可以运行代码分析工具。|
|安全 （更改信任级别）|否||
...
|自动设置格式|重新格式化使用标准的缩进和样式的代码。|否|
|书签|使用此选项可在编辑器中保存位置。|是|
|更改缩进|缩进或取消对选定的行。|是|
|[查找和替换文本](/visualstudio/ide/finding-and-replacing-text)|可以在文件、 项目或解决方案中，搜索并可能将更改文本。|是|
|转到.NET Framework API 的定义|当将光标置于.NET Framework API 时，将显示从.NET Framework 元数据生成的代码。|否|
...
|功能|描述|F # 中支持？|
|-------|-----------|----------------|
|自动实现接口|生成为接口方法的代码存根 （stub）。|否|
|代码片段|常见的编码构造的库中的代码注入主题。|否|
|完成单词|通过完成单词和名称，键入时键入的节省。|是|
|使用第一个完成模式|启用时，可使 word 完成功能选择第一个匹配项，当你键入时，而不是等待你选择一个或按**CTRL + SPACE**。|否|
|生成代码元素|使您能够生成各种构造的存根 （stub） 代码。|否|
|列表成员|当你键入成员访问运算符 （.） 时，显示类型的成员。|是|
|组织 Using / 打开|将引用的命名空间组织**使用**C# 中的语句或**打开**F # 中的指令。|否|
...
|编辑并继续|使代码可以修改和编译期间不必停止再重新启动调试器调试正在运行的程序。|否|
...
|工具|描述|F # 中支持？|
|----|-----------|----------------|
|调用层次结构|显示在代码中调用函数的嵌套的结构。|否|
|代码度量|收集有关你的代码，如行计数信息。|否|
|类视图|提供基于类型的项目中的代码视图。|否|
...
|属性窗口|显示并允许在具有焦点的开发环境中对象的属性的编辑。|是|
