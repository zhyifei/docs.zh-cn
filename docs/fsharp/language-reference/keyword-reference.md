...
|`downto`|[循环`for...to`表达式](loops-for-to-expression.md)|`for`在表达式中, 在反向计数时使用。|
...
|`function`|[match 表达式](match-expressions.md)<br /><br />[Lambda 表达式:趣味关键字](./functions/lambda-expressions-the-fun-keyword.md)|用作`fun`关键字的更短替代`match` , 以及 lambda 表达式中在单个参数上具有模式匹配的表达式。|
...
|`val`|[显式字段：`val` 关键字](./members/explicit-fields-the-val-keyword.md)<br /><br />[签名](signatures.md)<br /><br />[成员](./members/index.md)|在有限的情况下, 在签名中用于指示值或声明成员的类型。|
