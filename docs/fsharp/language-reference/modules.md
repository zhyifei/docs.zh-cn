...
F#模块是代码构造的分组F# , 如类型、值、函数值和绑定中`do`的代码。 它实现为只有静态成员的公共语言运行时 (CLR) 类。 模块声明有两种类型, 具体取决于整个文件是否包含在模块中: 顶级模块声明和局部模块声明。 顶级模块声明在模块中包含整个文件。 顶级模块声明只能显示为文件中的第一个声明。
...
如果文件中有多个模块, 则必须对每个模块使用本地模块声明。 如果声明了封闭命名空间, 则这些模块是封闭命名空间的一部分。 如果未声明封闭命名空间, 模块将成为隐式创建的顶级模块的一部分。 下面的代码示例显示了一个包含多个模块的代码文件。 编译器隐式创建一个名为`Multiplemodules` `MyModule1` 、 `MyModule2`和的顶级模块, 它们嵌套在该顶级模块中。
...
您可以打开该模块或一个或多个命名空间来简化代码。 有关打开命名空间和模块的详细信息, [请参阅导入声明:`open`关键字。](import-declarations-the-open-keyword.md)
...
F#4.1 引入了模块的概念, 允许所有包含的代码相互递归。  这是通过`module rec`实现的。  `module rec`使用可以减少无法在类型和模块之间编写相互引用代码的一些难题。  下面是一个示例:
...
请注意, 异常`DontSqueezeTheBananaException`和类`Banana`都相互引用。  此外, 模块`BananaHelpers`和类`Banana`也相互引用。  如果从F# `rec` 模块中删除关键字,则无法在中进行表示。`RecursiveModule`
