...
## <a name="how-to-run-the-code-samples"></a>如何运行的代码示例

>[!NOTE]
两个选项运行的代码示例是[试用 F #](http://www.tryfsharp.org/Create) （需要 Silverlight） 和[用于 Azure 笔记本的 F #](https://notebooks.azure.com/Microsoft/libraries/fsharp/html/FSharp%20for%20Azure%20Notebooks.ipynb)在 Microsoft Azure 上。

若要运行这些代码示例的最快方法是使用[F # Interactive](tutorials/fsharp-interactive/index.md)。  只需复制/粘贴的代码示例并那里运行它们。  或者，你可以设置要编译并运行代码作为控制台应用程序的项目。  请参阅[开始](./get-started/index.md)部分以了解详细信息。
