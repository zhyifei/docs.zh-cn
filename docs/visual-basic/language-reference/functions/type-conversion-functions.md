...
|`CStr`|[String 数据类型](../../../visual-basic/language-reference/data-types/string-data-type.md)|`CStr` 的返回取决于 `expression` 参数。 请参阅[CStr 函数的返回值](../../../visual-basic/language-reference/functions/return-values-for-the-cstr-function.md)。|
...
从 Visual Basic 15.8 开始，当你将以下方法返回的 <xref:System.Single> 或 <xref:System.Double> 值传递给整数转换函数之一（`CByte`、`CShort`、`CInt`、`CLng`、`CSByte`、`CUShort`、`CUInt`、`CULng`）时，将优化浮点到整数转换的性能：
...
`CChar` 的输入参数的数据类型必须为 `Char` 或 `String`。 不能使用 `CChar` 将数字转换为字符，因为 `CChar` 无法接受数字数据类型。 下面的示例获取一个表示码位（字符代码）的数字，并将其转换为相应的字符。 它使用 <xref:Microsoft.VisualBasic.Interaction.InputBox%2A> 函数获取数字的字符串，`CInt` 将字符串转换为类型 `Integer`，并使用 `ChrW` 将数字转换为类型 `Char`。
...
下面的示例使用 `CObj` 函数将数值转换为 `Object`。 `Object` 变量本身只包含一个四字节指针，该指针指向分配给它的 `Double` 值。
