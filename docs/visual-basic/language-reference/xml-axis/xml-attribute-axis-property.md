...
## <a name="parts"></a>部件  
 `object`  
 必需。 一个 <xref:System.Xml.Linq.XElement> 对象或 <xref:System.Xml.Linq.XElement> 对象的集合。  
...
|`name`|必需。 本地属性名称。 请参阅已[声明的 XML 元素和属性的名称](../../../visual-basic/programming-guide/language-features/xml/names-of-declared-xml-elements-and-attributes.md)。|  
...
## <a name="example"></a>示例  
 下面的示例演示如何在 XML 中以声明方式同时创建 XML 元素的特性，并通过将特性添加到 <xref:System.Xml.Linq.XElement> 对象的实例来动态创建属性。 `type` 属性以声明方式创建，并动态创建 `owner` 属性。  
