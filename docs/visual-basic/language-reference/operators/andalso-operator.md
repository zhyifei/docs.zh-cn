...
## <a name="data-types"></a>数据类型  
 `AndAlso`仅对定义运算符[布尔数据类型](../../../visual-basic/language-reference/data-types/boolean-data-type.md)。 Visual Basic 将根据需要向每个操作数转换`Boolean`之前对表达式求值。 如果将结果分配到数值类型，Visual Basic 会将其转换从`Boolean`为该类型，以便`False`变得`0`和`True`变得`-1`。
