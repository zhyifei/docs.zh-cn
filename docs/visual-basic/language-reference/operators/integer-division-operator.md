...
## <a name="result"></a>结果  
 结果是 @no__t 的整数商除以 `expression2`，这会丢弃所有余数，只保留整数部分。 这称为 "*截断*"。  
...
## <a name="remarks"></a>备注  
 在执行除法运算之前，Visual Basic 会尝试将任何浮点数值表达式转换为 `Long`。 如果 `Option Strict` @no__t 为-1，则会发生编译器错误。 如果 `Option Strict` @no__t 为-1，则当值超出[Long 数据类型](../../../visual-basic/language-reference/data-types/long-data-type.md)的范围时，可能会出现 @no__t。 转换为 `Long` 也服从于*银行家的舍入*。 有关详细信息，请参阅[类型转换函数](../../../visual-basic/language-reference/functions/type-conversion-functions.md)中的 "小数部分"。  
...
## <a name="attempted-division-by-zero"></a>尝试被零除  
 如果 `expression2` 的计算结果为零，则 `\` 运算符将引发 @no__t 2 异常。 对于操作数的所有数值数据类型都是如此。  
...
## <a name="example"></a>示例  
 下面的示例使用 @no__t 0 运算符来执行整数除法。 结果是一个整数，表示两个操作数的整数商，余数被丢弃。  
