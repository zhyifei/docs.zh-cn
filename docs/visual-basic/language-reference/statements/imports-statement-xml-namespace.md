...
Xml 文本（称为*本地 xml 命名空间*）中定义的 xml 命名空间优先于由 `Imports` 语句定义为 GLOBAL 的 xml 命名空间。 `Imports` 语句定义的 XML 命名空间优先于为 Visual Basic 项目导入的 XML 命名空间。 如果 XML 文本定义了 XML 命名空间，则该本地命名空间不适用于嵌入的表达式。
