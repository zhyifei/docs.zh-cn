...
|术语|Definition|  
|---|---|  
|`condition`|必需。 `Boolean` 表达式。 如果 `Nothing``condition`，Visual Basic 将其视为 `False`。|  
|`statements`|可选。 `While`后面的一个或多个语句，在每次 `True``condition` 时运行。|  
...
> "Do ..." 中也使用了 `While` 关键字[Loop 语句](../../../visual-basic/language-reference/statements/do-loop-statement.md)、 [Skip while 子句](../../../visual-basic/language-reference/queries/skip-while-clause.md)和[Take while 子句](../../../visual-basic/language-reference/queries/take-while-clause.md)。  
 ...
 `condition` 通常是对两个值进行比较导致的，但它可以是计算结果为[布尔数据类型](../../../visual-basic/language-reference/data-types/boolean-data-type.md)值（`True` 或 `False`）的任何表达式。 此表达式可以包含已转换为 `Boolean`的另一种数据类型的值，如数值类型。  
...
## <a name="example"></a>示例  
 下面的示例读取文本文件中的所有行。 <xref:System.IO.File.OpenText%2A> 方法将打开文件并返回读取字符的 <xref:System.IO.StreamReader>。 在 `While` 条件下，`StreamReader` 的 <xref:System.IO.StreamReader.Peek%2A> 方法决定文件是否包含其他字符。  
