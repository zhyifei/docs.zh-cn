...
- `attributelist`
  ...
  @No__t_0 特性指示枚举实例的值可以包含多个枚举成员，并且每个成员表示枚举值中的一个位域。
...
- `enumerationname`
  ...
  必须的。 枚举的名称。 有关有效名称的信息，请参阅已[声明的元素名称](../../../visual-basic/programming-guide/language-features/declared-elements/declared-element-names.md)。
...
- `memberlist`
  ...
  必须的。 在此语句中声明的成员常量的列表。 多个成员出现在单独的源代码行上。
...
@No__t_0 语句可以声明枚举的数据类型。 每个成员都采用枚举的数据类型。 可以指定 `Byte`、`Integer`、`Long`、`SByte`、`Short`、`UInteger`、`ULong` 或 `UShort`。
...
@No__t_0 语句可以初始化 `memberlist` 中所选成员的内容。 使用 `initializer` 提供要分配给成员的表达式。
...
枚举通常表示互斥的值。 通过在 `Enum` 声明中包含 <xref:System.FlagsAttribute> 特性，可以将多个值分配给枚举的实例。 @No__t_0 特性指定将枚举视为位域（即一组标志）。 它们称为*按位*枚举。
...
下面的示例演示如何使用按位枚举。 可以将多个值分配给位枚举的实例。 @No__t_0 声明包括 <xref:System.FlagsAttribute> 属性，该属性指示可将枚举视为一组标志。
