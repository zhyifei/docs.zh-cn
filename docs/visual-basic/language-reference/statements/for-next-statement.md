...
|`datatype`|可选。 Data type of `counter`. For more information, see [Counter Argument](#BKMK_Counter) later in this topic.|
|`start`|必须的。 Numeric expression. `counter` 的初始值。|
|`end`|必须的。 Numeric expression. The final value of `counter`.|
|`step`|可选。 Numeric expression. The amount by which `counter` is incremented each time through the loop.|
|`statements`|可选。 One or more statements between `For` and `Next` that run the specified number of times.|
|`Continue For`|可选。 Transfers control to the next loop iteration.|
|`Exit For`|可选。 Transfers control out of the `For` loop.|
|`Next`|必须的。 Terminates the definition of the `For` loop.|
...
|----------------------------|-----------------------------------|-------------------------------------------------------------------------------------------------------------|
|No|是|No, because `counter` is already defined. If the scope of `counter` isn't local to the procedure, a compile-time warning occurs.|
...
|是|是|Yes, but only if the existing `counter` variable is defined outside the procedure. That variable remains separate. If the scope of the existing `counter` variable is local to the procedure, a compile-time error occurs.|
|是|No|可以。|
