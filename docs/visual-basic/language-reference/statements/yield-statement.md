...
|术语|定义|  
|---|---|  
|`expression`|必须的。 可隐式转换为迭代器函数或包含 `Yield` 语句 `Get` 访问器的类型的表达式。|  
  
## <a name="remarks"></a>备注  
 @No__t_0 语句一次返回集合中的一个元素。 @No__t_0 语句包含在迭代器函数或 `Get` 访问器中，后者对集合执行自定义迭代。  
  
 使用 For Each ... 的方法使用迭代器函数[Next 语句](../../../visual-basic/language-reference/statements/for-each-next-statement.md)或 LINQ 查询。 @No__t_0 循环的每次迭代都会调用迭代器函数。 在迭代器函数中到达 `Yield` 语句时，将返回 `expression`，并且保留代码中的当前位置。 下次调用迭代器函数时，将从该位置重新开始执行。  
...
## <a name="exception-handling"></a>异常处理  
 @No__t_0 语句可以在 Try ... 的 `Try` 块内[Catch 。Finally 语句](../../../visual-basic/language-reference/statements/try-catch-finally-statement.md)。 具有 `Yield` 语句的 `Try` 块可以具有 `Catch` 块，并且可以具有 `Finally` 块。  
...
```  
 ...
 在 `For Each` 循环迭代时，将为 <xref:System.Collections.IEnumerator.MoveNext%2A> 调用 `elements` 方法。 此调用将执行 `MyIteratorFunction` 的主体，直至到达下一个 `Yield` 语句。 @No__t_0 语句返回一个表达式，该表达式不仅确定了循环体的使用 `element` 变量的值，还确定了元素的 <xref:System.Collections.Generic.IEnumerator%601.Current%2A> 属性，这是一个 `IEnumerable (Of String)`。  
...
## <a name="example"></a>示例  
 下面的示例 `Yield` 包含一个位于[For 。下一个](../../../visual-basic/language-reference/statements/for-next-statement.md)循环。 @No__t_1 中[每](../../../visual-basic/language-reference/statements/for-each-next-statement.md)个语句体的每次迭代都会创建对 `Power` 迭代器函数的调用。 对迭代器函数的每个调用将继续到 `Yield` 语句的下一次执行（在 `For…Next` 循环的下一次迭代期间发生）。  
