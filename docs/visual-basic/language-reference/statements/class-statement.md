...
## <a name="remarks"></a>备注  
 @No__t-0 语句定义一种新的数据类型。 *类*是面向对象编程（OOP）的基本构建基块。 有关详细信息，请参阅[对象和类](../../../visual-basic/programming-guide/language-features/objects-and-classes/index.md)。  
 ...
 类的每个实例都具有独立于所有其他实例的生存期。 此生存期的开始时间是由新的[Operator](../../../visual-basic/language-reference/operators/new-operator.md)子句或函数（如 <xref:Microsoft.VisualBasic.Interaction.CreateObject%2A>）创建的。 当指向实例的所有变量均设置为[Nothing](../../../visual-basic/language-reference/nothing.md)或其他类的实例时，它将结束。  
...
- **继承。** 如果类使用[Inherits 语句](../../../visual-basic/language-reference/statements/inherits-statement.md)，则只能指定一个基类或接口。 类不能从多个元素继承。  
     ...
     类不能继承自具有限制性更强的访问级别的另一个类。 例如，@no__t 0 类不能继承自 @no__t 类。  
...
- @no__t 0Object 生存期：如何创建和销毁对象 @ no__t-0
