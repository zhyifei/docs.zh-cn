...
|`send`|如果发生内部编译器错误，并且计算机和用户策略设置允许，则编译器会将数据发送给 Microsoft。<br /><br /> 如果[Windows 错误报告](/windows/desktop/wer/windows-error-reporting)系统设置启用了报告功能，则该选项 `-errorreport:send` 会尝试自动向 Microsoft 发送错误信息。 |
