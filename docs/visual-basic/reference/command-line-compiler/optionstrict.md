...
## <a name="arguments"></a>参数  
 `+` &#124; `-`  
 可选。 @No__t-0 选项限制隐式类型转换。 此选项的默认值为 `-optionstrict-`。 @No__t-0 选项与 `-optionstrict` 相同。 可对许可类型语义使用这两者。  
  
 `custom`  
 必需。 不遵守严格语言语义时发出警告。  
  
## <a name="remarks"></a>备注  
 如果 `-optionstrict+` 有效，则只能隐式进行扩大类型转换。 隐式收缩类型转换（例如，将 @no__t 0 类型的对象赋给整数类型对象）会被报告为错误。  
  
 若要为隐式收缩类型转换生成警告，请使用 `-optionstrict:custom`。 使用 `-nowarn:numberlist` 可忽略特定警告，`-warnaserror:numberlist` 可将特定的警告视为错误。  
