...
构造函数和析构函数控制对象的创建和析构。 中`Sub New`的`Sub Finalize`和过程 Visual Basic 初始化和销毁对象; 它们替换 Visual Basic `Class_Initialize` 6.0 `Class_Terminate`及更早版本中使用的和方法。
...
创建派生类的实例时，首先执行基类的 `Sub New` 构造函数，再执行派生类中的构造函数。 之所以发生此情况，原因在于 `Sub New` 构造函数中代码的首行使用语法 `MyBase.New()` 调用类层级中直接位于其上方的类的构造函数。 然后, 为类层次结构中的每个类调用构造函数,直到达到基类的构造函数。`Sub New` 此时，先执行基类的构造函数中的代码，再执行所有的派生类中每个构造函数内的代码，最后执行大多数派生类中的代码。
