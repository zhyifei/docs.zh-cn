...
-   不同于允许多个继承的语言[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]允许仅单一继承类中有; 即，派生的类可以具有只有一个基类。 虽然类中不允许多个继承，类可以实现多个接口，可以有效地实现相同目的。  
...
## <a name="inheritance-modifiers"></a>继承修饰符  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]引入了以下类级语句和修饰符以支持继承：  
