...
## <a name="passing-by-value"></a>按值传递  
 传递了参数*按值*通过指定[ByVal](../../../../visual-basic/language-reference/modifiers/byval.md)过程定义中的相应参数的关键字。 当你使用此传递机制，[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]将基础的编程元素的值复制到过程中的本地变量。 过程代码中调用代码没有任何对基础元素的访问。  
  
## <a name="passing-by-reference"></a>通过引用传递  
 传递了参数*通过引用*通过指定[ByRef](../../../../visual-basic/language-reference/modifiers/byref.md)过程定义中的相应参数的关键字。 当你使用此传递机制，[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]使过程基础的编程元素的直接引用中调用代码。  
  
## <a name="passing-mechanism-and-element-type"></a>传递机制和元素类型  
 所选的传递机制不相同的基础的元素类型的分类。 按值或按引用传递是指什么[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]提供给该过程代码。 值类型或引用类型引用的编程元素在内存中的存储方式。  
