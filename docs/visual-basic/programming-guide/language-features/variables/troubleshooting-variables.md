...
## <a name="wrong-variable-being-used"></a>正在使用的错误变量  
 当你有多个变量具有相同的名称时，Visual Basic 编译器将尝试解析该名称对每个引用。 如果变量具有不同的范围，编译器将解析对范围最小的声明的引用。 如果它们具有相同的范围，解析将失败，编译器将引发错误。 有关详细信息，请参阅 [References to Declared Elements](../../../../visual-basic/programming-guide/language-features/declared-elements/references-to-declared-elements.md)。  
