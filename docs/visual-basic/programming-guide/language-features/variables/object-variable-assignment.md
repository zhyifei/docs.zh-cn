...
可以通过使用[New](../../../../visual-basic/language-reference/operators/new-operator.md)关键字在声明中包含初始化。 以下声明语句 `testUri` 和 `ver` 中声明对象变量，并将特定对象分配给它们。 每个都使用适当类的重载构造函数之一来初始化对象。
...
将对象变量设置为 `Nothing` 会使变量与任何特定对象的关联停止。 这样可以防止意外更改变量来更改对象。 它还允许您测试对象变量是否指向有效对象，如下面的示例所示。
