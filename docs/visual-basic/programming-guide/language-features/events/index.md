...
### <a name="raising-events"></a>引发事件  
 事件类似于消息，指示某重要事件已发生。 广播消息的行为称为*引发*事件。 在 Visual Basic 中，将用 `RaiseEvent` 语句引发事件，如以下示例中所示：  
 ...
 *事件处理程序*是在相应事件发生时调用的过程。 可以将签名一致的任意有效子例程用作事件处理程序。 不过，不能将函数用作事件处理程序，因为它不能向事件源返回值。  
 ...
 Visual Basic 对事件处理程序使用标准命名约定，这些事件处理程序将事件发送方的名称、下划线和事件的名称组合在一起。 例如，`Click` 按钮的 `button1` 事件将命名为 `Sub button1_Click`。  
