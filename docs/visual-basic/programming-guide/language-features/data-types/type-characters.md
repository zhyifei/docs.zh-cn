...
|数值，没有小数部分，比 `Integer`更大|`Long`|`2147483648`|  
|数值，有小数部分|`Double`|`1.2`|  
|括在双引号内|`String`|`"A"`|  
...
对于以下类型没有文本类型字符：`Boolean`， `Byte`， `Date`， `Object`， `SByte`，或`String`数据类型或任何复合数据类型，例如数组或结构。
...
从 Visual Basic 2017 开始，可以使用下划线字符 (`_`) 作为组分隔符，以增强整型文本的可读性。 下面的示例使用`_`以便将二进制文本分组到 8 位组：
...
从 Visual Basic 15.5 开始，您还可以使用下划线字符 (`_`) 作为前缀和十六进制、 二进制或八进制数字之间的前导分隔符。 例如：
