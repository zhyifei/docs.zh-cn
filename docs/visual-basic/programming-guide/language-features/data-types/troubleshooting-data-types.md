...
@No__t_2 命名空间中 <xref:System.Convert> 类的 <xref:System.Convert.ToInt32%2A> 方法将 `True` 转换为 + 1。
...
@No__t_0 数据类型不会扩大到[Char 数据类型](../../../../visual-basic/language-reference/data-types/char-data-type.md)。 这意味着，如果要将文本分配给 `Char` 变量，则必须进行收缩转换或强制文本 `Char` 类型。
