...
类型转换的一个重要的注意事项是转换的结果是否在目标数据类型的范围内。  
 ...
 A*扩大转换*将值更改为数据类型，可允许的任何可能的值的原始数据。  扩大转换保留源值，但可以更改它的表示形式。 发生这种情况是如果你转换到整型转换`Decimal`，或从`Char`到`String`。  
  
 *“收缩转换”* 将值更改为可能无法保存某些可能值的数据类型。 例如，小数部分的值将舍入为整数类型、 和转换为数字类型转换时`Boolean`缩减为`True`或`False`。  
  
## <a name="widening-conversions"></a>扩大转换  
 下表显示了标准的扩大转换。  
  
|数据类型|为数据类型扩大<sup>1</sup>|  
|---|---|  
...
|[Nothing](../../../../visual-basic/language-reference/nothing.md)|任何数据类型或对象类型。|  
 ...
 <sup>1</sup>按照定义，每个数据类型扩大到自身。  
  
 <sup>2</sup>从转换`Integer`， `UInteger`， `Long`， `ULong`，或`Decimal`到`Single`或`Double`可能导致丢失精度，但永远不会导致丢失量值。 在这个意义上它们不会产生信息丢失。  
  
 <sup>3</sup>看起来有点令人惊讶从派生类型及其基类型之一的转换扩大转换。 原因是派生的类型包含的基类型的所有成员，因此它有资格作为基类型的实例。 按相反方向的基类型不包含由派生类型定义的任何新成员。  
  
 扩大转换在运行时始终成功，并且永远不会导致数据丢失。 你始终可以隐式执行它们，无论[Option Strict 语句](../../../../visual-basic/language-reference/statements/option-strict-statement.md)设置类型检查切换到`On`或`Off`。  
  
## <a name="narrowing-conversions"></a>收缩转换  
 标准的收缩转换包括：  
  
-   在前面的扩大转换的反向说明表 （只不过每种类型扩大到自身）  
...
-   从数据类型或对象类型到派生自它的类型转换  
 ...
 收缩转换执行并非总是在运行时，成功，失败或导致数据丢失。 如果目标数据类型不能接收所转换的值，则会发生错误。 例如，数值的转换可能导致溢出。 编译器不允许可隐式执行收缩转换，除非[Option Strict 语句](../../../../visual-basic/language-reference/statements/option-strict-statement.md)设置类型检查切换到`Off`。  
...
### <a name="when-to-use-narrowing-conversions"></a>何时使用收缩转换  
 当你知道源值可以转换为无错误或数据丢失的目标数据类型时，你可以使用收缩转换。 例如，如果你有`String`，你知道包含"True"False"，你可以使用`CBool`关键字来将其转换为`Boolean`。  
