...
## <a name="conversion-of-strings-to-numbers"></a>字符串转换为数字  
 ...
 时[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]将字符串转换为数字值，它使用**区域选项**Windows 中指定的设置**控制面板**解释千位分隔符、 小数分隔符和货币符号。 这意味着转换可能会成功下一个但不是能是其他设置。 例如，`"$14.20"`是可接受的英语 （美国） 区域设置中但不是在任何法语的区域设置。  
