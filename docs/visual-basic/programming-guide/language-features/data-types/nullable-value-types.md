...
有时，你将使用在某些情况下没有已定义的值的值类型。 例如，在数据库中的字段可能需要区分具有分配的值有意义的而不让分配的值。 值类型可以扩展以使其正常值或 null 值。 调用此类扩展*为 null 的类型*。  
 ...
 无法构造为 null 的类型，如数组、 引用类型上`String`，或类。 基础类型必须是值类型。 有关更多信息，请参见 [Value Types and Reference Types](../../../../visual-basic/programming-guide/language-features/data-types/value-types-and-reference-types.md)。  
