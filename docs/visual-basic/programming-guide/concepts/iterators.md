...
> [!NOTE]
> 对于除了简单迭代器示例之外的主题中的所有示例 ，都包含用于 `System.Collections` 和 `System.Collections.Generic` 命名空间的 [Imports](../../../visual-basic/language-reference/statements/imports-statement-net-namespace-and-type.md) 语句。  
  
## <a name="BKMK_SimpleIterator"></a>简单迭代器  
 下面的示例包含`Yield`一个位于[For 。下一个](../../../visual-basic/language-reference/statements/for-next-statement.md)循环。 在 `Main` 中，`For Each` 语句体的每次迭代都会创建一个对迭代器函数的调用，并将继续到下一个 `Yield` 语句。  
...
## <a name="BKMK_CollectionClass"></a>创建集合类  
 ...
 方法通过使用语句每次返回一个字符串，并且`Iterator`修饰符在函数声明中。 `Yield` `GetEnumerator`  
...
## <a name="BKMK_TryBlocks"></a>Try 块  
 Visual Basic 允许`Yield` `Try`在[Try 块 。Catch 。Finally 语句](../../../visual-basic/language-reference/statements/try-catch-finally-statement.md)。 带有语句的`Catch`块可以有块，并且可以有`Finally`块。 `Try` `Yield`  
  
 下面的示例在`Try`迭代`Catch`器函数`Finally`中包含、和块。 迭代`Finally`器函数中的块在`For Each`迭代完成之前执行。  
...
```  
 ...
 语句不能`Catch`位于块或`Finally`块中。 `Yield`  
  
 如果主体（而不是迭代器方法）引发异常，则`Finally`不`Catch`会执行迭代器函数中的块，但会执行迭代器函数中的块。 `For Each` Iterator `Catch`函数内的块仅捕获 iterator 函数内发生的异常。  
...
```  
 ...
 如果验证是在迭代器函数中，则在开始第一次迭代`For Each`的开始之前，将无法执行验证。  
...
## <a name="BKMK_GenericList"></a>对泛型列表使用迭代器  
 ...
 `BottomToTop`属性声明`Iterator`包含关键字。  
...
## <a name="BKMK_SyntaxInformation"></a>语法信息  
 ...
 在 Visual Basic 中，迭代器方法不能`ByRef`具有任何参数。  
  
 在 Visual Basic 中，"Yield" 不是保留字，仅在`Iterator`方法或`get`访问器中使用时具有特殊意义。  
...
## <a name="BKMK_Technical"></a>技术实现  
 ...
 在 `For Each…Next` 循环（或对 `IEnumerator.MoveNext` 的直接调用）的每次后续迭代中，下一个迭代器代码体都会在上一个 `Yield` 语句之后恢复。 然后，它将继续执行`Yield`下一条语句，直到到达迭代器主体的末尾，或者`Exit Function`遇到`Return`或语句为止。  
