...
2. 将鼠标指针停留在循环控制变量 `studentRecord` 以查看其数据类型。 @No__t_0 的类型将推断为要 `Student`，因为 `studentQuery` 返回 `Student` 实例的集合。
...
1. 在 `Where` 语句和查询的 `Select` 语句之间添加以下 `Order By` 子句。 @No__t_0 子句根据每个学生的姓氏，按字母顺序从 A 到 Z 的顺序对结果进行排序。
