...
当 `TaskOfT_MethodAsync` 从 await 表达式中调用时，await 表达式将检索存储在由 `leisureHours` 返回的任务中的整数值（`TaskOfT_MethodAsync` 的值）。 有关 await 表达式的详细信息，请参阅[Await 运算符](../../../../visual-basic/language-reference/operators/await-operator.md)。
...
通过从应用程序 `TaskOfT_MethodAsync` 中分离对 `Await` 的调用，你可以更好地了解此操作，如下面的代码所示。 对非立即等待的方法 `TaskOfT_MethodAsync` 的调用返回 `Task(Of Integer)`，正如你从方法声明预料的一样。 该任务指派给示例中的 `integerTask` 变量。 因为 `integerTask` 是 <xref:System.Threading.Tasks.Task%601>，所以它包含类型 <xref:System.Threading.Tasks.Task%601.Result> 的 `TResult` 属性。 在这种情况下，TResult 表示整数类型。 `Await` 应用于 `integerTask`，await 表达式的计算结果为 <xref:System.Threading.Tasks.Task%601.Result%2A> 的 `integerTask` 属性内容。 此值分配给 `result2` 变量。
...
使用 await 语句而不是 await 表达式来调用和等待 `Task_MethodAsync`，这类似于同步 `Sub` 或返回 void 的方法的调用语句。 在这种情况下，`Await` 运算符的应用程序不会生成值。
