...
通过 `DeleteFile` 对象的 `My.Computer.FileSystem` 方法，可以删除文件。 它提供的选项包括：是否将已删除的文件发送到“回收站”  、是否要求用户确认应删除该文件，以及当用户取消操作时要执行的操作。  
...
- 使用 `DeleteFile` 方法删除文件，并为 `SendToRecycleBin` 参数指定 `recycle`。 以下代码演示如何删除名为 `test.txt` 的文件并将其发送到“回收站”  。  
