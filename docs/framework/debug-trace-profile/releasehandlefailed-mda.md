...
ms.locfileid: "77216113"
---
# <a name="releasehandlefailed-mda"></a>releaseHandleFailed MDA
当 `releaseHandleFailed` 或 <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A> 派生的类的 <xref:System.Runtime.InteropServices.SafeHandle> 方法返回 <xref:System.Runtime.InteropServices.CriticalHandle> 时，激活 `false` 托管调试助手 (MDA) 通知开发人员。  
  
## <a name="symptoms"></a>症状  
 资源或内存泄漏。  如果 <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A> 或 <xref:System.Runtime.InteropServices.SafeHandle> 派生的类的 <xref:System.Runtime.InteropServices.CriticalHandle> 方法失败，则由类所封装的资源可能尚未释放或清理。  
  
## <a name="cause"></a>原因  
 如果用户创建派生自 <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A> 或 <xref:System.Runtime.InteropServices.SafeHandle> 的类，则他们必须提供 <xref:System.Runtime.InteropServices.CriticalHandle> 方法的实现；因此，这些情况是特定于单个资源的。 但是，需求如下所示：  
...
- 确保用以初始化带有有效原始句柄值的 <xref:System.Runtime.InteropServices.SafeHandle> 的代码拥有句柄。 如果在没有将 <xref:System.Runtime.InteropServices.SafeHandle> 参数设置到基构造函数中的 `ownsHandle` 时，围绕代码没有的句柄构造 `false`，则 <xref:System.Runtime.InteropServices.SafeHandle> 和实际句柄拥有者都可以关闭该句柄，如果 <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A> 失去争用，这将会导致 <xref:System.Runtime.InteropServices.SafeHandle> 中出错。  
...
- 可以跟踪句柄之前关闭的位置，通过将调试器断点放在用来释放该句柄的本机例程上（例如 <xref:System.Runtime.InteropServices.SafeHandle.ReleaseHandle%2A> 函数），当最终调用 `CloseHandle` 方法时，会导致失败。 这在压力方案中或甚至是中型功能测试中都不可能实现，由于此类例程处理的通信通常非常繁忙。 它可能有助于检测调用本机释放方法的代码，以便捕获调用方的标识，或者可能捕获完整堆栈跟踪，以及被释放句柄的值。  句柄值可以与此 MDA 报告的值进行比较。  
