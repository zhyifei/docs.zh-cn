...
## <a name="configuring-trace-switches"></a>配置跟踪开关  
 ...
 使用 .config 文件配置开关。 对于 Web 应用程序，这是与项目关联的 Web.config 文件。 在 Windows 应用程序中，此文件的名称为（应用程序名称） .exe。在已部署的应用程序中，此文件必须与可执行文件位于同一文件夹中。  
 ...
 创建开关实例时，还可以通过指定 displayName 和 description 这两个参数来初始化此实例。 构造函数的 displayName 参数设置 *类实例的* 属性<xref:System.Diagnostics.Switch.DisplayName%2A?displayProperty=nameWithType><xref:System.Diagnostics.Switch>。 displayName 是用于在 .config 文件中配置开关的名称，description 参数应返回开关及其所控制的消息的简要说明。  
...
> [!NOTE]
> 在 .NET Framework 2.0 版中，你可以使用文本指定开关值。 例如，为 `true` 指定 <xref:System.Diagnostics.BooleanSwitch> 或表示枚举值的文本，例如为 `Error` 指定 <xref:System.Diagnostics.TraceSwitch>。 行 `<add name="myTraceSwitch" value="Error" />` 等于 `<add name="myTraceSwitch" value="1" />`。  
...
2. 如果项目不包含配置文件（app.config 或 Web.config），则从“项目”菜单中选择“添加新项”。  
    ...
    - **Visual C#：** 在“添加新项”对话框中，选择“XML 文件”。 将此文件命名为**app.config**。在 XML 编辑器中，在 XML 声明的后面添加以下 XML：  
...
3. 在 `<configuration>` 标记之后但在 `</configuration>` 标记前面，添加适当的 XML 以配置开关。 以下示例演示了一个 DisplayName 属性为 **的 BooleanSwitch，以及一个 DisplayName 属性为** 的 TraceSwitch`DataMessageSwitch``TraceLevelSwitch`。  
...
4. 如果需要开启 BooleanSwitch，如前面示例中所示的 **，请将“Value”更改为 0 以外的任何整数**`DataMessagesSwitch`。  
  
5. 如果需要开启 TraceSwitch，如前面示例中所示的 **，请将“Value”更改为适当的级别设置 (1-4)** `TraceLevelSwitch`。  
