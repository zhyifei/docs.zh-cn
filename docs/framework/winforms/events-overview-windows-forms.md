...
委托可绑定到单个方法或多个方法，后者又称为多路广播。 为事件创建委托时，您（或 Windows）通常会创建多路广播事件。 极少的例外情况是，某个事件会导致特定过程（例如显示对话框），而该过程在逻辑上不在每个事件中重复多次。 有关如何创建多播委托的信息，请参阅[如何合并委托（多路广播委托）](../../csharp/programming-guide/delegates/how-to-combine-delegates-multicast-delegates.md)。
