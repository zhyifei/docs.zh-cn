...
在下面的过程中, 你将使用 Visual Studio 创建显示<xref:System.Windows.Forms.RichTextBox>控件的 MDI 子窗体, 该窗体类似于大多数字处理应用程序。 通过将<xref:System.Windows.Forms>控件替换为其他控件 (如<xref:System.Windows.Forms.DataGridView>控件或控件的组合), 您可以创建 mdi 子窗口 (以及通过扩展的 mdi 应用程序), 并且可能会有多种可能性。

## <a name="create-mdi-child-forms"></a>创建 MDI 子窗体

1. 在 Visual Studio 中创建新的 Windows 窗体应用程序项目。 在窗体的 "**属性**" 窗口中, <xref:System.Windows.Forms.Form.IsMdiContainer%2A>将其`true`属性设置`WindowsState`为, `Maximized`并将其属性设置为。

   这将该表单指定为适合子窗口的 MDI 容器。

2. 将 <xref:System.Windows.Forms.MenuStrip> 控件从 `Toolbox` 中拖到窗体上。 将其`Text`属性设置为 " **File**"。

3. 单击 " **Items** " 属性旁边的省略号 (...), 然后单击 "**添加**" 以添加两个子工具栏菜单项。 将这些`Text`项的属性设置为 "**新建**" 和 "**窗口**"。
...
7. 在 "**属性**" 窗口中, `Anchor`将属性设置为**Top, Left** , 并将属性设置为 " `Dock` **Fill**"。
...
9. 插入类似于下面的代码, 以便在用户单击 "**新建**" 菜单项时创建新的 MDI 子窗体。
   ...
   > [!NOTE]
   > 在下面的示例中，事件处理程序处理 `MenuItem2` 的 <xref:System.Windows.Forms.Control.Click> 事件。 请注意, 根据您的应用程序体系结构的具体情况, 您的**新**菜单项可能`MenuItem2`不是。
   ...
   在C++中, 在 Form1 `#include`的顶部添加以下指令:

   ```cpp
   #include "Form2.h"
   ```

10. 在 "**属性**" 窗口顶部的下拉列表中, 选择与 "**文件**" 菜单条对应的菜单条, <xref:System.Windows.Forms.MenuStrip.MdiWindowListItem%2A>并将属性设置为窗口。 <xref:System.Windows.Forms.ToolStripMenuItem>
