...
<xref:System.Windows.Forms.Clipboard>类提供了可用于与 Windows 操作系统剪贴板功能进行交互的方法。 许多应用程序的数据用作临时存储库使用剪贴板。 例如，文字处理器剪切和粘贴操作期间使用剪贴板。 剪贴板功能也很有用信息传输到另一个应用程序。  
 ...
 某些应用程序以增加数据有可能使用其他应用程序的数量以多种格式剪贴板上存储数据。 剪贴板格式是用于标识格式的字符串。 使用标识的格式的应用程序可以检索剪贴板上的关联的数据。 <xref:System.Windows.Forms.DataFormats>类提供了供你使用的预定义的格式名称。 此外可以使用自己的格式名称，或使用对象的类型作为其格式。 有关将数据添加到剪贴板的信息，请参阅[如何：将数据添加到剪贴板](../../../../docs/framework/winforms/advanced/how-to-add-data-to-the-clipboard.md)。  
