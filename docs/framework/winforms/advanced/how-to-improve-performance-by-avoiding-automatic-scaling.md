...
GDI + 可能会自动将图像缩放在绘制时，这会降低性能。 或者，可以控制图像缩放通过将传递到目标矩形的尺寸<xref:System.Drawing.Graphics.DrawImage%2A>方法。  
