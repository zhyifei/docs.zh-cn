...
下表介绍用户如何通过键盘和鼠标与 <xref:System.Windows.Forms.DataGridView> 控件交互。  
  
> [!NOTE]
> 若要自定义键盘行为，可以处理标准键盘事件，如 <xref:System.Windows.Forms.Control.KeyDown>。 但在编辑模式下，承载的编辑控件接收键盘输入，而对于 <xref:System.Windows.Forms.DataGridView> 控件不发生键盘事件。 若要处理编辑控件事件，请将处理程序附加到 <xref:System.Windows.Forms.DataGridView.EditingControlShowing> 事件处理程序中的编辑控件。 或者，您可以通过重写 <xref:System.Windows.Forms.DataGridView.ProcessDialogKey%2A> 和 <xref:System.Windows.Forms.DataGridView.ProcessDataGridViewKey%2A> 方法，自定义 <xref:System.Windows.Forms.DataGridView> 子类中的键盘行为。  
...
|Tab|如果 <xref:System.Windows.Forms.DataGridView.StandardTab%2A> 属性值为 `false`，则将焦点移动到当前行中的下一个单元格。 如果焦点已在该行的最后一个单元格中，则将焦点移到下一行的第一个单元格。 如果焦点位于控件中的最后一个单元格，则将焦点移动到父容器的 tab 键顺序中的下一个控件。<br /><br /> 如果 `true`<xref:System.Windows.Forms.DataGridView.StandardTab%2A> 属性值，则会将焦点移到父容器的 tab 键顺序中的下一个控件。|  
|Shift + Tab|如果 <xref:System.Windows.Forms.DataGridView.StandardTab%2A> 属性值为 `false`，则将焦点移动到当前行中的上一个单元格。 如果焦点已在该行的第一个单元格中，则将焦点移到上一行中的最后一个单元格。 如果焦点位于控件中的第一个单元格，则将焦点按父容器的 tab 键顺序移到上一个控件。<br /><br /> 如果 `true`<xref:System.Windows.Forms.DataGridView.StandardTab%2A> 属性值，则将焦点按父容器的 tab 键顺序移到上一个控件。|  
|Ctrl + Tab|如果 `false`<xref:System.Windows.Forms.DataGridView.StandardTab%2A> 属性值，则会将焦点移到父容器的 tab 键顺序中的下一个控件。<br /><br /> 如果 <xref:System.Windows.Forms.DataGridView.StandardTab%2A> 属性值为 `true`，则将焦点移动到当前行中的下一个单元格。 如果焦点已在该行的最后一个单元格中，则将焦点移到下一行的第一个单元格。 如果焦点位于控件中的最后一个单元格，则将焦点移动到父容器的 tab 键顺序中的下一个控件。|  
|Ctrl+Shift+Tab|如果 `false`<xref:System.Windows.Forms.DataGridView.StandardTab%2A> 属性值，则将焦点按父容器的 tab 键顺序移到上一个控件。<br /><br /> 如果 <xref:System.Windows.Forms.DataGridView.StandardTab%2A> 属性值为 `true`，则将焦点移动到当前行中的上一个单元格。 如果焦点已在该行的第一个单元格中，则将焦点移到上一行中的最后一个单元格。 如果焦点位于控件中的第一个单元格，则将焦点按父容器的 tab 键顺序移到上一个控件。|  
