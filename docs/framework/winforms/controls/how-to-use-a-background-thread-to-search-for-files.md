...
---
# <a name="how-to-use-a-background-thread-to-search-for-files"></a><span data-ttu-id="af540-102">如何：使用后台线程搜索文件</span><span class="sxs-lookup"><span data-stu-id="af540-102">How to: Use a Background Thread to Search for Files</span></span>
<span data-ttu-id="af540-103"><xref:System.ComponentModel.BackgroundWorker>组件替换，并添加了功能<xref:System.Threading>命名空间; 但是，<xref:System.Threading>命名空间保留向后兼容性和将来使用，如果您选择。</span><span class="sxs-lookup"><span data-stu-id="af540-103">The <xref:System.ComponentModel.BackgroundWorker> component replaces and adds functionality to the <xref:System.Threading> namespace; however, the <xref:System.Threading> namespace is retained for both backward compatibility and future use, if you choose.</span></span> <span data-ttu-id="af540-104">有关详细信息，请参阅[BackgroundWorker 组件概述](backgroundworker-component-overview.md)。</span><span class="sxs-lookup"><span data-stu-id="af540-104">For more information, see [BackgroundWorker Component Overview](backgroundworker-component-overview.md).</span></span>  
  
 <span data-ttu-id="af540-105">Windows 窗体使用单线程单元 (STA) 模型，因为在本机 Win32 windows 的本质上是单线程单元的基于 Windows 窗体。</span><span class="sxs-lookup"><span data-stu-id="af540-105">Windows Forms uses the single-threaded apartment (STA) model because Windows Forms is based on native Win32 windows that are inherently apartment-threaded.</span></span> <span data-ttu-id="af540-106">STA 模型意味着可以在任何线程上创建一个窗口，但它不能切换线程一次创建，并对它的所有函数调用必须在其创建的线程上都发生。</span><span class="sxs-lookup"><span data-stu-id="af540-106">The STA model implies that a window can be created on any thread, but it cannot switch threads once created, and all function calls to it must occur on its creation thread.</span></span> <span data-ttu-id="af540-107">Windows 窗体之外.NET Framework 中的类使用自由线程模型。</span><span class="sxs-lookup"><span data-stu-id="af540-107">Outside Windows Forms, classes in the .NET Framework use the free threading model.</span></span> <span data-ttu-id="af540-108">有关.NET Framework 中的线程处理的信息，请参阅[线程处理](../../../standard/threading/index.md)。</span><span class="sxs-lookup"><span data-stu-id="af540-108">For information about threading in the .NET Framework, see [Threading](../../../standard/threading/index.md).</span></span>  
  
 <span data-ttu-id="af540-109">STA 模型要求，需要从外部控件的创建线程调用控件上的任何方法必须进行封送处理到 （执行） 控件的创建线程。</span><span class="sxs-lookup"><span data-stu-id="af540-109">The STA model requires that any methods on a control that need to be called from outside the control's creation thread must be marshaled to (executed on) the control's creation thread.</span></span> <span data-ttu-id="af540-110">类的基类<xref:System.Windows.Forms.Control>提供了多种方法 (<xref:System.Windows.Forms.Control.Invoke%2A>， <xref:System.Windows.Forms.Control.BeginInvoke%2A>，和<xref:System.Windows.Forms.Control.EndInvoke%2A>) 实现此目的。</span><span class="sxs-lookup"><span data-stu-id="af540-110">The base class <xref:System.Windows.Forms.Control> provides several methods (<xref:System.Windows.Forms.Control.Invoke%2A>, <xref:System.Windows.Forms.Control.BeginInvoke%2A>, and <xref:System.Windows.Forms.Control.EndInvoke%2A>) for this purpose.</span></span> <span data-ttu-id="af540-111"><xref:System.Windows.Forms.Control.Invoke%2A> 使同步方法调用异步方法。<xref:System.Windows.Forms.Control.BeginInvoke%2A>进行异步方法的调用。</span><span class="sxs-lookup"><span data-stu-id="af540-111"><xref:System.Windows.Forms.Control.Invoke%2A> makes synchronous method calls; <xref:System.Windows.Forms.Control.BeginInvoke%2A> makes asynchronous method calls.</span></span>  
  
 <span data-ttu-id="af540-112">如果使用多线程处理中的资源密集型任务在控件中，用户界面可以保持响应状态时在后台线程上执行占用大量资源的计算。</span><span class="sxs-lookup"><span data-stu-id="af540-112">If you use multithreading in your control for resource-intensive tasks, the user interface can remain responsive while a resource-intensive computation executes on a background thread.</span></span>  
  
 <span data-ttu-id="af540-113">下面的示例 (`DirectorySearcher`) 显示为与指定的搜索字符串相匹配的文件使用后台线程以递归方式搜索目录，然后填充具有搜索结果的列表框的多线程的 Windows 窗体控件。</span><span class="sxs-lookup"><span data-stu-id="af540-113">The following sample (`DirectorySearcher`) shows a multithreaded Windows Forms control that uses a background thread to recursively search a directory for files matching a specified search string and then populates a list box with the search result.</span></span> <span data-ttu-id="af540-114">此示例所示的关键概念是，如下所示：</span><span class="sxs-lookup"><span data-stu-id="af540-114">The key concepts illustrated by the sample are as follows:</span></span>  
  
-   <span data-ttu-id="af540-115">`DirectorySearcher` 启动一个新线程来执行搜索。</span><span class="sxs-lookup"><span data-stu-id="af540-115">`DirectorySearcher` starts a new thread to perform the search.</span></span> <span data-ttu-id="af540-116">线程执行`ThreadProcedure`反过来调用帮助器方法`RecurseDirectory`方法来执行实际搜索并填充列表框。</span><span class="sxs-lookup"><span data-stu-id="af540-116">The thread executes the `ThreadProcedure` method that in turn calls the helper `RecurseDirectory` method to do the actual search and to populate the list box.</span></span> <span data-ttu-id="af540-117">但是，填充列表框需要跨线程调用，接下来两个项目符号项中所述。</span><span class="sxs-lookup"><span data-stu-id="af540-117">However, populating the list box requires a cross-thread call, as explained in the next two bulleted items.</span></span>  
  
-   <span data-ttu-id="af540-118">`DirectorySearcher` 定义`AddFiles`方法将文件添加到列表框中; 但是，`RecurseDirectory`不能直接调用`AddFiles`因为`AddFiles`可以仅在创建 STA 线程中执行`DirectorySearcher`。</span><span class="sxs-lookup"><span data-stu-id="af540-118">`DirectorySearcher` defines the `AddFiles` method to add files to a list box; however, `RecurseDirectory` cannot directly invoke `AddFiles` because `AddFiles` can execute only in the STA thread that created `DirectorySearcher`.</span></span>  
  
-   <span data-ttu-id="af540-119">唯一方式`RecurseDirectory`可以调用`AddFiles`是通过跨线程调用 — 即，通过调用<xref:System.Windows.Forms.Control.Invoke%2A>或<xref:System.Windows.Forms.Control.BeginInvoke%2A>封送`AddFiles`的创建线程`DirectorySearcher`。</span><span class="sxs-lookup"><span data-stu-id="af540-119">The only way `RecurseDirectory` can call `AddFiles` is through a cross-thread call — that is, by calling <xref:System.Windows.Forms.Control.Invoke%2A> or <xref:System.Windows.Forms.Control.BeginInvoke%2A> to marshal `AddFiles` to the creation thread of `DirectorySearcher`.</span></span> <span data-ttu-id="af540-120">`RecurseDirectory` 使用<xref:System.Windows.Forms.Control.BeginInvoke%2A>，以便可以以异步方式进行调用。</span><span class="sxs-lookup"><span data-stu-id="af540-120">`RecurseDirectory` uses <xref:System.Windows.Forms.Control.BeginInvoke%2A> so that the call can be made asynchronously.</span></span>  
  
-   <span data-ttu-id="af540-121">封送处理一种方法需要函数指针或回调的等效项。</span><span class="sxs-lookup"><span data-stu-id="af540-121">Marshaling a method requires the equivalent of a function pointer or callback.</span></span> <span data-ttu-id="af540-122">这是.NET Framework 中使用委托来完成。</span><span class="sxs-lookup"><span data-stu-id="af540-122">This is accomplished using delegates in the .NET Framework.</span></span> <span data-ttu-id="af540-123"><xref:System.Windows.Forms.Control.BeginInvoke%2A> 采用委托作为参数。</span><span class="sxs-lookup"><span data-stu-id="af540-123"><xref:System.Windows.Forms.Control.BeginInvoke%2A> takes a delegate as an argument.</span></span> <span data-ttu-id="af540-124">`DirectorySearcher` 因此定义一个委托 (`FileListDelegate`)，将绑定`AddFiles`的实例`FileListDelegate`在其构造函数，并传递到此委托实例<xref:System.Windows.Forms.Control.BeginInvoke%2A>。</span><span class="sxs-lookup"><span data-stu-id="af540-124">`DirectorySearcher` therefore defines a delegate (`FileListDelegate`), binds `AddFiles` to an instance of `FileListDelegate` in its constructor, and passes this delegate instance to <xref:System.Windows.Forms.Control.BeginInvoke%2A>.</span></span> <span data-ttu-id="af540-125">`DirectorySearcher` 此外定义完成搜索时封送一个事件委托。</span><span class="sxs-lookup"><span data-stu-id="af540-125">`DirectorySearcher` also defines an event delegate that is marshaled when the search is completed.</span></span>  
...
## <a name="using-the-multithreaded-control-on-a-form"></a><span data-ttu-id="af540-126">使用窗体上的多线程的控件</span><span class="sxs-lookup"><span data-stu-id="af540-126">Using the Multithreaded Control on a Form</span></span>  
 <span data-ttu-id="af540-127">下面的示例演示如何多线程`DirectorySearcher`可以在窗体上使用控件。</span><span class="sxs-lookup"><span data-stu-id="af540-127">The following example shows how the multithreaded `DirectorySearcher` control can be used on a form.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="af540-128">请参阅</span><span class="sxs-lookup"><span data-stu-id="af540-128">See also</span></span>
- <xref:System.ComponentModel.BackgroundWorker>
- [<span data-ttu-id="af540-129">使用 .NET Framework 开发自定义 Windows 窗体控件</span><span class="sxs-lookup"><span data-stu-id="af540-129">Developing Custom Windows Forms Controls with the .NET Framework</span></span>](developing-custom-windows-forms-controls.md)
- [<span data-ttu-id="af540-130">基于事件的异步模式概述</span><span class="sxs-lookup"><span data-stu-id="af540-130">Event-based Asynchronous Pattern Overview</span></span>](../../../standard/asynchronous-programming-patterns/event-based-asynchronous-pattern-overview.md)
