...
1. 添加一个名为 <xref:System.Windows.Forms.BindingSource> 的 `bindingSource1` 组件和两个分别名为 <xref:System.Windows.Forms.TextBox> 和 `textBox1` 的 `textBox2` 控件。  
...
3. 将名为 <xref:System.Windows.Forms.BindingNavigator> 的 `bindingNavigator1` 控件添加到窗体。  
  
4. 将 <xref:System.Windows.Forms.BindingNavigator.BindingSource%2A>的 `bindingNavigator1` 属性设置为 `bindingSource1`。 可以使用设计器或用代码执行此操作。  
