...
6. 找到语句, `Partial Public Class ValueButton`并将此控件继承<xref:System.Windows.Forms.UserControl>的类型从更改为<xref:System.Windows.Forms.Button>。 `Class` 这允许继承的控件继承<xref:System.Windows.Forms.Button>控件的所有功能。
...
2. 选择 "Visual Basic 项目" 节点, 然后单击 " **Windows 窗体应用程序**"。
...
6. 在 "**工具箱**" 的 "**所有 Windows 窗体**" 选项卡上 , 双击 " <xref:System.Windows.Forms.Label>标签" 将控件添加到窗体。
