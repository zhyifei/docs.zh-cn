...
2. 在 Visual Basic 项目列表中, 选择 " **Windows 控件库**" 项目模板, 在`ctlClockLib` "**名称**" 框中键入, 然后单击 **"确定"** 。
...
2. 在“工具箱”中，展开“公共控件”节点，然后双击“标签”。
     ...
     <xref:System.Windows.Forms.Label> 名`Label1`为的控件将添加到设计器图面上的控件中。
...
4. 在“工具箱”中，展开“组件”节点，然后双击“计时器”。
     ...
     <xref:System.Windows.Forms.Timer>由于是一个组件, 因此它在运行时不具有可视表示形式。 因而它不会和控件一起出现在设计器图面上，而是出现在“组件设计器”（设计器图面底部的一栏）中。
...
## <a name="testing-the-control"></a>测试控件
 控件不是独立的项目，它们必须托管在容器中。 测试控件的运行时行为，并使用“UserControl 测试容器”运用其属性。 有关详细信息，请参阅[如何：测试 UserControl](how-to-test-the-run-time-behavior-of-a-usercontrol.md)的运行时行为。
...
4. 使用鼠标将<xref:System.Windows.Forms.Label>控件拖到显示框的正下方。 在“属性”窗口中，设置下列属性。
...
9. 在 "**工具箱**" 中, 找到并双击 " **DateTimePicker** " 将<xref:System.Windows.Forms.DateTimePicker>控件添加到<xref:System.Windows.Forms.Label>窗体, 然后双击 "**标签**" 添加控件。
...
15. 在“调试”菜单上，单击“启动调试”。
     ...
     测试程序随即启动。 请注意, `ctlAlarmClock`控件中的当前时间已更新, 并且开始时间显示<xref:System.Windows.Forms.DateTimePicker>在控件中。

16. 单击 "小时" 的显示位置。<xref:System.Windows.Forms.DateTimePicker>
