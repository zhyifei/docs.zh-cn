...
- 如果更改属性会影响控件的可视显示, 请从<xref:System.Windows.Forms.Control.Invalidate%2A> `set`访问器调用方法 (控件继承自<xref:System.Windows.Forms.Control>的方法)。 <xref:System.Windows.Forms.Control.Invalidate%2A>进而调用<xref:System.Windows.Forms.Control.OnPaint%2A>方法, 这会重绘控件。 多次调用<xref:System.Windows.Forms.Control.Invalidate%2A>将导致一次调用以<xref:System.Windows.Forms.Control.OnPaint%2A>提高效率。  
...
```  
 ...
 以下代码片段将类型转换器和 UI 类型编辑器与属性 `Value` 相关联。 在这种`Value`情况下, 它是一个整数并且具有默认类型转换器<xref:System.ComponentModel.TypeConverterAttribute> , 但该特性应用了一个`FlashTrackBarValueConverter`允许设计器将其显示为百分比的自定义类型转换器 ()。 UI 类型编辑器 `FlashTrackBarValueEditor` 允许以可视化方式显示百分比。 此示例还显示了<xref:System.ComponentModel.TypeConverterAttribute>或<xref:System.ComponentModel.EditorAttribute>特性指定的类型转换器或编辑器将重写默认转换器。  
