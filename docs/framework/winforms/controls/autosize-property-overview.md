...
> [!NOTE]
> 若要最大化生产效率，Windows 窗体设计器在 Visual Studio shadows<xref:System.Windows.Forms.Control.AutoSize%2A>属性<xref:System.Windows.Forms.Form>类。 在设计时，窗体行为就如同<xref:System.Windows.Forms.Control.AutoSize%2A>属性设置为`false`，无论其实际的设置。 在运行时，进行任何特殊的便利设施，和<xref:System.Windows.Forms.Control.AutoSize%2A>应用属性所指定的属性设置。
