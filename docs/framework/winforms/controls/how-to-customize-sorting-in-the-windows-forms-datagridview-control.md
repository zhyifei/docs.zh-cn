...
<xref:System.Windows.Forms.DataGridView> 控件提供自动排序，但根据需要，你可能需要自定义排序操作。 例如，你可以使用编程排序来创建替代的用户界面 (UI)。 或者，你可以处理 <xref:System.Windows.Forms.DataGridView.SortCompare> 事件或调用 `Sort(IComparer)` 方法的 <xref:System.Windows.Forms.DataGridView.Sort%2A> 重载，以便进行更灵活的排序，例如对多个列进行排序。  
...
## <a name="custom-sorting-using-the-icomparer-interface"></a>使用 IComparer 接口自定义排序  
 下面的代码示例演示使用 `Sort(IComparer)` 方法的 <xref:System.Windows.Forms.DataGridView.Sort%2A> 重载进行自定义排序，它通过实现 <xref:System.Collections.IComparer> 接口来执行多个列的排序。  
