...
7. 向窗<xref:System.Windows.Forms.ListView>体添加`listView1`一个名为的控件, 并将其定位在<xref:System.Windows.Forms.SplitContainer>控件的右侧。 在属性窗口`listview1`中, 执行以下操作:
    ...
    3. 通过![单击<xref:System.Windows.Forms.ListView.Columns%2A>属性中的 "Visual](./media/visual-studio-ellipsis-button.png)Studio 属性窗口中的省略号按钮 (...) 来打开" ColumnHeader 集合编辑器 "。 添加三列, 并分别<xref:System.Windows.Forms.ColumnHeader.Text%2A>将其`Name`属性`Type`设置为`Last Modified`、和。 单击“确定”关闭对话框。
