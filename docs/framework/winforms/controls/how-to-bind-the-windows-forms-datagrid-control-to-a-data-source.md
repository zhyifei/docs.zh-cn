...
1. 编写代码以填充数据集。  
     ...
     你使用的确切代码取决于数据集获取数据的位置。 如果直接从数据库填充数据集, 则通常调用数据适配器的`Fill`方法, 如下面的示例所示, 该示例将填充名`DsCategories1`为的数据集:  
    ...
    ```  
     ...
     如果数据集是从 XML Web service 中填充的, 则通常在代码中创建服务的实例, 然后调用其方法之一来返回数据集。 然后, 将数据集从 XML Web service 合并到本地数据集。 下面的示例演示如何创建名`CategoriesService`为的 XML Web service 的实例, 调用其`GetCategories`方法, 并将生成的数据集合并到名`DsCategories1`为的本地数据集:  
...
2. 调用控件的<xref:System.Windows.Forms.DataGrid.SetDataBinding%2A>方法, 将数据源和数据成员传递给它。 <xref:System.Windows.Forms.DataGrid> 如果不需要显式传递数据成员, 请传递一个空字符串。  
    ...
    > 如果是第一次绑定网格, 则可以设置控件的<xref:System.Windows.Forms.DataGrid.DataSource%2A>和<xref:System.Windows.Forms.DataGrid.DataMember%2A>属性。 但是, 一旦设置了这些属性, 就不能对其进行重置。 因此, 建议你始终使用<xref:System.Windows.Forms.DataGrid.SetDataBinding%2A>方法。  
     ...
     下面的示例演示如何以编程方式绑定到数据集中名`DsCustomers1`为的 Customers 表:  
