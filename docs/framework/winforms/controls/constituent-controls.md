...
组成用户控件的控件（也称作“构成控件”**）在自定义图形呈现方面的灵活性相对较差。 所有 Windows 窗体控件都通过自己的<xref:System.Windows.Forms.Control.OnPaint%2A>方法处理自己的呈现。 由于此方法受到保护，开发人员无法对其进行访问，因此在绘制控件时无法阻止其执行。 然而，这并不意味着不能添加影响构成控件外观的代码。 附加呈现可通过添加事件处理程序来完成。 例如，假设您创作的按钮<xref:System.Windows.Forms.UserControl>名为`MyButton`。 如果希望具有超出 提供的其他呈现，<xref:System.Web.UI.WebControls.Button>则可以向用户控件添加类似于以下内容的代码：  
...
> [!NOTE]
> 某些 Windows 窗体控件（<xref:System.Windows.Forms.TextBox>如 ）由 Windows 直接绘制。 在这些情况下，永远不会调用<xref:System.Windows.Forms.Control.OnPaint%2A>该方法，因此永远不会调用上述示例。  
