...
在 Windows 通信基础 （WCF） 中，该服务规定了客户端的安全要求。 即，由服务指定要使用的安全模式以及客户端是否必须提供凭据。 因此，保证客户端安全的过程非常简单：使用从服务那里获得的元数据（如果已发布）来生成客户端。 元数据指定如何配置客户端。 如果服务要求客户端提供凭据，您必须获得能够满足要求的凭据。 本主题进一步详细讨论此过程。 有关创建安全服务的详细信息，请参阅[保护服务](securing-services.md)。  
  
## <a name="the-service-specifies-security"></a>服务指定安全性  
 默认情况下，WCF 绑定已启用安全功能。 （例外情况是<xref:System.ServiceModel.BasicHttpBinding>.）因此，如果服务是使用 WCF 创建的，则更有可能实现安全性以确保身份验证、机密性和完整性。 在这种情况下，服务提供的元数据将指示建立安全通信通道需要哪些条件。 如果服务元数据不包含安全要求，则无法对服务强行实施安全方案，例如 Secure Sockets Layer (SSL) over HTTP。 但是，如果服务要求客户端提供凭据，则客户端开发人员、部署人员或管理员必须提供客户端用来向该服务证明自己身份的实际凭据。  
  
## <a name="obtaining-metadata"></a>获取元数据  
 创建客户端时，第一步是获取客户端将与其通信的服务的元数据。 有两种方法可以做到这一点。 首先，如果服务发布元数据交换 （MEX） 终结点或通过 HTTP 或 HTTPS 使其元数据可用，则可以使用[服务模型元数据实用程序工具 （Svcutil.exe） 下载](servicemodel-metadata-utility-tool-svcutil-exe.md)元数据，该工具同时为客户端生成代码文件以及配置文件。 （有关使用该工具的详细信息，请参阅[使用 WCF 客户端访问服务](accessing-services-using-a-wcf-client.md)。如果服务不发布 MEX 终结点，并且也没有通过 HTTP 或 HTTPS 使其元数据可用，则必须与服务创建者联系，获取描述安全要求和元数据的文档。  
...
## <a name="validating-security"></a>验证安全性  
 ...
 但是，如果您选择从一个知之甚少的源下载客户端和元数据，请务必验证代码所使用的安全措施。 例如，一定不要简单地创建一个向服务发送个人或财务信息的客户端，除非该服务要求保密性和完整性（这是最基本的要求）。 您应该信任服务的所有者，只要您愿意披露此类信息，因为此类信息对他们可见。  
...
2. 指定一个实际客户端凭据。 实际客户端凭据称为*客户端凭据值*，以将其与类型区分开来。 例如，如果客户端凭据类型指定了证书，您必须提供由该服务所信任的证书颁发机构颁发的 X.509 证书。  
  
### <a name="determining-the-client-credential-type"></a>确定客户端凭据类型  
 如果生成了配置文件 Svcutil.exe 工具，请检查[\<绑定>](../configure-apps/file-schema/wcf/bindings.md)部分以确定需要哪些客户端凭据类型。 该节包含指定安全需求的绑定元素。 具体而言，\<请检查每个绑定的安全>元素。 该元素包含 `mode` 属性，该属性可设置为以下三个可能的值之一：`Message`、`Transport` 或 `TransportWithMessageCredential`。 该属性的值确定模式，而模式则确定哪个子元素有效。  
  
 元素`<security>`可以包含 或`<transport>``<message>`元素，也可以同时包含两者。 有效元素是指与安全模式匹配的那个元素。 例如，下面的代码指定安全模式为 `"Message"`，`<message>` 元素的客户端凭据类型为 `"Certificate"`。 在这种情况下，`<transport>` 元素可以忽略。 但是，`<message>` 元素指定必须提供 X.509 证书。  
...
- 在客户端代码中对其进行编程（使用 `SetCertificate` 方法）。  
 ...
 通过为客户端添加>配置文件部分[\<并使用](../configure-apps/file-schema/wcf/behaviors.md)`clientCredentials`元素（如下所示）的行为。  
  
#### <a name="setting-a-clientcredentials-value-in-code"></a>在代码\<中设置客户端凭据>值  
 要在<xref:System.ServiceModel.ClientBase%601.ClientCredentials%2A><xref:System.ServiceModel.ClientBase%601>[\<代码中设置客户端凭据>](../configure-apps/file-schema/wcf/clientcredentials.md)值，必须访问类的属性。 该属性返回一个 <xref:System.ServiceModel.Description.ClientCredentials> 对象，使用该对象可以访问各种凭据类型，如下表所示。  
...
#### <a name="setting-a-clientcredentials-value-in-configuration"></a>设置\<客户端凭据>配置中的值  
 凭据值通过使用终结点行为作为[\<客户端凭据>](../configure-apps/file-schema/wcf/clientcredentials.md)元素的子元素来指定。 所使用的元素取决于客户端凭据类型。 例如，下面的示例显示使用<[\<客户端证书>](../configure-apps/file-schema/wcf/clientcertificate-of-clientcredentials-element.md)设置 X.509 证书的配置。  
...
```  
 ...
 要在配置中设置客户端凭据，>[\<](../configure-apps/file-schema/wcf/endpointbehaviors.md)配置文件中添加终结点行为。 此外，添加的行为元素必须使用`behaviorConfiguration`[\<\<客户端>元素的终结点>](../configure-apps/file-schema/wcf/endpoint-of-client.md)属性链接到服务的终结点，如以下示例所示。 `behaviorConfiguration` 属性的值必须与行为 `name` 属性的值相匹配。  
...
> 有些客户端凭据值无法使用应用程序配置文件来设置，例如，用户名和密码值或 Windows 用户和密码值。 这种凭据值只能在代码中指定。  
 ...
 有关设置客户端凭据的详细信息，请参阅[：指定客户端凭据值](how-to-specify-client-credential-values.md)。  
