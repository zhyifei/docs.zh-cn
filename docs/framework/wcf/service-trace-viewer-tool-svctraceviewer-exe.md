...
## <a name="configuring-tracing"></a>配置跟踪  
 ...
 你可以配置跟踪使用应用程序的配置文件 — 为 Web.config; 对于 Web 承载的应用程序，或*Appname*.config 自承载应用程序。 下面是一个示例：  
...
```  
 ...
 此示例在名为 SdrConfigExample.e2e 的根目录中创建一个文件。 当你使用跟踪查看器打开文件"打开和查看 WCF 跟踪文件"部分中所述时，你可以看到已发送的所有消息。  
...
1.  在服务跟踪查看器启动后，单击**文件**，然后指向**打开**。 定位到跟踪文件的存储位置。  
  
2.  双击要打开的跟踪文件。  
  
    > [!NOTE]
    >  按住 Shift 的同时单击多个跟踪文件，可以同时选中并打开这些文件。 服务跟踪查看器合并所有文件的内容并显示为一个视图。 例如，可以同时打开客户端和服务的跟踪文件。 如果在配置中已启用消息日志记录和活动传播，则此功能十分有用。 通过这种方式，可以检查客户端和服务之间的消息交换。 你也可以将多个文件拖动到查看器中，或使用**项目**选项卡。有关详细信息，请参见“管理项目”一节。  
...
##### <a name="activity-view"></a>活动视图  
 后打开跟踪文件，你可以看到分组到活动中，显示在跟踪**活动**的左窗格中的视图。  
  
 **活动**视图显示活动名称，该活动中的跟踪数持续时间以及开始时间和结束时间。  
  
 单击所列出的任一活动，该活动中的跟踪都会显示在右侧跟踪窗格中。 然后，选择一个跟踪即可查看其详细信息。  
  
 你可以选择多个活动通过按**Ctrl**或**Shift**键并单击所需的活动。 跟踪窗格显示所选活动的所有跟踪。  
  
 你可以双击活动以将其显示在**Graph**视图。 另一种方法是选择一个活动，并切换到**Graph**视图。  
  
> [!NOTE]
>  活动"000000000000"是一个特殊活动，不能在图形视图中显示。 因为所有其他活动都与它链接，因此显示此活动会严重影响性能。  
...
##### <a name="graph-view"></a>图形视图  
 服务跟踪查看器最强大的功能之一是**Graph**视图，它以图表形式显示给定活动的跟踪数据。 通过图表形式，可以查看事件的逐步执行过程，可以查看当数据在多个活动之间移动时活动之间的相互关系。  
 ...
 在**Graph**视图中，每个垂直列表示的活动，并在列中的每个块代表一个跟踪。 活动按进程（或线程）分组。 活动之间的小箭头表示传输。 进程之间的大箭头表示消息交换。 选定的活动始终为黄色。  
...
1.  在传输图标左侧的上查找以"+"号的传输跟踪。  
  
2.  单击"+"，或按**Ctrl**和"+"使用键盘。  
  
3.  下一个活动出现在图形中。  
  
4.  A"-"传输图标左侧会出现。 单击"-"号或按 Ctrl 和"-"，会折叠活动传输。  
...
###### <a name="layout-mode"></a>布局模式  
 查看器有两种布局模式：**过程**和**线程**。 此设置定义最大组织单位。 默认布局模式是**过程**，这意味着活动按进程在图中分组。  
...
##### <a name="trace-pane"></a>跟踪窗格  
 ...
 你可以通过右键单击跟踪并选择跟踪的原始 XML 复制到剪贴板**复制到剪贴板的跟踪**。  
  
##### <a name="detail-pane"></a>详细信息窗格  
 服务跟踪查看器的右下窗格为“详细信息”窗格。 它提供三个选项卡来查看跟踪的详细信息。  
  
 **格式化**视图更有条理的方式显示的信息。 它以表和树的形式列出所有已知的 XML 元素，使得信息的读取和理解更加容易。  
  
 **XML**视图显示对应于所选跟踪的 XML。 它支持突出显示和语法着色。 当你使用**查找**来搜索字符串时，它会突出显示搜索结果。  
  
 **消息**视图显示消息日志跟踪消息部分的 xml。 选择非消息跟踪时，该视图不可见。  
...
#### <a name="using-the-filter-toolbar"></a>使用筛选器工具栏  
 筛选器工具栏显示在工具的顶部。 如果不存在，则可以激活在**视图**菜单。 此工具栏具有三个组件：  
  
-   查找：**查找**定义在筛选操作中查找的主题。 例如，如果你想要在进程 X 的上下文中找到已发出的所有跟踪，将此字段设置为 X 和**搜索中**进程名称字段。 如果选择了某个基于时间的筛选器，则此字段更改为 DateTime 选择器控件。  
  
-   搜索范围：此字段定义要应用的筛选器类型。  
  
-   级别：级别设置定义筛选器允许的最小跟踪级别。 例如，如果级别设置为“错误及严重错误”，则只会显示“错误”和“严重”级别的跟踪。 此筛选器与“查找”和“搜索范围”指定的条件结合使用。  
  
 **立即筛选**按钮用于启动筛选操作。 某些筛选器，尤其是当它们应用于大型数据集时，需要很长时间才能完成操作。 你可以通过按取消筛选操作**停止**下状态栏中显示的按钮**操作**菜单。  
...
1.  指定你在寻找值**查找内容**字段。 在此示例中，输入要搜索的线程 ID。  
...
-   “查找”对话框提供了更多查找选项。 它是可通过访问**编辑**菜单上，或通过快捷键 Ctrl + f。  
  
 “查找”工具栏显示在查看器的顶部。 如果不存在，则可以激活在**视图**菜单。 此工具栏具有两个组件：  
...
#### <a name="step-forward-or-backward"></a>逐步向前或逐步向后  
 如果你将每个跟踪都看作程序中行，则逐步向前是代码的非常类似于"逐过程"在 Visual Studio 集成开发环境 (IDE)。 区别在于，在跟踪中还可以逐步向后。 逐步向前意味着移动到活动中的下一个跟踪。  
  
-   逐步向前： 使用**活动**菜单上或按"F10"。 此外可以在跟踪窗格中"向下"使用箭头键。  
...
#### <a name="follow-transfer"></a>追踪传输  
 ...
 "追踪传输"是在调试器中类似于"单步执行"。 它追踪从 A 到 B 的传输。它对其他跟踪没有任何影响。  
  
 有两种追踪传输的方法：使用鼠标或使用键盘：  
  
-   使用鼠标：在跟踪窗格中双击传输跟踪。  
  
-   通过键盘： 选择传输跟踪，并在中使用"追踪传输"**活动**菜单上或按"F11"  
...
#### <a name="jump-to-next-or-previous-transfer"></a>跳转到下一个或上一个传输  
 在分析当前活动或选定的活动（如果选择了多个活动）时，可能希望快速找到它传输到的活动。 "跳转到下一个传输"，可在活动中查找下一个传输跟踪。 一旦找到传输跟踪，你可以使用"追踪传输"单步执行下一个活动。  
  
-   跳转到下一个传输： 使用**活动**菜单上或按"Ctrl + F10"。  
  
-   跳转到上一个传输： 使用**活动**菜单上或按"Ctrl + F9"。  
  
#### <a name="navigate-in-graph-view"></a>在图形视图中定位  
 尽管在活动窗格和跟踪窗格中导航是类似于调试，但使用**Graph**视图提供更好的体验，在导航窗格中。 有关详细信息，请参阅"关系图视图"部分。  
  
### <a name="loading-large-trace-files"></a>加载大型跟踪文件  
 跟踪文件可能非常大。 例如，如果启用跟踪，在"详细"级别，生成的跟踪文件运行几分钟的时间很容易就可以达到数百兆字节甚至更大，具体取决于网络速度和通信模式。  
 ...
 为了避免慢速加载和响应时间在分析大型跟踪文件，服务跟踪查看器提供了一种称为"部分加载"，即一次仅加载跟踪的一小部分功能。 例如，您可能有一个大于 1GB 的跟踪文件，在服务器上已运行了几天。 当发生某些错误，又想分析该跟踪时，将不必打开整个跟踪文件。 您可以加载可能发生错误的某个时间段内的跟踪。 由于缩小了范围，服务跟踪查看器工具可以更快地加载文件，而您可以使用一个较小的数据集来标识这些错误。  
...
## <a name="understanding-trace-icons"></a>了解跟踪图标  
 以下是该服务跟踪查看器工具使用中的图标列表**活动**视图中， **Graph**视图和**跟踪**窗格中表示不同项。  
...
## <a name="troubleshooting"></a>疑难解答  
 如果你没有写入注册表的权限，你收到以下错误消息"Microsoft 服务跟踪查看器未注册到系统"当你使用"`svctraceviewer /register`"命令注册工具。 如果出现这种情况，您应使用对注册表有写访问权限的帐户登录。  
