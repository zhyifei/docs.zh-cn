...
### <a name="sending-declared-faults"></a>发送已声明的错误  
 ...
 若要向客户端传送 `GreetingFault` 错误信息，需要捕捉相应的错误条件，并引发一个类型为 <xref:System.ServiceModel.FaultException%601?displayProperty=nameWithType> 的新 `GreetingFault`，其参数为一个新的 `GreetingFault` 对象，如下面的代码示例所示。 如果客户端是 WCF 客户端应用程序，它会将其视为托管异常的类型是<xref:System.ServiceModel.FaultException%601?displayProperty=nameWithType>类型的`GreetingFault`。  
  
 [!code-csharp[FaultContractAttribute#5](../../../samples/snippets/csharp/VS_Snippets_CFX/faultcontractattribute/cs/services.cs#5)]
 [!code-vb[FaultContractAttribute#5](../../../samples/snippets/visualbasic/VS_Snippets_CFX/faultcontractattribute/vb/services.vb#5)]  
  
### <a name="sending-undeclared-faults"></a>发送未声明的错误  
 发送未声明的错误可能非常有用，可以快速诊断和调试 WCF 应用程序，但其用途中的问题，因为调试工具是有限。 一般来讲，在进行调试时，建议您使用 <xref:System.ServiceModel.Description.ServiceDebugBehavior.IncludeExceptionDetailInFaults%2A?displayProperty=nameWithType> 属性。 在将此值设置为 true 时，客户端会将此类错误视为类型为 <xref:System.ServiceModel.FaultException%601> 的 <xref:System.ServiceModel.ExceptionDetail> 异常。  
...
## <a name="handling-faults"></a>处理错误  
 在 WCF 客户端在通信过程中发生的客户端应用程序感兴趣的 SOAP 错误作为托管异常引发。 尽管有许多可以在任何程序执行过程中发生的异常，使用 WCF 客户端编程模型的应用程序可以期望处理由通信的以下两个类型的异常。  
...
> [!NOTE]
>  当 WCF 服务具有<xref:System.ServiceModel.ServiceBehaviorAttribute.IncludeExceptionDetailInFaults%2A?displayProperty=nameWithType>或<xref:System.ServiceModel.Description.ServiceDebugBehavior.IncludeExceptionDetailInFaults%2A?displayProperty=nameWithType>属性设置为`true`客户端会将其视为的未声明<xref:System.ServiceModel.FaultException%601>类型的<xref:System.ServiceModel.ExceptionDetail>。 客户端可以捕捉这一特定错误，也可以在 <xref:System.ServiceModel.FaultException> 的 catch 块中处理该错误。  
...
### <a name="handle-exceptions-when-closing-the-channel"></a>在关闭通道时处理异常  
 大部分前面的讨论必须显式发送客户端，当客户端应用程序调用 WCF 客户端对象上的操作的消息处理在处理应用程序消息，即过程中发送的错误。  
...
> [!NOTE]
>  这些示例代码没有使用 `using` 协定。 由于关闭通道可能会引发异常，它被建议应用程序创建 WCF 客户端第一次，然后打开使用，并关闭 WCF 客户端在同一个 try 块。 有关详细信息，请参阅[WCF 客户端概述](../../../docs/framework/wcf/wcf-client-overview.md)和[避免问题与 Using 语句](../../../docs/framework/wcf/samples/avoiding-problems-with-the-using-statement.md)。  
