...
### <a name="logging-sensitive-information"></a>记录敏感信息  
 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 不会修改应用程序特定的标头和正文中的任何数据。 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 也不会跟踪应用程序特定的标题或正文数据中的个人信息。  
...
-   启用记录已知 PII：当启用记录已知 PII 时发出此事件。 发生这种情况时`enableLoggingKnownPii`属性中`machineSettings`Machine.config 文件元素设置为`true`，和`logKnownPii`属性`source`App.config 或 Web.config 文件中的元素设置为`true`.  
  
-   不允许记录已知 PII：当不允许记录已知 PII 时发出此事件。 发生这种情况时`logKnownPii`属性`source`App.config 或 Web.config 文件中的元素设置为`true`，但`enableLoggingKnownPii`属性中`machineSettings`Machine.config 文件元素设置为`false`. 不引发异常。  
