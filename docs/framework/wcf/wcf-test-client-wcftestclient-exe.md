...
通常, 你可以在以下位置中找到 WCF 测试客户端 (wcftestclient.exe): `C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\Common7\IDE` -社区可以是 "企业"、"专业" 或 "社区" 中的一个, 具体取决于安装的 Visual Studio 级别。
...
对于数据集参数, 请单击 **...** 按钮 "**编辑 ...** " 在显示数据网格的新窗口中进行编辑。 请注意 "**复制数据集**" 和 "**粘贴数据集**" 按钮的外观。 如果第一次编辑时 DataSet 对象的架构未知，则 DataGrid 为空。 您必须将具有相同架构的 DataSet 对象粘贴到 DataGrid 中的当前对象。 （请注意，在粘贴操作之前需要从其他位置复制架构。）您还可以通过单击 "**复制数据集**" 按钮来复制 dataset 对象以供将来使用。
...
"**工具**->**选项**客户端配置" 选项卡包含 "启动服务时始终重新生成配置" 选项 (默认情况下启用)。-> 此选项指定每次 WCF 测试客户端加载服务时, 它会根据最新的服务协定和服务 App.config 文件重新生成一个配置文件。
...
你还可以在命令行中启动 WCF 测试客户端, 使用`/ProjectPath`开关指定新的所需路径来存储生成的文件, 或者`/RestoreProjectPath`使用开关还原默认位置。 语法如下所示：
...
- 绑定WSFederationbinding、任何上下文绑定和 Https 绑定, WebHttpbinding (Json 响应消息支持)。
