...
2.  通过单击创建 WCF 服务库项目**文件**菜单并选择**新建**，**项目**。 在**新项目**对话框中，在对话框的左侧展开**Visual C#** 对于 C# 项目或**其他语言**然后**Visual Basic** Visual Basic 项目。 在所选的语言下选择**WCF**并将在对话框的中心部分上显示的项目模板列表。 选择**WCF 服务库**，并且在**名称**文本框中输入`GettingStartedLib`，在对话框底部的**解决方案名称**文本框中输入`GettingStarted`。  
