...
- 使用向导:单击 "**创建新服务**" 链接 。 在 "任务窗格" 或 "摘要" 页上, 启动向导。 也可以在 "**文件**" 菜单-> "**添加新项**"。

- 手动创建:你可以右键单击 "**服务**" 节点, 然后选择 "**新建服务**"。
...
> [!NOTE]
> 无法在此工具中编辑[ \<baseAddressPrefixFilters >](../../../docs/framework/configure-apps/file-schema/wcf/baseaddressprefixfilters.md)的值。 若要添加或修改此元素，应使用文本编辑器或 Visual Studio。
...
- 创建者向导:单击 "**创建新客户端 ...** " 链接 在窗口左下方的**任务窗格**上, 或在 "摘要"**页**上启动向导。 也可以在 "**文件**" 菜单-> "**添加新项**"。 该向导会提示您指向从中生成客户端配置的服务配置的位置。 然后，您可以选择要连接的服务终结点。
...
如果使用标准终结点, 则当你尝试删除它时, 会显示一条警告消息:**标准终结点正在使用中。如果现在删除，请确保删除其他配置部分中(例如，从服务终结点或客户端终结点中)对该标准终结点的所有引用。否则，配置将无效，并且下次将无法打开。是否确实要删除标准终结点？ "**
...
"**诊断**" 节点显示配置文件中的所有诊断设置。 它使你能够打开或关闭性能计数器、启用或禁用 Windows Management Instrumentation (WMI)、配置 WCF 跟踪以及配置 WCF 消息日志记录。 **诊断**节点中的设置对应于配置文件中`system.diagnostics`的 < > 部分`<diagnostics>`和部分`<system.serviceModel>` 。
...
2. 单击 "**切换性能计数器**"。 性能计数器包含三种状态:Off (默认值)、ServiceOnly 和 All。 单击链接可在这三种状态之间切换设置。
...
3. 单击 "**跟踪级别**" 链接以调整跟踪级别。 有六个跟踪级别:Off、严重、错误、警告、信息和详细信息。 **活动跟踪**和**传播活动**选项使您能够使用 WCF 活动跟踪功能。
...
3. 单击 "**日志级别**" 链接以调整日志级别。 有三个日志级别:格式不正确, 服务和传输。
...
**诊断** **摘要页**可以完成配置诊断中的最常见任务。 但是, 如果要手动编辑 "侦听器" 和 "源" 设置, 则必须在 "**消息日志记录**"、"**侦听器**和**源**" 节点中展开 "**诊断**" 节点并编辑设置。
...
4. 在 "**文件**" 菜单中, 指向 "**添加新项**", 然后单击 "**服务**"。 “新建服务元素向导”随即打开。
...
不能保证配置编辑器生成的服务配置文件是安全的。 请参阅[安全](../../../docs/framework/wcf/feature-details/security.md)文档, 了解如何保护 WCF 服务。
