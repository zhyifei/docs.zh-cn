...
-   **宿主级**： 在宿主共享级别，缓存是仅供工作流服务主机中承载的工作流实例。 缓存还可在进程范围的缓存中的工作流服务主机之间共享。  
  
-   **实例级别**： 在实例共享级别，缓存可供其整个生存期内的特定工作流实例，但不可供其他工作流实例。  
  
-   **无缓存**： 缓存已关闭默认情况下如果必须使用在配置中定义的终结点的工作流。 在这种情况下，由于打开缓存可能会造成不安全的情况，因此也建议关闭缓存。 例如，在针对每次发送都需要使用不同的标识（不同的凭据或使用模拟）时。  
