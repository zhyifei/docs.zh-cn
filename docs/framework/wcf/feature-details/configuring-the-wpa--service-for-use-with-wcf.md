...
## <a name="configuring-a-site-with-non-http-bindings"></a>使用非 HTTP 绑定配置站点  
 ...
 applicationHost.config 是一个 XML 文本文件，可以使用任何标准文本编辑器（如记事本）打开。 但是，IIS 7.0 命令行配置工具 （appcmd.exe） 是添加非 HTTP 站点绑定的首选方式。  
...
## <a name="enabling-an-application-to-use-non-http-protocols"></a>使应用程序可以使用非 HTTP 协议  
 您可以在应用程序级别启用或禁用单个网络协议。 下面的命令说明如何为在 `Default Web Site` 中运行的应用程序同时启用 HTTP 和 net.tcp 协议。  
  
```console  
appcmd.exe set app "Default Web Site/appOne" /enabledProtocols:net.tcp  
```  
  
 还可以在\<应用程序中设置已启用的协议列表，>存储在应用程序Host.config中的站点 XML 配置的元素。  
...
## <a name="building-a-wcf-service-that-uses-was-for-non-http-activation"></a>生成一个将 WAS 用于非 HTTP 激活的 WCF 服务  
 执行安装和配置 WAS 的步骤（请参阅[：安装和配置 WCF 激活组件](../../../../docs/framework/wcf/feature-details/how-to-install-and-configure-wcf-activation-components.md)），将服务配置为使用 WAS 进行激活类似于配置 IIS 中托管的服务。  
