...
您可以使用 Svcutil.exe 从正在运行的服务中下载元数据并将元数据保存到本地文件。 对于 HTTP 和 HTTPS URL 方案，Svcutil.exe 尝试使用 Ws-metadataexchange 和[XML Web Service 发现](https://go.microsoft.com/fwlink/?LinkId=94950)检索元数据。 对于所有其他 URL 架构，Svcutil.exe 仅使用 WS-MetadataExchange。  
...
> [!CAUTION]
> 当运行 Svcutil.exe 来获取公开两个不同服务协定的服务的元数据时，每个服务协定分别包含同一名称的操作时，Svcutil.exe 会显示错误，指出 "无法从 ... 中获取元数据"例如，如果你有一个服务，该服务公开一个`ICarService`名为的服务协定，该协定具有一个操作`Get(Car c)` ，并且该服务公开了一个具有`Get(Book b)`操作的名`IBookService`为的服务协定。 若要解决此问题，请执行下列操作之一：
>
> - 重命名其中的一项操作。
...
3. <`url`> 参数指定提供元数据的服务终结点的 URL 或联机托管的元数据文档。 <`epr`> 参数指定一个 XML 文件的路径，该文件包含支持 ws-metadataexchange 的服务`EndpointAddress`终结点的 ws-addressing。  
