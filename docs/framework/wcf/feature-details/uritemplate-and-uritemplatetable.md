...
```  
 ...
 在路径表达式中，首尾斜杠是可选的。 查询表达式和片段表达式可完全省略。 路径由一系列由分隔的段组成 /，每个段都可以具有文本值、 一个变量名 （写在 {大括号} 中） 或通配符 (编写为\*)。 在上一模板中，“\weather\”段为文本值，而“{state}”和“{city}”为变量。 变量采用其名称从其大括号的内容和更高版本可以使用一个具体的值，以创建替换它们*关闭 URI*。 通配符是可选的但只能出现在其中进行逻辑匹配"路径其余部分"的 URI 的末尾。  
  
 查询表达式（如果有）指定一系列用“&”分隔的无序名称/值对。 查询表达式的元素可以是文本对 (x=2) 或变量对 (x={var})。 只有查询的右侧可以有变量表达式。 不允许 {someName} = {someValue}。 不允许使用不成对的值 (?x)。 没有任何空的查询表达式和查询表达式包含只需单个之间的区别？（同时意味着"任何查询"）。  
...
-   "{shoe} /boat/？ 平台 = {shoe}"– 重复变量名。  
  
-   "？ x = 2&x = 3"– 查询字符串中的名称/值对必须是唯一的即使它们是文本。  
  
-   "？ x = 2 （& a)"– 查询字符串的格式不正确。  
  
-   "？ 2 2&x = {shoe}"– 查询字符串必须是名称/值对。  
  
-   "？ y = 2 （& a) （& a) X = 3"– 查询字符串必须是名称/值对，名称不能以开头 &。  
...
### <a name="matching-and-compound-path-segments"></a>匹配和复合路径段  
 复合路径段允许定义在单个路径段内具有多个变量的 UriTemplate。 例如，在下面的模板字符串:"地址 / {state}。{city}"相同的段内定义了两个变量 （state 和 city）。 该模板会匹配诸如"http://example.com/Washington.Redmond"的 URL，但它也会匹配"http://example.com/Washington.Redmond.Microsoft"的 URL。 在后一种情况下，state 变量将包含"Washington"，city 变量将包含"Redmond.Microsoft"。 这时，任何文本（“/”除外）都将与 {city} 变量相匹配。 如果你希望模板不要匹配"额外"的文本，可将变量放在单独的模板段中，例如:"地址 / {state} / {city}。  
...
### <a name="template-equivalence"></a>模板等效性  
 两个模板可认为是*结构等效*时所有模板的文本匹配且它们具有相同的段中的变量。 例如，以下模板是结构等效的：  
