...
- 不透明二进制流  
 ...
 这意味着 WCF WEB HTTP 编程模型可以处理任何类型的数据，但您可能对 <xref:System.IO.Stream>进行编程。  
