...
- 用于建立安全上下文 (使用传输安全会话或消息安全会话) 的过程已关闭 (<xref:System.ServiceModel.NonDualMessageSecurityOverHttp.EstablishSecurityContext%2A>在消息安全或传输无法建立安全性的情况下, 属性设置为) `false`会话用于传输安全案例。 HTTPS 就是此类传输的一个示例）。  
...
## <a name="token-caches-allow-replay-using-obsolete-data"></a>令牌缓存允许使用过时数据的重放  
 WCF 使用本地安全机构 (LSA) `LogonUser`函数通过用户名和密码对用户进行身份验证。 由于登录功能是一种代价高昂的操作, WCF 使你可以缓存代表经过身份验证的用户的令牌, 以提高性能。 该缓存机制保存来自 `LogonUser` 的结果，以备将来之用。 默认情况下, 此机制处于禁用状态;若要启用它, 请<xref:System.ServiceModel.Security.UserNamePasswordServiceCredential.CacheLogonTokens%2A>将属性`true`设置为, 或`cacheLogonTokens` [ \<使用 userNameAuthentication >](../../../../docs/framework/configure-apps/file-schema/wcf/usernameauthentication.md)的属性。  
 ...
 缓解此操作:将`cachedLogonTokenLifetime`值设置为用户所需的最短时间范围, 以减少攻击时段。  
  
## <a name="issued-token-authorization-expiration-reset-to-large-value"></a>颁发的令牌授权:过期重置为大值  
