...
-   <xref:System.ServiceModel.ConcurrencyMode>：最多允许每个实例上下文同时拥有一个对该实例上下文中的消息进行处理的线程。 其他希望使用同一个实例上下文的线程必须一直阻塞，直到原始线程退出该实例上下文为止。  
  
-   <xref:System.ServiceModel.ConcurrencyMode>：每个服务实例都可以拥有多个同时处理消息的线程。 若要使用此并发模式，服务实现必须是线程安全的。  
  
-   <xref:System.ServiceModel.ConcurrencyMode>：每个服务实例一次只能处理一个消息，但可以接受可重入的操作调用。 仅当服务通过 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 客户端对象提供服务时才会接受这些调用。  
  
> [!NOTE]
>  理解和开发能够安全地使用多个线程的代码可能比较困难。 在使用 <xref:System.ServiceModel.ConcurrencyMode> 或 <xref:System.ServiceModel.ConcurrencyMode> 值之前，应确保已针对这些模式对服务进行了适当设计。[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)] <xref:System.ServiceModel.ServiceBehaviorAttribute.ConcurrencyMode%2A>.  
  
 并发的使用与实例化模式有关。 在 <xref:System.ServiceModel.InstanceContextMode>``实例化过程中，与并发没有关系，因为每个消息都由一个新的 <xref:System.ServiceModel.InstanceContext> 处理，因此 <xref:System.ServiceModel.InstanceContext> 中处于活动状态的线程永远都不会超过一个。  
  
 下面的代码示例演示如何将 <xref:System.ServiceModel.ServiceBehaviorAttribute.ConcurrencyMode%2A> 属性设置为 <xref:System.ServiceModel.ConcurrencyMode>。  
...
## 会话与 InstanceContext 设置进行交互  
 会话和 <xref:System.ServiceModel.InstanceContext> 根据协定中 <xref:System.ServiceModel.SessionMode> 枚举的值和服务实现上的 <xref:System.ServiceModel.ServiceBehaviorAttribute.InstanceContextMode%2A?displayProperty=fullName> 属性的组合进行交互，该组合控制着通道和特定服务对象之间的联系。  
  
 下表显示了在给定服务的 <xref:System.ServiceModel.ServiceContractAttribute.SessionMode%2A?displayProperty=fullName> 属性值和 <xref:System.ServiceModel.ServiceBehaviorAttribute.InstanceContextMode%2A?displayProperty=fullName> 属性值组合的情况下，支持会话或不支持会话的传入通道的结果。  
...
|---------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|  
|PerCall|-   会话通道的行为：每个调用都具有一个会话和 <xref:System.ServiceModel.InstanceContext>。<br />-   无会话通道的行为：将引发异常。|-   会话通道的行为：每个调用都具有一个会话和 <xref:System.ServiceModel.InstanceContext>。<br />-   无会话通道的行为：每个调用都具有一个 <xref:System.ServiceModel.InstanceContext>。|-   会话通道的行为：将引发异常。<br />-   无会话通道的行为：每个调用都具有一个 <xref:System.ServiceModel.InstanceContext>。|  
|PerSession|-   会话通道的行为：每个通道都具有一个会话和 <xref:System.ServiceModel.InstanceContext>。<br />-   无会话通道的行为：将引发异常。|-   会话通道的行为：每个通道都具有一个会话和 <xref:System.ServiceModel.InstanceContext>。<br />-   无会话通道的行为：每个调用都具有一个 <xref:System.ServiceModel.InstanceContext>。|-   会话通道的行为：将引发异常。<br />-   无会话通道的行为：每个调用都具有一个 <xref:System.ServiceModel.InstanceContext>。|  
|Single|-   会话通道的行为：所有调用都具有一个会话和一个 <xref:System.ServiceModel.InstanceContext>。<br />-   无会话通道的行为：将引发异常。|-   会话通道的行为：所创建的单一实例或用户指定的单一实例都具有一个会话和 <xref:System.ServiceModel.InstanceContext>。<br />-   无会话通道的行为：所创建的单一实例或用户指定的单一实例都具有一个 <xref:System.ServiceModel.InstanceContext>。|-   会话通道的行为：将引发异常。<br />-   无会话通道的行为：所创建的每个单一实例或用户指定的单一实例都具有一个 <xref:System.ServiceModel.InstanceContext>。|  
