...
1. `workflowUnhandledException`在 <`behavior` `action` > 元素内的 < > 元素中添加一个 < > 元素, 使用特性指定发生未经处理的异常时要执行的操作, 如下面的示例中所示。`serviceBehaviors`  
    ...
    ```  
     ...
     可以将 <`workflowUnhandledException`> 元素的属性设置为以下值之一:`action`  
     ...
     **terminate**  
     ...
     有关的详细信息<xref:System.ServiceModel.Activities.Description.WorkflowUnhandledExceptionBehavior>, 请参阅[工作流服务主机扩展性](../../../../docs/framework/wcf/feature-details/workflow-service-host-extensibility.md)。  
