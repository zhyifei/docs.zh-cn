...
|---------------------------------|--------------------------------------|----------------------------------------------|------------------------------|  
|强制|真|WS-AT|事务必须以可以互操作的 WS-AT 格式流动。|  
|强制|真|OleTransactions|事务必须以 WCF OleTransactions 格式流动。|  
|强制|假|不适用|不适用，因为这是无效的配置。|  
|Allowed|真|WS-AT|事务可以以可互操作的 WS-AT 格式流动。|  
|Allowed|真|OleTransactions|事务可能以 WCF OleTransactions 格式流动。|  
|Allowed|假|任意值|不流动事务。|  
...
## <a name="enabling-transaction-flow-at-the-endpoint-level"></a>在终结点级别启用事务流  
 除了<xref:System.ServiceModel.TransactionFlowAttribute>属性提供的方法级别事务流设置外，WCF 还为事务流提供了一个终结点范围的设置，以允许管理员在更高级别控制事务流。  
 ...
 WCF 提供的大多数持续绑定都包含`transactionFlow`和`transactionProtocol`特性，使你能够将特定的绑定配置为接受传入事务。 有关设置配置元素的详细信息，请参阅[ \<binding >](../../../../docs/framework/misc/binding.md)。  
