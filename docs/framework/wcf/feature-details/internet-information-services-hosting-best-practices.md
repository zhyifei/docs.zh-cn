...
本主题概述承载 Windows Communication Foundation (WCF) 服务的一些最佳做法。  
  
## <a name="implementing-wcf-services-as-dlls"></a>将 WCF 服务实现为 DLL  
 实现 WCF 允许你之外重用该服务的 Web 应用程序模型，例如，可能没有 Internet 信息服务 (IIS) 部署的测试环境中为部署到 Web 应用程序的 \bin 目录的 DLL 的服务。  
...
> [!NOTE]
>  使 WCF 用于消息层可靠性和安全性的协议利用可变的内存中状态。 WCF 可靠会话和安全会话可能会由于应用程序回收而意外终止。 IIS 承载的应用程序利用这些协议应该依赖于关联应用程序层状态 （例如，应用程序层结构或自定义关联标头） 或禁用的 WCF 提供的会话密钥之外的内容IIS 进程回收的托管应用程序。  
  
## <a name="optimizing-performance-in-middle-tier-scenarios"></a>在中间层方案中优化性能  
 为获得最佳性能的*中间层方案*-调用其他服务以响应传入消息的服务 — 一次实例化 WCF 服务客户端向远程服务和跨多个传入重用它请求。 实例化 WCF 服务客户端是相对于进行服务调用上预先存在的客户端实例，代价高昂的操作和中间层方案通过跨请求缓存远程客户端来产生明显的性能提升。 WCF 服务客户端是线程安全的因此不需要跨多个线程同步对客户端访问。  
...
## <a name="wcf-in-multi-homed-or-multi-named-scenarios"></a>多宿主或多名称方案中的 WCF  
 你可以部署内的 IIS Web 场，其中的一组计算机共享公共外部名称的 WCF 服务 (如http://www.contoso.com)但按不同的主机名单独寻址 (例如，http://www.contoso.com可能将流量定向到两台不同计算机名为http://machine1.internal.contoso.com和http://machine2.internal.contoso.com)。 此部署方案完全支持由 WCF，但需要承载 WCF 服务在服务的元数据 （Web Services 描述语言） 显示正确的 （外部） 主机名的 IIS 网站的特殊配置。  
  
 若要确保正确的主机名显示在生成 WCF 服务元数据，配置的 IIS 网站中承载 WCF 服务以使用显式主机名的默认标识。 例如，驻留在 www.contoso.com 场内的计算机应使用的 IIS 站点绑定 *:80:www.contoso.com 为 HTTP 和\*: 443:www.contoso.com 为支持 HTTPS。  
...
## <a name="enabling-asynchronous-processing"></a>启用异步处理  
 默认情况下以同步方式处理发送到承载在 IIS 6.0 及更早版本的 WCF 服务的消息。 ASP.NET 在各自的线程 （ASP.NET 工作线程） 上调入 WCF 和 WCF 使用另一个线程来处理该请求。 WCF 在完成其处理之前会保持 ASP.NET 工作线程。 这将导致同步处理请求。 以异步方式处理请求使可伸缩性更好，因为它减少了处理的请求 – WCF 不保持 ASP.NET 线程在处理请求时所需的线程数。 对于运行 IIS 6.0，因为没有方法来限制打开到服务器的传入请求的计算机不建议使用异步行为*拒绝服务*(DOS) 攻击。 从 IIS 7.0 开始，引入了并发请求限制：`[HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\ASP.NET\2.0.50727.0]"MaxConcurrentRequestsPerCpu`。 借助这一新的限制，可安全地使用异步处理。  默认情况下，在 IIS 7.0 中会注册异步处理程序和模块。 如果关闭了此功能，可在应用程序的 Web.config 文件中手动启用对请求的异步处理。 所使用的设置取决于 `aspNetCompatibilityEnabled` 设置。 如果已将 `aspNetCompatibilityEnabled` 设置为 `false`，请按照下面的配置代码段所示配置 `System.ServiceModel.Activation.ServiceHttpModule`。  
...
## <a name="see-also"></a>请参阅  
 [服务主机示例](http://msdn.microsoft.com/library/f703a3f6-0fba-418a-a92f-7ce75ccfa47e)  
