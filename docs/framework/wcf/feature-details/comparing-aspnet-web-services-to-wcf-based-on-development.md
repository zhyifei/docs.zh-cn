...
Windows Communication Foundation (WCF) 具有一个 ASP.NET 兼容模式选项来启用 WCF 应用程序进行编程和配置如 ASP.NET Web 服务，并且还可以模仿它们的行为。 下列各节比较 ASP.NET Web 服务和 WCF 基于要求来使用这两种技术开发应用程序。  
...
-   <xref:System.Xml.Serialization> 命名空间中的许多属性类型均可添加到 .NET Framework 类及其成员中，用于控制类实例的 XML 表示方式。  
 ...
 WCF 应用程序开发通常也可以使用的复杂类型定义。 WCF 可为使用相同的.NET Framework 类作为 ASP.NET Web 服务。  
  
 WCF<xref:System.Runtime.Serialization.DataContractAttribute>和<xref:System.Runtime.Serialization.DataMemberAttribute>可以添加到.NET Framework 类型，以指示该类型的实例将要序列化为 XML，以及的特定字段或属性的类型进行序列化，如下面的示例代码中所示。  
...
> [!NOTE]
>  尽管 ASP.NET Web 服务使用<xref:System.Xml.Serialization.XmlSerializer>，和 WCF ASP.NET 兼容模式会使 WCF 服务模拟 ASP.NET Web 服务的行为，ASP.NET 兼容性选项不限制使用<xref:System.Xml.Serialization.XmlSerializer>。 用户仍然可以将 <xref:System.Runtime.Serialization.DataContractSerializer> 用于以 ASP.NET 兼容模式运行的服务。  
...
```  
 ...
 通过定义一个或多个 WCF 终结点提供了一个 WCF 服务。 终结点由地址、绑定和服务协定定义。 地址用于定义服务的位置。 绑定用于指定与服务的通信方式。 服务协定用于定义服务可执行的操作。  
...
```  
 ...
 <xref:System.ServiceModel.ServiceContractAttribute>指定接口定义 WCF 服务约定，和<xref:System.ServiceModel.OperationContractAttribute>指示，如果有的话，接口的方法的定义服务协定的操作。  
...
|NetTcpBinding|在网络的 WCF 软件实体之间的安全、 可靠且高性能通信。|  
|NetNamedPipeBinding|在同一计算机上的 WCF 软件实体之间的安全、 可靠且高性能通信。|  
|NetMsmqBinding|通过使用 MSMQ 的 WCF 软件实体之间的通信。|  
|MsmqIntegrationBinding|在 WCF 软件实体和通过使用 MSMQ 的另一个软件实体之间的通信。|  
|NetPeerTcpBinding|通过使用 Windows 对等网络 WCF 软件实体之间的通信。|  
 ...
 WCF 应用程序的自定义绑定很容易定义为 WCF 用于实现单个协议的绑定元素类的集合。 可以将新的绑定元素编写为表示其他协议。  
...
```  
 ...
 在 IIS 5.1 或 6.0 中，Windows 进程激活服务 (WAS) 中提供的 IIS 7.0 一部分以及任意.NET 应用程序中，可以很容易承载 WCF 服务。 若要在 IIS 5.1 或 6.0 中承载某项服务，此服务必须使用 HTTP 作为通信传输协议。  
...
4.  将配置文件复制到虚拟目录下，并将其命名为 Web.config。  
 ...
 若要托管.NET 应用程序中的 WCF 服务，将类库程序集引用的应用程序，此服务类型编译和服务使用向主机应用程序进行编程<xref:System.ServiceModel.ServiceHost>类。 下面是所需基本编程的示例：  
...
```  
 ...
 WCF 服务的任何终结点提供的地址是主机的相对于终结点的基址的地址。 对于每个通信传输协议，宿主都可以有一个基址。 在上面的配置文件配置示例中，为终结点选定的 <xref:System.ServiceModel.BasicHttpBinding> 将 HTTP 用作传输协议，因此终结点的地址 `EchoService` 与宿主的 HTTP 基址相关。 对于在前面的示例的宿主，HTTP 基址是http://www.contoso.com:8000/。 对于在 IIS 或 WAS 中承载的服务，其基址为服务的服务文件的 URL。  
  
 仅服务承载于 IIS 或 WAS，并将配置使用 HTTP 作为传输协议以独占方式，可使用 WCF 的 ASP.NET 兼容模式选项。 启用此选项需要执行下列步骤。  
...
2.  管理员必须将应用程序配置为使用 ASP.NET 兼容模式。  
    ...
    ```  
     ...
     WCF 应用程序还可以配置为使用.asmx 扩展作为服务文件而非.svc。  
    ...
    ```  
     ...
     该选项可以将保存你无需修改配置为使用.asmx 服务文件的 Url 时修改服务使用 WCF 客户端。  
  
## <a name="client-development"></a>客户端开发  
 ASP.NET Web 服务的客户端通过使用命令行工具 WSDL.exe 生成，该工具可将 .asmx 文件的 URL 作为输入提供。 WCF 提供的相应工具是[ServiceModel 元数据实用工具 (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)。 它会生成服务协定的定义和 WCF 客户端类的定义的代码模块。 它还使用服务的地址和绑定生成一个配置文件。  
 ...
 没有通过 ASP 生成的 WCF 客户端类中的名称能保证。NET 的 WSDL.exe 工具，默认情况下，匹配由 Svcutil.exe 工具生成的 WCF 客户端类中的名称。 尤其是，在默认情况下，必须使用 <xref:System.Xml.Serialization.XmlSerializer> 序列化的类的属性名将在 Svcutil.exe 工具生成的代码中获得后缀 Property，这与 WSDL.exe 工具的情况并不一样。  
...
```  
 ...
 WCF 提供了属性， <xref:System.ServiceModel.MessageContractAttribute>， <xref:System.ServiceModel.MessageHeaderAttribute>，和<xref:System.ServiceModel.MessageBodyMemberAttribute>描述由服务发送和接收的 SOAP 消息的结构。  
...
```  
 ...
 此语法将生成消息结构的显式表示形式，同时 ASP.NET Web 服务的代码也将暗示消息结构。 此外，在 ASP.NET 语法中，消息头表示为服务的属性，如`ProtocolHeader`属性在前面的示例中，而在 WCF 语法中，它们是更准确地表示为消息的属性。 此外，WCF 允许消息标头添加到终结点的配置。  
...
```  
 ...
 使用 IIS 51 中承载 HTTP 终结点使用查询 WSDL WCF 服务的.svc 文件发出 HTTP GET 请求，6.0 或 WAS 会导致 WCF 描述服务的 WSDL 响应。 如果 httpGetEnabled 设置为 true，则使用查询 WSDL 对 .NET 应用程序所承载服务的 HTTP 基址发出 HTTP GET 请求具有相同的效果。  
  
 但是，WCF 还响应 Ws-metadataexchange 请求的它将生成为描述服务的 WSDL。 ASP.NET Web 服务没有对 WS-MetadataExchange 请求的内置支持。  
  
 可以广泛地自定义 WCF 生成的 WSDL。 <xref:System.ServiceModel.Description.ServiceMetadataBehavior> 类提供了一些用于自定义 WSDL 的功能。 若要不生成 WSDL，而不是在给定的 URL 中使用静态 WSDL 文件，则还可以配置 WCF。  
...
## <a name="exception-handling"></a>异常处理  
 ...
 WCF 服务中未经处理的异常不是返回到客户端作为 SOAP 错误，以防止通过异常意外公开的敏感信息。 为了进行调试，系统将提供一个配置设置以将未处理的异常返回客户端。  
...
```  
 ...
 WCF 提供了用于状态管理的可扩展对象。 可扩展对象就是实现 <xref:System.ServiceModel.IExtensibleObject%601> 的对象。 最重要的可扩展对象为 <xref:System.ServiceModel.ServiceHostBase> 和 <xref:System.ServiceModel.InstanceContext>。 `ServiceHostBase` 允许您保持同一个宿主上所有服务类型的所有实例都可访问的状态，而 `InstanceContext` 允许您保持可以被某个服务类型的同一个实例中运行的任意代码访问的状态。  
  
 此处，服务类型， `TradingSystem`，具有<xref:System.ServiceModel.ServiceBehaviorAttribute>，指定通过相同的 WCF 客户端实例的所有调用将都路由到服务类型的同一个实例。  
...
```  
 ...
 而 ASP.NET 基础上提供了控制状态信息的位置中<xref:System.Web.HttpContext>实际存储类，WCF，至少在其初始版本中，提供了不能控制可扩展对象的存储位置。 后者实质上非常最佳原因选择 ASP.NET 兼容模式的 WCF 服务。 如果必须管理可配置状态，则选择 ASP.NET 兼容模式使您可以按照它们在 ASP.NET 中的使用方式使用 <xref:System.Web.HttpContext> 类的功能，而且还可以通过使用存储的 <xref:System.Web.HttpContext> 类配置状态信息的管理位置。  
  
## <a name="security"></a>安全性  
 用于保证 ASP.NET Web 服务安全的选项就是那些用于保证任意 IIS 应用程序安全的选项。 因为不仅在 IIS 中，还在任何.NET 可执行文件中，可以承载 WCF 应用程序，保护 WCF 应用程序的选项必须进行独立于 IIS 的功能。 但是，为 ASP.NET Web 服务提供的功能也已可用于在 ASP.NET 兼容性模式下运行的 WCF 服务。  
  
### <a name="security-authentication"></a>安全性：身份验证  
 IIS 可提供对应用程序的访问进行控制的功能，您可以使用此功能选择匿名访问或各种身份验证模式：Windows 身份验证、摘要式身份验证、基本身份验证和 .NET Passport 身份验证。 Windows 身份验证选项可以用于控制对 ASP.NET Web 服务的访问。 但是，当在 IIS 中承载 WCF 应用程序时，必须配置 IIS 以允许匿名访问应用程序，以便该身份验证可由 WCF 本身，它支持在其他各个选项中的 Windows 身份验证。 其他内置选项包括用户名令牌、X.509 证书、SAML 令牌和 CardSpace 卡，但是也可以定义自定义身份验证机制。  
  
### <a name="security-impersonation"></a>安全性：模拟  
 ASP.NET 提供一个标识元素，ASP.NET Web 服务可通过使用该元素模拟当前请求提供了某个特定用户或任何用户的凭据。 该元素可以用于在 ASP.NET 兼容模式下运行的 WCF 应用程序中配置模拟。  
  
 WCF 配置系统提供其自身标识元素用于指定将特定用户模拟。 此外，WCF 客户端和服务可以独立配置进行模拟。 当客户端发送请求时，可将它们配置为模拟当前用户。  
...
### <a name="security-authorization-using-access-control-lists"></a>安全性：使用访问控制列表进行授权  
 访问控制列表 (ACL) 可用于限制对 .asmx 文件的访问。 但是，在 ASP.NET 兼容模式中的除忽略是 WCF.svc 文件上的 Acl。  
...
### <a name="security-role-based-authorization"></a>安全性：基于角色的授权  
 ...
 角色提供程序机制实际上可以独立于 ASP.NET 在任意.NET 应用程序，包括 WCF 应用程序中使用。 下面的示例配置为 WCF 应用程序演示如何使用 ASP.NET 角色提供程序是通过选择一个选项<xref:System.ServiceModel.Description.ServiceAuthorizationBehavior>。  
...
### <a name="security-claims-based-authorization"></a>安全性：基于声明的授权  
 WCF 的最重要的创新之一是它全面支持，以授权对基于声明的受保护资源的访问。 声明包含类型、权限和值等，例如驾驶执照。 它提出一组有关持有人的声明，其中一个是持有人的出生日期。 此声明的类型为出生日期，而声明的值为司机的出生日期。 声明授予持有人的权限指定持有人可对声明的值执行的操作。 如果是要声明司机的出生日期，权限为拥有：司机拥有该出生日期但是不能更改它。 基于声明的授权包含基于角色的授权，因为角色也是一种声明类型。  
  
 基于声明的授权可通过将一组声明与操作的访问要求进行比较实现，根据比较的结果，可获得操作的访问权或遭到拒绝。 在 WCF 中，您可以指定要用于运行基于声明的授权，再一次通过分配到的值类`ServiceAuthorizationManager`属性<xref:System.ServiceModel.Description.ServiceAuthorizationBehavior>。  
...
```  
 ...
 用于运行基于声明的授权的类必须派生自只有一种重写方法（即 <xref:System.ServiceModel.ServiceAuthorizationManager>）的 `AccessCheck()`。 每当服务操作调用，并且提供时，WCF 将调用该方法<xref:System.ServiceModel.OperationContext>具有的用户的声明的对象中其`ServiceSecurityContext.AuthorizationContext`属性。 WCF 执行的工作的任何安全令牌从组合有关用户的声明进行身份验证，这使提供的用户的评估这些声明是否满足需要操作任务。  
  
 WCF 自动组合从任何一种安全声明令牌是非常重要的创新，因为它使基于完全独立于身份验证机制的声明的授权代码。 与此相反，使用 ACL 或 ASP.NET 中的角色进行授权则与 Windows 身份验证息息相关。  
  
### <a name="security-confidentiality"></a>安全性：机密性  
 通过将 IIS 中的应用程序配置为使用安全超文本传输协议 (HTTPS)，可以在传输级别确保与 ASP.NET Web 服务交换的消息的机密性。 相同可以在 IIS 中托管的 WCF 应用程序。 但是，在 IIS 外部承载的 WCF 应用程序，还可以配置为使用安全传输协议。 更重要，WCF 应用程序还可以配置为之前传输这些，使用 Ws-security 协议保护消息的安全。 如果使用 WS-Security 协议保护消息正文的安全，则消息正文到达最终目的地之前在中介中加密传输。  
  
## <a name="globalization"></a>全球化  
 通过 ASP.NET 配置语言，您可以为这些服务单独指定区域性。 在 ASP.NET 兼容模式下，WCF 不支持该配置设置除外。 若要将本地化不使用 ASP.NET 兼容模式的 WCF 服务，请将区域性特定的程序集，此服务类型编译，并为每个区域性特定的程序集具有单独的区域性特定终结点。  
