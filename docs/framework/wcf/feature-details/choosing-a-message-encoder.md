...
ms.locfileid: "80345253"
---
# <a name="choosing-a-message-encoder"></a>选择消息编码器
本主题讨论在 Windows 通信基础 （WCF） 中包含的消息编码器中选择的条件：二进制、文本和消息传输优化机制 （MTOM）。  
...
客户端和服务都需要同意发送和接收压缩消息，因此必须在客户端和服务上的二进制MessageEncode 元素上配置 compressFormat 属性。 如果未配置服务或客户端使用压缩，则会引发 ProtocolException；但另一方面，启用压缩时应仔细考虑。 如果网络带宽是一个瓶颈，则压缩非常有用。 如果 CPU 是瓶颈，则压缩会减小吞吐量。 必须在模拟环境中进行相应的测试，以确定这对于应用程序是否有好处  
