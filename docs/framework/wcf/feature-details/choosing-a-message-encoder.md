...
ms.locfileid: "33494857"
---
# <a name="choosing-a-message-encoder"></a>选择消息编码器
本主题讨论选择包含 Windows Communication Foundation (WCF) 中的消息编码器的条件： 二进制、 文本和消息传输优化机制 (MTOM)。  
  
 在 WCF 中，指定如何通过终结点之间跨网络传输数据*绑定*，这组成的序列*绑定元素*。 消息编码器由绑定堆栈中的消息编码绑定元素表示。 绑定包括多个可选协议绑定元素（如安全绑定元素或可靠消息绑定元素）、一个必需的消息编码绑定元素以及一个必需的传输绑定元素。  
...
-   <xref:System.ServiceModel.Channels.BinaryMessageEncodingBindingElement>二进制消息编码器，使用精简的二进制格式和针对 WCF 到 WCF 的通信进行了优化并因此不可互操作。 这也是 WCF 提供的所有编码器中的大多数的高性能的编码器。  
  
-   <<!--zz xref:System.ServiceModel.Channels.MTOMMessageEncodingBindingElement --> `System.ServiceModel.Channels.MTOMMessageEncodingBindingElement`>、 绑定元素，指定的字符编码和消息版本管理的消息使用 MTOM 编码。 MTOM 是一种用于在 WCF 消息中传输二进制数据的有效技术。 MTOM 编码器会尝试在效率和互操作性之间建立平衡。 MTOM 编码以文本形式传输大多数 XML，但是会通过按原样（即不转换为文本）的方式传输来优化大型二进制数据块。 就效率而言，在 WCF 提供了，编码器之间 MTOM 是之间 （最慢） 的文本和二进制 （最快）。  
...
|可靠性|可靠性是编码器传输错误的复原能力。 也可以在消息层、传输层和应用程序层提供可靠性。 所有标准 WCF 编码器假定其他层提供可靠性。 编码器几乎没有从传输错误恢复的能力。|无|  
...
|互操作性|此因素是指 WCF 编码器与非 WCF 服务进行互操作的能力。|Text<br /><br /> MTOM（部分）|  
