...
-   R3515： 请求消息发送到终结点必须有`ReplyTo`标头`[address]`属性等于"http://www.w3.org/2005/08/addressing/anonymous"，或不具有`ReplyTo`根本，如果终结点使用 WSDL 1.1 SOAP 1.x HTTP 绑定，并且有策略备选方法的标头与`wsap10:UsingAddressing`断言，但不`cdp:CompositeDuplex`附加。  
