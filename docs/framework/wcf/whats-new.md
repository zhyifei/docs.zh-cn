...
|receiveTimeout|SMSvcHost.exe|30 秒|[配置 Net.TCP 端口共享服务](https://msdn.microsoft.com/library/b6dd81fa-68b7-4e1b-868e-88e5901b7ea0)|
...
## <a name="webservicehost-and-default-endpoints"></a>WebServiceHost 和默认终结点
 在 Visual Studio 2010 中，WebServiceHost 会自动创建默认终结点，而无论你是否显式指定了终结点。 在 Visual Studio 2012 及更高版本中，WebServiceHost 仅创建默认终结点，如果显式不添加任何终结点。 如果客户端预期使用默认终结点，那么您可以显式添加终结点，并将客户端指向该终结点。 或者，您可以通过向应用程序的配置文件添加以下设置来告诉 WCF 恢复到上一个行为。
