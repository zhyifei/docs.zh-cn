...
-   如果从中间层应用程序调用操作。 ([!INCLUDE[crabout](../../../includes/crabout-md.md)]这种情况下，请参阅[中间层客户端应用程序](../../../docs/framework/wcf/feature-details/middle-tier-client-applications.md)。)  
...
```  
 ...
 SampleMethodTaskAsync 操作返回\<字符串 > 因为逻辑运算返回字符串。 有关基于任务的异步模式的详细信息，请参阅[异步模式](http://go.microsoft.com/fwlink/?LinkId=232504)。  
...
```  
 ...
 有关基于事件的异步模式的详细信息，请参阅[The Event-Based 异步模式](http://go.microsoft.com/fwlink/?LinkId=232515)。  
...
#### <a name="iasyncresult-asynchronous-pattern"></a>IAsyncResult 异步模式  
 ...
 有关示例，请参阅[如何： 实现异步服务操作](../../../docs/framework/wcf/how-to-implement-an-asynchronous-service-operation.md)。  
...
```  
 ...
 这会生成一个服务协定，在该服务协定中，每个操作都作为 `<Begin>` 方法（其 <xref:System.ServiceModel.OperationContractAttribute.AsyncPattern%2A> 属性设置为 `true`）和相应的 `<End>` 方法来建模。 有关完整的示例使用<xref:System.ServiceModel.ChannelFactory%601>，请参阅[如何： 调用操作以异步方式使用通道工厂](../../../docs/framework/wcf/feature-details/how-to-call-operations-asynchronously-using-a-channel-factory.md)。  
