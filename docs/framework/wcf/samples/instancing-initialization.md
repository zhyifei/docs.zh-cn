...
此示例通过定义接口扩展了[池](../../../../docs/framework/wcf/samples/pooling.md)采样, 该`IObjectControl`接口通过激活和停用来自定义对象的初始化。 客户端调用向池中返回对象以及不向池中返回对象的方法。  
...
```  
 ...
 方法提供了*清理初始化*功能。 `ReleaseInstance` 通常，该池为其生存期保持最少数量的对象。 但是，可能有过度使用的时期，此时需要在池中创建更多对象以达到配置中指定的最大限制。 最终，当池变得不是很活跃时，那些多余的对象可能会变成额外的开销。 因此，当 `activeObjectsCount` 达到 0 时，会启动一个空闲计时器，该计时器将触发并执行清除循环。  
...
- 终结点行为:它们允许自定义特定的服务终结点, 包括 EndpointDispatcher。  
  
- 合同行为:它们允许在客户端或服务<xref:System.ServiceModel.Dispatcher.ClientRuntime>上<xref:System.ServiceModel.Dispatcher.DispatchRuntime>分别自定义或类。  
  
- 操作行为:它们允许在客户端或服务<xref:System.ServiceModel.Dispatcher.ClientOperation>上<xref:System.ServiceModel.Dispatcher.DispatchOperation>分别自定义或类。  
...
## <a name="hooking-activation-and-deactivation"></a>挂钩激活和停用  
 ...
 为了模拟此功能，此示例声明了一个具有上述成员的公共接口 (`IObjectControl`)。 此接口然后由用来提供上下文特定初始化的服务类来实现。 必须修改 <xref:System.ServiceModel.Dispatcher.IInstanceProvider> 实现才能满足这些要求。 现在, 每次通过调用`GetInstance`方法获取对象时, 必须检查该对象是否实现`IObjectControl.` `Activate` (如果它存在), 必须相应地调用方法。  
