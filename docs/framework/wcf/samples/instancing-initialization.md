...
此示例通过定义接口（`IObjectControl`）扩展了[池](../../../../docs/framework/wcf/samples/pooling.md)采样，该接口通过激活和停用来自定义对象的初始化。 客户端调用向池中返回对象以及不向池中返回对象的方法。  
...
## <a name="hooking-activation-and-deactivation"></a>挂钩激活和停用  
 ...
 为了模拟此功能，此示例声明了一个具有上述成员的公共接口 (`IObjectControl`)。 此接口然后由用来提供上下文特定初始化的服务类来实现。 必须修改 <xref:System.ServiceModel.Dispatcher.IInstanceProvider> 实现才能满足这些要求。 现在，每次通过调用 `GetInstance` 方法获取对象时，必须检查该对象是否实现 `IObjectControl.` 如果是这样，则必须相应地调用 `Activate` 方法。  
