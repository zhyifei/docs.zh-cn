...
```  
 ...
 服务公开两个终结点，一个用来与使用配置文件 (Web.config) 定义的服务进行通信，另一个用来使用 WS-MetadataExchange 协议来公开服务的 WSDL 文档。 终结点由地址、绑定和协定组成。 绑定配置了标准[ \<的 wsHttpBinding >](../../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md)元素, 默认为使用消息安全性。 此示例将 `clientCredentialType` 属性设置为 Certificate 以要求进行客户端身份验证。  
...
```  
 ...
 此行为指定在客户端向服务进行身份验证时使用服务凭据。 服务器证书使用者名称是在`findValue` [ \<serviceCredentials >](../../../../docs/framework/configure-apps/file-schema/wcf/servicecredentials.md)元素的属性中指定的。  
