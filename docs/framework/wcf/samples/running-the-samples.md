...
1. 如果服务由 IIS 承载，请确保可以使用浏览器通过输入以下地址来访问服务： `http://localhost/servicemodelsamples/service.svc`。 在响应中应显示确认页。 如果未显示 "确认" 页，请参阅[WCF 示例的故障排除提示](https://docs.microsoft.com/previous-versions/dotnet/netframework-3.5/ms751511(v=vs.90))。  
  
2. 如果服务是自承载的，请从 \service\bin（在语言特定文件夹内）中运行 Service.exe。 服务活动显示在服务控制台窗口上。  
  
3. 从语言特定文件夹下的 \client\bin\\中运行。 客户端活动将显示在客户端控制台窗口上。  
...
1. 如果服务在 IIS 中承载：  
    ...
    1. 在服务计算机上创建一个名为 ServiceModelSamples 的虚拟目录。 [Windows Communication Foundation 示例的一次性安装过程](../../../../docs/framework/wcf/samples/one-time-setup-procedure-for-the-wcf-samples.md)随附的批处理文件 setupvroot.bat 可用于创建磁盘目录和虚拟目录。  
...
2. 如果服务在 IIS 中承载：  
    ...
    1. 通过输入地址 `http://localhost/servicemodelsamples/service.svc`来使用浏览器激活服务。  
