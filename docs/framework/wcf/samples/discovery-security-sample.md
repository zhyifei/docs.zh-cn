...
-   **服务**: 公开 ICalculatorService 协定的自承载服务。 该服务标记为可发现。 用户通过指定证书的存储位置、名称和主题名称或其他唯一标识符来指定用于对消息签名的证书的详细信息，并指定客户端证书（用于检查传入消息的签名的证书）所处的存储区。 基于这些详细信息，生成并使用增加了安全性的 UdpDiscoveryEndpoint。  
...
2.  若要生成和运行示例，请在 Visual Studio 中打开 Security.sln 文件，然后选择**全部重新生成**。 更新解决方案属性以启动多个项目： 选择**启动**的对除 discoverysecurechannels 以外的所有项目。 正常运行解决方案。  
