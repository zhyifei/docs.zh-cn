...
- **AnnouncementListener**:用于侦听联机和脱机公告并使用安全公告终结点的自承载服务。  
...
1. 从 Visual Studio 的开发人员命令提示执行 bat 脚本。 本示例使用证书对消息进行签名和验证。 该脚本使用 Makecert.exe 创建证书，然后使用 Certmgr.exe 安装这些证书。 该脚本必须使用管理员特权运行。  
