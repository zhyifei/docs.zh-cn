...
此示例使用 WCF 创建两个 rss 源: 一个`movies`源和一个`channels`源。 用于激活服务的 url 不包含扩展, 并且是从`Application_Start` <xref:System.Web.HttpApplication>类派生的`Global`类的方法中注册的。  
...
3. 在地址栏中, 将`movies`添加到 URL, 使其读取`http://localhost:[port]/movies`并按 enter。  
  
     电影源显示在浏览器中。  
  
4. 在地址栏中, 将`channels`添加到 URL, 以便读取`http://localhost:[port]/channels`并按 enter。  
...
5. 在地址栏中, 将`movies`添加到 URL, 以便读取`http://localhost:[port]/movies`并按 enter。  
  
     电影源显示在浏览器中。  
  
6. 在地址栏中, 将`channels`添加到 URL, 以便读取`http://localhost:[port]/channels`并按 enter。  
...
> [!NOTE]
> 如果默认应用程序池版本设置为版本[!INCLUDE[netfx40_long](../../../../includes/netfx40-long-md.md)] 2, 则必须将其更新为。  
