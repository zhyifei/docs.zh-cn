...
## <a name="sample-details"></a>示例详细信息  
 ...
 需要注意操作的返回类型。 WCF REST 编程模型本身只支持 XML 和 JSON 响应格式时操作返回的类型而不<xref:System.ServiceModel.Channels.Message>。 但是，当使用 <xref:System.ServiceModel.Channels.Message> 作为返回类型时，开发人员可以完全控制应如何格式化消息内容。  
