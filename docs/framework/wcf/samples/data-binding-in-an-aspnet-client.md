...
>  本主题的最后介绍了此示例的设置过程和生成说明。  
 ...
 在 ASP.NET 客户端.aspx 页上，定义 DataGrid Web 控件以包含由服务返回的数据的图形表示形式。 .Aspx 页面上的代码调用天气数据的 WCF 服务，并将数据返回到的数组`WeatherData`对象。 DataGrid 通过将它的 `DataSource` 属性设置为该数组来指定从何处获取数据。 数据绑定通过调用 DataGrid 的 `DataBind` 方法发生。 所有这些代码，包含在。`aspx` 页面的`Page_Load`方法，因此每次用户刷新浏览器页面，将数据更新数据网格中。  
