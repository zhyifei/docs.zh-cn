...
> 本主题的最后介绍了此示例的设置过程和生成说明。  
 ...
 Windows 通信基础 （WCF） 可以在两种传输模式下进行通信- 缓冲或流式处理。 在默认的缓冲传输模式中，必须完整传递消息，接收方才能读取该消息。 在流传输模式下，接收方可以在完整传递消息之前开始处理该消息。 当传递的信息很长且可以依次处理时，流模式非常有用。 当消息过长以致于无法全部缓冲时，流模式也非常有用。  
