...
>  本主题的最后介绍了此示例的设置过程和生成说明。  
 ...
 从 Windows Communication Foundation (WCF) 客户端上的通信方法引发的异常是正常或异常。 意外异常包括灾难性故障（如 `OutOfMemoryException`）和编程错误（如 `ArgumentNullException` 或 `InvalidOperationException`）。 通常是没有有效的方法来处理意外的错误，因此通常调用 WCF 客户端通信方法时应不捕获它们。  
  
 应从 WCF 客户端上的通信方法的例外情况包括`TimeoutException`， `CommunicationException`，任何派生的类和`CommunicationException`。 这些警报表示可以安全地处理通过中止 WCF 客户端并报告通信故障进行通信时出现问题。 因为外部因素可能导致任何应用程序中出现这些错误，所以正确的应用程序必须捕获这些异常并在发生异常时进行恢复。  
