...
---
# 池
本示例演示如何扩展 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 来支持对象池。本示例演示如何创建在语法和语义上与企业服务的 `ObjectPoolingAttribute` 属性功能相似的属性。对象池可以显著提高应用程序的性能。但是，如果使用不正确也可能产生负面影响。对象池有助于减少重新创建经常使用且要求频繁进行初始化的对象的开销。但是，如果调用缓冲池对象上的方法要花大量时间完成，则对象池在刚达到最大池大小时就会对其他请求排队。因此可能不能为某些对象创建请求提供服务，这时将引发超时异常。  
  
> [!NOTE]
>  本主题的末尾介绍了此示例的设置过程和生成说明。  
  
 创建 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 扩展的第一步是确定要使用的扩展点。  
  
 在 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 中，术语“调度程序”指的是一个运行时组件，它负责将传入消息转换成用户服务上的方法调用，并将该方法的返回值转换成传出消息。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 服务为每个终结点创建一个调度程序。如果与 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 客户端关联的协定是一个双工协定，则该客户端必须使用调度程序。  
...
## 对象池  
 自定义 <xref:System.ServiceModel.Dispatcher.IInstanceProvider> 实现为服务提供所需的对象池语义。因此，此示例有一个为池提供 <xref:System.ServiceModel.Dispatcher.IInstanceProvider> 自定义实现的 `ObjectPoolingInstanceProvider` 类型。当 `Dispatcher` 调用 <xref:System.ServiceModel.Dispatcher.IInstanceProvider.GetInstance%28System.ServiceModel.InstanceContext%2CSystem.ServiceModel.Channels.Message%29> 方法时，自定义实现将在内存池中寻找现有对象，而不是创建新的实例。如果找到一个对象，则返回该对象。否则，将创建新对象。下面的示例代码演示了 `GetInstance` 的实现。  
...
```  
 ...
 自定义 `ReleaseInstance` 实现将已释放的实例重新添加回池中并减少 `ActiveObjectsCount` 值。`Dispatcher` 可能从不同的线程调用这些方法，因此需要同步访问 `ObjectPoolingInstanceProvider` 类中的类级别成员。  
...
```  
 ...
 `ReleaseInstance` 方法提供“清除初始化”功能。通常，池为其生存期维护最少数量的对象。但是，可能有过度使用的时期，此时需要在池中创建更多对象以达到配置中指定的最大限制。最终，当池变得不是很活跃时，那些多余的对象可能成为额外的开销。因此，当 `activeObjectsCount` 到达 0 时，会启用一个空闲计时器，该计时器将触发并执行清除循环。  
...
-   `ServiceHostBase`：此参数提供当前正在初始化的 <xref:System.ServiceModel.ServiceHostBase>。  
 ...
 在自定义 <xref:System.ServiceModel.Description.IServiceBehavior> 实现中，会实例化一个新的 `ObjectPoolingInstanceProvider` 实例，并将该实例分配到 ServiceHostBase 的每个 <xref:System.ServiceModel.Dispatcher.DispatchRuntime> 的 <xref:System.ServiceModel.Dispatcher.DispatchRuntime.InstanceProvider%2A> 属性。  
...
>   
>  `<安装驱动器>:\WF_WCF_Samples\WCF\Extensibility\Instancing\Pooling`  
