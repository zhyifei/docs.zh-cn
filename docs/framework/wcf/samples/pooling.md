...
>  本主题的最后介绍了此示例的设置过程和生成说明。  
 ...
 在[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]术语*调度程序*指负责将传入消息转换为用户的服务上的方法调用，并从该方法的返回值转换为传出的运行时组件消息。 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 服务为每个终结点创建一个调度程序。 如果与 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 客户端关联的协定是一个双工协定，则该客户端必须使用调度程序。  
...
## <a name="the-iinstanceprovider"></a>IInstanceProvider  
 在 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 中，调度程序使用实现 <xref:System.ServiceModel.Dispatcher.DispatchRuntime.InstanceProvider%2A> 接口的 <xref:System.ServiceModel.Dispatcher.IInstanceProvider> 创建服务类的实例， 此接口有三个方法：  
