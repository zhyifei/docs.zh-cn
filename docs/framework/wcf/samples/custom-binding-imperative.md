...
该示例演示如何编写命令性代码来定义和使用自定义绑定，而无需使用配置文件或生成的 Windows 通信基础 （WCF） 客户端。 本示例将 HTTP 传输提供的功能与可靠会话通道结合在一起，用于创建基于 HTTP 的可靠绑定。 此示例基于实现计算器服务的[入门](../../../../docs/framework/wcf/samples/getting-started-sample.md)。  
