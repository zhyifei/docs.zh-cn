...
此示例演示如何管理包含大量项的联合源。 在服务器上，此示例演示如何延迟在源中创建各个 <xref:System.ServiceModel.Syndication.SyndicationItem> 对象，一直到项就要被写入网络流之前。  
 ...
 演示使用 Visual C# 迭代器（使用`yield return`关键字构造）。 有关迭代器的详细信息，请参阅 MSDN 上的"使用迭代器"主题。  
...
```  
 ...
 因此，项流永远不会完全缓冲到内存中。 可以通过在`yield return``ItemGenerator.GenerateItems()`方法内的语句上设置断点来观察此行为，并注意到在服务返回`StreamedFeed()`方法结果后首次遇到此断点。  
...
```  
 ...
 因此，在遍历 `ReadItems()` 结果的客户端应用程序准备好使用每个项之前，并不会从网络中读取每个项。 您可以通过在`yield return`语句`StreamedAtom10FeedFormatter.DelayReadItems()`内设置断点来观察此行为，并注意到在调用`ReadFrom()`完成后首次遇到此断点。  
