...
## <a name="wcf-integration-with-other-microsoft-technologies"></a>WCF 与其他 Microsoft 技术的集成  
 ...
 要与 WCF 配对的第一个技术是 Windows Workflow Foundation (WF)。 工作流简化应用程序开发的工作流中的步骤封装为"活动"。 在 Windows Workflow Foundation 的第一个版本，开发人员必须创建工作流主机。 Windows Workflow Foundation 的下一版本与 WCF 集成。 允许 WCF 服务; 中轻松承载任何工作流您可以执行此操作通过自动选择 WF/WCF 作为项目类型中[!INCLUDE[vs_current_long](../../../includes/vs-current-long-md.md)]。  
