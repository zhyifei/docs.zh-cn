...
ms.locfileid: "70795796"
---
# <a name="custom-message-formatters"></a>自定义消息格式化程序
消息内容通常为 XML 格式，该格式通常不便于在应用程序中使用。 应用程序操作对象，并且获取和设置其属性。 Windows Communication Foundation （WCF）使用*数据协定*将<xref:System.ServiceModel.Channels.Message>对象转换为可由应用程序轻松处理的对象。 我们将这些过程称为序列化和反序列化。 请注意，这些词同样用于描述传输层对消息连网格式所进行的序列化和反序列化，这是一个不相关的过程。  
