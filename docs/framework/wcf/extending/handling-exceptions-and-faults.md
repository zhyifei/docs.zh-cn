...
## <a name="exceptions"></a>例外  
 在引发异常时需要牢记两件事情：第一，异常的类型必须允许用户编写能对异常做出适当反应的正确代码。 第二，异常必须为用户提供足够的信息，使用户能了解究竟哪里出现了故障，该故障带来的影响以及如何修复该故障。 以下各节提供有关 Windows 通信基础 （WCF） 通道的异常类型和消息的指导。 “异常的设计准则”文档中也有关于 .NET 中的异常的一般性指南。  
  
### <a name="exception-types"></a>异常类型  
 由通道引发的所有异常都必须是 <xref:System.TimeoutException?displayProperty=nameWithType>、<xref:System.ServiceModel.CommunicationException?displayProperty=nameWithType> 或者是派生自 <xref:System.ServiceModel.CommunicationException> 的类型 （还可能会引发诸如 <xref:System.ObjectDisposedException> 之类的异常，但这仅仅是为了指示调用代码误用了通道。 如果通道使用正确，则只能引发给定的异常。WCF 提供了七种异常类型，<xref:System.ServiceModel.CommunicationException>它们派生自通道并设计为由通道使用。 还存在其他从 <xref:System.ServiceModel.CommunicationException> 派生的异常，这些异常设计用于系统的其他部分。 这些异常类型包括：  
...
|<xref:System.ServiceModel.CommunicationObjectAbortedException>|<xref:System.ServiceModel.ICommunicationObject>正在使用的状态已中止（有关详细信息，请参阅[了解状态更改](understanding-state-changes.md)）。 与<xref:System.ServiceModel.CommunicationObjectFaultedException>类似 ，此异常表示应用程序调用<xref:System.ServiceModel.ICommunicationObject.Abort%2A>了对象，可能来自另一个线程，并且该对象由于这个原因不再可用。|如果存在，则提供有关内部异常的详细信息。|新建一个对象。 请注意，根据首先导致 <xref:System.ServiceModel.ICommunicationObject> 中止的原因，可能还需要完成其他工作来进行恢复。|  
...
|<xref:System.ServiceModel.ProtocolException>|正如终结点的策略所述，通信协议在终结点之间不匹配。 例如，组帧内容类型不匹配或者超过了最大消息大小。|如果存在，则提供有关特定协议错误的更多信息。 例如，如果出错原因是由于超过了 MaxReceivedMessageSize，则 <xref:System.ServiceModel.QuotaExceededException> 为内部异常。|恢复：请确保发送方的协议设置与接收方的协议设置相匹配。 执行此操作的一种方法是重新导入服务终结点的元数据（策略），并使用生成的绑定重新创建通道。|  
...
### <a name="exception-messages"></a>异常消息  
 ...
 所发生的情况。 使用与用户体验相关的术语提供问题的明确描述。 例如，“Invalid configuration section”（配置节无效）是糟糕的异常消息。 这会令用户想知道哪个配置节有误以及它为何有误。 改进的消息是"无效配置部分\<自定义>"。 “Cannot add the transport named myTransport to the binding named myBinding because the binding already has a transport named myTransport”（无法将名为 myTransport 的传输添加到名为 myBinding 的绑定中，因为该绑定已经有一个名为 myTransport 的传输）是较好的消息。 这是一条非常具体的消息，使用用户可以在应用程序的配置文件中轻松识别的术语和名称。 但是，仍缺少几个关键部分。  
...
## <a name="communicating-faults"></a>传达错误  
 ...
 ![处理异常和故障](./media/wcfc-soap1-1andsoap1-2faultcomparisonc.gif "wcfc_SOAP1-1AndSOAP1-2FaultComparisonc")  
...
```  
 ...
 `Code` 属性对应于 `env:Code`（或 SOAP 1.1 中的 `faultCode`），可用来标识错误的类型。 SOAP 1.2 定义了五个 `faultCode` 所允许的值（例如，Sender（发送方）和 Receiver（接收方）），并定义了一个可以包含任何子代码值的 `Subcode` 元素 （有关允许的故障代码及其含义的列表，请参阅[SOAP 1.2 规范](https://www.w3.org/TR/soap12-part1/#tabsoapfaultcodes)。SOAP 1.1 具有稍微不同的机制：它定义四`faultCode`个值（例如客户端和服务器），可以通过定义全新的值或使用点表示法创建更具体的`faultCodes`值（例如客户端.身份验证）来扩展这些值。  
 ...
 如果想要以编程方式区分某个错误，则应创建新的错误子代码（如果使用的是 SOAP 1.1，则应创建新的错误代码）。 这与创建新的异常类型相似。 应避免在 SOAP 1.1 错误代码中使用点表示法 [（WS-I 基本配置文件](http://www.ws-i.org/Profiles/BasicProfile-1.1-2004-08-24.html#SOAP_Custom_Fault_Codes)还阻止使用故障代码点表示法。  
...
```  
 ...
 该`Reason`属性对应于`env:Reason`（或`faultString`SOAP 1.1 中）与异常消息类似的错误条件的人类可读描述。 `FaultReason` 类（和 SOAP `env:Reason/faultString`）对于为实现全球化而提供多种翻译具有内置的支持。  
...
```  
 ...
 故障详细信息内容使用各种方法（包括`GetDetail`\<T> 和`GetReaderAtDetailContents`（） 在 MessageFault 上公开。 错误详细信息是一个不透明的元素，其中包含有关错误的附加详细信息。 如果您希望错误中包含一些任意的结构化详细信息，则这非常有用。  
...
```  
 ...
 用来生成自定义错误的每个通道都必须实现 `FaultConverter` 并通过调用 `GetProperty<FaultConverter>` 来返回它。 自定义`OnTryCreateFaultMessage`实现必须将异常转换为故障或委托给内部通道的`FaultConverter`。 如果通道是传输，则必须将异常或委托给编码器`FaultConverter`或 WCF 中提供的默认值。 `FaultConverter` 默认的 `FaultConverter` 会转换与 WS-Addressing 和 SOAP 所指定的错误消息相对应的错误。 下面是一个示例 `OnTryCreateFaultMessage` 实现。  
...
3. 针对堆栈中单个层的错误，例如，像 WS-RM 序列号错误之类的错误。  
 ...
 类别 1. 的错误通常是 WS-Addressing 和 SOAP 错误。 WCF`FaultConverter`提供的基类将转换与 WS 寻址和 SOAP 指定的错误消息对应的错误，因此您不必自己处理这些异常的转换。  
...
1. 如果图层认为消息无效，则图层应执行其"无效消息"处理。 此类处理特定于该层，但是可以包括丢弃消息、跟踪或引发已转换为错误的异常。 这方面的示例包括对未正确保护的消息的安全接收，或者对序列号有误的消息的 RM 接收。  
  
2. 否则，如果消息是专门应用于图层的错误消息，并且该消息在图层的交互之外没有意义，则图层应处理错误条件。 这方面的示例包括“RM 序列被拒绝”错误，这类错误对于 RM 通道上方的各层没有意义，表示从挂起操作引发的 RM 通道错误。  
...
### <a name="mustunderstand-processing"></a>MustUnderstand 处理  
 SOAP 定义一个用于指示接收方未理解必需的标头的通用错误。 此错误称为 `mustUnderstand` 错误。 在 WCF 中，自定义`mustUnderstand`通道从不生成故障。 相反，位于 WCF 通信堆栈顶部的 WCF 调度程序会检查以查看基础堆栈已理解标记为 MustToto_true 的所有标头。 如果均未理解，则此时会生成一个 `mustUnderstand` 错误 （用户可以选择关闭此 `mustUnderstand` 处理，并让应用程序接收所有消息头。 在这种情况下，应用程序负责执行`mustUnderstand`处理。生成的错误包括一个未理解标头，其中包含所有未被理解的"Must因"标头的名称。  
...
```  
 ...
 如果故障是 `IsMustUnderstandFault` 故障，`true` 返回 `mustUnderstand`。 如果具有指定名称和命名空间的标头以 NotUnderstood 标头的形式包括在该错误中，则 `WasHeaderNotUnderstood` 将返回 `true`。  否则，将返回 `false`。  
...
- 除了核心组件之外，您还可以使用[服务跟踪查看器工具 （SvcTraceViewer.exe）](../service-trace-viewer-tool-svctraceviewer-exe.md)查看和搜索 WCF 跟踪。 该工具专为 WCF 生成并使用 编写的跟踪文件而设计<xref:System.Diagnostics.XmlWriterTraceListener>。 下图演示了跟踪中涉及到的各种组件。  
  
 ![处理异常和故障](./media/wcfc-tracinginchannelsc.gif "wcfc_TracingInChannelsc")  
...
### <a name="tracing-from-a-custom-channel"></a>从自定义通道跟踪  
 ...
 在实例化 <xref:System.Diagnostics.TraceSource> 时，所指定的字符串将成为该源的名称。 此名称用来配置（启用/禁用/设置跟踪级别）跟踪源。 此名称还显示在跟踪输出本身当中。 自定义通道应使用唯一的源名称来帮助该跟踪输出的读取器了解跟踪信息的出处。 常见的做法是将正在写入信息的程序集的名称用作跟踪源的名称。 例如，WCF 使用 System.ServiceModel 作为从系统.ServiceModel 程序集中写入的信息的跟踪源。  
...
#### <a name="integrating-with-the-trace-viewer"></a>与跟踪查看器集成  
 通道生成的跟踪可以通过用作<xref:System.Diagnostics.XmlWriterTraceListener?displayProperty=nameWithType>跟踪侦听器以[服务跟踪查看器工具 （SvcTraceViewer.exe）](../service-trace-viewer-tool-svctraceviewer-exe.md)可读的格式输出。 此操作无需由通道开发人员来执行， 相反，需要在应用程序的配置文件中配置此跟踪侦听器的应用程序用户（或对应用程序进行故障排除的人员）。 例如，下面的配置将 <xref:System.ServiceModel?displayProperty=nameWithType> 和 `Microsoft.Samples.Udp` 中的跟踪信息均输出到名为 `TraceEventsFile.e2e` 的文件中：  
...
```  
 ...
 WCF 跟踪查看器了解前面显示`TraceRecord`的元素的架构，并从其子元素中提取数据，并将其以表格格式显示。 在跟踪结构化应用程序数据以帮助 Svctraceviewer.exe 用户读取数据时，通道应使用此架构。
