...
底层是一个必需的传输元素。 你可以使用自己的传输，或者 Windows Communication Foundation （WCF）提供的以下传输绑定元素之一：
