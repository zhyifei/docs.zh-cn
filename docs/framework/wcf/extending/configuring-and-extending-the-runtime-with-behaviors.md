...
> [!NOTE]
>  也可以使用支持配置的行为，并通过将这些行为插入到 machine.config 配置文件并锁定这些项来强制执行公司的应用程序策略。 有关说明和示例，请参阅[如何： 在企业中锁定下终结点](../../../../docs/framework/wcf/extending/how-to-lock-down-endpoints-in-the-enterprise.md)。  
...
```  
 ...
 其中`Microsoft.WCF.Documentation.EndpointBehaviorMessageInspector`是行为扩展类型和`HostApplication`是该类编译到程序集的名称。  
