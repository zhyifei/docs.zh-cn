...
## <a name="extending-bindings-and-binding-elements"></a>扩展绑定和绑定元素  
 Windows Communication Foundation (WCF) 包含涵盖了种类繁多的方案的系统提供的绑定。 (有关详细信息，请参阅[系统提供的绑定](../../../../docs/framework/wcf/system-provided-bindings.md)。)可能有些时候，但是，当你需要创建和使用不包含在 WCF 中的绑定。 下面的方案需要创建新绑定。  
...
## <a name="channels-bindings-and-binding-elements"></a>通道、绑定和绑定元素  
 ...
 传输通道和协议通道的主要要求是它们需要实现必需的通道接口。 若要创建有效的通道层，它们必须具有关联的工厂和侦听器，等等。 若要使用需要有关联的绑定元素的 WCF 中的通道实现派生自<xref:System.ServiceModel.Channels.BindingElement>每个通道并且应该有相关的绑定扩展元素，以便包括到配置文件派生自<xref:System.ServiceModel.Configuration.BindingElementExtensionElement>.  
