...
|-------------|----------------------|--------------------------|-----------------------------|------------------|------------|--------------------------|-------------------------------|  
|<xref:System.ServiceModel.BasicHttpBinding>|Basic Profile 1.1（基本配置文件 1.1）|（无）、传输、消息、混合|（无）|（无）|n/a|文本、(MTOM)|是<br /><br /> （缓冲式）|  
...
|<xref:System.ServiceModel.UdpBinding>|.NET**注意：**互操作性可以通过实现此绑定实现的标准-UDP 上的 SOAP 规范。|（无）|（无）|（无）|n/a|（文本）|否|  
