...
承载 Windows Communication Foundation (WCF) 服务使用编写时[!INCLUDE[netfx35_long](../../../includes/netfx35-long-md.md)]上机运行[!INCLUDE[netfx40_long](../../../includes/netfx40-long-md.md)]，可能会收到<xref:System.ServiceModel.ProtocolException>包含以下文本。  
...
```  
 ...
 接下来查找 <`compilers`> 元素，然后更改具有值为 4.0 的 CompilerVersion 提供程序选项。 默认情况下，有两个 <`compiler`> 下的元素 <`compilers`> 元素。 您必须同时更新这两个元素的 CompilerVersion 提供程序选项，如下面的示例所示。  
...
2.  添加`targetFramework`属性设为 <`compilation`> 元素，如以下示例所示。  
...
3.  找到 <`compilers`> 元素，然后更改具有值为 4.0 的 CompilerVersion 提供程序选项。 默认情况下，有两个 <`compiler`> 下的元素 <`compilers`> 元素。 您必须同时更新这两个元素的 CompilerVersion 提供程序选项，如下面的示例所示。  
