...
|<xref:System.Activities.DynamicActivity>|与 WF 设计器和运行时系统通过提供接口类似于 DOM 的方法来构造活动<!--zz <xref:System.ComponentModel.IcustomTypeDescriptor>--> `IcustomTypeDescriptor`，从而允许新活动创建，但不定义新类型。|  
...
## <a name="authoring-activities-using-nativeactivity"></a>使用 NativeActivity 创作活动  
 从 <xref:System.Activities.NativeActivity> 派生的活动，与从 <xref:System.Activities.CodeActivity> 派生的活动一样，可通过重写 <xref:System.Activities.NativeActivity.Execute%2A> 来创建命令性功能，除此之外还可以通过传递给 <xref:System.Activities.NativeActivityContext> 方法的 <xref:System.Activities.NativeActivity.Execute%2A> 访问工作流运行时的所有功能。 此上下文提供支持，包括安排和取消子活动、 执行<xref:System.Activities.ActivityAction>和<!--zz <xref:System.Activities.ActivityFunc>-->`ActivityFunc`对象，流动事务流入工作流、 调用异步进程、 取消和中止执行、 访问权限执行属性和扩展以及书签 （恢复暂停工作流的句柄）。  
