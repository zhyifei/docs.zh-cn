...
####  <a name="CompiledXaml"></a> 编译型的 Xaml  
 编译型 XAML 工作流支持 C# 表达式，此种工作流编译成类型，作为面向 [!INCLUDE[netfx_current_short](../../../includes/netfx-current-short-md.md)] 的 C# 工作流项目的组成部分。 编译型的 XAML 是 Visual Studio 中的工作流创作的默认类型和 C# 工作流项目中创建 Visual Studio 面向[!INCLUDE[netfx_current_short](../../../includes/netfx-current-short-md.md)]使用 C# 表达式。  
