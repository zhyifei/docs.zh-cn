...
此示例演示如何编写将跟踪记录写入 SQL 数据库的自定义 SQL 跟踪参与者。 Windows 工作流基础 （WF） 提供工作流跟踪，以深入了解工作流实例的执行。 跟踪运行时在工作流执行过程中会发出工作流跟踪记录。 有关工作流跟踪的详细信息，请参阅[工作流跟踪和跟踪](../workflow-tracking-and-tracing.md)。
...
3. 在视觉工作室 2010 中打开 SqlTrackingSample.sln。

4. 按**Ctrl**+**移位**+**B**可构建解决方案。
...
7. 浏览器显示 StockPriceService 页，其中包含本地服务 WSDL 地址。 复制此地址。
   ...
   本地服务 WSDL 地址的一个`http://localhost:65193/StockPriceService.xamlx?wsdl`示例是 。

8. 使用文件资源管理器运行 WCF 测试客户端 （WcfTestClient.exe）。 它位于*微软视觉工作室 10.0_Common7_IDE 目录中*。

9. 在 WCF 测试客户端中，单击 **"文件**"菜单并选择"**添加服务**"。 将本地服务地址粘贴到文本框中。 单击 **“确定”**，关闭对话框。

10. 在 WCF 测试客户端中，双击**GetStockPrice**。 这将打开需要`GetStockPrice`一个参数的操作，键入值`Contoso`并单击 **"调用**"。

11. 发出的跟踪记录将写入一个 SQL 数据库中。 若要查看跟踪记录，请在 SQL Management Studio 中打开 TrackingSample 数据库，然后导航到表。 对表运行一个选择查询，将显示存储在相关表中的跟踪记录内的数据。

   有关 SQL 服务器管理工作室的详细信息，请参阅[介绍 SQL 服务器管理工作室](/sql/ssms/sql-server-management-studio-ssms)。 [在此处](https://aka.ms/ssmsfullsetup)下载 SQL 服务器管理工作室。

## <a name="uninstall-the-sample"></a>卸载示例

1. 在示例目录中运行跟踪清理.cmd 脚本 *（\WF_基本\跟踪\Sql 跟踪*）。
