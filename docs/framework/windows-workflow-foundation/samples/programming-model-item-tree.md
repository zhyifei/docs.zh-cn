...
此示例演示如何使用 Windows Presentation Foundation （WPF）树视图中的声明性数据绑定导航 <xref:System.Activities.Presentation.Model.ModelItem> 树。

## <a name="sample-details"></a>示例详细信息
 <xref:System.Activities.Presentation.Model.ModelItem> 树是 Windows 工作流设计器基础结构用来公开有关正在编辑的基础实例的数据的抽象。 下图是工作流设计器中的基础结构的各层的描述。
