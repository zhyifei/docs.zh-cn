...
## <a name="discussion"></a>讨论  
 在此示例中，使用实现 Windows Communication Foundation (WCF) 服务[!INCLUDE[wf1](../../../../includes/wf1-md.md)]并且具有一系列<xref:System.ServiceModel.Activities.Receive>活动。 此工作流建立了一个简单的贷款审批过程模型，在此模型中，工作流应会收到与贷款审批相关的三个通知。 Windows Communication Foundation (WCF) 客户端应用程序的服务的期望的相反顺序发送三个关联的通知。 由于已在服务上启用缓冲接收功能，因此在工作流准备好接收消息时，将在服务上对每条无序消息进行缓冲和处理。  
...
```  
 ...
 同样，也将使用 <xref:System.ServiceModel.NetMsmqBinding> 配置客户端终结点。 通过使用生成的客户端代码和配置**添加服务引用**Visual Studio 的功能。 下面的示例是在 App.config 文件中生成的客户端终结点。  
...
4.  当系统询问您想要创建的虚拟目录为 LoanService 项目，是否选择**是**。  
  
#### <a name="to-set-up-the-service-queues"></a>设置服务队列  
  
1.  按 F5 运行 LoanClient 应用程序，这将创建队列并激活 Service1.xamlx 中定义的服务。  
  
2.  打开**计算机管理**通过在命令提示符下运行 Compmgmt.msc 控制台。  
  
3.  在中**计算机管理**控制台中，展开**服务**，**应用程序**，**消息队列**，**专用队列**.  
  
4.  右击 loanservice/service1.xamlx 队列，然后选择**属性**。  
  
5.  选择**安全**选项卡，并添加**每个人接收消息**，**扫视消息**并**发送消息**权限。  
  
6.  打开 [!INCLUDE[iis60](../../../../includes/iis60-md.md)] 管理器。  
  
7.  浏览到**服务器**，**站点**， **Default Web site**，**专用**， **LoanService**和选择**高级的选项**  
...
1.  打开**计算机管理**通过在命令提示符下运行 Compmgmt.msc 控制台。  
  
2.  展开**服务**并**应用程序**，**消息队列**，**专用队列**。  
