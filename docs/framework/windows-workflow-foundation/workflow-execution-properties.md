...
---
# 工作流执行属性
通过线程本地存储区 \(TLS\)，CLR 可为每个线程维护执行上下文。此执行上下文管理已知的线程属性，例如，线程标识、环境事务、当前权限集，不包括用户定义的线程属性（如已命名的槽）。  
...
## 创建和使用工作流执行属性  
 通常，通过聚焦于消息上的属性工作流执行属性执行 <xref:System.Activities.IExecutionProperty> 接口，<xref:> System.ServiceModel.Activities.ISendMessageCallback?qualifyHint=False&autoUpgrade=True 和 <xref:> System.ServiceModel.Activities.IReceiveMessageCallback?qualifyHint=False&autoUpgrade=True相反。若要创建工作流执行属性，请创建一个实现 <xref:System.Activities.IExecutionProperty> 接口并实现成员 <xref:System.Activities.IExecutionProperty.SetupWorkflowThread%2A> 和 <xref:System.Activities.IExecutionProperty.CleanupWorkflowThread%2A> 的类。通过这些成员，该执行属性可在包含该属性的活动（包括任何子活动）的每个工作脉冲期间正确设置和关闭线程本地存储区。在本示例中，创建了设置 `Console.ForegroundColor` 的 `ConsoleColorProperty`。  
  
> [!NOTE]
>  本主题中的以下代码示例基于 [执行属性](../../../docs/framework/windows-workflow-foundation/samples/execution-properties.md) 示例。  
...
```  
 ...
 活动创作者可通过在该活动的执行重写中注册此属性来使用它。在本示例中，定义了一个 `ConsoleColorScope` 活动，该活动通过将 `ConsoleColorProperty` 添加到当前 <xref:System.Activities.NativeActivityContext> 的 <xref:System.Activities.NativeActivityContext.Properties%2A> 集合来注册该属性。  
