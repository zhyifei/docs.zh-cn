...
2. 通过对**InstanceStore**调用 <xref:System.Runtime.DurableInstancing.InstanceStore.CreateInstanceHandle%2A> 方法来获取实例的句柄。  
  
3. 通过对**InstanceStore**调用 <xref:System.Runtime.DurableInstancing.InstanceStore.Execute%2A> 方法来对实例句柄调用命令。  
  
4. 检查**InstanceStore**返回的 <xref:System.Runtime.DurableInstancing.InstanceView>，以确定命令的结果。
