...
事务将多个任务绑定在一起。 例如，假设应用程序执行两个任务。 首先，它在数据库中创建一个新表。 然后，它调用一个专用对象，来收集数据、设置数据格式以及将数据插入新表中。 这两个任务是相关的，甚至是相互依赖的，以便只有在可以用数据填充表时才创建新表。 在单个事务范围内执行这两个任务时，会在它们之间强制建立连接。 如果第二个任务失败，则第一个任务会回滚到创建新表之前的点。  
 ...
 在创建事务时，可以指定应用于该事务的隔离级别。 由<xref:System.Transactions.IsolationLevel>枚举定义的隔离级别确定了其他事务对受事务影响的数据所拥有的访问级别。  
  
 您可以使用 ADO.NET、 <xref:System.EnterpriseServices>或<xref:System.Transactions>由命名空间提供的事务性编程模型来创建事务。 [System.object 提供的功能](features-provided-by-system-transactions.md)主题讨论了可用于使用<xref:System.Transactions>命名空间编写事务应用程序的功能。  
