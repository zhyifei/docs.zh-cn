...
|跟踪级别|说明|  
|----------|--------|  
|严重|表示发生了类似于下面的严重故障：<br /><br /> -   可造成用户功能立即丧失的错误。<br />-   需要管理员采取操作才能避免功能丧失的事件。<br />-   代码挂起。<br />-   此跟踪级别也可为解释其他严重跟踪提供足够的上下文。这可以帮助确定导致严重故障的操作序列。|  
...
## 跟踪代码  
 下表列出了由 <xref:System.Transactions> 基础结构生成的跟踪代码。该表中包括跟踪代码标识符、跟踪的 <xref:System.Diagnostics.EventTypeFilter.EventType%2A> 枚举级别以及包含在跟踪的 **TraceRecord** 中的额外数据。此外，跟踪的相应跟踪级别也存储在 **TraceRecord** 中。  
...
|TransactionScopeCreated|Info|TransactionScopeResult，可为以下各项：<br /><br /> -   新事务。<br />-   已传递的事务。<br />-   已传递的依赖事务。<br />-   使用当前事务。<br />-   没有事务。<br /><br /> 新的当前 TransactionTraceId|  
|TransactionScopeDisposed|Info|范围的“预计”当前事务的 TransactionTraceId。|  
|TransactionScopeIncomplete|Warning|范围的“预计”当前事务的 TransactionTraceId。|  
|TransactionScopeNestedIncorrectly|Warning|范围的“预计”当前事务的 TransactionTraceId。|  
|TransactionScopeCurrentTransactionChanged|Warning|旧的当前 TransactionTraceId、其他 TransactionTraceId|  
|TransactionScopeTimeout|Warning|范围的“预计”当前事务的 TransactionTraceId。|  
