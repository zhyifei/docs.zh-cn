...
> [!NOTE]
> 为了避免**OverflowException**，我们建议你还在将<xref:System.Data.DataRow>值存储在中之前，先处理从 Oracle NUMBER 类型到有效 .NET Framework 类型的任何转换。 您可以使用<xref:System.Data.Common.DataAdapter.FillError>事件来确定是否发生了**OverflowException** 。 有关<xref:System.Data.Common.DataAdapter.FillError>事件的详细信息，请参阅[处理 DataAdapter 事件](handling-dataadapter-events.md)。  
