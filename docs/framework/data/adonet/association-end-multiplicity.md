...
-   很多 (\*):指示在关联端存在零行、 一行或多个实体类型实例。  
 ...
 关联的特征通常由关联端重数表示。 例如，如果关联端有一 （1） 和许多的重数 (\*)，该关联称为一个对多关联。 在下面的示例中，`PublishedBy` 关联是一个一对多关联（一个出版商可以出版很多书，而一本书只能由一个出版商出版）。 
  `WrittenBy` 关联是一个多对多关联（一本书可以有多位作者，而一位作者可以著有多本书）。  
