...
## <a name="ceilingexpression"></a>天花板（expression）
...
## <a name="floorexpression"></a>楼层（expression）
...
|`function` | 可选。 一个`Int32` ，表示要执行的操作的类型。 如果省略函数或其值为0（默认值）， `numeric_expression`则将舍入。 如果指定了 0 以外的值，则将截断 `numeric_expression`。 |
...
## <a name="squareexpression"></a>方形（expression）
...
- **SQL Server 2012 及更高版本：** [数学函数（Transact-sql）](/sql/t-sql/functions/mathematical-functions-transact-sql)
