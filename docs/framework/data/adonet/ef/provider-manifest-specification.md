...
本节讨论数据存储提供程序如何可以支持数据存储中的类型和功能。  
 ...
 提供程序支持的类型由基础数据库直接或间接地提供支持。 这些类型不一定是确切的存储类型，但提供程序用于支持实体框架的类型。 提供程序/存储类型在实体数据模型 (EDM) 条目中进行了说明。  
  
 在 EDM 条目中指定了数据存储支持的函数的参数和返回类型。  
  
## <a name="requirements"></a>要求  
 实体框架和数据存储需要能够在不丢失或截断的情况下以已知类型来回传递数据。  
  
 提供程序清单必须可由工具在设计时加载，而不必打开与数据存储的连接。  
  
 实体框架区分大小写，但基础数据存储可能不是。 在清单中定义和使用 EDM 项目（例如标识符和类型名称）时，它们必须使用实体框架区分大小写。 如果可能区分大小写的数据存储元素出现在提供程序的清单中，则在提供程序清单中需要维持大小写区分。  
  
 实体框架需要所有数据提供程序的提供程序清单。 如果尝试使用实体框架中没有提供程序清单的提供程序，则将出现错误。  
  
 下表描述了实体框架在提供程序交互出现异常时将引发的异常类型：  
...
### <a name="writing-a-provider-with-symmetric-type-mapping"></a>使用对称类型映射编写提供程序  
 您可以为实体框架编写提供程序，其中每个存储类型映射到单个 EDM 类型，而不考虑映射方向。 对于具有非常简单的映射（与 EDM 类型对应）的提供程序类型，您可以使用对称解决方案，因为此类型系统很简单或者匹配 EDM 类型。  
...
### <a name="writing-a-provider-with-asymmetric-type-mapping"></a>使用非对称类型映射编写提供程序  
 为实体框架编写数据存储提供程序时，某些类型的 EDM 到提供程序类型映射可能与提供程序到 EDM 类型映射不同。 例如，无限制的 EDM PrimitiveTypeKind.String 可能会映射到提供程序上的 nvarchar(4000)，而 nvarchar(4000) 将映射到 EDM PrimitiveTypeKind.String(MaxLength=4000)。  
...
#### <a name="using-a-data-store-connection"></a>使用数据存储连接  
 当数据存储连接可用时，调用<xref:System.Data.Common.DbProviderServices.GetProviderManifestToken%2A?displayProperty=nameWithType>以返回传递给 方法的<xref:System.Data.Common.DbProviderServices.GetProviderManifest%2A>令牌，该方法将返回<xref:System.Data.Common.DbProviderManifest>。 此方法委托给提供程序的`GetDbProviderManifestToken`实现。  
...
#### <a name="using-a-provider-manifest-token"></a>使用提供程序清单标记  
 对于脱机方案，此标记从 SSDL 表示形式中选取。 SSDL 允许您指定提供程序清单令牌（有关详细信息，请参阅[架构元素 （SSDL）。](/ef/ef6/modeling/designer/advanced/edmx/ssdl-spec#schema-element-ssdl) 例如，如果无法打开某个连接，则 SSDL 会具有一个提供程序清单标记，用于指定有关清单的信息。  
...
|属性名称|数据类型|必选|默认值|说明|  
|--------------------|---------------|--------------|-------------------|-----------------|  
|名称|String|是|不适用|提供程序特定的数据类型名称|  
|PrimitiveTypeKind|PrimitiveTypeKind|是|不适用|EDM 类型名称|  
...
|属性名称|数据类型|必选|默认值|说明|  
|--------------------|---------------|--------------|-------------------|-----------------|  
|名称|String|是|不适用|函数的标识符/名称|  
|ReturnType|String|否|Void|函数的 EDM 返回类型|  
|Aggregate|Boolean|否|False|如果函数为聚合函数，则为 True。|  
|BuiltIn|Boolean|否|True|如果函数内置于数据存储中，则返回 True|  
|StoreFunctionName|String|否|\<Name>|数据存储中的函数名称。  考虑了函数名称的重定向级别。|  
|NiladicFunction|Boolean|否|False|如果函数不需要任何参数且在调用时不使用任何参数，则返回 True。|  
|ParameterType<br /><br /> 语义|ParameterSemantics|否|AllowImplicit<br /><br /> 转换|有关查询管道应如何处理参数类型替换的选项：<br /><br /> - 仅精确匹配<br />- 允许隐式促销<br />- 允许隐式转换|  
...
|属性名称|数据类型|必选|默认值|说明|  
|--------------------|---------------|--------------|-------------------|-----------------|  
|名称|String|是|不适用|参数的标识符/名称。|  
|类型|String|是|不适用|参数的 EDM 类型。|  
|“模式”|参数<br /><br /> 方向|是|不适用|参数的方向：<br /><br /> - 在<br />- 出<br />- 出|  
