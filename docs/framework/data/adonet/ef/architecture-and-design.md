...
![关系图](./media/de1ca705-4f7c-4d2d-ace5-afefc6d3cefa.gif "de1ca705-4f7c-4d2d-ace5-afefc6d3cefa")
...
该符号还会添加到符号表中，以将输入绑定名称链接到它（"b"，\<symbol_b >）。

如果后续节点重用该 SqlSelectStatement，则它将一个项添加到符号表以将其输入绑定名链接到此符号。 在我们的示例中，输入绑定名称为 "a" 的 DbProjectExpression 将重复使用 SqlSelectStatement 并向表中添加（\< symbol_b >）。
...
其中 \<leftSqlSelectStatement > 和 \<rightSqlSelectStatement > 通过访问每个输入获得 Sqlselectstatement，\<p > 是对应的运算（例如 UNION ALL）。
...
根据表达式类型（Any 或 All），DbQuantifierExpression 转换为：
...
若要为区别名和列生成唯一名称，请使用 \<existing_name > _n，其中 n 是尚未使用的最小别名。 所有别名的全局列表加大了对层叠重命名的需要。
