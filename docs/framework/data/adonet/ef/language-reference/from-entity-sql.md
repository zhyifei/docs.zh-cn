...
> [!NOTE]
>  `CROSS` 和`OUTER APPLY`运算符在 SQL Server 2005 中引入。 在某些情况下，查询管道可能生成包含 `CROSS APPLY` 和/或 `OUTER APPLY` 运算符的 Transact-SQL。 因为某些后端提供程序，包括 SQL Server 2005 之前版本的 SQL Server 不支持这些运算符，则不能对这些后端提供程序执行此类查询。  
>   
