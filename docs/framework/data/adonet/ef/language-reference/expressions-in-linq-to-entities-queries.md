...
表达式是求值结果可以为单个值、对象、方法或命名空间的一段代码。 表达式可以包含文本值、方法调用、运算符及其操作数，或者简单名称。 简单名称可以是变量名、类型成员名、方法参数名、命名空间名或类型名。 表达式可以使用运算符（运算符又可使用其他表达式作为参数）或方法调用（方法调用的参数又可以是其他方法调用）。 因此，表达式可以非常简单，也可以极其复杂。  
 ...
 在 LINQ to Entities 查询中，表达式可以包含任何内容类型中所允许的<xref:System.Linq.Expressions>命名空间，包括 lambda 表达式。 可在 LINQ to Entities 查询表达式是可以使用的表达式的一个超集查询[!INCLUDE[adonet_ef](../../../../../../includes/adonet-ef-md.md)]。  对 [!INCLUDE[adonet_ef](../../../../../../includes/adonet-ef-md.md)] 的查询中使用的表达式仅限于 `ObjectQuery<T>` 和基础数据源所支持的运算。  
...
> [!NOTE]
>  特定的语言构造，如C# `unchecked`，LINQ to Entities 中没有任何意义。  
