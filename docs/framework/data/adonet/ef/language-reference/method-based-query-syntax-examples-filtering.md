...
本主题中的示例演示如何使用`Where`和`Where…Contains`方法通过基于方法的查询语法来查询[AdventureWorks 销售模型](https://archive.codeplex.com/?p=msftdbprodsamples)。 请注意，Where 。`Contains` 不能用作[已编译查询](compiled-queries-linq-to-entities.md)的一部分。  
