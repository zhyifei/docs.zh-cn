...
|通配符|描述|示例|  
|------------------------|-----------------|-------------|  
|%|包含零个或零个以上字符的任何 `string`。|`title like '%computer%'` 在标题中的任何位置查找单词 `"computer"` 的所有标题。|  
|_（下划线）|任意单个字符。|`firstname like '_ean'` 将查找以 `"ean` 结尾的所有四个字母的名字，如 "Dean" 或 "小红"。|  
|[ ]|指定范围 ([a-f]) 或集合 ([abcdef]) 中的任意单个字符。|`lastname like '[C-P]arsen'` 将查找以 "arsen" 结尾并以 C 和 P 之间的任何单个字符开头的姓氏（如 Carsen 或 Larsen）。|  
|[^]|不在指定范围 ([^a-f]) 或集合 ([^abcdef]) 中的任意单个字符。|`lastname like 'de[^l]%'` 查找所有以 "de" 开头的名称，并且不将 "l" 包含为下列字母。|  
  
> [!NOTE]
> [!INCLUDE[esql](../../../../../../includes/esql-md.md)] LIKE 运算符和 ESCAPE 子句不适用于 `System.DateTime` 或 `System.Guid` 值。  
  
 LIKE 支持 ASCII 模式匹配和 Unicode 模式匹配。 当所有参数都为 ASCII 字符时，将执行 ASCII 模式匹配。 如果一个或多个参数为 Unicode，则所有参数都会转换为 Unicode，并执行 Unicode 模式匹配。 在将 Unicode 与 LIKE 一起使用时，尾随空格有意义；但对非 Unicode，尾随空格则没有意义。 @No__t_0 的模式字符串语法与 Transact-sql 的模式字符串语法相同。  
