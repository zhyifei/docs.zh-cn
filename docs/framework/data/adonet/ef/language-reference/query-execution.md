...
> [!TIP]
> 若要获取表格式的查询运算符的方便摘要，以便快速标识运算符的执行行为，请参阅[按执行方式分类标准查询运算符（C#）](../../../../../csharp/programming-guide/concepts/linq/classification-of-standard-query-operators-by-manner-of-execution.md)。
...
## <a name="store-execution"></a>存储区执行  
 ...
 本节介绍在客户端本地执行代码的方案。 有关本地执行的表达式类型的详细信息，请参阅[LINQ to Entities 查询中的表达式](expressions-in-linq-to-entities-queries.md)。  
