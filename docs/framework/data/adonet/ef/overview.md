...
[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)] 使开发人员可以采用特定于域的对象和属性（如客户和客户地址）的形式使用数据，而不必自己考虑存储这些数据的基础数据库表和列。 借助[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]，开发人员在处理数据时能够以更高的抽象级别工作，并且能够以相比传统应用程序更少的代码创建和维护面向数据的应用程序。 由于是 .NET Framework 的组件， [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]因此应用程序可以在任何安装了版本 3.5 SP1 的 .NET Framework 计算机上运行。 [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]
...
## <a name="give-life-to-models"></a>为模型指定生存期
 ...
 通过使开发人员能够在域模型中查询实体和关系（称为中的[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]*概念*模型），并依赖[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]来将这些操作转换为数据源，从而为模型带来了各种关系： [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]特定命令。 这使应用程序不再对特定数据源具有硬编码的依赖性。
...
- 概念架构定义语言 (CSDL) 定义概念模型。 CSDL 是[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)][实体数据模型](../entity-data-model.md)的实现。 文件扩展名为 .csdl。
...
[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]使用这些模型和映射文件来创建、读取、更新和删除对概念模型中的实体和关系的操作，以便与数据源中的等效操作。 [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]甚至支持将概念模型中的实体映射到数据源中的存储过程。 有关详细信息，请参阅[CSDL、SSDL 和 MSL 规范](./language-reference/csdl-ssdl-and-msl-specifications.md)。
...
## <a name="map-objects-to-data"></a>将对象映射到数据
 ...
 现有解决方案只能通过将面向对象的类和属性映射到关系表和列来尝试弥合这种通常称为“阻抗不匹配”的差异。 不采用这种传统方法，而[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]是将逻辑模型中的关系表、列和外键约束映射到概念模型中的实体和关系。 这在定义对象和优化逻辑模型方面都增加了灵活性。 实体数据模型工具根据概念模型生成可扩展的数据类。 这些类是分部类，可以通过开发人员添加的其他成员进行扩展。 默认情况下，为特定概念模型生成的类派生自基类，这些基类提供服务以将实体具体化为对象以及跟踪和保存更改。 开发人员可以使用这些类像处理通过关联相关的对象一样处理实体和关系。 开发人员还可以自定义针对某一概念模型生成的类。 有关详细信息，请参阅使用[对象](working-with-objects.md)。

## <a name="access-and-change-entity-data"></a>访问和更改实体数据

[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]不仅仅是另一种对象关系映射解决方案，从本质上讲，它的作用是使应用程序能够访问和更改概念模型中以实体和关系形式表示的数据。 [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]使用模型和映射文件中的信息将对概念模型中表示的实体类型的对象查询转换为特定于数据源的查询。 查询结果具体化为管理的[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]对象。 [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]提供以下方法来查询概念模型并返回对象：
...
[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)] 中包含 EntityClient 数据提供程序。 此提供程序管理连接，将实体查询转换为特定于数据源的查询，并返回[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]用于将实体数据具体化为对象的数据读取器。 当不需要对象具体化时，通过使应用程序执行 [!INCLUDE[esql](../../../../../includes/esql-md.md)] 查询并使用返回的只读数据读取器，还可以像标准 ADO.NET 数据提供程序一样使用 EntityClient 提供程序。 有关详细信息，请参阅[实体框架的 EntityClient Provider](entityclient-provider-for-the-entity-framework.md)。
...
[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)] 包含一个支持规范命令目录树的最新 SqlClient 数据提供程序。 有关详细信息，请参阅[SqlClient for the 实体框架](sqlclient-for-the-entity-framework.md)。

## <a name="entity-data-model-tools"></a>实体数据模型工具

与[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]运行时一起，Visual Studio 包含映射和建模工具。 有关详细信息，请参阅[建模和映射](modeling-and-mapping.md)。

## <a name="learn-more"></a>了解详细信息

若要了解有关的[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]详细信息，请参阅：

[入门](getting-started.md)-提供有关如何使用快速[入门](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/bb399182(v=vs.100))快速启动和运行的信息，其中演示了如何创建简单[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]的应用程序。

[实体框架术语](terminology.md)-定义在文档中[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)] [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]使用的实体数据模型和和所引入的许多术语。

[实体框架资源](resources.md)-提供概念性主题的链接和指向用于构建[!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]应用程序的外部主题和资源的链接。
