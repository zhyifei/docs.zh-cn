...
本主题介绍了一些开发人员在他们使用 ADO.NET 中的浮点数时经常遇到的问题。 这些问题是由计算机存储浮点数的方式所导致的，并不是特定提供程序（如 <xref:System.Data.SqlClient> 或 <xref:System.Data.OracleClient>）所特有的。  
 ...
 若要最大限度地减少这些影响，应在 numeric 类型之间使用您可用的最近似匹配。 例如，如果您正在使用 SQL Server，精确数值可能会更改，如果将实际类型的 TRANSACT-SQL 值转换为 float 类型的值。 在.NET Framework 中，将转换<xref:System.Single>到<xref:System.Double>还可能产生意外的结果。 在这两种情况下，比较好的方法是使应用程序中的所有值都使用同一 numeric 类型。 您也可以使用固定精度的十进制类型，或者在使用浮点数之前将它们强制转换为固定精度的十进制类型。  
