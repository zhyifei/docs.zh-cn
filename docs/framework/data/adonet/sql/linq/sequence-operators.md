...
## 与 .NET 的差异  
 除 `Average` 外，所有受支持的序列运算符在公共语言运行库 \(CLR\) 中均按预期方式工作。  `Average` 返回的值的类型与执行求平均值运算的类型相同，而在 CLR 中，`Average` 则始终返回 <xref:System.Double> 或 <xref:System.Decimal>。  如果源参数被显式强制转换为 double\/decimal，或者选择器强制转换为 double\/decimal，则产生的 SQL 也将进行这种转换，因而结果将同预期的一样。  
