...
## <a name="class-library-projects"></a>类库项目  
 对象关系设计器在项目的`app.config`文件中创建一个连接字符串。 在类库项目中，不使用 `app.config` 文件。 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 使用在设计时文件中提供的连接字符串。 更改 `app.config` 中的值不会更改应用程序连接到的数据库。  
...
- 使用你自己的代码先删除阻止删除父对象的子对象。  
 ...
 有关详细信息，请参阅[如何：删除数据库](how-to-delete-rows-from-the-database.md)中的行。  
...
- `Imports` `using` C#对于和<xref:System.Linq> ，你有一个（Visual Basic）或（）指令。 <xref:System.Data.Linq>  
...
## <a name="groupby-invalidoperationexception"></a>GroupBy InvalidOperationException  
 如果在按 <xref:System.Linq.Enumerable.GroupBy%2A> 表达式进行分组的 `boolean` 查询（如 `group x by (Phone==@phone)`）中有一个列值为 null，则会引发此异常。 由于表达式为`boolean`，因此会将该键推断`boolean`为，而不`nullable` `boolean`是。 当转换的比较产生 null 时，会尝试将分配`nullable` `boolean` `boolean`给，并引发异常。  
