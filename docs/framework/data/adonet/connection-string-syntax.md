...
> [!IMPORTANT]
> 关键字的`Persist Security Info`默认设置为`false`。 如果将其设置为 `true` 或 `yes`，则允许在打开连接后通过连接获取安全敏感信息（包括用户 ID 和密码）。 保持`Persist Security Info`设置`false`以确保不受信任的源无法访问敏感的连接字符串信息。  

### <a name="windows-authentication-with-sqlclient"></a>使用 SqlClient 进行 Windows 身份验证
 以下每种语法形式都使用 Windows 身份验证连接到本地服务器上的**AdventureWorks**数据库。  
...
### <a name="sql-server-authentication-with-sqlclient"></a>SQL Server 身份验证与 SqlClient
...
当您连接到 Azure SQL 数据库或 Azure SQL 数据仓库并提供格式`user@servername`的登录名时，请确保登录`servername`名中的值与 提供`Server=`的值匹配。
...
要连接到 SQL Server 的命名实例，请使用*服务器名称\实例名称*语法。  
...
### <a name="type-system-version-changes"></a>类型系统版本更改  
 中的`Type System Version`关键字<xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType>指定 SQL Server 类型的客户端表示形式。 有关 <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> 关键字的更多信息，请参见 `Type System Version`。  
...
## <a name="using-trustservercertificate"></a>使用 TrustServerCertificate  
 仅当`TrustServerCertificate`使用有效证书连接到 SQL Server 实例时，关键字才有效。 当 `TrustServerCertificate` 设置为 `true` 时，传输层将使用 SSL 来加密通道并跳过证书链以验证信任。  
...
### <a name="enabling-encryption"></a>启用加密  
 要在服务器上未预配证书时启用加密，必须在 SQL 服务器配置管理器中设置**强制协议加密****和信任服务器证书**选项。 在这种情况下，如果服务器上未设置可验证的证书，加密将使用不带验证的自签名服务器证书。  
  
 应用程序设置无法降低在 SQL Server 中配置的安全级别，但可以增强安全级别。 应用程序可以通过将`TrustServerCertificate`和`Encrypt`关键字设置为 来请求`true`加密，从而保证即使在尚未预配服务器证书且尚未为客户端配置**强制协议加密**的情况下也会进行加密。 但是，如果未在客户端配置中启用 `TrustServerCertificate`，则仍需要提供服务器证书。  
...
|是|否|忽略|忽略|仅当存在可验证的服务器证书时，才会进行加密;否则，连接尝试失败。|  
|是|是|否（默认值）|忽略|始终加密，但可能使用自签名的服务器证书。|  
|是|是|是|否（默认值）|仅当存在可验证的服务器证书时，才会进行加密;否则，连接尝试失败。|  
...
> [!IMPORTANT]
> 可以在通用数据链路 （UDL） 文件中为**OleDbConnection**提供连接信息;但是，您应该避免这样做。 UDL 文件未加密，会以明文形式公开连接字符串信息。 因为 UDL 文件对您的应用程序来说是一个基于文件的外部资源，所以无法使用 .NET Framework 保护该文件。 **SqlClient**不支持 UDL 文件。  
