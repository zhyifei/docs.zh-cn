...
## <a name="example"></a>示例  
 本示例使用以下存储的过程以插入到一个新类别**Northwind** **类别**表。 存储的过程采用的值**CategoryName**列作为输入的参数，并使用 scope_identity （） 函数来检索标识字段的新值**CategoryID**，并将其返回输出参数。 RETURN 语句使用 @@ROWCOUNT函数返回插入的行数。  
