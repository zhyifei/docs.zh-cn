...
> [!NOTE]
> `DataAdapter`对于的`Fill` 操作`Get`和的方法，将从.NETFramework数据提供程序返回的类型推断.NETFramework类型。`DataReader` ADO.NET 中的[数据类型映射](data-type-mappings-in-ado-net.md)中介绍了 Microsoft SQL Server、OLE DB 和 ODBC 数据类型的推断 .NET Framework 类型和访问器方法。  
