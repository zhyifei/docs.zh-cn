...
## <a name="handling-errors-as-infomessages"></a>将错误作为信息性消息处理  
 通常，只有从服务器发出的信息性消息和警告消息才会触发 <xref:System.Data.SqlClient.SqlConnection.InfoMessage> 事件。 但是，当发生实际错误时，启动服务器操作的执行**NonQuery**或**ExecuteReader**方法将停止，并引发异常。  
...
## <a name="working-with-the-statechange-event"></a>使用 StateChange 事件  
 当**连接**状态更改时，将发生**状态更改**事件。 **状态更改**事件接收<xref:System.Data.StateChangeEventArgs>，使您能够通过使用**原始状态**和**当前状态**属性确定**连接**状态的更改。 **原始状态**属性是一个<xref:System.Data.ConnectionState>枚举，用于指示**连接**在更改之前的状态。 **当前状态**是一<xref:System.Data.ConnectionState>个枚举，指示**连接**更改后的状态。  
  
 以下代码示例使用**StateChange**事件在**连接**状态更改时向控制台写入消息。  
