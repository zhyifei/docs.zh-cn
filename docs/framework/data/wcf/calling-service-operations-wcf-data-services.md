...
ms.locfileid: "79174350"
---
# <a name="calling-service-operations-wcf-data-services"></a>调用服务操作（WCF 数据服务）
开放数据协议 （OData） 定义数据服务的服务操作。 WCF 数据服务使您能够将此类操作定义为数据服务上的方法。 与其他数据服务资源一样，这些服务操作也通过 URI 进行寻址。 服务操作可以返回实体类型的集合、单个实体类型实例的集合和基元类型（如整数和字符串）的集合。 服务操作还可以返回 `null`（在 Visual Basic 中为 `Nothing`）。 WCF 数据服务客户端库可用于访问支持 HTTP GET 请求的服务操作。 这些种类的服务操作定义为应用了 <xref:System.ServiceModel.Web.WebGetAttribute> 的方法。 有关详细信息，请参阅[服务操作](service-operations-wcf-data-services.md)。  
  
 服务操作在实现 OData 的数据服务返回的元数据中公开。 在元数据中，服务操作表示为 `FunctionImport` 元素。 在生成强类型 <xref:System.Data.Services.Client.DataServiceContext> 时，“添加服务引用”和 DataSvcUtil.exe 工具将忽略此元素。 因此，无法在上下文中找到一种方法用来直接调用服务操作。 但是，您仍可以使用 WCF 数据服务客户端以以下两种方式之一调用服务操作：  
...
## <a name="considerations-for-calling-service-operations"></a>调用服务操作的注意事项  
 使用 WCF 数据服务客户端调用服务操作时，请考虑以下事项。  
  
- 异步访问<xref:System.Data.Services.Client.DataServiceContext.BeginExecute%2A>/<xref:System.Data.Services.Client.DataServiceContext.EndExecute%2A>数据服务时，必须在 上<xref:System.Data.Services.Client.DataServiceContext>或使用等效异步方法或在<xref:System.Data.Services.Client.DataServiceQuery%601.BeginExecute%2A>/<xref:System.Data.Services.Client.DataServiceQuery%601.EndExecute%2A>上 使用<xref:System.Data.Services.Client.DataServiceQuery%601>方法。  
  
- WCF 数据服务客户端库无法实现返回基元类型集合的服务操作的结果。  
  
- WCF 数据服务客户端库不支持调用 POST 服务操作。 服务操作由 HTTP POST 调用，后者通过使用 <xref:System.ServiceModel.Web.WebInvokeAttribute> 配合 `Method="POST"` 参数定义。 要通过使用 HTTP POST 请求调用服务操作，必须使用 <xref:System.Net.HttpWebRequest>。  
...
- 请考虑对强类型 <xref:System.Data.Services.Client.DataServiceContext> 分部类创建扩展方法，该分部类通过工具生成，使用 <xref:System.Data.Services.Client.DataServiceContext.CreateQuery%2A> 或 <xref:System.Data.Services.Client.DataServiceContext.Execute%2A> 方法来调用服务操作。 这样您就可以直接从上下文调用服务操作。 有关详细信息，请参阅博客文章"[服务操作"和 WCF 数据服务客户端](https://docs.microsoft.com/archive/blogs/astoriateam/service-operations-and-the-wcf-data-services-client)。  
  
- 当您用于<xref:System.Data.Services.Client.DataServiceContext.CreateQuery%2A>调用服务操作时，客户端库通过执行保留字符的百分比编码（如<xref:System.Data.Services.Client.DataServiceQuery%601.AddQueryOption%2A>ampersand（&）和字符串中单引号转义来自动转义提供给 的字符。 但是，当您调用其中一个*Execute*方法来调用服务操作时，必须记住执行任何用户提供的字符串值的此转义。 URI 中的单引号转义为单引号对。  
  
## <a name="examples-of-calling-service-operations"></a>调用服务操作示例  
 本节包含以下示例，说明如何使用 WCF 数据服务客户端库调用服务操作：  
  
- [调用执行&lt;&gt; T 返回实体集合](calling-service-operations-wcf-data-services.md#ExecuteIQueryable)  
  
- [使用&lt;CreateQuery&gt; T 返回实体集合](calling-service-operations-wcf-data-services.md#CreateQueryIQueryable)  
  
- [调用执行&lt;&gt; T 返回单个实体](calling-service-operations-wcf-data-services.md#ExecuteSingleEntity)  
  
- [调用执行&lt;&gt; T 返回基元值的集合](calling-service-operations-wcf-data-services.md#ExecutePrimitiveCollection)  
  
- [调用执行&lt;&gt; T 返回单个基元值](calling-service-operations-wcf-data-services.md#ExecutePrimitiveValue)  
...
### <a name="calling-executet-to-return-a-collection-of-entities"></a>调用执行\<T> 以返回实体集合  
...
### <a name="using-createqueryt-to-return-a-collection-of-entities"></a>使用 createQuery\<T>返回实体集合  
...
### <a name="calling-executet-to-return-a-single-entity"></a>调用执行\<T> 返回单个实体  
...
### <a name="calling-executet-to-return-a-collection-of-primitive-values"></a>调用执行\<T> 返回基元值的集合  
...
### <a name="calling-executet-to-return-a-single-primitive-value"></a>调用执行\<T> 以返回单个基元值  
