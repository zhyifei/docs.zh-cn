...
ms.locfileid: "75346160"
---
# <a name="batching-operations-wcf-data-services"></a>批处理操作（WCF 数据服务）
Open Data Protocol （OData）支持批处理发送到基于 OData 的服务的请求。 有关详细信息，请参阅[OData：批处理](https://www.odata.org/documentation/odata-version-2-0/batch-processing/)。 在 WCF 数据服务中，使用 <xref:System.Data.Services.Client.DataServiceContext>的每个操作（如执行查询或保存更改）都会导致向数据服务发送一个单独的请求。 为保持操作集的合理范围，可以显式定义操作批。 这可确保批处理中的所有操作都在单个 HTTP 请求中发送到数据服务，使服务器能够以原子方式处理操作，并减少与数据服务的往返次数。  
