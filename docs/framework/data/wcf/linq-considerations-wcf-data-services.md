...
本主题提供有关使用 WCF 数据服务客户端时组成和执行 LINQ 查询的方式以及使用 LINQ 查询实现开放数据协议 （OData） 的数据服务的限制。 有关针对基于 OData 的数据服务撰写和执行查询的详细信息，请参阅[查询数据服务](querying-the-data-service-wcf-data-services.md)。  
  
## <a name="composing-linq-queries"></a>编写 LINQ 查询  
 LINQ 允许针对实现 <xref:System.Collections.Generic.IEnumerable%601> 的对象集合编写查询。 Visual Studio 中的 **"添加服务参考"** 对话框和 DataSvcUtil.exe 工具都用于生成 OData 服务的表示形式，作为从<xref:System.Data.Services.Client.DataServiceContext>继承的实体容器类以及表示在源中返回的实体的对象。 这些工具还可为由服务作为源公开的集合的实体容器类生成属性。 封装了数据服务的类的每个属性都会返回一个 <xref:System.Data.Services.Client.DataServiceQuery%601>。 因为 <xref:System.Data.Services.Client.DataServiceQuery%601> 类实现 LINQ 定义的 <xref:System.Linq.IQueryable%601> 接口，所以可以针对数据服务公开的源编写 LINQ 查询，客户端库会将其转换为一个查询请求 URI，在执行时将此 URI 发送给数据服务。  
  
> [!IMPORTANT]
> LINQ 语法中可表示的查询集比 OData 数据服务使用的 URI 语法中启用的查询集要广泛。 如果无法将查询映射到目标数据服务中的 URI，则会引发 <xref:System.NotSupportedException>。 有关详细信息，请参阅本主题中的[不支持的 LINQ 方法](linq-considerations-wcf-data-services.md#unsupportedMethods)。  
...
[!code-vb[Astoria Northwind Client#AddQueryOptionsLinqSpecific](../../../../samples/snippets/visualbasic/VS_Snippets_Misc/astoria_northwind_client/vb/source.vb#addqueryoptionslinqspecific)]
 ...
 此 LINQ 查询将转换为以下查询 URI，该 URI 针对基于北风的快速[启动](quickstart-wcf-data-services.md)数据服务执行：  
  
```http
http://localhost:12345/Northwind.svc/Orders?Orderby=ShippedDate&?filter=Freight gt 30  
```  
  
 有关 LINQ 的一般信息，请参阅[语言集成查询 （LINQ） - C#](../../../csharp/programming-guide/concepts/linq/index.md)或[语言集成查询 （LINQ） - 可视基本](../../../visual-basic/programming-guide/concepts/linq/index.md)。  
...
[!code-vb[Astoria Northwind Client#AddQueryOptionsLinqExpressionSpecific](../../../../samples/snippets/visualbasic/VS_Snippets_Misc/astoria_northwind_client/vb/source.vb#addqueryoptionslinqexpressionspecific)]
 ...
 WCF 数据服务客户端能够将这两种组合查询转换为查询 URI，并且可以通过将查询方法追加到查询表达式来扩展 LINQ 查询。 通过向查询表达式或 <xref:System.Data.Services.Client.DataServiceQuery%601> 追加方法语法来编写 LINQ 查询时，运算将按照方法的调用顺序添加到查询 URI 中。 这等效于调用 <xref:System.Data.Services.Client.DataServiceQuery%601.AddQueryOption%2A> 方法将每个查询选项添加到查询 URI 中。  
...
## <a name="executing-linq-queries"></a>执行 LINQ 查询  
 ...
 客户端分两个部分来执行 LINQ 查询。 只要有可能，将首先在客户端上计算查询中的 LINQ 表达式，然后生成基于 URI 的查询并将其发送至数据服务，以针对服务中的数据进行计算。 有关详细信息，请参阅[查询数据服务中的](querying-the-data-service-wcf-data-services.md)["客户端与服务器执行"](querying-the-data-service-wcf-data-services.md#executingQueries)部分。  
  
 当 LINQ 查询无法在符合 OData 的查询 URI 中转换时，尝试执行时引发异常。 有关详细信息，请参阅[查询数据服务](querying-the-data-service-wcf-data-services.md)。  
  
## <a name="linq-query-examples"></a>LINQ 查询示例  
 以下各节中的示例演示了可针对 OData 服务执行的 LINQ 查询类型。  
...
<a name="expand"></a>
### <a name="expand"></a>展开  
 查询 OData 数据服务时，可以请求包括与查询目标实体相关的实体包含返回的源。 针对 LINQ 查询的目标实体集调用 <xref:System.Data.Services.Client.DataServiceQuery%601.Expand%2A> 的 <xref:System.Data.Services.Client.DataServiceQuery%601> 方法，相关实体集名称作为 `path` 参数提供。 有关详细信息，请参阅[加载延迟内容](loading-deferred-content-wcf-data-services.md)。  
...
## <a name="unsupported-linq-methods"></a>不支持的 LINQ 方法  
 下表包含 LINQ 方法的类不受支持，并且不能包含在针对 OData 服务的查询中：  
...
|分页运算符|以下分页运算符不能用于 <xref:System.Data.Services.Client.DataServiceQuery%601>：<br /><br /> -   <xref:System.Linq.Enumerable.ElementAt%2A><br />-   <xref:System.Linq.Enumerable.Last%2A><br />-   <xref:System.Linq.Enumerable.LastOrDefault%2A><br />-   <xref:System.Linq.Enumerable.SkipWhile%2A><br />-   <xref:System.Linq.Enumerable.TakeWhile%2A><br/><br/>**注：** 在空序列上执行的分页运算符返回 null。|  
|其他运算符|以下运算符也不支持针对<xref:System.Data.Services.Client.DataServiceQuery%601>：<br /><br /> - <xref:System.Linq.Enumerable.Empty%2A><br />- <xref:System.Linq.Enumerable.Range%2A><br />- <xref:System.Linq.Enumerable.Repeat%2A><br />- <xref:System.Linq.Enumerable.ToDictionary%2A><br />- <xref:System.Linq.Enumerable.ToLookup%2A>|  
  
<a name="supportedExpressions"></a>
## <a name="supported-expression-functions"></a>支持的表达式函数  
 支持以下通用语言运行时 （CLR） 方法和属性，因为它们可以转换为查询表达式，以便包含在请求 URI 中到 OData 服务：  
  
|<xref:System.String> 成员|支持的 OData 功能|  
|-----------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|  
...
|<xref:System.DateTime>成员<sup>1</sup>|支持的 OData 功能|  
|-------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|  
...
|<xref:System.DateTime.Year>|`int year(DateTime p0)`|  
 ...
 <sup>1</sup>还支持 Visual Basic<xref:Microsoft.VisualBasic.DateAndTime?displayProperty=nameWithType>中的等效<xref:Microsoft.VisualBasic.DateAndTime.DatePart%2A>日期和时间属性和方法。  
  
|<xref:System.Math> 成员|支持的 OData 功能|  
|---------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|  
...
|<xref:System.Linq.Expressions.Expression> 成员|支持的 OData 功能|  
|---------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|  
