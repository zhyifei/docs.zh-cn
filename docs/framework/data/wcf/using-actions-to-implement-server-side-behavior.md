...
#### <a name="tryresolveserviceaction"></a>TryResolveServiceAction  
 此方法搜索指定的[ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103)) ，如果找到`true` [ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103)) ，则返回。 如果找到，则在`out`参数中`serviceAction`返回[ServiceAction](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/hh544089(v=vs.103)) 。  
...
3. 存储来自 Invoke() 的任何结果，以便可以使用 GetResult() 检索这些结果  
 ...
 此接口需要两种方法：调用和 GetResult。 Invoke 调用用于实现动作行为的委托，而 GetResult 返回此动作的结果。  
