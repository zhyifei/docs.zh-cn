---
title: "服务操作（WCF 数据服务） | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 服务操作（WCF 数据服务）
使用 [!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)] 可以在数据服务中定义服务操作，以便在服务器上公开方法。  与其他数据服务资源一样，服务操作也是通过 URI 进行寻址。  使用服务操作可以在数据服务中公开业务逻辑；例如，实现验证逻辑，应用基于角色的安全性或公开专用查询功能。  服务操作是添加到从 <xref:System.Data.Services.DataService%601> 派生的数据服务的方法。像所有其他数据服务资源一样，您可以提供服务操作方法的参数。例如，以下服务操作 URI（基于[快速启动](../../../../docs/framework/data/wcf/quickstart-wcf-data-services.md)数据服务）将值 `London` 传递到 `city` 参数：  
...
```  
 ...
 使用 <xref:System.Data.Services.DataService%601> 的 <xref:System.Data.Services.DataService%601.CurrentDataSource%2A> 可以直接访问数据服务正在使用的数据源。  有关详细信息，请参阅[如何：定义服务操作](../../../../docs/framework/data/wcf/how-to-define-a-service-operation-wcf-data-services.md)。  
  
 有关如何从 .NET Framework 客户端应用程序调用服务操作的信息，请参见[调用服务操作](../../../../docs/framework/data/wcf/calling-service-operations-wcf-data-services.md)。  
...
## 对服务操作进行寻址  
 通过将方法名称放在 URI 的第一个路径段中，可以对服务操作进行寻址。  例如，下面的 URI 访问一个 `GetOrdersByState` 操作，其返回 `Orders` 对象的 <xref:System.Linq.IQueryable%601> 集合。  
...
|<xref:System.Linq.IQueryable%601>|除了作为服务操作名称的路径之外，还允许使用查询路径段。  也允许使用查询选项。|  
 ...
 可以将其他路径段或查询选项添加到该 URI，具体取决于服务操作的返回类型。  例如，以下 URI 访问 `GetOrdersByCity` 操作，该操作返回 `Orders` 对象的 <xref:System.Linq.IQueryable%601> 集合（按 `RequiredDate` 的降序顺序排序）以及相关的 `Order_Details` 对象：  
...
## 服务操作访问控制  
 服务操作的服务范围可见性由 <xref:System.Data.Services.IDataServiceConfiguration> 类的 <xref:System.Data.Services.IDataServiceConfiguration.SetServiceOperationAccessRule%2A> 方法控制，其方式与使用 <xref:System.Data.Services.IDataServiceConfiguration.SetEntitySetAccessRule%2A> 方法控制实体集可见性的方式大致相同。  例如，数据服务定义中的以下代码行用于访问 `CustomersByCity` 服务操作。  
