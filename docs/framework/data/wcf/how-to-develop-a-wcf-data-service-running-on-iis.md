...
本主题演示如何使用 WCF 数据服务创建一个数据服务，该服务基于 Internet Information Services （IIS）上运行的 ASP.NET Web 应用程序所承载的 Northwind 示例数据库。 有关如何创建与 ASP.NET 开发服务器上运行的 ASP.NET Web 应用程序相同的 Northwind 数据服务的示例，请参阅[WCF 数据服务快速入门](quickstart-wcf-data-services.md)。
