...
这是 WCF 数据服务快速入门的最后一任务。 在此任务中，你将向解决方案中添加一个控制台应用程序，将对[!INCLUDE[ssODataFull](../../../../includes/ssodatafull-md.md)]该源的引用添加到此新的客户端应用程序，并使用生成的客户端数据服务类和客户端库从客户端应用程序访问 OData 源.
...
2. 在 "**命名空间**" 文本框中`Northwind`，键入，然后单击 **"确定"** 。
...
3. 在**NorthwindClient**下的`using` **解决方案资源管理器**中，打开 mainwindow.xaml 文件的代码页，然后添加以下语句（`Imports`在 Visual Basic 中）。
