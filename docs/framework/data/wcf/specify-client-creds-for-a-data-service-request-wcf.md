---
title: 如何：为数据服务请求指定客户端凭据（WCF 数据服务）
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
ms.locfileid: "70790325"
---
# <a name="how-to-specify-client-credentials-for-a-data-service-request-wcf-data-services"></a>如何：为数据服务请求指定客户端凭据（WCF 数据服务）
...
## <a name="example"></a>示例  
 下面的示例来自作为 Windows Presentation Framework 应用程序主页的 Extensible Application Markup Language （XAML）文件的代码隐藏页。 该示例显示了一个 `LoginWindow` 实例，用于收集用户的身份验证凭据，然后在向数据服务发出请求时使用了这些凭据。  
