---
title: "使用 XML Web services 进行 XML 序列化 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
helpviewer_keywords: 
  ...
  - "SOAP, XML 序列化"
  - "XML 序列化, 特性"
  - "XML 序列化, XML Web services"
  - "XML Web services, XML 序列化"
...
XML 序列化是在 XML Web services 体系结构中使用的基础传输机制，由 [XmlSerializer](https://msdn.microsoft.com/en-us/library/system.xml.serialization.xmlserializer.aspx) 类执行。若要控制由 XML Web services 生成的 XML，可以将[用来控制 XML 序列化的特性](../../../docs/framework/serialization/attributes-that-control-xml-serialization.md)和[用来控制编码的 SOAP 序列化的特性](../../../docs/framework/serialization/attributes-that-control-encoded-soap-serialization.md)中列出的特性应用于用来创建 XML Web services \(.asmx\) 的文件的类、返回值、参数和字段。有关创建 XML Web services 的更多信息，请参见[Building XML Web Services Using ASP.NET](http://msdn.microsoft.com/zh-cn/01dfc27c-c68e-4910-a0aa-5e4c2a766b0c)。  
  
## 文本样式和编码样式  
 XML Web services 生成的 XML 可以用两种方式进行格式化，一种是文本方式，另一种是编码方式，如[Customizing SOAP Messages](http://msdn.microsoft.com/zh-cn/1d777288-c0d9-4e6a-b638-f010da031952)中所述。因此有两组控制 XML 序列化的特性。[用来控制 XML 序列化的特性](../../../docs/framework/serialization/attributes-that-control-xml-serialization.md)中列出的特性旨在控制文本样式的 XML。[用来控制编码的 SOAP 序列化的特性](../../../docs/framework/serialization/attributes-that-control-encoded-soap-serialization.md)中列出的特性用来控制编码样式。通过有选择地应用这些特性，可以调整应用程序，使其返回两种样式或其中一种。而且，这些特性可以根据需要应用于返回值和参数。  
  
### 使用两种样式的示例  
 创建 XML Web services 时，可以对方法使用两组特性。在下面的代码示例中，名为 `MyService` 的类包含两种 XML Web services 方法：`MyLiteralMethod` 和 `MyEncodedMethod`。这两种方法执行相同的功能，即返回 `Order` 类的实例。在 `Order` 类中，[XmlTypeAttribute](frlrfSystemXmlSerializationXmlTypeAttributeClassTopic) 和 [SoapTypeAttribute](frlrfSystemXmlSerializationSoapTypeAttributeClassTopic) 特性都应用于 `OrderID` 字段，且两组特性的 `ElementName` 特性设置为不同的值。  
