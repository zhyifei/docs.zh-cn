---
title: "如何：控制派生类的序列化 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 如何：控制派生类的序列化
使用 **XmlElementAttribute** 特性更改 XML 元素的名称不是自定义对象序列化的唯一方法。您还可以自定义 XML 流，具体方法为从现有类派生以及指示 <xref:System.Xml.Serialization.XmlSerializer> 实例如何序列化新类。  
  
 例如，假设有一个 `Book` 类，您可以从该类派生，并创建一个具有多个属性的 `ExpandedBook` 类。然而，当序列化或反序列化时，您必须指示 **XmlSerializer** 接受派生的类型。可以通过创建一个 <xref:System.Xml.Serialization.XmlElementAttribute> 实例并将其 **Type** 属性设置为派生类的类型来实现这一点。请将 **XmlElementAttribute** 添加至 <xref:System.Xml.Serialization.XmlAttributes> 实例。然后将 **XmlAttributes** 添加至 <xref:System.Xml.Serialization.XmlAttributeOverrides> 实例，并指定要重写的类型以及接受派生类的成员的名称。这在下面的示例中显示。  
