...
---
# <a name="repeatbutton-syles-and-templates"></a>RepeatButton 样式和模板
本主题介绍的样式和模板的<xref:System.Windows.Controls.Primitives.RepeatButton>控件。 你可以修改默认<xref:System.Windows.Controls.ControlTemplate>提供独特外观的控件。 有关详细信息，请参阅[通过创建 ControlTemplate 自定义现有控件的外观](../../../../docs/framework/wpf/controls/customizing-the-appearance-of-an-existing-control.md)。  
  
## <a name="repeatbutton-parts"></a>RepeatButton 部件  
 <xref:System.Windows.Controls.Primitives.RepeatButton>控件不具有任何已命名的部件。  
  
## <a name="repeatbutton-states"></a>RepeatButton 状态  
 下表列出的可视状态<xref:System.Windows.Controls.Primitives.RepeatButton>控件。  
...
## <a name="repeatbutton-controltemplate-example"></a>RepeatButton ControlTemplate 示例  
 下面的示例演示如何定义<xref:System.Windows.Controls.ControlTemplate>为<xref:System.Windows.Controls.Primitives.RepeatButton>控件。  
