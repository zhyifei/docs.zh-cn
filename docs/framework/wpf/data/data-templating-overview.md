...
在上面的示例中，我们使用 <xref:System.Windows.DataTemplate.Triggers%2A?displayProperty=nameWithType> 属性将触发器置于 <xref:System.Windows.DataTemplate> 中。 触发器的 <xref:System.Windows.Setter> 设置位于 <xref:System.Windows.DataTemplate>内的元素（<xref:System.Windows.Controls.Border> 元素）的属性的值。 但是，如果您的 `Setters` 与之相关的属性不是当前 <xref:System.Windows.DataTemplate>内的元素的属性，则更适合使用 <xref:System.Windows.Controls.ListBoxItem> 类的 <xref:System.Windows.Style> 设置属性（如果您绑定的控件是<xref:System.Windows.Controls.ListBox>）。 例如，如果你希望 <xref:System.Windows.Trigger> 在鼠标指向某个项时对该项的 <xref:System.Windows.UIElement.Opacity%2A> 值进行动画处理，则需要在 <xref:System.Windows.Controls.ListBoxItem> 样式中定义触发器。 有关示例，请参阅[样式设置和模板化示例简介](https://github.com/Microsoft/WPF-Samples/tree/master/Styles%20&%20Templates/IntroToStylingAndTemplating)。
...
此示例使用[system.windows.datatemplate>](xref:System.Windows.FrameworkTemplate.Resources%2A)属性。 此部分中定义的资源由 <xref:System.Windows.DataTemplate>中的元素共享。
