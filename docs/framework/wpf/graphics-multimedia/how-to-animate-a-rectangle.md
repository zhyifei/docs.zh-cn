...
## <a name="example"></a>示例  
 下面的示例使用的实例<xref:System.Windows.Media.Animation.RectAnimation>类进行动画处理<xref:System.Windows.Media.RectangleGeometry.Rect%2A>属性<xref:System.Windows.Media.RectangleGeometry>，其中的大小和位置的矩形的更改进行动画处理。  
