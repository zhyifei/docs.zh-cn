...
|不透明度|呈现半透明三维内容（如反射）时，请使用画笔或材料的不透明度属性（通过 <xref:System.Windows.Media.Brush.Opacity%2A> 或 <xref:System.Windows.Media.Media3D.DiffuseMaterial.Color%2A>），而不是通过将 `Viewport3D.Opacity` 设置为小于1的值来创建单独的半透明 <xref:System.Windows.Controls.Viewport3D>。|  
