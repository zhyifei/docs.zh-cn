...
-   指定从该处该值应使用取得联系的时间其<xref:System.Windows.Media.Animation.IKeyFrame.KeyTime%2A>属性。  
 ...
 **替代方法**  
 ...
 如果你想要进行动画处理没有相应 From/To/By 动画或关键帧动画的类型，请考虑使用<xref:System.Windows.Media.Animation.ObjectAnimationUsingKeyFrames>。 因为它属于弱类型，<xref:System.Windows.Media.Animation.ObjectAnimationUsingKeyFrames>可以对任何类型的值进行动画处理。 此方法的缺点在于<xref:System.Windows.Media.Animation.ObjectAnimationUsingKeyFrames>仅支持离散内插。  
 ...
 **实现说明**  
 ...
 相反，你注册<xref:System.Windows.Media.CompositionTarget.Rendering>包含你想要进行动画处理的对象的对象的事件。 每帧会调用一次此事件处理程序方法。 每次 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 将可视化树中的持久呈现数据封送到复合树时，都将调用事件处理程序方法。  
