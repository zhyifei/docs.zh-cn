...
此示例演示了一些更常见的操作, 这些操作可通过<xref:System.Windows.Documents.Table.RowGroups%2A>属性对表的行组执行。  
...
## <a name="example"></a>示例  
 下面的示例插入一个新<xref:System.Windows.Documents.TableRowGroup>的。  新列将插入到索引位置 0, 使其成为表中新的第一个行组。  
...
## <a name="example"></a>示例  
 下面的示例在表中向特定<xref:System.Windows.Documents.TableRowGroup> (由 index 指定) 添加多行。  
...
## <a name="example"></a>示例  
 下面的示例将多个单元格添加<xref:System.Windows.Documents.TableRow>到表中的特定 (由 index 指定)。  
...
## <a name="example"></a>示例  
 下面的示例返回由表托管<xref:System.Windows.Documents.TableRowGroup>的元素的数目。  
