...
> [!NOTE]
> Windows SDK 包含一组可用于 @no__t 0 应用程序的示例 OpenType 字体。 这些字体提供本主题余下部分所述的大多数功能。 有关详细信息，请参阅[示例 OpenType 字体包](sample-opentype-font-pack.md)。  
...
- 字体中包含的显式脚本和语言信息，使文本处理应用程序可相应调整其行为。  
 ...
 本概述的其余部分介绍了一些由 <xref:System.Windows.Documents.Typography> 对象的属性公开的视觉上有趣的 OpenType 功能的广度和灵活性。 有关此对象的详细信息，请参阅[版式类](#typography_class)。  
...
### <a name="superscripts-and-subscripts"></a>上标和下标  
 @No__t-0 属性允许您设置 OpenType 字体的上标和下标值。  
  
 以下文本显示 Palatino Linotype 字体的上标。  
  
 使用 OpenType 上标的![文本使用]opentype 上标(./media/opentype-font-features/opentype-superscripts.gif "文本")  
 ...
 使用(./media/opentype-font-features/opentype-subscripts.gif "opentype 下标的")![文本使用]opentype 下标  
...
### <a name="decorative-uses-of-superscripts-and-subscripts"></a>上标和下标的修饰用法  
 ...
 使用 opentype 上标和下标文本使用 opentype(./media/opentype-font-features/opentype-superscripts-subscripts.gif "上标和下标")的![文本]  
...
## <a name="capitals"></a>大写字母  
 ...
 使用(./media/opentype-font-features/opentype-capitals.gif "opentype 大写字母")的![文本使用 opentype 大写字母]  
...
### <a name="titling-capitals"></a>标题大写字母  
 ...
 使用 opentype 标题大写字母的![文本使用 opentype]标题(./media/opentype-font-features/opentype-titling-capitals.gif "大写字母")  
...
### <a name="capital-spacing"></a>大写字母间距  
 ...
 使用(./media/opentype-font-features/opentype-capital-spacing.gif "opentype 大写字母间距")的![文本使用]opentype 大写字母间距  
...
- **历史连字**。 重在历史性，并非专为可读性而设计。  
 ...
 使用(./media/opentype-font-features/opentype-standard-ligatures.gif "opentype 标准连字")的![使用 opentype 标准连字]的文本  
 ...
 使用 opentype(./media/opentype-font-features/opentype-discretionary-ligatures.gif "自由连字")的![文本使用 opentype 自由连字]  
  
 以下标记示例演示如何使用 <xref:System.Windows.Documents.Typography> 对象的属性定义 Pericles 字体的自由连字字形。  
  
 [!code-xaml[OpenTypeFontSamples#5](~/samples/snippets/csharp/VS_Snippets_Wpf/OpenTypeFontSamples/CS/PageOne.xaml#5)]  
  
 默认情况下，@no__t 中的 OpenType 字体会启用标准连字。 例如，如果使用 Palatino Linotype 字体，则标准连字“fi”、“ff”和“fl”显示为组合字符字形。 请注意，每个标准连字的字符对之间彼此相连。  
  
 使用 opentype![标准连字和 Palatino Linotype 的 Palatino Linotype 文本使用 opentype 标准连](./media/opentype-font-features/opentype-standard-ligatures-palatino.gif "")字的文本    
   
 但是，可禁用标准连字功能，从而使“ff”等标准连字显示为两个单独的字形，而不显示为一个组合字符字形。  
  
 使用禁用的 opentype 标准连字的文本使用禁用的![opentype]标准连(./media/opentype-font-features/disabled-opentype-standard-ligatures.gif "字")  
...
## <a name="swashes"></a>花体  
 ...
 使用 opentype 标准(./media/opentype-font-features/opentype-standard-swash-glyphs.gif "和花体")字型的使用![opentype 标准和花体的]文本  

 花体通常用作事件公告等简短文章中的修饰元素。 以下文本使用花体强调事件名称的大写字母。  
  
 使用 opentype 花体文本的![文本]使用(./media/opentype-font-features/opentype-swashes.gif "opentype 花体")  
...
### <a name="contextual-swashes"></a>连接形式花体  
 ...
 使用 opentype 上下文花体文本的![文本]使用(./media/opentype-font-features/opentype-contextual-swashes.gif "opentype 上下文花体")  
...
## <a name="alternates"></a>备用项  
 ...
 使用 opentype(./media/opentype-font-features/opentype-standard-glyphs.gif "标准")字形的![文本使用 opentype 标准字形]  

 Pericles OpenType 字体包含其他一些标志符号，它们提供标准字形集的样式备用项。 以下文本显示样式备用字形。  
  
 使用 opentype 样式备用标志符号文本使用 opentype 样式(./media/opentype-font-features/opentype-stylistic-alternate-glyphs.gif "备用字形")的![文本]  
 ...
 使用![opentype 样式备用标志符号的文本](./media/opentype-font-features/opentype-stylistic-alternate-glyphs-pericles.gif "使用 opentype 样式备用标志符号为 Pericles 字体使用 opentype 样式备用标志符号")
...
### <a name="random-contextual-alternates"></a>随机备用连接形式  
 ...
 使用 opentype 随机上下文替换文本的![文本](./media/opentype-font-features/opentype-random-contextual-alternates.gif "使用 opentype 随机上下文替换")项  
  
 以下标记示例演示如何使用 <xref:System.Windows.Documents.Typography> 对象的属性定义 Lindsey 字体的随机上下文替代项。  
...
### <a name="historical-forms"></a>历史形式  
 ...
 使用(./media/opentype-font-features/opentype-historical-forms.gif "opentype 历史")窗体文本使用 opentype 历史窗体的![文本]  
...
### <a name="fractions"></a>分数  
 ...
 使用 opentype 斜杠和堆积分数文本![的文本]使用(./media/opentype-font-features/opentype-slashed-stacked-fractions.gif "opentype 斜杠和堆积分数")  
...
### <a name="old-style-numerals"></a>旧样式数字  
 ...
 ![使用 opentype 旧样式数字](./media/opentype-font-features/opentype-old-style-numerals.gif "文本使用 opentype 旧样式数字")的文本  
    
 以下文本显示 Palatino Linotype 字体的标准数字，后跟旧样式数字。  
  
 ![使用 opentype 旧样式数字的文本]使用(./media/opentype-font-features/opentype-old-style-numeral-sets.gif "opentype 旧样式数字集设置文本")  
...
### <a name="proportional-and-tabular-figures"></a>比例数字和表格式数字  
 ...
 使用 opentype 按![比例 & 表格](./media/opentype-font-features/opentype-proportional-tabular-figures.gif "使用 opentype 比例和表格数字的")文本  
...
### <a name="slashed-zero"></a>斜线零  
 ...
 使用 opentype 斜杠零数字文本的![文本](./media/opentype-font-features/opentype-slashed-zero-numerals.gif "使用 opentype 斜杠零位")  
...
## <a name="typography-class"></a>版式类  
 @No__t-0 对象公开 OpenType 字体支持的功能集。 通过在标记中设置 @no__t 的属性，可以轻松地创作利用 OpenType 功能的文档。  
