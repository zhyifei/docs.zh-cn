...
## <a name="xaml-syntax-in-brief"></a>XAML 语法简述  
...
### <a name="property-element-syntax"></a>属性元素语法  
 ...
 属性元素开始标记的语法为 `<`*typeName*`.`*propertyName*`>`。 通常, 该标记的内容是该属性作为其值的类型的对象元素。 指定内容之后, 必须用结束标记关闭属性元素。 结束标记的语法为 `</`*typeName*`.`*propertyName*`>`。  
...
### <a name="collection-syntax"></a>集合语法  
 ...
 下面的示例演示了用于设置<xref:System.Windows.Media.GradientBrush.GradientStops%2A>属性值的集合语法:  
...
### <a name="xaml-content-properties"></a>XAML 内容属性  
 ...
 例如, <xref:System.Windows.Controls.Border>指定的<xref:System.Windows.Controls.Decorator.Child%2A>内容属性。 以下两个<xref:System.Windows.Controls.Border>元素的处理方式相同。 第一个元素利用内容属性语法并省略 `Border.Child` 属性元素。 第二个元素显式显示 `Border.Child`。  
...
- 类必须声明内容属性, 并且该内容属性必须为可分配给字符串的类型 (类型可以为<xref:System.Object>)。 <xref:System.Windows.Controls.ContentControl>例如, 任何使用<xref:System.Windows.Controls.ContentControl.Content%2A>作为其内容属性并且类型<xref:System.Object>为, 并且这在实际<xref:System.Windows.Controls.ContentControl>情况<xref:System.Windows.Controls.Button>下支持以下用法: `<Button>Hello</Button>`。  
...
```  
 ...
 此处, 每<xref:System.Windows.Controls.Button>个都是的<xref:System.Windows.Controls.StackPanel>子元素。 这是一个简单直观的标记，此标记由于两个不同的原因省略了两个标记。  
  
- **省略的 system.windows.controls.stackpanel> 属性元素:** <xref:System.Windows.Controls.StackPanel>派生自<xref:System.Windows.Controls.Panel>。 <xref:System.Windows.Controls.Panel>将<xref:System.Windows.Controls.Panel.Children%2A?displayProperty=nameWithType>定义为其 XAML 内容属性。  
  
- **省略的 System.windows.controls.uielementcollection> 对象元素:** 属性采用<xref:System.Windows.Controls.UIElementCollection>实现<xref:System.Collections.IList>的类型。 <xref:System.Windows.Controls.Panel.Children%2A?displayProperty=nameWithType> 根据处理集合 (如) <xref:System.Collections.IList>的 XAML 规则, 可以省略集合的元素标记。 (在这种情况<xref:System.Windows.Controls.UIElementCollection>下, 实际上不能进行实例化, 因为它不公开无参数的构造函数<xref:System.Windows.Controls.UIElementCollection> , 这就是该对象元素被注释掉的原因)。  
...
## <a name="case-and-white-space-in-xaml"></a>XAML 中的大小写和空白  
 ...
 WPF XAML 处理器和序列化程序将忽略或删除所有 nonsignificant 空格, 并将规范化任何有效空白。 这与 XAML 规范的默认空白行为建议一致。 通常，只有在 XAML 内容属性中指定字符串时，此行为的重要性才会体现出来。 简言之，XAML 将空格、换行符和制表符转化为空格，如果它们出现在一个连续字符串的任一端，则保留一个空格。 本主题未介绍 XAML 空白处理的完整说明。 有关详细信息, 请参阅[XAML 中的空白处理](../../xaml-services/whitespace-processing-in-xaml.md)。  
...
## <a name="markup-extensions"></a>标记扩展  
 ...
 例如, 以下标记使用特性语法设置<xref:System.Windows.FrameworkElement.Style%2A>属性的值。 属性采用类的实例, 该<xref:System.Windows.Style>实例在默认情况下无法由特性语法字符串实例化。 <xref:System.Windows.FrameworkElement.Style%2A> 但在本例中，特性引用了特定的标记扩展 [StaticResource](staticresource-markup-extension.md)。 处理该标记扩展时，将返回对以前在资源字典中作为键控资源进行实例化的某个样式的引用。  
  
 [!code-xaml[FEResourceSH_snip#XAMLOvwShortResources](~/samples/snippets/csharp/VS_Snippets_Wpf/FEResourceSH_snip/CS/page1.xaml#xamlovwshortresources)]  
[!code-xaml[FEResourceSH_snip#XAMLOvwShortResources2](~/samples/snippets/csharp/VS_Snippets_Wpf/FEResourceSH_snip/CS/page1.xaml#xamlovwshortresources2)]  
[!code-xaml[FEResourceSH_snip#XAMLOvwShortResources3](~/samples/snippets/csharp/VS_Snippets_Wpf/FEResourceSH_snip/CS/page1.xaml#xamlovwshortresources3)]  
  
 有关特定在 WPF 中实现的所有 XAML 标记扩展的参考列表，请参阅 [WPF XAML 扩展](wpf-xaml-extensions.md)。 有关由 system.exception 定义的标记扩展的引用列表, 并且更广泛地用于 .NET Framework xaml 实现, 请参阅[xaml 命名空间 (x:)语言功能](../../xaml-services/xaml-namespace-x-language-features.md)。 有关标记扩展概念的详细信息，请参阅[标记扩展和 WPF XAML](markup-extensions-and-wpf-xaml.md)。  
  
<a name="type_converters"></a>   
## <a name="type-converters"></a>类型转换器  
 在 [XAML 语法概述](xaml-overview-wpf.md#xaml_syntax_in_brief)一节中，曾提到特性值必须能够通过字符串进行设置。 对于某些类型<xref:System.String> <xref:System.DateTime> (如或<xref:System.Uri>), 对字符串转换为其他对象类型或基元值的方式的基本、本机处理。 但是很多 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 类型或这些类型的成员扩展了基本字符串特性处理行为，因此可以将更复杂的对象类型的实例指定为字符串和特性。  
  
 <xref:System.Windows.Thickness>结构是具有为 XAML 用法启用类型转换的类型的示例。 <xref:System.Windows.Thickness>指示嵌套矩形中的度量值, 并将其用作属性 (如) <xref:System.Windows.FrameworkElement.Margin%2A>的值。 通过将类型转换器置于上<xref:System.Windows.Thickness>, 使用的<xref:System.Windows.Thickness>所有属性都可以更轻松地在 XAML 中指定, 因为可以将这些属性指定为属性。 下面的示例使用类型转换和特性语法为提供值<xref:System.Windows.FrameworkElement.Margin%2A>:  
  
 [!code-xaml[XAMLOvwSupport#MarginTCE](~/samples/snippets/csharp/VS_Snippets_Wpf/XAMLOvwSupport/CSharp/page7.xaml#margintce)]  
  
 前面的特性语法示例等效于下面的更详细的语法示例, 其中的<xref:System.Windows.FrameworkElement.Margin%2A>是通过<xref:System.Windows.Thickness>包含对象元素的属性元素语法设置的。 的四个键属性<xref:System.Windows.Thickness>设置为新实例的属性:  
...
## <a name="xaml-root-elements-and-xaml-namespaces"></a>XAML 根元素和 XAML 命名空间  
 一个 XAML 文件只能有一个根元素，这样才能同时作为格式正确的 [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] 文件和有效的 XAML 文件。 对于典型的 wpf 方案, 使用在 WPF 应用程序模型中具有明显意义的根<xref:System.Windows.Window>元素 (例如, <xref:System.Windows.Controls.Page>对于页面, <xref:System.Windows.ResourceDictionary>为, 对于外部字典<xref:System.Windows.Application>为, 对于应用程序定义)。 下面的示例演示了[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]页面的典型 XAML 文件的根元素, 其中包含的根<xref:System.Windows.Controls.Page>元素。  
...
- [x:Key](../../xaml-services/x-key-directive.md):为中的<xref:System.Windows.ResourceDictionary>每个资源设置唯一键 (或其他框架中的类似字典概念)。 在典型的 WPF 应用程序标记中的所有 `x:` 用法中，`x:Key` 可能占到 90%。  
...
- [x:Type](../../xaml-services/x-type-markup-extension.md):基于类型<xref:System.Type>名称构造一个引用。 <xref:System.Type>这用于指定采用的特性 ( <xref:System.Windows.Style.TargetType%2A?displayProperty=nameWithType>例如), 尽管通常属性具有<xref:System.Type>本机字符串到转换的方式, 而[x:Type](../../xaml-services/x-type-markup-extension.md)标记扩展用法是可选的。  
  
           `x:` 前缀/XAML 命名空间中还有其他一些不太常见的编程构造。 有关详细信息, [请参阅 XAML 命名空间 (x:)语言功能](../../xaml-services/xaml-namespace-x-language-features.md)。  
...
## <a name="events-and-xaml-code-behind"></a>事件和 XAML 代码隐藏  
 大多数 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 应用程序既包括 XAML 标记，也包括代码隐藏。 在项目中, XAML 以`.xaml`文件形式编写, CLR 语言 (例如 Microsoft Visual Basic 或C# ) 用于编写代码隐藏文件。 在 WPF 编程和应用程序模型中对 XAML 文件进行标记编译时，XAML 文件的 XAML 代码隐藏文件的位置是通过如下方式来标识的：以 XAML 根元素的 `x:Class` 特性形式指定一个命名空间和类。  
...
### <a name="routed-events"></a>路由事件  
 路由事件是一个特殊的事件功能，该功能是 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 的基础。 路由事件使一个元素可以处理另一个元素引发的事件，前提是这些元素通过树关系连接在一起。 使用 XAML 特性指定事件处理时，可以对任何元素（包括未在类成员表中列出该特定事件的元素）侦听和处理该路由事件。 这是通过以所属类名限定事件名特性来实现的。 `StackPanel`例如, 正在进行`StackPanel`  /  `StackPanel`的示例中的父项可以通过指定上的特性`Button.Click`来注册子<xref:System.Windows.Controls.Primitives.ButtonBase.Click>元素按钮的事件的处理程序。 `Button`对象元素, 将处理程序名称作为属性值。 若要深入了解路由事件的工作原理，请参阅[路由事件概述](routed-events-overview.md)。  
  
<a name="x_name_and_xaml_named_elements"></a>   
## <a name="xaml-named-elements"></a>XAML 命名元素  
 默认情况下，通过处理 XAML 对象元素在对象图中创建的对象实例没有唯一标识符或对象引用。 相反，如果在代码中调用构造函数，则几乎总是使用构造函数结果为构造的实例设置变量，以便以后在代码中引用该实例。 为了对通过标记定义创建的对象提供标准化访问，XAML 定义了 [x:Name 特性](../../xaml-services/x-name-directive.md)。 可以在任何对象元素上设置 `x:Name` 特性的值。 在代码隐藏中，所选标识符等效于引用所构造的实例的实例变量。 在所有方面，命名元素以类似于对象实例的方式工作（名称引用实例），并且代码隐藏可以通过引用命名元素来处理应用程序内的运行时交互。 实例和变量之间的此连接由 WPF XAML 标记编译器完成, 更具体地涉及功能和模式, 如<xref:System.Windows.Markup.IComponentConnector.InitializeComponent%2A>本主题中所述的。  
  
 WPF 框架级别 XAML 元素继承<xref:System.Windows.FrameworkElement.Name%2A>属性, 该属性等效于 XAML 定义`x:Name`的属性。 其他某些类也为 `x:Name`（通常也定义为 `Name` 属性）提供属性级等效项。 一般而言，如果在所选元素/类型的成员表中找不到 `Name` 属性，则可以改用 `x:Name`。 这些`x:Name`值将为 XAML 元素提供一个标识符, 该标识符可在运行时用于特定子系统或通过实用工具方法<xref:System.Windows.FrameworkElement.FindName%2A>(如)。  
  
 下面的示例在<xref:System.Windows.FrameworkElement.Name%2A>一个<xref:System.Windows.Controls.StackPanel>元素上设置。 然后, <xref:System.Windows.Controls.Button>中的一个处理程序, <xref:System.Windows.Controls.StackPanel>它<xref:System.Windows.Controls.StackPanel>通过设置<xref:System.Windows.FrameworkElement.Name%2A>的通过其`buttonContainer`实例引用来引用。  
...
[!code-xaml[XAMLOvwSupport#NamedE2](~/samples/snippets/csharp/VS_Snippets_Wpf/XAMLOvwSupport/CSharp/page7.xaml#namede2)]  
 ...
 和变量一样，实例的 XAML 名称受范围概念约束，因此可以在可预测的某个范围内强制名称唯一。 定义页面的主标记表示一个唯一的 XAML 名称范围，而该 XAML 名称范围的边界是该页面的根元素。 但是，其他标记源（如样式或样式中的模板）可以在运行时与页面交互，这种标记源常常具有自己的 XAML 名称范围，这些名称范围不一定与页面的 XAML 名称范围相关联。 有关`x:Name`和 XAML 名称范围的详细信息, <xref:System.Windows.FrameworkElement.Name%2A>请参阅、 [x:Name 指令](../../xaml-services/x-name-directive.md)或[WPF XAML 名称范围](wpf-xaml-namescopes.md)。  
...
## <a name="attached-properties-and-attached-events"></a>附加属性和附加事件  
 ...
 下面的示例演示<xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType>了附加属性。 <xref:System.Windows.Controls.DockPanel>类为<xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType>定义访问器, 因此拥有附加属性。 类还包括迭代其子元素并专门检查每个元素的设置值的<xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType>逻辑。 <xref:System.Windows.Controls.DockPanel> 如果找到一个值，将在布局过程中使用该值定位子元素。 使用附加属性和此定位功能实际上是<xref:System.Windows.Controls.DockPanel>类的激发方案。 <xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType>  
...
## <a name="base-types-and-xaml"></a>基类型和 XAML  
 除了 XAML 的标记元素外, 基础 WPF XAML 及其 XAML 命名空间还与 CLR 对象相对应。 但是，并不是所有的类都能映射到元素。 在 CLR 对象模型中<xref:System.Windows.Controls.Primitives.ButtonBase>, 抽象类 (如) 和某些非抽象基类用于继承。 基类（包括抽象类）对于 XAML 开发仍然很重要，因为每个具体的 XAML 元素都从其层次结构中的某个基类继承成员。 通常，这些成员包括可以设置为元素特性的属性或者可以处理的事件。 <xref:System.Windows.FrameworkElement>是 WPF 框架级别[!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)]的[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]具体基类。 设计[!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)]时, 将使用各种形状、面板、修饰器或控件类, 它们都派生自<xref:System.Windows.FrameworkElement>。 相关基类<xref:System.Windows.FrameworkContentElement>支持面向文档的元素, 这些元素可适用于流布局演示, 使用在中<xref:System.Windows.FrameworkElement>特意镜像 api 的 api。 元素级和 CLR 对象模型的属性组合提供了一组可在大多数具体 XAML 元素上设置的公共属性, 而不考虑特定的 XAML 元素及其基础类型。  
