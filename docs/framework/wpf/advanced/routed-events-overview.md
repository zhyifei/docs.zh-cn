...
## <a name="what-is-a-routed-event"></a>什么是路由事件？  
 ...
 实现定义： 路由的事件是[!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)]由的实例支持的事件<xref:System.Windows.RoutedEvent>类，并由处理[!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)]事件系统。  
 ...
 ![“是”、“否”和“取消”按钮] (../../../../docs/framework/wpf/advanced/media/routedevent-ovw-1.gif "RoutedEvent_ovw_1")  
  
 在此简化的元素树中，源<xref:System.Windows.Controls.Primitives.ButtonBase.Click>事件是之一<xref:System.Windows.Controls.Button>元素，而所<xref:System.Windows.Controls.Button>被单击有机会处理该事件的第一个元素。 但是，如果没有处理程序附加到<xref:System.Windows.Controls.Button>作用于事件，则该事件将向上冒泡到<xref:System.Windows.Controls.Button>在元素树中，这是父<xref:System.Windows.Controls.StackPanel>。 可能的事件将冒泡到<xref:System.Windows.Controls.Border>，然后会到达到页树的根的元素 （未显示）。  
  
 换而言之，此事件路由<xref:System.Windows.Controls.Primitives.ButtonBase.Click>事件是：  
...
### <a name="how-routed-events-are-implemented"></a>路由事件的实现方式  
 ...
 下面的示例演示了自定义声明`Tap`路由的事件，包括注册和公开<xref:System.Windows.RoutedEvent>标识符字段和`add`和`remove`实现`Tap` [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)]事件。  
...
## <a name="why-use-routed-events"></a>为什么使用路由事件？  
 ...
 路由事件侦听器和路由事件源不必在其层次结构中共享公用事件。 任何<xref:System.Windows.UIElement>或<xref:System.Windows.ContentElement>可以是任何路由事件的事件侦听器。 因此，可以将有效的 [!INCLUDE[TLA2#tla_api](../../../../includes/tla2sharptla-api-md.md)] 集中可用的全套路由事件用作概念“接口”，应用程序中的不同元素通过这个接口来交换事件信息。 路由事件的这个“接口”概念特别适用于输入事件。  
...
-   某些[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]样式和模板化功能，如<xref:System.Windows.EventSetter>和<xref:System.Windows.EventTrigger>需要被引用的事件路由事件。 前面提到的事件标识符方案就是这样的。  
...
[!code-vb[EventOvwSupport#SimpleHandlerA](../../../../samples/snippets/visualbasic/VS_Snippets_Wpf/EventOvwSupport/visualbasic/default.xaml.vb#simplehandlera)]  
 ...
 在用代码创建的应用程序中为路由事件添加处理程序非常简单。 路由的事件处理程序总是可以添加一个帮助器方法通过<xref:System.Windows.UIElement.AddHandler%2A>(它是现有后备调用同一方法`add`。)但是，现有的 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 路由事件通常借助于支持机制来实现 `add` 和 `remove` 逻辑，这些实现允许使用特定于语言的事件语法来添加路由事件的处理程序，特定于语言的事件语法比 Helper 方法更直观。 下面是 Helper 方法的示例用法：  
...
### <a name="the-concept-of-handled"></a>“已处理”概念  
 ...
 但是，没有凭此侦听器仍然可以运行处理程序以响应路由事件的"附加"机制其中<xref:System.Windows.RoutedEventArgs.Handled%2A>是`true`事件数据中。 换言之，将事件数据标记为“已处理”并不会真的停止事件路由。 你仅可以使用的附加机制，在代码中，或在<xref:System.Windows.EventSetter>:  
  
-   在代码中，而不是适用的语言特定事件语法用于常规[!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)]事件，调用[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]方法<xref:System.Windows.UIElement.AddHandler%28System.Windows.RoutedEvent%2CSystem.Delegate%2CSystem.Boolean%29>添加您的处理程序。 请将 `handledEventsToo` 的值指定为 `true`。  
  
-   在<xref:System.Windows.EventSetter>，将其设置<xref:System.Windows.EventSetter.HandledEventsToo%2A>要属性`true`。  
  
 除了行为，<xref:System.Windows.RoutedEventArgs.Handled%2A>路由事件，这一概念中的状态生成<xref:System.Windows.RoutedEventArgs.Handled%2A>已将应用程序设计和编写事件处理程序代码的方式的影响。 您可以形成概念<xref:System.Windows.RoutedEventArgs.Handled%2A>视为一种简单的协议，公开的路由事件。 完全如何使用此协议负责，但是了解的概念设计的值<xref:System.Windows.RoutedEventArgs.Handled%2A>旨在一起使用，如下所示是：  
  
-   如果路由事件标记为“已处理”，则它不必由该路由中的其他元素再次处理。  
  
-   如果路由的事件不会标记为已处理，则在路由过程中前面其他侦听器已经选择不进行注册处理程序或已注册的选择不来操作的事件数据和设置的处理程序<xref:System.Windows.RoutedEventArgs.Handled%2A>到`true`。 （或者，当前侦听器很可能是路由中的第一个点。）当前侦听器上的处理程序现在有三个可能的操作过程：  
  
    -   不执行任何操作；该事件保持未处理状态，该事件将路由到下一个侦听器。  
  
    -   执行代码以响应该事件，但是所执行的操作被视为不足以保证将事件标记为“已处理”。 该事件将路由到下一个侦听器。  
  
    -   执行代码以响应该事件。 在传递到处理程序的事件数据中将该事件标记为“已处理”，因为所执行的操作被视为不足以保证将该事件标记为“已处理”。 事件仍将路由到下一步的侦听器，但与<xref:System.Windows.RoutedEventArgs.Handled%2A> = `true`中其事件数据，因此仅`handledEventsToo`侦听器能够进一步调用处理程序。  
  
 此概念设计强化由前面所述的路由行为： 它是难度 （尽管仍可以在代码或样式） 将附加即使在路由过程前面的处理程序已设置调用的路由事件的处理程序<xref:System.Windows.RoutedEventArgs.Handled%2A>到`true`。  
  
 有关详细信息<xref:System.Windows.RoutedEventArgs.Handled%2A>、 类处理的路由事件和有关时将其建议恰当地路由将事件标记为<xref:System.Windows.RoutedEventArgs.Handled%2A>，请参阅[标记作为 Handled，和类处理的路由事件](../../../../docs/framework/wpf/advanced/marking-routed-events-as-handled-and-class-handling.md)。  
...
## <a name="class-handlers"></a>类处理程序  
 如果你以某种方式从定义派生的类<xref:System.Windows.DependencyObject>，还可以定义，然后附加你的类声明或继承的事件成员的路由事件的类处理。 每当路由事件到达其路由中的元素实例时，都会先调用类处理程序，然后再调用附加到该类某个实例的任何实例侦听器处理程序。  
  
 有些 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 控件对某些路由事件具有固有的类处理。 路由事件可能看起来从未引发过，但实际上正对其进行类处理，如果使用某些技术，路由事件还是可以由实例处理程序进行处理。 此外，许多基类和控件会公开可用来替代类处理行为的虚拟方法。 若要深入了解如何解决不需要的类处理以及如何在自定义类中定义自己的类处理，请参阅[将路由事件标记为“已处理”和类处理](../../../../docs/framework/wpf/advanced/marking-routed-events-as-handled-and-class-handling.md)。  
...
## <a name="attached-events-in-wpf"></a>WPF 中的附加事件  
 ...
 有关 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 中附加事件的详细信息，请参阅[附加事件概述](../../../../docs/framework/wpf/advanced/attached-events-overview.md)。  
...
## <a name="wpf-input-events"></a>WPF 输入事件  
 路由事件在 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 平台中的常见应用之一是用于事件输入。 在 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 中，按照约定，隧道路由事件的名称以单词“Preview”开头。 输入事件通常成对出现，一个是浮升事件，另一个是隧道事件。 例如，<xref:System.Windows.ContentElement.KeyDown>事件和<xref:System.Windows.ContentElement.PreviewKeyDown>事件具有相同的签名，前者是冒泡输入的事件并且后者是隧道输入事件。 偶尔，输入事件只有浮升版本，或者有可能只有直接路由版本。 在文档中，路由事件主题交叉引用具有备用路由策略的类似路由事件（如果存在这类路由事件），托管的引用页面中的相关部分阐明每个路由事件的路由策略。  
  
 实现成对出现的 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 输入事件，使来自输入的单个用户操作（如按鼠标按钮）按顺序引发该对中的两个路由事件。 首先引发隧道事件并沿路由传播。 然后引发浮升事件并沿路由传播。 两个事件按原义共享相同的事件数据实例，因为<xref:System.Windows.UIElement.RaiseEvent%2A>引发冒泡事件的实现类中的方法调用侦听从隧道事件的事件数据，并将其重新在新引发的事件。 具有隧道事件处理程序的侦听器首先获得将路由事件标记为“已处理”的机会（首先是类处理程序，然后是实例处理程序）。 如果隧道路由中的某个元素将路由事件标记为“已处理”，则会针对浮升事件发送已处理的事件数据，而且将不调用等效的浮升输入事件的附加典型处理程序。 已处理的浮升事件看起来好像尚未引发。 此处理行为对于控件合成非常有用，因为在此情况下你可能希望所有基于命中测试的输入事件或者所有基于焦点的输入事件都由最终的控件（而不是它的复合部件）报告。 作为可支持控件类的代码的一部分，最后一个控件元素靠近合成中的根，因此将有机会首先对隧道事件进行类处理，或者有机会将该路由事件“替换”为更针对控件的事件。  
...
6.  针对根元素处理 `MouseDown`（浮升）。  
 ...
 路由事件处理程序委托提供对以下两个对象的引用：引发该事件的对象以及在其中调用处理程序的对象。 在其中调用处理程序的对象是由 `sender` 参数报告的对象。 首先引发事件的对象报告的<xref:System.Windows.RoutedEventArgs.Source%2A>事件数据中的属性。 路由的事件仍可引发以及由同一个对象，在这种情况下处理`sender`和<xref:System.Windows.RoutedEventArgs.Source%2A>是相同的 （这是用例和步骤 3 和 4 在事件处理示例列表）。  
  
 由于隧道和冒泡，父元素接收输入的事件其中<xref:System.Windows.RoutedEventArgs.Source%2A>是一个及其子代元素。 时请务必知道源元素的是，你可以通过访问确定源元素<xref:System.Windows.RoutedEventArgs.Source%2A>属性。  
  
 通常情况下，输入的事件标记<xref:System.Windows.RoutedEventArgs.Handled%2A>，进一步都不调用处理程序。 通常，一旦调用了用来对输入事件的含义进行特定于应用程序的逻辑处理的处理程序，就应当将输入事件标记为“已处理”。  
  
 有关此常规语句的例外<xref:System.Windows.RoutedEventArgs.Handled%2A>状态是输入事件处理程序注册为有意忽略<xref:System.Windows.RoutedEventArgs.Handled%2A>仍将任一路由过程调用的事件数据的状态。 有关详细信息，请参阅[预览事件](../../../../docs/framework/wpf/advanced/preview-events.md)或[将路由事件标记为“已处理”和类处理](../../../../docs/framework/wpf/advanced/marking-routed-events-as-handled-and-class-handling.md)。  
 ...
 一些类选择对某些输入事件进行类处理，其目的通常是重新定义用户驱动的特定输入事件在该控件中的含义并引发新事件。 有关详细信息，请参阅[将路由事件标记为“已处理”和类处理](../../../../docs/framework/wpf/advanced/marking-routed-events-as-handled-and-class-handling.md)。  
  
 有关输入以及在典型的应用程序方案中输入和事件如何交互的详细信息，请参阅[输入概述](../../../../docs/framework/wpf/advanced/input-overview.md)。  
  
<a name="events_styles"></a>   
## <a name="eventsetters-and-eventtriggers"></a>EventSetter 和 EventTrigger  
 在样式，你可以包括一些预声明[!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)]事件处理中标记的语法通过<xref:System.Windows.EventSetter>。 在应用样式时，所引用的处理程序会添加到带样式的实例中。 你可以声明<xref:System.Windows.EventSetter>仅为的路由事件。 下面是一个示例。 请注意，此处引用的 `b1SetColor` 方法位于代码隐藏文件中。  
  
 [!code-xaml[EventOvwSupport#XAML2](../../../../samples/snippets/csharp/VS_Snippets_Wpf/EventOvwSupport/CSharp/page2.xaml#xaml2)]  
  
 此处获得的优点是样式是可能包含大量可应用于你的应用程序中的任何按钮的其他信息并具有<xref:System.Windows.EventSetter>能属于该样式将提升甚至在标记级别的代码重用。 此外，<xref:System.Windows.EventSetter>提取处理程序一步进一步即可常规应用程序和页标记的方法名称。  
  
 将的路由的事件和动画功能相结合的另一个专用的语法[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]是<xref:System.Windows.EventTrigger>。 与<xref:System.Windows.EventSetter>，仅路由的事件可能用于<xref:System.Windows.EventTrigger>。 通常情况下，<xref:System.Windows.EventTrigger>声明为属于一种样式，但<xref:System.Windows.EventTrigger>也可以声明的一部分在页级别的元素上<xref:System.Windows.FrameworkElement.Triggers%2A>集合，或在<xref:System.Windows.Controls.ControlTemplate>。 <xref:System.Windows.EventTrigger>使您能够指定<xref:System.Windows.Media.Animation.Storyboard>运行路由的事件达到其路由中的某个元素时声明<xref:System.Windows.EventTrigger>为该事件。 利用<xref:System.Windows.EventTrigger>通过仅处理事件，从而导致启动现有情节提要是<xref:System.Windows.EventTrigger>提供更好地控制情节提要和其运行时行为。 有关详细信息，请参阅[在情节提要启动之后使用事件触发器来控制情节提要](../../../../docs/framework/wpf/graphics-multimedia/how-to-use-event-triggers-to-control-a-storyboard-after-it-starts.md)。  
  
<a name="more_about"></a>   
## <a name="more-about-routed-events"></a>有关路由事件的更多信息  
 本主题主要从以下角度讨论路由事件：描述基本概念；就如何以及何时响应各种基元素和控件中已经存在的路由事件提供指南。 但是，你可以在自定义类上创建自己的路由事件以及所有必要的支持（如专用的事件数据类和委托）。 路由的事件的所有者可以是任何类，但必须由引发并由处理路由的事件<xref:System.Windows.UIElement>或<xref:System.Windows.ContentElement>派生类才能起作用。 有关自定义事件的详细信息，请参阅[创建自定义路由事件](../../../../docs/framework/wpf/advanced/how-to-create-a-custom-routed-event.md)。  
