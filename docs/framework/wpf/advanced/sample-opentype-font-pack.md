...
本主题概述了随 Windows SDK 一起分发的示例 OpenType 字体。 示例字体支持应用程序可以使用的[!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)]扩展 OpenType 功能。  

<a name="overview"></a>
## <a name="fonts-in-the-opentype-font-pack"></a>OpenType 字体包中的字体  
 Windows SDK 提供了一组示例 OpenType 字体，可用于创建[!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)]应用程序。 这些示例字体在 Ascender Corporation 的许可下提供。 这些字体仅实现 OpenType 格式定义的总功能的子集。 下表列出了示例 OpenType 字体的名称。  
...
## <a name="installing-the-fonts"></a>安装字体  
 您可以选择将示例 OpenType 字体安装到默认的 Windows 字体目录 **[WINDOWS]字体**。 使用“字体”控制面板安装字体。 这些字体在计算机上后，引用默认 Windows 字体的所有应用程序都可以访问这些字体。 可以通过双击字体文件以多种字体大小来显示具有代表性的字符集。 下面的屏幕截图显示 Lindsey 字体文件 Linds.ttf。  
