...
本演练演示如何在基于的[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] [!INCLUDE[TLA#tla_winforms](../../../../includes/tlasharptla-winforms-md.md)]混合应用程序中本地化元素。

本演练涉及以下任务：

- 正在创建[!INCLUDE[TLA#tla_winforms](../../../../includes/tlasharptla-winforms-md.md)]宿主项目。
...
第一步是创建[!INCLUDE[TLA#tla_winforms](../../../../includes/tlasharptla-winforms-md.md)]应用程序项目，并[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]添加包含要本地化的内容的元素。

### <a name="to-create-the-host-project"></a>创建宿主项目

1. 创建一个名为`LocalizingWpfInWf`的**WPF 应用程序**项目。  （**文件** > **新** **项目视觉对象C#** 或 Visual Basic经典桌面 > **WPF 应用程序**）。 >  >  > 

2. 向项目[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]添加一个<xref:System.Windows.Controls.UserControl>名`SimpleControl`为的元素。

3. <xref:System.Windows.Forms.Integration.ElementHost>使用控件`SimpleControl`将元素放置在窗体上。 有关详细信息，请参见[演练：在 Windows 窗体](walkthrough-hosting-a-3-d-wpf-composite-control-in-windows-forms.md)中承载 3-D WPF 复合控件。

## <a name="adding-localizable-content"></a>添加可本地化的内容

接下来，您将添加[!INCLUDE[TLA#tla_winforms](../../../../includes/tlasharptla-winforms-md.md)]一个 "标签" 控件[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] ，并将该元素的内容设置为可本地化的字符串。

### <a name="to-add-localizable-content"></a>添加可本地化的内容

1. 在**解决方案资源管理器**中，双击**simplecontrol.xaml**以在中[!INCLUDE[wpfdesigner_current_short](../../../../includes/wpfdesigner-current-short-md.md)]打开它。
...
5. 按 F5 生成并运行该应用程序。
     ...
     元素和标签控件都显示文本 **"Hello"。** `SimpleControl`
...
2. 在 "**属性**" 窗口中，将窗体的可**本地化**属性的`true`值设置为。
...
7. `Form1` 在`InitializeComponent`调用之前，将以下代码复制到构造函数中。
...
10. 将以下行复制到项目文件`PropertyGroup`中的第一个。
...
3. 在**解决方案资源管理器**中，双击**Simplecontrol.xaml**以在代码编辑器中打开它。 你将看到`msbuild`该命令已将`Uid`属性添加到所有元素。 这有助于通过分配资源标识符进行本地化。
...
3. 用 Visual Studio 或其他文本编辑器打开 temp .csv 文件。 将字符串`"Hello"`替换为其西班牙语`"Hola"`翻译。
