...
下面的示例演示<xref:System.Windows.Controls.Image>未能<xref:System.Windows.FlowDirection>从包含它的中<xref:System.Windows.Controls.StackPanel>继承。
...
> [!NOTE]
> 下载文件中包含一个**ms_logo**文件。 该代码假定 .jpg 文件不在项目中，而是位于 C:\ 驱动器中的某个位置。 必须将 .jpg 从项目文件复制到 C:\ 驱动器或更改代码才能在项目内查找该文件。 若要执行此`Source="file://c:/ms_logo.jpg"`更改`Source="ms_logo.jpg"`，请执行此操作。
