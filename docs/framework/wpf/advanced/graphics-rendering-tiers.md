...
- **呈现层 0** - 无图形硬件加速。 所有图形功能都使用软件加速。 DirectX 版本级别小于版本 9.0。  
  
- **呈现层 1** - 某些图形功能使用图形硬件加速。 DirectX 版本级别大于或等于版本 9.0。  
  
- **呈现层 2** - 大多数图形功能都使用图形硬件加速。 DirectX 版本级别大于或等于版本 9.0。  
  
 该<xref:System.Windows.Media.RenderCapability.Tier%2A?displayProperty=nameWithType>属性允许您在应用程序运行时检索呈现层。 使用呈现层可确定设备是否支持某些硬件加速图形功能。 然后，应用程序就可以在运行时根据设备支持的呈现层采用不同的代码路径。  
...
> 从 .NET 框架 4 开始，渲染层 1 已重新定义为仅包括支持 DirectX 9.0 或更高级别的图形硬件。 支持 DirectX 7 或 8 的图形硬件现在定义为呈现层 0。  
 ...
 呈现层的值为 1 或 2 意味着，如果必要的系统资源可用并且尚未耗尽，则 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 的大部分图形功能会使用硬件加速。 这对应于大于或等于 9.0 的 DirectX 版本。  
  
 下表显示呈现层 1 和呈现层 2 的图形硬件需求差异：  
  
|Feature|第 1 层|第 2 层|  
|-------------|------------|------------|  
|直接X版本|必须高于或等于 9.0。|必须高于或等于 9.0。|  
...
|Feature|说明|  
|-------------|-----------|  
|3D 抗锯齿|3D 防锯齿仅在支持 Windows 显示驱动程序模型 （WDDM） 的操作系统上支持，例如 Windows Vista 和 Windows 7。|  
...
|使用栅格化内容<xref:System.Windows.Media.Imaging.RenderTargetBitmap>|使用<xref:System.Windows.Media.Imaging.RenderTargetBitmap.Render%2A>的方法呈现的任何内容<xref:System.Windows.Media.Imaging.RenderTargetBitmap>。|  
|使用的平铺内容<xref:System.Windows.Media.TileBrush>|将 的属性<xref:System.Windows.Media.TileBrush.TileMode%2A><xref:System.Windows.Media.TileBrush>设置为 的任何平铺内容<xref:System.Windows.Media.TileMode.Tile>。|  
...
|分层窗口|分层窗口允许 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 应用程序将内容呈现到非矩形窗口中的屏幕。 在支持 Windows 显示驱动程序模型 （WDDM） 的操作系统上，如 Windows Vista 和 Windows 7，分层窗口的硬件加速。 在其他系统上（如 Windows XP）上，分层窗口由软件呈现，没有硬件加速。<br /><br /> 您可以通过设置以下<xref:System.Windows.Window>属性来启用分层[!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]窗口：<br /><br /> -   <xref:System.Windows.Window.WindowStyle%2A> = <xref:System.Windows.WindowStyle.None><br />-   <xref:System.Windows.Window.AllowsTransparency%2A> = `true`<br />-   <xref:System.Windows.Controls.Control.Background%2A> = <xref:System.Windows.Media.Brushes.Transparent%2A>|  
...
### <a name="directx-diagnostic-tool"></a>DirectX 诊断工具  
 DirectX 诊断工具 Dxdiag.exe 旨在帮助您解决与 DirectX 相关的问题。 DirectX 诊断工具的默认安装文件夹是：  
  
 `~\Windows\System32`  
  
 运行 DirectX 诊断工具时，主窗口包含一组选项卡，允许您显示和诊断与 DirectX 相关的信息。 例如，"**系统**"选项卡提供有关您的计算机的系统信息，并指定计算机上安装的 DirectX 版本。  
