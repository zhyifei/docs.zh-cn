...
- **发现**:查找符合主机应用程序支持的协定的外接程序。

- **激活**:加载、运行和建立与外接程序的通信。

- **隔离**:使用应用程序域或进程来建立隔离边界, 以保护应用程序不受加载项的潜在安全性和执行问题的限制。

- **通信**:通过调用方法和传递数据, 允许外接程序和宿主应用程序跨隔离边界彼此进行通信。
...
WPF UI 类型不能远程处理。 为了解决此问题, WPF 扩展了 .NET Framework 外接程序模型, 以允许从主机应用程序显示外接程序创建的 WPF UI。 此支持由 WPF 通过两<xref:System.AddIn.Contract.INativeHandleContract>种类型提供: <xref:System.AddIn.Pipeline.FrameworkElementAdapters>由类实现的接口和两个静态方法: <xref:System.AddIn.Pipeline.FrameworkElementAdapters.ContractToViewAdapter%2A>和<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A>。 从较高层面来看，这些类型和方法按以下方式使用：

1. WPF 要求外接程序提供的用户界面是直接或间接从<xref:System.Windows.FrameworkElement>派生的类, 如形状、控件、用户控件、布局面板和页面。

2. 无论协定声明在外接程序和宿主应用程序之间传递 UI, 都必须将其声明为<xref:System.AddIn.Contract.INativeHandleContract> ( <xref:System.Windows.FrameworkElement>而非)。<xref:System.AddIn.Contract.INativeHandleContract>是可跨隔离边界传递的外接程序 UI 的可远程表示形式。

3. 在从外接程序的应用程序域中传递<xref:System.Windows.FrameworkElement>时, <xref:System.AddIn.Contract.INativeHandleContract>通过调用<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A>将作为打包成。

4. 向宿主应用程序的应用程序域传递后, <xref:System.AddIn.Contract.INativeHandleContract>必须<xref:System.Windows.FrameworkElement>通过调用<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ContractToViewAdapter%2A>将重新打包为。

如何<xref:System.AddIn.Contract.INativeHandleContract>使用<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ContractToViewAdapter%2A>、、 <xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A>和取决于具体的方案。 下面几节介绍每个编程模型的详细信息。
...
4. 必须<xref:System.Windows.FrameworkElement> <xref:System.AddIn.Contract.INativeHandleContract>先将外接程序返回的 UI 从转换为, 然后才能跨越隔离边界。

5. 在跨越隔离边界之后, 必须将返回的<xref:System.AddIn.Contract.INativeHandleContract> UI 从转换为。 <xref:System.Windows.FrameworkElement>

6. 宿主应用程序将显示返回<xref:System.Windows.FrameworkElement>的。
...
4. 在越过隔离边界<xref:System.Windows.FrameworkElement> <xref:System.AddIn.Contract.INativeHandleContract>之前, 外接程序必须从转换为。

5. 在越过隔离边界<xref:System.AddIn.Contract.INativeHandleContract> <xref:System.Windows.FrameworkElement>后, 外接程序必须从转换为。
...
- 必须对[!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)]应用程序清单进行专门配置, 以将管道 (文件夹和程序集) 和外接程序程序集下载到客户端计算机上的 ClickOnce 应用程序缓存中, 这与位于相同的文件夹中。 [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)]

- 用于发现和加载外接程序的[!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] 代码必须使用作为管道和外接程序位置的ClickOnce应用程序缓存。[!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)]
...
最后一步是配置应用程序清单，以包含要下载的管道程序集文件和外接程序程序集文件。 文件应位于[!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)]应用程序所占用的 ClickOnce 缓存中文件夹根目录的文件夹中。 通过执行以下操作，可以在 [!INCLUDE[TLA2#tla_visualstu](../../../../includes/tla2sharptla-visualstu-md.md)] 中实现该配置：
...
为确保外接程序可以引用源站点的文件，必须使用等效于主机应用程序的安全隔离来加载外接程序。 此安全级别由<xref:System.AddIn.Hosting.AddInSecurityLevel.Host?displayProperty=nameWithType>枚举值标识, 并在激活外接程序时传递<xref:System.AddIn.Hosting.AddInToken.Activate%2A>给方法。

<a name="WPFAddInModelArchitecture"></a>

## <a name="wpf-add-in-architecture"></a>WPF 外接程序体系结构

正如我们所看到的, 在最高级别, WPF 允许 .NET Framework <xref:System.Windows.FrameworkElement>外接程序使用<xref:System.AddIn.Contract.INativeHandleContract>、 <xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A>和<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ContractToViewAdapter%2A>实现用户界面 (直接或间接从派生)。 结果是, 主机应用程序返回了一个<xref:System.Windows.FrameworkElement> , 它从主机应用程序的 UI 中显示。
...
- 在外接程序端, WPF 会为将由主机应用程序显示的 UI 获取一个窗口句柄。 窗口句柄由派生自<xref:System.Windows.Interop.HwndSource>并实现<xref:System.AddIn.Contract.INativeHandleContract>的内部 WPF 类封装。 此类的实例由<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A>返回, 并从外接程序的应用程序域封送到主机应用程序的应用程序域。
...
<xref:System.AddIn.Contract.INativeHandleContract>摘要、 <xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A>、和<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ContractToViewAdapter%2A>存在, 以允许将 WPF UI 的窗口句柄从外接<xref:System.Windows.Interop.HwndHost>程序传递到主机应用程序, 在该应用程序中, 它由进行封装并显示宿主应用程序的 UI。

> [!NOTE]
> 由于宿主应用程序获取<xref:System.Windows.Interop.HwndHost>, 因此主机应用程序无法将返回的对象转换为由<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ContractToViewAdapter%2A>外接程序实现的类型<xref:System.Windows.Controls.UserControl>(例如)。

本质上, <xref:System.Windows.Interop.HwndHost>具有某些限制, 它们会影响主机应用程序使用它们的方式。 不过, WPF 扩展<xref:System.Windows.Interop.HwndHost>了多个外接程序方案的功能。 下面介绍这些优点和限制。

<a name="WPFAddInModelBenefits"></a>

## <a name="wpf-add-in-benefits"></a>WPF 外接程序的优点

因为 wpf 外接程序用户界面是使用派生自<xref:System.Windows.Interop.HwndHost>的内部类从主机应用程序显示的, 所以这些用户界面受<xref:System.Windows.Interop.HwndHost>与 WPF UI 服务 (如布局) 有关的功能的限制。呈现、数据绑定、样式、模板和资源。 不过, WPF 通过其他功能<xref:System.Windows.Interop.HwndHost>增强了其内部子类, 其中包括:

- 在宿主应用程序的 UI 和外接程序的 UI 之间进行 tab 键切换。 请注意, "外接程序是 UI" 编程模型要求外接程序端适配器进行重写<xref:System.AddIn.Pipeline.ContractBase.QueryContract%2A> , 以启用 tab 键切换, 无论外接程序是完全信任的还是部分信任的。
...
- 当外接程序以安全隔离 (即部分信任的安全沙盒) 运行时, 防止非法访问外接程序 UI 窗口句柄。 调用<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A>可确保安全:
  ...
  - 对于 "外接程序返回 UI" 编程模型, 传递跨隔离边界的外接程序 UI 的窗口句柄的唯一方法是调用<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A>。

  - 对于 "外接程序是 UI" 编程模型, 需要在外<xref:System.AddIn.Pipeline.ContractBase.QueryContract%2A>接程序端适配器上重写并调用<xref:System.AddIn.Pipeline.FrameworkElementAdapters.ViewToContractAdapter%2A> (如前面的示例所示), 就像调用外接程序端适配器的`QueryContract`实现时,主机端适配器。

- 提供多个应用程序域执行保护。 由于应用程序域的限制，因此即使存在隔离边界，外接程序应用程序域中引发的未经处理的异常也会导致整个应用程序出现故障。 不过, WPF 和 .NET Framework 外接程序模型提供一种简单的方法来解决此问题并提高应用程序的稳定性。 如果主机应用程序是 wpf 应用程序, 则显示<xref:System.Windows.Threading.Dispatcher> UI 的 WPF 外接程序将为运行应用程序域的线程创建。 通过处理<xref:System.Windows.Threading.Dispatcher.UnhandledException> WPF 外接程序的<xref:System.Windows.Threading.Dispatcher>事件, 可以检测应用程序域中发生的所有未经处理的异常。 <xref:System.Windows.Threading.Dispatcher> 可以<xref:System.Windows.Threading.Dispatcher.CurrentDispatcher%2A>从属性获取。
...
- 外接程序的 UI 不能使用<xref:System.Windows.Media.VisualBrush>呈现。 相反, 外接程序可能会拍摄生成的 UI 的快照, 以创建一个可以使用协定所定义的方法传递到主机应用程序的位图。

- <xref:System.Windows.Controls.MediaElement>在外接程序 UI 中, 无法从中播放媒体文件。

- 为外接程序 ui 生成的鼠标事件既不是由宿主应用程序接收也不会引发`IsMouseOver` , 主机应用程序 ui 的属性的`false`值为。

- 当焦点在外接程序 UI 中的控件之间移动时`GotFocus` , `LostFocus`和事件既不会接收, 也不会由主机应用程序引发。

- 在打印时, 包含外接程序 UI 的主机应用程序部分显示为白色。

- 如果主机应用程序<xref:System.Windows.Threading.Dispatcher>继续执行, 则在卸载 owner 外接程序之前, 必须手动关闭外接程序 UI 创建的所有调度程序 (请参见)。 协定可以实现一些方法, 这些方法允许主机应用程序在外接程序卸载之前通知外接程序, 从而允许外接程序 UI 关闭其调度程序。

- 如果外接程序 UI 是<xref:System.Windows.Controls.InkCanvas>或<xref:System.Windows.Controls.InkCanvas>包含, 则不能卸载外接程序。

<a name="PerformanceOptimization"></a>

## <a name="performance-optimization"></a>性能优化

默认情况下, 当使用多个应用程序域时, 每个应用程序所需的各种 .NET Framework 程序集都将加载到该应用程序的域中。 因此，创建新应用程序域和在应用程序域中启动应用程序所需的时间可能会影响性能。 不过, 通过指示应用程序在已加载的应用程序域之间共享程序集, .NET Framework 提供了一种减少开始时间的方法。 为<xref:System.LoaderOptimizationAttribute>此, 需要使用属性, 该属性必须应用于入口点方法 (`Main`)。 这种情况下，只能使用代码来实现应用程序定义（请参阅[应用程序管理概述](application-management-overview.md)）。
