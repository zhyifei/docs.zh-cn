...
|`pwzImageVersion`|[out] 可选。 当`GetRequestedRuntime`返回时，包含 CLR 版本对应于[ICLRRuntimeInfo](../../../../docs/framework/unmanaged-api/hosting/iclrruntimeinfo-interface.md)返回的接口。|  
...
|`pdwConfigFlags`|[out] 可选。 如果`GetRequestedRuntime`绑定过程中，使用配置文件，当它返回，`pdwConfigFlags`包含[METAHOST_CONFIG_FLAGS](../../../../docs/framework/unmanaged-api/hosting/metahost-config-flags-enumeration.md)值，该值指示是否[\<启动 >](../../../../docs/framework/configure-apps/file-schema/startup/startup-element.md)元素具有`useLegacyV2RuntimeActivationPolicy`属性集以及属性的值。 应用[METAHOST_CONFIG_FLAGS_LEGACY_V2_ACTIVATION_POLICY_MASK](../../../../docs/framework/unmanaged-api/hosting/metahost-config-flags-enumeration.md)屏蔽到`pdwConfigFlags`若要获取的值与相关`useLegacyV2RuntimeActivationPolicy`。|  
...
|`ppRuntime`|[out]当`GetRequestedRuntime`返回时，包含指向对应[ICLRRuntimeInfo](../../../../docs/framework/unmanaged-api/hosting/iclrruntimeinfo-interface.md)接口。|  
...
|ERROR_INSUFFICIENT_BUFFER|分配给 `pwzVerison` 的内存不足。<br /><br /> -或-<br /><br /> 分配给 `pwzImageVerison` 的内存不足。|  
