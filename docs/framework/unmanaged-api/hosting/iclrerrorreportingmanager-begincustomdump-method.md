...
## <a name="parameters"></a>参数  
 ...
 `dwNumItems`  
 中`items` 数组的长度。 如果 `dwFlavor` 不是 DUMP_FLAVOR_Mini，则 `dwNumItems` 应为零。  
  
 `items`  
 中[CustomDumpItem](../../../../docs/framework/unmanaged-api/hosting/customdumpitem-structure.md)实例的数组，指定要添加到小型转储中的项。 如果 `dwFlavor` 不是 DUMP_FLAVOR_Mini，`items` 应为 null。  
