...
## <a name="parameters"></a>参数  
 `dwMilliseconds`  
 [in]如果返回前, 等待的毫秒数当前`IHostManualEvent`不属于实例。  
  
 `option`  
 [in]之一[WAIT_OPTION](../../../../docs/framework/unmanaged-api/hosting/wait-option-enumeration.md)值，如果此主机应采取的操作，该值指示操作阻止。  
...
|HOST_E_DEADLOCK|宿主的等待时间间隔，期间检测到死锁，并选择当前`IHostManualEvent`实例作为死锁牺牲品。|  
