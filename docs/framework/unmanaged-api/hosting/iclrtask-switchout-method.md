...
通知公共语言运行时（CLR）当前[ICLRTask](../../../../docs/framework/unmanaged-api/hosting/iclrtask-interface.md)实例表示的任务不再处于可运行状态。  
...
## <a name="remarks"></a>备注  
 主机调用 `SwitchOut`，通知 CLR 它已暂时停止执行当前 `ICLRTask` 实例表示的任务，并将重新计划任务。  
