...
## <a name="remarks"></a>备注  
 仅在完整的、阻止的垃圾回收后更新统计信息。即，包含所有代并在收集过程中停止应用程序的集合。 例如，<xref:System.GC.Collect?displayProperty=nameWithType> 方法重载执行完整的阻塞式收集。 并发垃圾回收在后台发生，不会阻止应用程序。  
  
 `GetCurrentSurvived` 方法是托管 <xref:System.AppDomain.MonitoringSurvivedMemorySize%2A?displayProperty=nameWithType> 属性的非托管等效项。  
