...
## <a name="parameters"></a>参数  
 `newPriority`  
 中一个整数，表示当前 `IHostTask` 实例所表示的任务所请求的线程优先级值。  
  
## <a name="return-value"></a>返回值  
  
|HRESULT|描述|  
|-------------|-----------------|  
|S_OK|`SetPriority` 成功返回。|  
...
- THREAD_PRIORITY_TIME_CRITICAL  
 ...
 当用户代码修改了 <xref:System.Threading.Thread.Priority%2A?displayProperty=nameWithType> 的值时，CLR 将调用 `SetPriority`。 宿主可以定义自己的线程优先级分配算法，并可以随意忽略此请求。  
