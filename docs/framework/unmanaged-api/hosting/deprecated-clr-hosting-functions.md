...
本部分介绍了早期版本的托管 API 使用的非托管全局静态函数。  
 ...
 除了仅供 .NET Framework 使用的基础结构函数（`_Cor*` 函数）以外，.NET Framework 4 中已弃用这些函数。  
...
## <a name="clr-version-functions"></a>CLR 版本函数  
 ...
 已否决。 获取具有指定 CLSID 的类的相应 CLR 版本信息。  
...
## <a name="hosting-functions"></a>承载函数  
 ...
 已否决。 使用指定的清单和其他应用程序数据，在指定的网络路径下启动应用程序。  
  
 [CorMarkThreadInThreadPool 函数](../../../../docs/framework/unmanaged-api/hosting/cormarkthreadinthreadpool-function.md)  
 已否决。 标记当前正在执行的线程池线程以执行托管代码。 从 .NET Framework 版本2.0 开始，此函数不起作用。 这不是必需的，并且可以从代码中删除。  
 ...
 已否决。 基于指定的版本信息创建[ICorDebug](../../../../docs/framework/unmanaged-api/debugging/icordebug-interface.md)对象。  
