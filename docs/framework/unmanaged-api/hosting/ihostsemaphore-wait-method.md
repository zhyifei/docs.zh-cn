...
导致当前[IHostSemaphore](../../../../docs/framework/unmanaged-api/hosting/ihostsemaphore-interface.md)要等待，直到它拥有实例或指定的经历的时间量。  
...
#### <a name="parameters"></a>参数  
 `dwMilliseconds`  
 [in]如果在返回之前, 等待的毫秒数当前`IHostSemaphore`实例不拥有。  
  
 `option`  
 [in]之一[WAIT_OPTION](../../../../docs/framework/unmanaged-api/hosting/wait-option-enumeration.md)值，指定如果此主机应采取什么操作操作块。  
...
|HOST_E_DEADLOCK|宿主在等待期间，检测到死锁，并选择当前`IHostSemaphore`实例作为死锁牺牲品。|  
