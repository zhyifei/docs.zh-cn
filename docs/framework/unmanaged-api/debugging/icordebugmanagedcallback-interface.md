...
|方法|描述|  
|------------|-----------------|  
|[Break 方法](../../../../docs/framework/unmanaged-api/debugging/icordebugmanagedcallback-break-method.md)|当执行代码流中<xref:System.Reflection.Emit.OpCodes.Break>的指令时, 通知调试器。|  
...
## <a name="remarks"></a>备注  
 ...
 每个回调实现都必须调用[ICorDebugController:: Continue](../../../../docs/framework/unmanaged-api/debugging/icordebugcontroller-continue-method.md)以继续执行。 如果`ICorDebugController::Continue`在回调返回之前未调用, 则该进程将保持停止状态, 并且在调用之前`ICorDebugController::Continue`不会进行更多的事件回调。  
