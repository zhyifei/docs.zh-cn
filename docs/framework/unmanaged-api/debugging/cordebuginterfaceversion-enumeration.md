...
- .NET Framework 4 引入了多个接口版本。 例如，`ICorDebugThread3` 和 `ICorDebugThread4` 显示为 `ICorDebugThread` 接口的第三个和第四个版本。 .NET Framework 4 还引入了第一版`ICorDebugStackWalk`接口和第二个版本的`ICorDebugNativeFrame`接口 (`ICorDebugNativeFrame2`)。  
