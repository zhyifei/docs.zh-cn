...
## <a name="parameters"></a>参数  
 ...
 `pLibraryProvider`  
 中一种库提供程序回调接口，允许根据需要定位和加载特定于版本的调试库。 仅当`ppProcess`或`pFlags`不`null`为时，才需要此参数。  
 ...
 `pVersion`  
 [in，out]CLR 的版本。 对于输入，此值可以为`null`。 它还可以指向[CLR_DEBUGGING_VERSION](clr-debugging-version-structure.md)结构，在这种情况下，结构的`wStructVersion`字段必须初始化为0（零）。  
  
 输出时，将用`CLR_DEBUGGING_VERSION` CLR 的版本信息填充返回的结构。  
...
|CORDBG_E_UNSUPPORTED_DEBUGGING_MODEL|此运行时版本不支持此调试模型。 目前，CLR 版本在 .NET Framework 4 之前不支持调试模型。 此`pwszVersion`错误后，output 参数仍设置为正确的值。|  
|CORDBG_E_UNSUPPORTED_FORWARD_COMPAT|CLR 的版本高于此调试器支持的版本。 此`pwszVersion`错误后，output 参数仍设置为正确的值。|  
