...
-   A[目录](#Directory)此映射中包括的所有字典包含相对虚拟地址 (RVA)。  
  
-   一个字节对齐[堆](#Heap)包含对象实例化信息。 在最后一个目录输入后立即开始。  
...
-   前 4 个字节包含字典条目的数量（也就是说，字典中的相对虚拟地址数）。 我们将此值为*N*。如果设置了高位，则按相对虚拟地址以升序对条目排序。  
  
-   *N*的目录项，请按照。 每个条目由 8 个字节（两个 4 字节段）构成：  
