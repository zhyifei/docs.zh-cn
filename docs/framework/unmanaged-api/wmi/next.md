...
`pstrName`\
弄一个新`BSTR`的，它包含属性名称。 如果名称不是必需的`null` ，则可以将此参数设置为。

`pVal`\
弄用`VARIANT`属性的值填充的。 如果值不是必需的`null` ，则可以将此参数设置为。 如果函数返回错误代码，则不会`VARIANT`修改传递`pVal`到的。

`pvtType`\
弄指向`CIMTYPE`变量的指针`LONG` （在其中放置属性的类型的。） 此属性的值可以是`VT_NULL_VARIANT`，在这种情况下，需要确定属性的实际类型。 此参数还可以为`null`。

`plFlavor`\
弄`null`或一个值，该值用于接收有关属性的源的信息。 有关可能的值，请参阅 [备注] 部分。
...
如果`plFlavor`不`LONG`为，则值接收有关属性的来源的信息，如下所示： `null`
