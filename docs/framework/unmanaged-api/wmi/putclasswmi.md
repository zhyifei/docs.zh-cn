...
| `WBEM_FLAG_UPDATE_FORCE_MODE` | 0x40 | 强制更新的类时存在冲突的子类别。 例如，此标志强制执行更新，如果类限定符定义在子类中，并尝试添加相同限定符的基类与现有冲突。 在强制模式下，通过删除将被子类中发生冲突的限定符来解决本冲突。 |
