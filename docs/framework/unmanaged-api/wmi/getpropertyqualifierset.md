...
`wszMethod`\
中属性名称。 `wszProperty`必须指向有效`LPCWSTR`的。
...
因为系统属性没有限定符，所以如果您尝试`WBEM_E_SYSTEM_PROPERTY`获取系统属性的[IWbemQualifierSet](/windows/desktop/api/wbemcli/nn-wbemcli-iwbemqualifierset)指针，该函数将返回。
