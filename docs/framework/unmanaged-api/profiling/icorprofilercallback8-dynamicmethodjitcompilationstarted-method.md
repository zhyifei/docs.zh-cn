...
---
# <a name="icorprofilercallback8dynamicmethodjitcompilationstarted-method"></a>ICorProfiler回调8：:DynamicMethodJIT编译启动方法
[在 .NET 框架 4.7 和更高版本中支持]  
  
每当开始动态方法的 JIT 编译时，通知探查器。  
...
[在]`pILHeader`指向方法 IL 标头的第一个字节的指针。

[在]`cbILHeader` IL 标头中的字节数。

## <a name="remarks"></a>备注  

每当编译动态方法时，就会触发此回调。 这包括各种 IL 存根和 LCG 方法。 其目的是向探查器编写器提供足够的信息，以便向用户标识编译的方法。

> [!NOTE]
> `functionId`值不能用于解析到其元数据令牌，因为动态方法没有元数据。

指针`pILHeader`仅在回调期间有效。
