...
```  
 ...
 允许并假设`Fancy.AddTwice`内嵌元素调用到`SimpleAdd`。 探查器可以使用此枚举器来查找模块 B 中哪些以内联方式定义的所有方法`Simple.Add`，并枚举结果会`AddTwice`。  `inlineeModuleId` 是的模块标识符`A`，并`inlineeeMethodId`是标识符`Simple.Add(int a, int b)`。  
