...
## <a name="remarks"></a>备注  
 ...
 尽管只要存在模块的 ID，就可以调用 `GetModuleInfo2` 方法，但在探查器接收到[ICorProfilerCallback：： ModuleAttachedToAssembly](icorprofilercallback-moduleattachedtoassembly-method.md)回调之前，父程序集的 ID 将不可用。  
