...
|CORPROF_E_IRREVERSIBLE_INSTRUMENTATION_PRESENT|分离无法实现，因为探查器使用了检测到的 Microsoft 中间语言（MSIL）代码，或插入了 `enter` / `leave` 挂钩。 尚未尝试分离；探查器仍处于完全附加状态。<br /><br /> **注意**已检测 MSIL 是代码，由探查器使用[SetILFunctionBody](icorprofilerinfo-setilfunctionbody-method.md)方法提供。|  
