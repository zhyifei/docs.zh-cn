...
- 自定义类必须是公共的, 并且必须公开默认 (无参数) 公共构造函数。 （有关结构注释，请参阅下节内容。）  
...
### <a name="properties"></a>属性  
 如果你使用典型的 clr `get`和`set`访问器模式以及与语言相关的 keywording 将属性定义为公共 clr 属性, 则 XAML 类型系统可以将属性报告为具有提供给的适当信息的成员。<xref:System.Xaml.XamlMember>属性,<xref:System.Xaml.XamlMember.IsReadPublic%2A>例如和<xref:System.Xaml.XamlMember.IsWritePublic%2A>。  
 ...
 在缺少文本语法或本机 XAML 转换并且没有进一步间接寻址 (如标记扩展用法) 的情况下, 属性的类型 (<xref:System.Xaml.XamlMember.TargetType%2A>在 xaml 类型系统中) 必须能够通过将 t 转换为 xaml 对象编写器来将其返回到 xaml 对象编写器作为 CLR 类型的 arget 类型。  
  
 如果使用 XAML 2009, 则在未满足前面的注意事项时, 可以使用[X:Reference 标记扩展](x-reference-markup-extension.md)来提供值;但是, 这比类型定义问题更是使用问题。  
  
### <a name="events"></a>事件  
 如果将事件定义为公共 CLR 事件, 则 XAML 类型系统可将事件报告为成员, 其<xref:System.Xaml.XamlMember.IsEvent%2A> `true`形式为。 线路事件处理程序不在 .NET Framework XAML 服务功能的范围内;这留给了特定的框架和实现。  
...
- `target` 对象在实现中可以指定为更具体的类型。 可以使用它来确定可附加成员的使用范围;预期范围外的使用情况将引发无效的强制转换异常, 然后由 XAML 分析错误引发。 参数名称`target`不是必需的, 但在大多数实现`target`中按约定命名。  
  
- 返回值在实现中可以指定为更具体的类型。  
  
 若要支持<xref:System.ComponentModel.TypeConverter>可附加成员的属性用法的已启用文本语法, <xref:System.ComponentModel.TypeConverterAttribute>请应用`Get`于*PropertyName*访问器。 应用到`get`而不是`set` nonintuitive; 但是, 此约定可支持可序列化的只读可附加成员的概念, 这在设计器方案中非常有用。  
...
- 可以在实现中将对象指定为更具体的类型,与上一节中所述的逻辑和结果相同。`target`  
  
- `value` 对象在实现中可以指定为更具体的类型。  
  
 请记住, 此方法的值是来自 XAML 使用情况的输入, 通常采用属性格式。 从属性窗体中必须存在对文本语法的值转换器支持, 并且属性`Get`为*PropertyName*访问器。  
  
### <a name="attachable-member-stores"></a>可附加成员存储  
 通常, 访问器方法不能提供将可附加成员值置于对象图中的方法, 也不能从对象图中检索值并正确地对其进行序列化。 为了提供此功能, 之前`target`的访问器签名中的对象必须能够存储值。 存储机制应该与可附加成员原则一致, 成员可附加到可附加成员不在成员列表中的目标。 .NET Framework XAML 服务通过 api <xref:System.Xaml.IAttachedPropertyStore>和<xref:System.Xaml.AttachablePropertyServices>为可附加成员存储提供一种实现技术。 <xref:System.Xaml.IAttachedPropertyStore>由 XAML 编写器用于发现存储实现, 并且应在作为访问器的类型的类型`target`上实现。 静态<xref:System.Xaml.AttachablePropertyServices> api 在访问器的主体中使用, 并通过其<xref:System.Xaml.AttachableMemberIdentifier>引用可附加成员。  
...
## <a name="access-level"></a>访问级别  
 ...
 无论何时, 用户代码定义旨在作为重构 UI <xref:System.Windows.Controls.UserControl>行为的方法的一个示例, 而不是作为可能通过使用`public`访问级别声明支持类而隐含的任何可能的扩展机制的一部分。 如果将支持代码编译到`internal`将其作为 XAML 类型引用的同一程序集中, 则可以使用access声明此类。<xref:System.Windows.Controls.UserControl>  
  
 对于在完全信任的情况下加载 XAML 并使用<xref:System.Xaml.XamlObjectWriter>的应用程序, `internal`将始终启用加载具有访问级别的类。  
  
 对于在部分信任环境下加载 XAML 的应用程序, 可以使用<xref:System.Xaml.Permissions.XamlAccessLevel> API 来控制访问级别特性。 此外, 延迟机制 (如 WPF 模板系统) 必须能够传播任何访问级别权限, 并在最终运行时评估中保留这些权限;这是通过传递<xref:System.Xaml.Permissions.XamlAccessLevel>信息在内部处理的。  
...
### <a name="wpf-implementation"></a>WPF 实现  
 ...
 在 WPF XAML 术语中,*内部类型*是由同一程序集定义的类型, 该程序集也包括引用 XAML。 可以通过 XAML 命名空间映射此类类型, 该命名空间故意省略映射的 assembly 部分, 例如`xmlns:local="clr-namespace:WPFApplication1"`。  如果 BAML 引用内部类型, 并且该类型具有`internal`访问级别, 则会为`GeneratedInternalTypeHelper`该程序集生成一个类。 如果你想要避免`GeneratedInternalTypeHelper`这种情况, 则`public`必须使用访问级别, 或者必须将相关类因式分解为单独的程序集, 并使该程序集依赖。  
