...
> [!NOTE]
>  不使用大括号 ({})，具体而言左大括号 （{），为字符串格式的一个元素。 这些字符保留作为标记扩展序列的入口和出口。  
