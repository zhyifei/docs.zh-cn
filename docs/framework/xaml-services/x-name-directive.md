...
## <a name="wpf-usage-notes"></a>WPF 用法说明  
 ...
 Wpf 使用 Microsoft Visual Basic 应用程序目标，并包括与 XAML 文件`Page`添加的编译期间创建生成操作，单独的参考属性`WithEvents`关键字具有的所有元素`x:Name`，以支持`Handles`事件处理程序委托的语法。 此属性始终是公共的。 有关详细信息，请参阅 [Visual Basic 和 WPF 事件处理](../../../docs/framework/wpf/advanced/visual-basic-and-wpf-event-handling.md)。  
