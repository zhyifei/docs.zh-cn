...
|||  
|-|-|  
|`object`|XAML 类型的对象元素声明，由 CLR 泛型类型支持。 如果 `object` 引用的 XAML 类型不是来自默认的 XAML 命名空间，`object` 需要一个前缀来指示 `object` 存在的 XAML 命名空间。|  
...
## <a name="remarks"></a>备注  
 ...
 请注意，`x:TypeArguments` 的这一定义特定于 .NET Framework XAML 服务和使用 CLR 支持。 可以在[\[的 MS-XAML\] 部分](https://docs.microsoft.com/previous-versions/msp-n-p/ff650760(v=pandp.10))中找到语言级定义5.3.11。  
...
### <a name="liststring"></a>列出\<字符串 >  
 `<scg:List x:TypeArguments="sys:String" ...>` 使用 <xref:System.String> 类型参数实例化新的 <xref:System.Collections.Generic.List%601>。  
  
### <a name="dictionarystringstring"></a>Dictionary\<String，String >  
 `<scg:Dictionary x:TypeArguments="sys:String,sys:String" ...>` 使用两个 <xref:System.String> 类型参数实例化新的 <xref:System.Collections.Generic.Dictionary%602>。  
  
### <a name="queuekeyvaluepairstringstring"></a>队列 < KeyValuePair\<String，String > >  
 `<scg:Queue x:TypeArguments="scg:KeyValuePair(sys:String,sys:String)" ...>` 实例化一个新的 <xref:System.Collections.Generic.Queue%601>，它具有与内部约束类型参数 <xref:System.String> 和 <xref:System.String>的 <xref:System.Collections.Generic.KeyValuePair%602> 约束。  
...
- 根元素必须映射到至少具有一个类型参数的泛型类型。 示例为 <xref:System.Windows.Navigation.PageFunction%601>。 页函数是 WPF 中 XAML 泛型用法支持的主要方案。  
...
## <a name="xaml-2009-or-xaml-2006-with-no-wpf-30-or-wpf-35-dependency"></a>不带 WPF 3.0 或 WPF 3.5 依赖项的 XAML 2009 或 XAML 2006  
 ...
 如果使用 XAML 2009，而不是映射 CLR 基类型以获取公共语言基元的 XAML 类型，则可以将[常见 XAML 语言基元的内置类型](built-in-types-for-common-xaml-language-primitives.md)用作 `typeString`中的信息项。 例如，可以声明以下内容（前缀映射未显示，但 x 是 xaml 2009 的 xaml 语言命名空间）：  
  
```xaml  
<my:BusinessObject x:TypeArguments="x:String,x:Int32"/>  
```  
  
 在 WPF 中，在面向 .NET Framework 4 时，可以将 XAML 2009 功能与 `x:TypeArguments` 一起使用，但仅适用于松散 XAML （非标记编译的 XAML）。 WPF 的已编译标记的 XAML 以及 XAML 的 BAML 形式当前不支持 XAML 2009 关键字和功能。 如果需要对 XAML 进行标记编译，则必须在 "XAML 2006 和 WPF 一般 XAML 用法" 一节中所述的限制下运行。  
