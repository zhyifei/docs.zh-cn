...
## <a name="where-is-mef-available"></a>MEF 适用于何处？
 MEF 是 [!INCLUDE[net_v40_short](../../../includes/net-v40-short-md.md)] 的组成部分，适用于所有使用 .NET Framework 的地方。 可以在客户端应用程序（不论其是否使用 Windows 窗体或其他技术）或使用 ASP.NET 的服务端应用程序里使用 MEF。
...
- 向系统添加参考。MEF 位于 ComponentModel.Composition 程序集中。

- 打开 Module1.vb 或 Program.cs，然后添加 System.ComponentModel.Composition 和 System.ComponentModel.Composition.Hosting 的 `Imports` 或 `using` 语句。 这两个名称空间包含你开发可扩展应用程序将需要的 MEF 类型。
...
如果使用的是 Visual Basic，则在 Module1.vb 中添加一个名为 `Program` 的公用类。

将以下行添加到 Module1.vb 或 Program.cs 中的 `Program` 类中：
