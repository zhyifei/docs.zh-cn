...
---
# <a name="apis-that-rely-on-reflection"></a>依赖反射的 API
...
```  
 ...
 但即使当你为未实例化的泛型类型添加元数据时，调用 <xref:System.Type.MakeGenericType%2A?displayProperty=fullName> 方法也会引发 [MissingMetadataException](../../../docs/framework/net-native/missingmetadataexception-class-net-native.md) 异常：  
...
```  
 ...
 `AppClass<T>` 上的每个不同的实例化都要求一个不同的指令（如果该实例化是使用 <xref:System.Type.MakeGenericType%2A?displayProperty=fullName> 方法创建的并且不静态使用）。  
