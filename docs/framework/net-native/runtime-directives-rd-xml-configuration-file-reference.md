...
## 运行时指令和策略  
 ...
 任何由元素属性指定的策略均适用于未对该策略指定值的子元素。  例如，如果策略由[类型](../../../docs/framework/net-native/type-element-net-native.md)元素指定，则该策略适用于包含的所有尚未指定策略的类型和成员。  
