---
title: "&lt;类型参数&gt;元素 (.NET Native) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|特性|特性类型|说明|  
|---------------|--------------------|-----------------|  
|`Name`|常规|必需的特性。 类型的参数的名称<xref:System.Type>。 例如，对于方法签名 `Type.GetInterfaceMap(Type interfaceType)`，`Name` 特性的值为“接口类型”。|  
...
|值|描述|  
|-----------|-----------------|  
|*parameter_name*|类型的参数的名称<xref:System.Type>。 例如，对于方法签名 `Type.GetInterfaceMap(Type interfaceType)`，`Name` 特性的值为“接口类型”。|  
...
## <a name="remarks"></a>备注  
 `<TypeParameter>`元素是类似于[ <> \> ](../../../docs/framework/net-native/parameter-element-net-native.md)元素，但它可以只应用于类型的参数<xref:System.Type>。 它将策略应用到任何在运行时间由 `Name` 特性指定的以类型参数为代表的类型。  
...
```  
 ...
 指定表示的运行时类型的元数据`type`参数应可用于序列化。 如果这些运行时指令适用于一个包含以下源代码的项目：  
