...
---
# <a name="method-element-net-native"></a>\<方法>元素（.NET 本机）
...
|值|说明|  
|-----------|-----------------|  
|method_name**|方法名称。 该方法的类型由父[\<类型>](type-element-net-native.md)或[\<类型即时>](typeinstantiation-element-net-native.md)元素定义。|  
...
|元素|说明|  
|-------------|-----------------|  
|[\<参数>](parameter-element-net-native.md)|将策略应用到传递到方法的参数类型。|  
|[\<泛型参数>](genericparameter-element-net-native.md)|将策略应用到一个泛型类型或方法的参数类型。|  
|[\<暗示类型>](impliestype-element-net-native.md)|如果该策略已应用到以包含 `<Method>` 元素为代表的方法，将该策略应用到一个类型。|  
|[\<类型参数>](typeparameter-element-net-native.md)|将策略应用到以传递到方法为代表的 <xref:System.Type> 自变量类型。|  
...
## <a name="remarks"></a>备注  
 ...
 你无法通过使用 `<Method>` 元素为一个构造函数定义运行时反射策略。 `Activate`而是使用[\<程序集>、](assembly-element-net-native.md)[\<命名空间>、](namespace-element-net-native.md)[\<类型>](type-element-net-native.md)或[\<类型即时性>](typeinstantiation-element-net-native.md)元素的属性。  
...
## <a name="example"></a>示例  
 ...
 通过使用[\<Type>](type-element-net-native.md)元素定义其元数据必须存在的类型，以及添加`<Method>`元素以确保存在可动态调用的方法重载的实现，可以全部消除这些异常。 以下的 default.rd.xml 文件可以清除这些异常并允许实例没有错误地执行。  
