---
title: "基于声明的标识模型 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.reviewer: ""
ms.suite: ""
...
### 安全令牌服务 \(STS\)  
 安全令牌服务 \(STS\) 是根据 WS\-Trust 和 WS 联合身份验证协议生成、签名并颁发安全令牌的服务组件。  尽管实现这些协议需要做大量工作，但 WIF 会为您完成所有这些工作，使得协议中的非专业人士只需做极少量的工作即可启动并运行 STS。  您可以使用预生成的 STS（如 [Active Directory® 联合身份验证服务 \(AD FS\) 2.0](http://go.microsoft.com/fwlink/?LinkID=247516)）和云 STS（如 [Microsoft Azure 访问控制服务 \(ACS\)](http://go.microsoft.com/fwlink/?LinkID=247517)），如果您希望颁发自定义标记或提供自定义身份验证或授权，则可使用 WIF 生成您自己的自定义 STS。  利用 WIF，您可以轻松生成自己的 STS。  
...
## 基于声明的标识模型的基本方案  
 ...
 ![依赖合作伙伴身份验证流](../../../docs/framework/security/media/conc-relying-partner-processc.png "conc\_relying\_partner\_processc")  
