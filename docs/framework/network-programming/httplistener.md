...
## HTTP.SYS  
 <xref:System.Net.HttpListener> 类在 HTTP.sys 的基础上构建，后者是为 Windows 处理所有 HTTP 流量的内核模式侦听器。  HTTP.sys 提供连接管理、带宽限制以及 Web 服务器日志记录。  使用 `HttpCfg.exe` 工具可添加 SSL 证书。  有关详细信息，请参见[服务器](http://go.microsoft.com/fwlink/?LinkID=178285)文档中的有关 [HttpCfg.exe](http://go.microsoft.com/fwlink/?LinkID=178284) 工具的文档。  
