...
如果要使用 .NET Framework 开发 Windows 服务，则可以使用[*installutil.exe*](../tools/installutil-exe-installer-tool.md)命令行实用工具或[PowerShell](/powershell/scripting/overview)快速安装服务应用程序。 想要发布用户可以安装和卸载的 Windows 服务的开发者，应使用 InstallShield。 有关详细信息，请参阅[创建安装程序包（Windows 客户端）](https://docs.microsoft.com/visualstudio/deployment/deploying-applications-services-and-components#create-an-installer-package-windows-client)。
...
1. 从 "**开始**" 菜单中选择 " **windows powershell** " 目录，然后选择 " **windows powershell**"。

2. 访问你的项目的已编译可执行文件所在的目录。

3. 使用项目的输出和服务名称运行[**新的服务**](/powershell/module/microsoft.powershell.management/new-service)cmdlet 作为参数：
...
2. 用服务的名称作为参数运行[**Remove-service**](/powershell/module/microsoft.powershell.management/remove-service) cmdlet：
