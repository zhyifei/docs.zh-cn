---
title: "封送类、结构和联合 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|结构中的 null 值。|传递 null 引用（Visual Basic 中为 **Nothing**），而不传递对值类型的引用。|HandleRef 示例|  
...
-   从 PinvokeLib.dll 导出的 **TestArrayInStruct**。  
 ...
 [PinvokeLib.dll](http://msdn.microsoft.com/zh-cn/5d1438d7-9946-489d-8ede-6c694a08f614) 是一种自定义的非托管库，包含上述函数和 4 种结构（**MYPERSON**、**MYPERSON2**、**MYPERSON3** 和 **MYARRAYSTRUCT**）的实现。  这些结构包含以下元素：  
...
-   `MyPerson` 仅包含字符串成员。  [CharSet](../../../docs/framework/interop/specifying-a-character-set.md) 字段在传递到非托管函数时将字符串设置为 ANSI 格式。  
...
-   `TestArrayInStruct` 将对 `MyArrayStruct` 类型的引用声明为其参数。  
 ...
 作为方法参数的结构按值传递，除非此参数包含 **ref**（Visual Basic 中为 **ByRef**）关键字。  例如，`TestStructInStruct` 方法将对 `MyPerson2` 类型对象的引用（地址的值）传递到非托管代码。  为了处理 `MyPerson2` 指向的结构，此示例创建了具有指定大小的缓冲区，并同时使用 <xref:System.Runtime.InteropServices.Marshal.AllocCoTaskMem%2A?displayProperty=fullName> 和 <xref:System.Runtime.InteropServices.Marshal.SizeOf%2A?displayProperty=fullName> 方法返回其地址。  然后，此示例将该托管结构的内容复制到非托管的缓冲区。  最后，此示例使用 <xref:System.Runtime.InteropServices.Marshal.PtrToStructure%2A?displayProperty=fullName> 方法将非托管缓冲区的数据封送到托管对象，并使用 <xref:System.Runtime.InteropServices.Marshal.FreeCoTaskMem%2A?displayProperty=fullName> 方法释放非托管的内存块。  
...
## FindFile 示例  
 此示例演示了如何将包含第二、嵌入结构的结构传递到非托管函数。  它还演示了如何使用 <xref:System.Runtime.InteropServices.MarshalAsAttribute> 属性在结构中声明固定长度的数组。  在此示例中，嵌入的结构元素将添加到父结构。  有关未平展的嵌入结构的示例，请参阅[“结构”示例](http://msdn.microsoft.com/zh-cn/96a62265-dcf9-4608-bc0a-1f762ab9f48e)。  
...
```  
 ...
 在此示例中，`FindData` 类包含原始结构和嵌入结构中每个元素的对应数据成员。  如果存在 2 个原始字符缓冲区，类将替换字符串。  **MarshalAsAttribute** 将 <xref:System.Runtime.InteropServices.UnmanagedType> 枚举设置为 **ByValTStr**，它用于标识非托管结构中出现的具有固定长度的内联字符数组。  
...
-   从 PinvokeLib.dll 导出的 **TestUnion**。  
 ...
 [PinvokeLib.dll](http://msdn.microsoft.com/zh-cn/5d1438d7-9946-489d-8ede-6c694a08f614) 是一种自定义的非托管库，包含上述函数和两个联合（**MYUNION** 和 **MYUNION2**）的实现。  联合包含以下元素：  
...
```  
 ...
 `MyUnion2_1` 和 `MyUnion2_2` 分别包含值类型（整数）和字符串。  在托管代码中，值类型和引用类型不允许重叠。  此示例使用方法重载以使调用方在调用同一个非托管函数时能够使用这两种类型。  `MyUnion2_1` 的布局是显式的且具有准确的偏移值。  与此相反，`MyUnion2_2` 的布局是按顺序的，因为不允许引用类型使用显式布局。  <xref:System.Runtime.InteropServices.MarshalAsAttribute> 属性将 <xref:System.Runtime.InteropServices.UnmanagedType> 枚举设置为 **ByValTStr**，它用于标识在联合的非托管表示形式中出现的具有固定长度的内联字符数组。  
...
## OutArrayOfStructs 示例  
 ...
 此示例使用 [PinvokeLib.dll](http://msdn.microsoft.com/zh-cn/5d1438d7-9946-489d-8ede-6c694a08f614) 中定义（且位于源文件）的包装器函数和平台调用。  它使用 `TestOutArrayOfStructs` 函数和 `MYSTRSTRUCT2` 结构。  结构包含以下元素：  
