---
title: "COM 可调用包装 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|**IerrorInfo**|提供以下内容的文字描述：错误、错误源、帮助文件，帮助上下文以及定义错误的接口的 GUID（.NET 类始终为 **GUID\_NULL**）。|  
...
|**Iunknown**|提供 **IUnknown** 接口的标准实现，COM 客户端使用该接口管理 CCW 的生存期并提供类型强制。|  
 ...
 **ClassInterfaceType.None** 值防止在类元数据被导出到类型库时生成类接口。  在前面的示例中，COM 客户端只能通过 `IExplicit` 接口访问 `LoanApp` 类。  
...
### 避免缓存调度标识符 \(DispId\)。  
 ...
 若要避免在使用类接口时中断后期绑定 COM 客户端，请应用具有 **ClassInterfaceType.AutoDispatch** 值的 **ClassInterfaceAttribute**。  此值实现仅支持调度的类接口，但将省略类型库中的接口说明。  没有接口说明，客户端就无法在编译时缓存 DispId。  尽管这是类接口的默认接口类型，但你可以显式地应用属性值。  
...
```  
 ...
 若要在运行时获取接口成员的 DispId，COM 客户端可以调用 **IDispatch.GetIdsOfNames**。  若要调用接口上的方法，请将返回的 DispId 作为参数传递到 **IDispatch.Invoke**。  
...
## 请参阅  
 ...
 [Simulating COM Interfaces](http://msdn.microsoft.com/zh-cn/ad2ab959-e2be-411b-aaff-275c3fba606c)   
