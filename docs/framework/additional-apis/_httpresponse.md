...
ms.locfileid: "32745911"
---
# <a name="httpwebrequesthttpresponse-field"></a>HttpWebRequest。\_HttpResponse 字段

`HttpWebRequest._HttpResponse` 是<xref:System.Net.HttpWebResponse>包含通过 HTTP 请求的 HTTP 响应详细信息。 它可以是`null`直到接收到 HTTP 响应。
...
> [!WARNING]
> `HttpWebRequest._HttpResponse`字段是内部，且不是在你的代码中直接使用。
> 
