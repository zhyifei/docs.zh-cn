...
|功能|更改|影响|范围|  
|-------------|------------|------------|-----------|  
|长路径支持|自定位 [!INCLUDE[net_v462](../../../includes/net-v462-md.md)] 的应用程序起，支持长路径（最多可包含 32000 个字符），260（或 `MAX_PATH`）个字符的路径长度上限已被撤消。|对于定位 [!INCLUDE[net_v462](../../../includes/net-v462-md.md)] 的应用，以前引发 <xref:System.IO.PathTooLongException> 的代码路径不再引发异常。 有关详细信息，请参阅[缓解：长路径支持](~/docs/framework/migration-guide/mitigation-long-path-support.md)。|次要|  
