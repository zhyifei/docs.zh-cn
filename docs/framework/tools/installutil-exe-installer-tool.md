---
title: "Installutil.exe（安装程序工具） | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|`/LogFile=`\[*filename*\]|指定在其中记录安装进度的日志文件的名称。  默认情况下，如果省略 `/LogFile` 选项，则会创建名为 *assemblyname*.InstallLog 的日志文件。  如果 *filename* 被省略，则不生成任何日志文件。|  
...
-   *assemblyname*.InstallState \- 包含用于卸载该程序集的数据。  
 ...
 Installutil.exe 使用反射检查指定的程序集以及查找将 <xref:System.ComponentModel.RunInstallerAttribute?displayProperty=fullName> 特性设置为 `true` 的所有 <xref:System.Configuration.Install.Installer> 类型。  该工具然后对 <xref:System.Configuration.Install.Installer> 类型的每个实例执行 <xref:System.Configuration.Install.Installer.Install%2A?displayProperty=fullName> 或 <xref:System.Configuration.Install.Installer.Uninstall%2A?displayProperty=fullName> 方法。  Installutil.exe 以事务性方式执行安装；也就是说，如果有一个程序集未能安装，则 Installutil.exe 会回滚其他所有程序集的安装。  卸载不是事务性的。  
...
```  
 ...
 下列命令显示了 InstallUtil.exe 的命令语法和选项的说明。  如果已向安装程序的 <xref:System.Configuration.Install.Installer.HelpText%2A?displayProperty=fullName> 属性分配了帮助文本，则它还会在 `myAssembly.exe` 中显示安装程序组件支持的选项的说明的列表。  
