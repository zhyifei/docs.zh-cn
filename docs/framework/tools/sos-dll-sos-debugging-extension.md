...
|**EEHeap** [**-gc**] [**-loader**]|显示有关内部公共语言运行时数据结构所使用的进程内存的信息。<br /><br /> **-gc** 和 **-loader** 选项将此命令的输出限制为垃圾回收器或加载程序数据结构。<br /><br /> 有关垃圾回收器的信息列出了托管堆中每个段的范围。  如果指针落在由 **-gc** 给定的段范围内，则该指针是一个对象指针。|  
