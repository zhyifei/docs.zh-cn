...
|**/output:** *22*|指定输出文件名和扩展名。 默认情况下，输出文件名与第一个源文件的名称相同。 默认扩展名为 .exe。 如果指定 **/dll** 选项，则默认扩展名为 .dll。 请注意：指定 /output:myfile.dll 不设置 /dll 选项。 如果不指定 **/dll**，则结果是名为 myfile.dll 的可执行文件。|  
