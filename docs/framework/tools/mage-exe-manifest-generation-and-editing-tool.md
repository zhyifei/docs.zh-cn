...
清单生成和编辑工具 (Mage.exe) 是一种命令行工具，可以支持创建和编辑应用程序和部署清单。 作为命令行工具，Mage.exe 可以从批处理脚本和基于 Windows 的其他应用程序（包括 [!INCLUDE[vstecasp](../../../includes/vstecasp-md.md)] 应用程序）运行。  
 ...
 Mage.exe 和 MageUI.exe 的两个版本作为组件包含在 [!INCLUDE[vs_dev10_long](../../../includes/vs-dev10-long-md.md)] 安装程序中。 若要查看版本信息，请运行 MageUI.exe，依次选择“帮助”和“关于”。 本文档介绍 Mage.exe 和 MageUI.exe 的 4.0.x.x 版本。  
...
|**-cf, -CertFile** `filePath`||所有文件类型。|指定用于对清单进行签名的 X509 数字证书的位置。 如果该证书需要密码，此选项可以与“-Password”选项结合使用。<br/><br/>自 .NET Framework 4.6.2 SDK（使用 Visual Studio、Windows SDK 和 .NET Framework 4.6.2 开发人员工具包发行）起，Mage.exe 通过 CNG 和 CAPI 证书对清单进行签名。|  
