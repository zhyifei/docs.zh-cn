...
|           | 描述 |
| --------- | ----------- |
| **href**  | 必需的特性。<br><br>要包含的配置文件的 URL。 **Href**特性支持的唯一格式是 `file://`。 支持本地文件和 UNC 文件。 |
...
- 所有链接的配置文件合并到一个文件中，这与 C/C++中的 `#include` 指令的行为类似。

- 仅在应用程序配置文件中允许 **\<linkedConfiguration >** 元素;它在*machine.config*中被忽略。

- 检测和终止循环引用。 也就是说，如果 **\<** 一系列配置文件的元素形成循环，则检测并停止循环。
