...
|instanceEncodingOption|一个可选值，指定在将实例状态信息保存到持久性存储区之前，是否使用 GZip 算法压缩此信息。 此值的类型为 `System.Activities.DurableInstancing.InstanceEncodingAction`。 此属性的可能值为"None"，它指定不压缩和"GZip"，指定该实例数据进行压缩，并使用 gzip 算法。|  
