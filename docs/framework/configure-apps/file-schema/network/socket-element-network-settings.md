...
## <a name="remarks"></a>备注  
 ...
 @No__t 的默认值为-0，`alwaysUseCompletionPortsForConnect` 属性为**false**。  
  
 @No__t-0 可用于从适用的配置文件中获取 @no__t 的属性的当前值。 @No__t-0 可用于从适用的配置文件中获取 @no__t 的属性的当前值。  
  
 @No__t-0 特性指定要用于套接字的默认 <xref:System.Net.Sockets.IPProtectionLevel?displayProperty=nameWithType>。 @No__t-0 属性可将 IPv6 套接字的限制配置为指定的作用域，如具有相同的链接本地或站点本地前缀的地址。 此选项使应用程序可以对 IPv6 套接字设置访问限制。 通过应用此类限制，可让在专用局域网上运行的应用程序能够通过简单的方式很好地增强自身的安全性，以便防范外部攻击。 此选项扩大或缩小侦听套接字的范围，在适当的时候启用从公共用户和私有用户进行的不受限制的访问，或根据需要限制对同一站点的访问。  
...
- 在套接字上接收数据包的 UDP 应用程序。  
 ...
 @No__t-0 属性设置的可能值与 <xref:System.Net.Sockets.IPProtectionLevel?displayProperty=nameWithType> 枚举中指定的已定义保护级别相对应，如下所示：  
