...
2.  指定是否应应用 IRI 分析规则。 这可以在 machine.config 或应用配置文件中完成。  
  
 启用 IRI 分析 (启用 iriParsing = `true`) 将执行规范化和字符根据最新 IRI 检查规则中 RFC 3987。 默认值是`false`，将执行规范化和字符根据 RFC 2396 检查和 RFC 3986 （对于 IPv6 文本）。  
