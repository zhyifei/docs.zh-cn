...
## <a name="example"></a>示例  
 下面的示例演示如何使用 **\<g s >** 元素包含加密名称映射和 OID 映射。 此示例将配置运行时，以便 <xref:System.Security.Cryptography.HashAlgorithm.Create%2A?displayProperty=nameWithType> 返回 `MyHashClass` 对象，`MyCryptoClass` 类映射到对象标识符1.3.36.2.1。  
