---
title: "&lt;msmqIntegration&gt; | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|deadLetterQueue|指定要使用的死信队列类型。<br /><br /> 有效值包括<br /><br /> -   Custom：自定义死信队列。<br />-   None：没有要使用的死信队列。<br />-   System：使用系统死信队列。<br /><br /> 此属性的类型为 DeadLetterQueue。|  
...
|serializationFormat|指定用于序列化作为 MSMQ 消息的一部分发送的对象的格式化程序。  有效值为<br /><br /> -   ActiveX：序列化 COM 对象时使用 ActiveX 格式化程序。<br />-   Binary：将对象序列化为二进制数据包。<br />-   ByteArray：将对象序列化为字节数组。<br />-   Stream：将对象序列化为流。<br />-   Xml：将对象序列化为 XML 数据包。  默认值为 XML。<br /><br /> 此属性的类型为 <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat>。|  
