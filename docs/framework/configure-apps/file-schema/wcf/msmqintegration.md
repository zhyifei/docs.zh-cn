...
|deadLetterQueue|指定要使用的死信队列类型。<br /><br /> 有效值包括<br /><br /> 自定义： 自定义死信队列。<br />-None： 没有死信队列是要使用。<br />-系统： 使用系统死信队列。<br /><br /> 此属性的类型为 DeadLetterQueue。|  
...
|serializationFormat|指定用于序列化作为 MSMQ 消息的一部分发送的对象的格式化程序。 有效值为<br /><br /> -ActiveX： 序列化 COM 对象时，使用 ActiveX 格式化程序。<br />二进制： 将对象序列化为二进制数据包。<br />-ByteArray： 将对象序列化为一个字节数组。<br />-Stream： 序列化到流对象。<br />Xml： 将对象序列化为 XML 数据包。 默认值为 XML。<br /><br /> 此属性的类型为 <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat>。|  
