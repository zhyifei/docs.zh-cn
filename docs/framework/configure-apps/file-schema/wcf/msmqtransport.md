...
|queueTransferProtocol|指定此绑定使用的排队通信信道传输协议。 有效值为<br /><br /> 本机： 使用本机 MSMQ 协议。<br />-Srmp： 使用 Soap 可靠消息传递协议 (SRMP)。<br />-SrmpSecure： 使用 Soap 可靠消息传递协议安全 (SRMPS) 传输。<br /><br /> 此属性的类型为 <xref:System.ServiceModel.QueueTransferProtocol>。<br /><br /> 由于 MSMQ 不支持 Active Directory 寻址使用 SOAP 可靠消息协议时，不应为 Srmp 或 Srmps 设置此属性时`u``seActiveDirectory`设置为`true`。|  
