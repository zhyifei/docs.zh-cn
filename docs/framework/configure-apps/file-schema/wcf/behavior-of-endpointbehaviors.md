...
|[\<enableWebScript >](enablewebscript.md)|启用终结点行为，此行为使得可以使用 ASP.NET AJAX 网页中的服务。 该行为仅应与 \<webHttpBinding > 标准绑定或 \<w > 绑定元素一起使用。|  
...
|[\<Wcf-webhttp >](webhttp.md)|通过配置指定终结点上的 WebHttpBehavior。 此行为与 \<webHttpBinding > 标准绑定结合使用时，将为 WCF 服务启用 Web 编程模型。|  
