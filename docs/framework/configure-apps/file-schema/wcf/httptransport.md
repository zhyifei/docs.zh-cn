...
## <a name="remarks"></a>备注  
 `httpTransport` 元素是创建实现 HTTP 传输协议的自定义绑定的起始点。 HTTP 是用于互操作性用途的主要传输。 通过 Windows Communication Foundation (WCF) 以确保与其他非互操作性支持此传输[!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)]Web 服务堆栈。  
