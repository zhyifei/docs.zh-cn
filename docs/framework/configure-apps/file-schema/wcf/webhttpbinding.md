...
|maxReceivedMessageSize|一个正整数，指定采用此绑定配置的通道上可以接收的最大消息大小（字节），包括消息头。 如果消息超出此限制，则发送方将收到错误。 接收方将删除该消息，并在跟踪日志中创建事件项。 默认值为 65536。 **注意：** 仅在 ASP.NET 兼容模式下增大此值。 还应增加 `httpRuntime` 的值（请参阅[HttpRuntime 元素（ASP.NET 设置架构）](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/e1f13641(v=vs.100))）。|  
...
## <a name="remarks"></a>备注  
 WCF Web 编程模型允许开发人员通过 HTTP 请求（这些请求使用 "纯旧 XML" （POX）样式消息，而不是基于 SOAP 的消息）来公开 WCF Web 服务。 要使客户端使用 HTTP 请求与服务进行通信，必须使用 \<WebHttpBehavior > 附加到[\<webHttpBinding >](webhttpbinding.md)来配置服务的终结点。  
