...
|maxReceivedMessageSize|一个正整数，指定采用此绑定配置的通道上可以接收的最大消息大小（字节），包括消息头。 如果消息超出此限制，则发送方将收到错误。 接收方将删除该消息，并在跟踪日志中创建事件项。 默认值为 65536。 **注意：** 在 ASP.NET 兼容模式下，单独增加此值是不够的。 您还应增加的值`httpRuntime`(请参阅[httpRuntime 元素 （ASP.NET 设置架构）](https://msdn.microsoft.com/library/e9b81350-8aaf-47cc-9843-5f7d0c59f369))。|  
