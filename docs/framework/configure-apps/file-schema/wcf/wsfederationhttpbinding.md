...
|useDefaultWebProxy|一个布尔值，指示是否使用系统的自动配置 HTTP 代理。  如果此属性为 `true`，则代理地址必须为 `null`（即，不设置代理地址）。  默认值为 `true`。|  
  
### 子元素  
  
|元素|描述|  
|--------|--------|  
|[\<安全性\>](../../../../../docs/framework/configure-apps/file-schema/wcf/security-of-wsfederationhttpbinding.md)|定义消息的安全设置。  此元素的类型为 <xref:System.ServiceModel.Configuration.WSFederationHttpSecurityElement>。|  
...
|[reliableSession](http://msdn.microsoft.com/zh-cn/9c93818a-7dfa-43d5-b3a1-1aafccf3a00b)|指定是否在通道终结点之间建立可靠会话。|  
...
## 备注  
 ...
 中所包含的相应堆栈相同：[\<安全性\>](../../../../../docs/framework/configure-apps/file-schema/wcf/security-of-wsfederationhttpbinding.md)设置为默认值 <xref:System.ServiceModel.WSFederationHttpSecurityMode>。  
  
 `wsFederationHttpBinding` 控制 [\<message\>](../../../../../docs/framework/configure-apps/file-schema/wcf/message-element-of-wsfederationhttpbinding.md)中的消息安全设置的详细信息。  请注意，由于绑定一旦创建后，就无法更改它所使用的安全性，因此 [\<安全性\>](../../../../../docs/framework/configure-apps/file-schema/wcf/security-of-wsfederationhttpbinding.md) 元素仅提供 get 访问。  
 ...
 有关此绑定的更多信息，请参见[如何：创建 WSFederationHttpBinding](../../../../../docs/framework/wcf/feature-details/how-to-create-a-wsfederationhttpbinding.md)。  
