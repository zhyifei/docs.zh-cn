...
[\<安全 >](security-of-wsfederationhttpbinding.md)设置为默认值 <xref:System.ServiceModel.WSFederationHttpSecurityMode.Message>。

`wsFederationHttpBinding` 在[\<message >](message-element-of-wsfederationhttpbinding.md)中控制消息安全设置的详细信息。 请注意， [\<security >](security-of-wsfederationhttpbinding.md)元素只提供 get 访问权限，因为在创建绑定后，不能更改绑定使用的安全性。
