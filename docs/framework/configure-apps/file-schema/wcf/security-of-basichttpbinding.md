...
---
# <a name="security-of-basichttpbinding"></a>\<basicHttpBinding 的安全 > \<
...
|-----------|-----------------|  
|None|-传输过程中消息不受保护。|  
|传输|使用 HTTPS 传输提供安全性。 用 HTTPS 保证 SOAP 消息的安全。 使用服务的 X.509 证书向客户端对服务进行身份验证。 使用所提供的 ClientCredentialType 对客户端进行身份验证。 请参阅[\<传输 >](transport-of-basichttpbinding.md)。|  
...
|元素|描述|  
|-------------|-----------------|  
|[\<传输 >](transport-of-basichttpbinding.md)|定义基本 HTTP 服务的传输安全设置。 此元素与 <xref:System.ServiceModel.HttpTransportSecurity> 相对应。|  
...
|元素|描述|  
|-------------|-----------------|  
|绑定|[\<basicHttpBinding >](basichttpbinding.md)的绑定元素。|  
