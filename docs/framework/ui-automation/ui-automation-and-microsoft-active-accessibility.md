...
> 本文档适用于想要使用 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 命名空间中定义的托管 <xref:System.Windows.Automation> 类的 .NET Framework 开发人员。 有关 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]的最新信息，请参阅 [Windows 自动化 API：UI 自动化](/windows/win32/winauto/entry-uiauto-win32)。  
 ...
 Microsoft Active Accessibility 是使应用程序可访问的早期解决方案。 [!INCLUDE[TLA#tla_uiautomation](../../../includes/tlasharptla-uiautomation-md.md)]是 Microsoft Windows 的新辅助功能模型，旨在满足辅助技术产品和自动化测试工具的需求。 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]提供了许多与活动可访问性改进。  
  
 本主题包括 和[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]解释这些功能与活动可访问性有何不同。  
  
<a name="Programming_Languages_compare"></a>
## <a name="programming-languages"></a>编程语言  
<活动可访问性基于组件对象模型 （COM），支持双接口，因此可编程为 C/C++、Microsoft Visual Basic 6.0 和脚本语言。 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]（包括用于标准控件的客户端提供程序库）以托管代码编写，并且 UI 自动化客户端应用程序最容易使用 C# 或 Visual Basic .NET 进行编程。 作为接口实现的 UI 自动化提供程序可以用托管代码或 C/C++ 编写。  
  
<a name="Support_in_Windows_Presentation_Foundation_"></a>
## <a name="support-in-windows-presentation-foundation"></a>Windows Presentation Foundation 中的支持  
 Windows 演示基础 （WPF） 是创建用户界面的新模型。 WPF 元素不包含对活动可访问性的本机支持;因此，WPF 元素不包含对活动可访问性的支持。但是，它们支持[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]，其中包括对活动辅助功能客户端的桥接支持。 只有专门为 WPF[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]编写的客户端才能充分利用 WPF 的辅助功能，例如对文本的丰富支持。  
  
<a name="Servers_and_Clients_compare"></a>
## <a name="servers-and-clients"></a>服务器和客户端  
 在活动可访问性中，服务器和客户端主要通过服务器的`IAccessible`实现直接进行通信。  
 ...
 UI 自动化提供程序可以为活动辅助功能客户端提供信息，而主动辅助功能服务器可以为 UI 自动化客户端应用程序提供信息。 但是，由于活动辅助功能未公开与[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]、这两种模型不完全兼容的信息。  
  
<a name="UI_Elements_compare"></a>
## <a name="ui-elements"></a>UI 元素  
 活动辅助功能[!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)]将元素作为`IAccessible`接口或子标识符呈现。 很难比较两个 `IAccessible` 指针来确定它们是否引用同一元素。  
...
## <a name="tree-views-and-navigation"></a>树视图和导航  
 ...
 在活动可访问性中，许多与最终用户无关的自动化元素在树中公开。 客户端应用程序需要查看所有元素以确定哪些元素是有意义的。  
 ...
 在"活动可访问性"中，元素之间的导航是空间（例如，移动到屏幕上左侧的元素）、逻辑（例如，移动到下一个菜单项，或对话框中选项卡顺序中的下一个项目）或分层项（例如，将容器中的第一个子级或从子级移动到其父级）。 分层导航非常复杂，因为子元素并不总是实现 `IAccessible`的对象。  
  
 在 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]中，所有 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] 元素都是支持相同基本功能的 <xref:System.Windows.Automation.AutomationElement> 对象。 （从提供程序的角度来看，它们是实现从 继承的<xref:System.Windows.Automation.Provider.IRawElementProviderSimple>接口的对象。导航主要是分层的：从父母到孩子，从一个兄弟姐妹到下一个兄弟。 （同级之间的导航具有逻辑元素，因为它可能遵循选项卡顺序。您可以使用<xref:System.Windows.Automation.TreeWalker>类从任何起始点导航，使用树的任何筛选视图。 你还可以通过使用 <xref:System.Windows.Automation.AutomationElement.FindFirst%2A> 和 <xref:System.Windows.Automation.AutomationElement.FindAll%2A>导航到特定的子级或继承项；例如，很容易检索对话框中支持指定控件模式的所有元素。  
  
 中的[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]导航比在活动辅助功能中更一致。 某些元素（如下拉列表和弹出窗口）在"活动辅助功能"树中出现两次，并且来自它们的导航可能会产生意外的结果。 实际上，无法正确实现钢筋控件的活动可访问性。 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 能够重排根目录并进行重新布置，因此可以在树的任何位置放置元素，不必考虑窗口所有者构建的层次结构。  
  
<a name="Roles_and_Control_Types"></a>
## <a name="roles-and-control-types"></a>角色和控件类型  
 活动辅助功能使用`accRole`属性`IAccessible::get_actRole`（ ） 检索元素在 中的角色的描述，[!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)]如ROLE_SYSTEM_SLIDER或ROLE_SYSTEM_MENUITEM。 元素的角色是确定其可用功能的主要线索。 通过使用固定的方法（如 `IAccessible::accSelect` 和 `IAccessible::accDoDefaultAction`）可实现与控件的交互。 客户端应用程序和 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] 之间的交互仅限于可通过 `IAccessible`完成的交互。  
 ...
 下表显示了活动辅助功能角色对[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]控件类型的映射。  
  
|活动辅助功能角色|[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 控件类型|  
|----------------------------------------------------------------------|----------------------------------------------------------------------------------------|  
...
## <a name="states-and-properties"></a>状态和属性  
 在活动可访问性中，元素支持一组公共属性，某些属性（如`accState`）必须描述非常不同的内容，具体取决于元素的角色。 服务器必须实现可返回属性的 `IAccessible` 的所有方法，返回的属性甚至与该元素不相关。  
  
 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]定义更多属性，其中一些属性对应于活动可访问性中状态。 某些属性是所有元素共有的，但其他属性特定于控件类型和控件模式。 属性由唯一标识符进行区分，并且大多数属性都可通过使用单个方法 <xref:System.Windows.Automation.AutomationElement.GetCurrentPropertyValue%2A> 或 <xref:System.Windows.Automation.AutomationElement.GetCachedPropertyValue%2A>进行检索。 许多属性还可以轻松从 <xref:System.Windows.Automation.AutomationElement.Current%2A> 和 <xref:System.Windows.Automation.AutomationElement.Cached%2A> 属性访问器进行检索。  
...
|STATE_SYSTEM_UNAVAILABLE|<xref:System.Windows.Automation.AutomationElement.IsEnabledProperty>|Y|  
 ...
 以下状态不是由大多数活动辅助功能控制服务器实现的，或者在 中[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]没有等效状态。  
...
|STATE_SYSTEM_EXTSELECTABLE|活动辅助功能服务器未广泛实现|  
|STATE_SYSTEM_MARQUEED|活动辅助功能服务器未广泛实现|  
|STATE_SYSTEM_SELFVOICING|活动辅助功能服务器未广泛实现|  
|STATE_SYSTEM_TRAVERSED|在 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]中不可用|  
|STATE_SYSTEM_ALERT_HIGH|活动辅助功能服务器未广泛实现|  
|STATE_SYSTEM_ALERT_MEDIUM|活动辅助功能服务器未广泛实现|  
|STATE_SYSTEM_ALERT_LOW|活动辅助功能服务器未广泛实现|  
|STATE_SYSTEM_FLOATING|活动辅助功能服务器未广泛实现|  
...
## <a name="events"></a>事件  
 与[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]活动辅助功能中的事件机制不同，事件机制不依赖于 Windows 事件路由（与窗口句柄紧密连接），并且不需要客户端应用程序设置挂钩。 可以对事件订阅进行微调，不仅能订阅特定事件，甚至还能订阅树的特定部分。 通过跟踪所侦听的事件，提供程序也可微调其事件的引发。  
  
 对于客户端来说检索引发事件的元素也很简单，因为这些事件会被直接传递到事件回调。 如果在客户端订阅该事件时，缓存请求处于活动状态，则会自动预提取元素的属性。  
  
 下表显示了活动辅助功能胜利事件和[!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]事件的对应关系。  
...
|EVENT_OBJECT_REORDER|在活动辅助功能中未始终使用。 在 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]中没有定义直接对应的事件。|  
