...
|功能|实现|  
|--------|--------|  
|向 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 公开提供程序|为响应发送到控制窗口的 WM\_GETOBJECT 消息，返回实现 <xref:System.Windows.Automation.Provider.IRawElementProviderSimple>（或派生接口）的对象。 对于片段，这必须是片段根的提供程序。|  
...
>   
>  应为承载在 [!INCLUDE[TLA#tla_winforms](../../../includes/tlasharptla-winforms-md.md)] 控件中的提供程序返回 <xref:System.Windows.Automation.AutomationElementIdentifiers.IsKeyboardFocusableProperty>。 在这种情况下，默认的窗口提供程序可能无法检索正确值。  
>   
...
> [!NOTE]
>  与根不同，片段的元素必须从 <xref:System.Windows.Automation.Provider.IRawElementProviderSimple.HostRawElementProvider%2A> 返回 `null` 引用，因为它们没有直接承载在窗口中，并且没有默认的提供程序可以支持往返它们的导航。  
 ...
 片段根仅支持子元素的导航。 例如，方向是 <xref:System.Windows.Automation.Provider.NavigateDirection> 时，列表框将返回列表中的第一项，当方向是 <xref:System.Windows.Automation.Provider.NavigateDirection> 时，将返回最后一项。 片段根不支持导航到父级或同级；这由宿主窗口提供程序进行处理。  
...
## 请参阅  
 ...
 [Simple Provider Sample](http://msdn.microsoft.com/zh-cn/c10a6255-e8dc-494b-a051-15111b47984a)   
