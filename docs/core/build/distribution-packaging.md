...
包维护人员可以决定提供共享框架和 SDK 的预览版。 可以使用 `dotnet-sdk-[major].[minor].[sdk feat]xx`、`aspnetcore-runtime-[major].[minor]` 或 `dotnet-runtime-[major].[minor]` 包提供预览版。 对于预览版，包的主版本号必须设为 0。 这样最终版本将被安装为此包的升级版。
