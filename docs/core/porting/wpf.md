...
>[!IMPORTANT]
>虽然本文使用 C# 作为目标语言，但其中的步骤同样适用于 VB.NET，唯一的区别是 VB.NET 分别使用 .vbproj 和 .vb 文件，而不是 .csproj 和 .cs 文件     。
...
>[!NOTE]
> Visual Studio 2017 不支持 .NET Core 3.0 项目。 **Visual Studio 2019** 支持 .NET Core 3.0 项目，但对 .NET Core WPF 可视化设计器的支持有限。 要使用获得完全支持的可视化设计器，解决方案中必须包含可与 .NET Core 项目共享其文件的 .NET Framework WPF 项目。
...
01. 使用 [Windows 兼容包][compat-pack]可有助于进行迁移。
    ...
    某些 API 可在 .NET Framework 中使用，但不可用于 .NET Core 3.0。 [Windows 兼容包][compat-pack]添加了其中许多 API，可有助于使 WPF 应用与 .NET Core 兼容。
